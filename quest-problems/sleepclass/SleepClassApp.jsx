...
import { SleepClassProgressiveCode, downloadSleepClassPDF, getSleepClassSections } from "./components";
import { makeSleepCh1, makeSleepCh2 } from "./chapters";

const A = "#059669";

export default function SleepClassApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeSleepCh1(false));
  const [ch2Q, setCh2Q] = useState(() => makeSleepCh2(false, "py"));

  useEffect(() => {
    setCh2Q(prev => makeSleepCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeSleepCh1, 1: (e) => makeSleepCh2(e, codeLang) };
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 12, alignItems: "center" }}>
          ...
          {showCodeControls && (
            <div style={{ display: "flex", gap: 0, alignItems: "stretch", flexShrink: 0 }}>
              ...
              <button onClick={() => downloadSleepClassPDF(E, getSleepClassSections(E), codeLang)} style={{
