...
def sentences(s):
    ...
    # ⚠️ 2026-09-24: 줄바꿈이 **문장 끝이 아니라 그냥 줄바꿈**인 자리가 많다.
    # CodeWalk 말풍선은 폭에 맞추려고 **문장 한가운데서** 줄을 바꾼다 —
    #   "The sum of (subset sum)^K over every subset,\nmod 998244353."
    # 영어 문장은 대문자로 시작하므로, 줄바꿈 **바로 뒤가 소문자면 이어지는 줄**이다.
    # 한국어는 대소문자가 없어 이 규칙에 안 걸린다(한국어 줄은 어차피 문장부호로 끝난다).
    # 이걸 안 하면 sumk·tichu·presents 처럼 **뜻이 같은데 영어만 부푼** 헛경보가 쌓이고,
    # 그때마다 ALLOW 에 예외를 하나씩 넣게 된다 — 예외를 쌓지 말고 잣대를 고친다.
    t = re.sub(r"\n(?=\s*[a-z])", " ", t)
