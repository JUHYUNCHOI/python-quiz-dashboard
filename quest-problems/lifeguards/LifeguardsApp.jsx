...
import { LifeguardsProgressiveCode, downloadLifeguardsPDF, getLifeguardsSections, getLifeguardsWalk } from "./components";
import { makeLifeguardsCh1, makeLifeguardsCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#2563eb";

export default function LifeguardsApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeLifeguardsCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeLifeguardsCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeLifeguardsCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeLifeguardsCh1, 1: (e) => makeLifeguardsCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <LifeguardsProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "lifeguards-codewalk") return <CodeWalk E={E} lang={codeLang} {...getLifeguardsWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <LifeguardsProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "lifeguards-codewalk") return <CodeWalk E={E} lang={codeLang} {...getLifeguardsWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadLifeguardsPDF(E, getLifeguardsSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`life-${tab}-${cur}-${lang}`} text={step.narr} />}
