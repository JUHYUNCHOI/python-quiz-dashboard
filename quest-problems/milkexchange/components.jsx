...
const FULL_CPP = [
  ...
  "    cin.tie(nullptr);",
  "",
  "    auto input = sys.stdin.readline;",
  "",
  "    long long N, M; cin >> N >> M;",
  "    vector<long long> cap; { long long _x; while (cin >> _x) cap.push_back(_x); if (!cin) cin.clear(); } // adapt: read N values",
  "    vector<long long> milk; { long long _x; while (cin >> _x) milk.push_back(_x); if (!cin) cin.clear(); } // adapt: read N values",
  "    auto direction = input().split()  # 'L' or 'R' for each cow;",
  "",
  "    // for step in range(min(M, 2 * N)):",
  "        auto new_milk = [0] * N;",
  "        for (long long i = 0; i < N; i++) {",
  "            auto give = min(1, milk[i]);",
  "            if (direction[i] == 'L') {",
  "                auto target = (i - 1) % N;",
  "            else {",
  "                auto target = (i + 1) % N;",
...
export function MilkExchangeProgressiveCode({ E, lang = "py", sections }) {
...
export function downloadMilkExchangePDF(E, sections, lang = "py") {
  const win = window.open("", "_blank");
  if (!win) { alert(t(E, "Pop-up blocked.", "팝업 차단됨.")); return; }
  const esc = (s) => String(s).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
  const langLabel = lang === "py" ? "🐍 Python" : "💻 C++";
  const fileTitle = t(E, "MilkExchange — Full Study Guide", "MilkExchange — 종합 풀이 노트");
