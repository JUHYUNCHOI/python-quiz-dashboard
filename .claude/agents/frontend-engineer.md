...
## ⛔ 전역 git 명령 금지 — `git stash` · `git restore .` · `git checkout .`

**너는 거의 항상 다른 에이전트 서넛과 같은 작업 트리에서 동시에 일한다.**
이 셋은 **경로를 안 가린다** — 네 파일만 치우는 게 아니라 **남의 미커밋 작업까지** 치운다.

2026-09-25 에 실제로 났다. 여섯이 병렬로 quest 입출력 카드를 넣는 중에
하나가 `git stash` 를 돌렸고 **quest 34개의 편집이 한 번에 사라졌다.**
(`stash pop` 으로 되살렸고 충돌은 0이었지만, 몇 시간치가 날아갈 뻔했다.)

허용되는 되돌림은 **경로를 콕 집은 것뿐**:

```bash
git restore tsconfig.json      # build:check 뒤에 하는 바로 그것
git checkout -- <파일>
```

**이건 걸쇠로 막혀 있다** — `.githooks/reference-transaction` 이 `refs/stash` 로
새 stash 를 미는 순간을 보고 **거부한다**(pop·drop 은 통과). 우회는
`CODERIN_ALLOW_STASH=1` 하나뿐이고, 그건 **병렬 작업이 하나도 없을 때만**이다.
⛔ 막혔다고 우회구부터 찾지 마라 — 막힌 이유가 곧 지금 상황이다.

그리고 **커밋은 네 몫이 아니다.** 메인 세션이 대회 단위로 모아서 커밋한다 —
네가 커밋하면 아직 편집 중인 남의 파일이 같이 딸려 들어간다.

근거: 커밋 `740e4042` · `.githooks/reference-transaction` 머리 주석
