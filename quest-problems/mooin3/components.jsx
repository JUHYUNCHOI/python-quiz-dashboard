...
const M3_FAST_PY = [
  "import sys",
  "",
  "# 손으로 짠 이분 탐색 — bisect 모듈 안 써도 같은 일.",
  "# arr 의 [lo, hi) 안에서 arr[idx] >= val 인 가장 작은 idx 반환.",
  "def first_ge(arr, val, lo, hi):",
  "    while lo < hi:",
  "        mid = (lo + hi) // 2",
  "        if arr[mid] < val:",
  "            lo = mid + 1",
  ...
  "    return lo",
  "",
  "# arr[idx] > val 인 가장 작은 idx (즉 위와 같지만 등호 위치만 다름).",
  "def first_gt(arr, val, lo, hi):",
  "    while lo < hi:",
  "        mid = (lo + hi) // 2",
  "        if arr[mid] <= val:",
  "            lo = mid + 1",
  ...
  "s = data[p].decode(); p += 1",
  "",
  "# positions[c] = chr(c+97) 가 등장하는 인덱스들 (왼→오 순서로 추가하니 자동 정렬)",
  ...
  "        posc = positions[c]",
  "        sz = len(posc)",
  "        lo = first_gt(posc, i, 0, sz)   # j > i 인 첫 인덱스",
  "        hi = first_ge(posc, k, 0, sz)   # j >= k 인 첫 인덱스",
  "        if lo >= hi: continue",
  "        mid = (i + k) // 2",
  "        at = first_ge(posc, mid, lo, hi)",
  "        # 꼭대기 근처 후보 2 개만 확인 (at, at - 1)",
  "        if at < hi:",
  "            j = posc[at]",
  "            v = (j - i) * (k - j)",
  "            if v > best: best = v",
...
const M3_FAST_CPP = [
  ...
  "// arr 의 [lo, hi) 안에서 arr[idx] >= val 인 가장 작은 idx 반환.",
  "int first_ge(const vector<int>& arr, int val, int lo, int hi) {",
  "    while (lo < hi) {",
  "        int mid = (lo + hi) / 2;",
  ...
  "}",
  "",
  "// arr[idx] > val 인 가장 작은 idx",
  "int first_gt(const vector<int>& arr, int val, int lo, int hi) {",
  "    while (lo < hi) {",
  "        int mid = (lo + hi) / 2;",
  "        if (arr[mid] <= val) lo = mid + 1;",
  "        else hi = mid;",
  ...
  "            int k = last_same[c][r];",
  "            if (k <= i) continue;",
  "            int sz = positions[c].size();",
  "            int lo = first_gt(positions[c], i, 0, sz);",
  "            int hi = first_ge(positions[c], k, 0, sz);",
  "            if (lo >= hi) continue;",
  "            int mid = (i + k) / 2;",
  "            int at = first_ge(positions[c], mid, lo, hi);",
  "            // 꼭대기 근처 후보 2 개만 확인 (at, at - 1)",
  "            if (at < hi) {",
  "                int j = positions[c][at];",
  "                long long v = (long long)(j - i) * (k - j);",
  "                if (v > best) best = v;",
  "            }",
  "            if (at - 1 >= lo) {",
  "                int j = positions[c][at - 1];",
  "                long long v = (long long)(j - i) * (k - j);",
  "                if (v > best) best = v;",
  "            }",
  "        }",
