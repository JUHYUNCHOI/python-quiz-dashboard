...
x = 9 → sum = 18`,
        ...
        // ── 3단계: 참조 설명 ──────────────────────
        {
          ...
          content: `> 📌 여기서 잠깐 \`&\` (참조) 를 살짝 보고 넘어가요. range-for 를 제대로 쓰려면 꼭 필요해서 미리 소개. **자세한 참조 문법은 "참조와 함수" 레슨에서 본격적으로** 배워요.

방금 배운 \`for (int x : v)\`에는 숨겨진 비밀이 있어요. x를 아무리 수정해도 **원본이 안 바뀌어요.**
