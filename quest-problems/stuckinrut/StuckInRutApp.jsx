...
import { StuckInRutProgressiveCode, downloadStuckInRutPDF, getStuckInRutSections, getStuckInRutWalk } from "./components";
import { makeStuckCh1, makeStuckCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#8b5cf6";

export default function StuckInRutApp(props = {}) {
  ...
  const makers  = { 0: makeStuckCh1, 1: (e) => makeStuckCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <StuckInRutProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "stuck-walk") return <CodeWalk E={E} lang={codeLang} {...getStuckInRutWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <StuckInRutProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "stuck-walk") return <CodeWalk E={E} lang={codeLang} {...getStuckInRutWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadStuckInRutPDF(E, getStuckInRutSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`stuc-${tab}-${cur}-${lang}`} text={step.narr} />}
