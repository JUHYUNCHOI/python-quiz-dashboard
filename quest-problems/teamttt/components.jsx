// 🔒 USACO_VERIFIED — cpid=831, teamttt (2018 Open Bronze #1)
...
const FULL_PY = [
  ...
  "with open('tttt.in', 'r') as file:",
  "    lines = file.readlines()",
  "",
  "grid = []",
  "for r in range(3):",
  "    grid.append(lines[r].strip())",
  "",
  "# 8 lines: 3 rows + 3 cols + 2 diagonals",
  "all_lines = []",
  "for r in range(3):",
  "    all_lines.append([grid[r][0], grid[r][1], grid[r][2]])",
  "for c in range(3):",
  "    all_lines.append([grid[0][c], grid[1][c], grid[2][c]])",
  "all_lines.append([grid[0][0], grid[1][1], grid[2][2]])",
  "all_lines.append([grid[0][2], grid[1][1], grid[2][0]])",
  "",
  ...
  "        teams.add((pair[0], pair[1]))",
  "",
  "with open('tttt.out', 'w') as file:",
  "    file.write(str(len(singles)) + '\\n')",
  "    file.write(str(len(teams)) + '\\n')",
];
...
const FULL_CPP = [
  ...
  "    for (int i = 0; i < 3; i++) fin >> grid[i];",
  "",
  "    // 8 lines: 3 행 + 3 열 + 2 대각선",
  "    set<char> singles;",
  "    set<string> teams;  // 두 글자 정렬한 \"AB\" 형태",
  "    int rows_idx[8][3][2] = {",
  ...
  "    };",
  "    for (int k = 0; k < 8; k++) {",
  "        char a = grid[rows_idx[k][0][0]][rows_idx[k][0][1]];",
  "        char b = grid[rows_idx[k][1][0]][rows_idx[k][1][1]];",
  "        char c = grid[rows_idx[k][2][0]][rows_idx[k][2][1]];",
  "        set<char> distinct;",
  "        distinct.insert(a);",
  "        distinct.insert(b);",
  "        distinct.insert(c);",
  ...
  "            string pair_str = \"\";",
  "            for (char ch : distinct) pair_str += ch;  // set 은 정렬됨",
  "            teams.insert(pair_str);",
  "        }",
  "    }",
  "    fout << singles.size() << \"\\n\";",
  "    fout << teams.size() << \"\\n\";",
