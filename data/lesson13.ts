      ...
      steps: [
        {
          ...
          content: `\`range\` 는 한마디로 **숫자를 차례로 뽑아주는 자판기**예요. 버튼을 어떻게 누르느냐(숫자를 몇 개 넣느냐)에 따라 뽑히는 숫자가 달라져요. **1 개 / 2 개 / 3 개** 넣을 수 있고, 각각 뜻이 달라요.
...
for i in range(5):
    print(i)  # 0, 1, 2, 3, 4
\`\`\`
시작 버튼을 안 눌렀으니 파이썬이 알아서 **0 부터** 시작해요. (왜 1 이 아니라 0? — 파이썬은 첫 번째를 0 번이라고 세거든요. 엘리베이터로 치면 1 층이 아니라 'G(지상)층' 부터.)
...
for i in range(1, 6):
    print(i)  # 1, 2, 3, 4, 5
\`\`\`
0 말고 **내가 원하는 숫자부터** 시작하고 싶을 때. 1 부터 세고 싶으면 \`range(1, ...)\`.
...
for i in range(0, 10, 2):
    print(i)  # 0, 2, 4, 6, 8
\`\`\`
한 칸씩이 아니라 **두 칸씩 건너뛰기**. 간격이 2 면 짝수만, 3 이면 세 칸씩.

**왜 끝은 안 포함일까?** \`range(시작, 끝)\` 은 *"끝에 닿기 직전까지"* 라고 약속돼 있어요. 그래야 \`range(0, 5)\` 가 딱 **5 개** (0,1,2,3,4) 로 떨어져서 "끝 − 시작 = 개수" 라는 계산이 깔끔하게 맞거든요.
...
**왜 문자열이 for 에 들어갈까?** 문자열은 사실 *글자들이 한 줄로 늘어선 것* 이에요. \`"사과"\` 는 \`사\`, \`과\` 두 글자가 순서대로 붙어 있는 거죠 (기차 칸처럼). for 문은 "줄 서 있는 걸 앞에서부터 하나씩 꺼내는" 도구라서, range 가 숫자를 하나씩 뽑듯이 문자열에서는 **글자를 하나씩** 뽑아줘요.

문법: **for 변수 in 문자열:**
\`ch\` 라는 변수에 **사 → 과** 가 차례로 들어가요. (변수 이름은 마음대로 — 글자라서 character 의 \`ch\` 를 자주 써요.)
...
**왜 \`total = 0\` 을 for *밖에* 둘까?** total 은 숫자를 모아 담는 **빈 저금통** 이에요. 저금통은 돈을 넣기 *전에 한 번만* 비워두면 되죠. 만약 \`total = 0\` 을 for 안에 넣으면 매번 저금통을 다시 비워서 — 넣은 게 계속 사라져 합계가 안 쌓여요!
