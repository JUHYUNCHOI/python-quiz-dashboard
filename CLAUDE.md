...
printf '<입력>' | python3 scripts/run-quest-code.py <id>     # ⭐ **화면이 말하는 답 = 🔒 코드가 내는 답인가**
                                                            #   quest 의 정답 배열을 찾아 **실제로 돌려** 준다 (`--cpp` 는 컴파일해서,
                                                            #   `--show` 는 코드만). 2026-09-26: 「과정 시뮬」 셋을 만들며 **만들기 전에**
                                                            #   예제를 이렇게 검증했는데 **셋 중 하나가 떨어졌다** —
                                                            #   `swaptowin` 은 화면 손풀이가 **3번**인데 코드는 **1번**(`1 1 1 3`) 이었다.
                                                            #   화면이 「같은 줄 안에서 먼저 찾는다」는 **코드 자신의 순서를 어기고** 있었고,
                                                            #   그대로 시뮬을 만들었으면 **틀린 길을 가르치며 완성**됐을 것이다.
                                                            #   (`photoshoot` 이 그 사고의 잔해다 — 이름이 비슷한 **다른 문제**의
...
node scripts/check-fixed-bar-overlap.mjs <id> --tab "⚡ 코드"  # 고정 바가 본문 버튼의 **탭을 가져가나** (모바일)
                                                            ...
                                                            #   **특정 스크롤 구간에서만** 가려진다 → `see-screen` 은 **원리상 0건**이다.
                                                            #   2026-09-26 실측: `mcc20citytour` 시뮬의 «▶ 다음» 이 스크롤 130px 에서
                                                            #   66% 덮이고, 누르면 **시뮬을 통째로 건너뛰고** 코드 쪽으로 점프한다.
                                                            #   돌아와도 시뮬은 **1/20 으로 리셋**된다(걸음이 컴포넌트 로컬 useState).
                                                            #   **푸터·헤더 같은 페이지 크롬은 「참고」로 내리고 본문만 센다** —
                                                            #   푸터는 한 번 가서 한 번 누르면 되지만, 시뮬 ◀▶ 는 **한 자리에 머문 채
                                                            #   스무 번** 누른다. 거기가 갈림길이다.
```
...
> ⚠️ **그런데 이 "1개뿐" 이 또 조용히 틀렸다 — 같은 날 두 시간 뒤에 잡혔다.** `cowgym` 검토자가
> *"코드를 화면에 그대로 보여주고 퀴즈까지 내는데 왜 0건이지?"* 라고 물었다. 원인은 이 저장소
> quest 168개가 쓰는 **공유 `CodeBlock`**(`components/quest/shared.tsx`)·12개가 쓰는 자체
> `CodeSnippet` 처럼, 코드 글자를 **JSX children 이 아니라 `lines={[...]}` 프롭으로 넘기는** 모양이다 —
> 컴포넌트 **정의부**(`{lines.map(...)}`)만 스캔하고 **호출부**(`lines={[...]}`)는 안 봐서
> 코드가 실제로 있는 자리를 놓쳤다. `lines_prop_blocks()` 를 추가해(`lines={NAME}`·`lines={pick(A,B)}`
> 이름 참조도 quest 폴더 안 `const NAME=[...]` 를 찾아 푼다) **6건 · quest 4개(billboard·cowgym·
> mixmilk·shellgame)로 다시 늘었고, 4개 전부 손으로 열어 진짜였다.**
> ⚠️ **반대 방향(🔒 코드가 쓰는데 화면 어디서도 안 가르친 것)은 일부러 안 짠다** — 시험 삼아
> 돌려 보니 `map`·`sum` 같은 입출력 상용구가 **175건·quest 25개+** 로 쏟아져서 오탐이 압도적이었다
> (2026-09-23). `mcc20kitty` 처럼 이 반대 방향 결함은 **사람이 읽어야** 잡힌다.
> ⚠️ **모노스페이스(`fontFamily: 'JetBrains Mono'`)로 스타일된 코드 블록 + `lines=` 프롭 호출,
> 이 두 모양만 본다** — narr 의 "max() 를 쓰면…" 같은 스치는 언급은 일부러 뺐다. `CodeWalk` 로
> 이미 옮긴 quest는 그 컴포넌트가 🔒 최종 코드 배열을 그대로 보여주므로(글자를 새로 안 쳐서)
> 구조적으로 안 걸린다 — 이 구멍은 **CodeWalk 로 아직 안 옮긴 옛 수기(手記) 코드 블록**에서만 난다.
> **판정이 아니다** — 가르친 방법을 일부러 버리고 더 나은 방법으로 갈아탄 자리일 수 있다.
> **0건이 결백이 아니다** — 위 두 모양 밖의 세 번째 모양이 또 있을 수 있다.
...
> 🆕 **quest 를 닫기 전에 묻는 질문 (2026-09-23)** — 기계가 원리상 못 잡는다:
>   **“인트로 시뮬의 숫자와 계획/코드 단계의 숫자가 **같은 예제로 맞아떨어지나?**”**
>   왜 생겼나 — `strangefn` 을 PM 이 한 번 닫았는데 **선생님이 화면 보시고 다시 여셨다.**
>   **시뮬은 브루트(f 를 4번 되풀이)를 가르치는데 코드는 공식을 쓰고 있었다.**
>   학생마다 "코드 탭에서 막혔다" 고는 했는데 **아무도 "이 시뮬이 저 코드로 이어지나"
>   를 묻지 않아서 재검증 여덟 번을 뚫었다.**
>   `memory/usaco_quest_learning_flow.md` 의 **계획** 단계가 화면에 없으면
>   그 내용이 **힌트 뒤로 숨는다** — 실제로 7쪽 힌트가 코드가 할 세 단계 전부를
>   담고 있었다(`feedback_students_copy_the_answer` 의 "힌트2 = 정답 전문" 과 같은 모양).
>   **코드 쪽 앞에 «계획» 쪽이 있나부터 봐라.**
...
⚠️ **이 둘은 quest 가 커밋 전이면 막는다.** 2026-09-23, 같은 날 두 번 —
   `cowsignal` 학생이 "설명이 없다" 고 보고했는데 있었고(커밋 전 화면을 본 것),
   `strangefn` 학생이 수정 **전** 표·시뮬 숫자를 보고했다. **레이스가 아니라 순서
   문제였다** — 편집이 끝나기 전에 검토자를 보냈다. 이제 `git status --porcelain --
   quest-problems/<id>/` 이 비어 있지 않으면 exit 3 으로 막는다. 방금 고친 걸
   미리 보고 싶으면 `--allow-dirty` — 막지 않되 크게 경고를 찍는다(보고에
   "커밋 전 상태로 봤다" 고 적을 것). 새 스크립트를 따로 안 만든 이유 —
   검토자가 quest 를 볼 때 반드시 거치는 문이 이미 이 둘이라, 여기 안 박으면
   "먼저 이거 돌려라" 를 또 잊는다.

⚠️ **이 둘은 HMR 노이즈(깨진 청크)를 만나면 조용히 틀리지 않고 재시도한다.**
   다른 사람이 quest 파일을 저장하는 중이면 Turbopack 이 반쯤 써진 파일을 읽어
   `Uncaught SyntaxError: Invalid or unexpected token` 같은 깨진 청크를 낸다
   (2026-09-23 확인 — 프로덕션은 정적 청크라 이 문제가 없다). PM 판정: "전면
   직렬화(한 번에 한 quest 만 편집)는 오늘의 병렬 처리량을 죽이는 과잉 대응이다" —
   그래서 **막지 않는다.** 노이즈 신호가 찍히면 2.5초 뒤 재시도(최대 6번), 그래도
   안 없어지면 **크게 떠들고 exit 3** — 조용히 무시하지 않는다.
...
> ⚠️ **선생님이 "~된건가? / 해결됐나?" 처럼 현황을 판정해 달라고 물으시면,**
>   배포 질문이 아니어도 **`project-lead` 를 불러라.** 특히 관련 `memory/feedback_*.md`
>   가 이미 있는(= 두 번째 이상 지적된) 문제는, 내가 직접 diff·로그를 본 결과라도
>   **결론은 project-lead 검증 없이 말하지 마라.**
>   (2026-09-18~21 에 선생님이 *"프로젝트 매니저?"* 를 **다섯 번** 물으셨다.
>    다섯 번 다 **내가 혼자 판단해서 결론을 보고한 직후**였다.)
...
>   **이건 걸쇠로 막혀 있다** — `.githooks/pre-push` 가 `check-deploy-budget.py` 를 돌려
>   오늘 이미 밀었으면 **푸시 자체를 막는다.** (`git config core.hooksPath .githooks`)
>   ⛔ `--no-verify` 로 넘기지 마라. 넘겨야 할 때는 선생님이 말씀하신다.
>   왜 걸쇠까지 왔나 — 이 얘기를 하는 메모리 문서가 **이미 셋** 있었고 CLAUDE.md 에도
>   적었고 세는 스크립트도 만들었는데, 다 **내가 기억해서 돌려야** 작동했다.
>   project-lead(2026-09-21): *"지금 상태는 문서 네 번째를 하나 더 쓴 것과 다르지 않다."*
