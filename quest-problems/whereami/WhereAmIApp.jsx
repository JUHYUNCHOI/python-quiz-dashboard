...
import { WhereAmIProgressiveCode, downloadWhereAmIPDF, getWhereAmISections, getWhereAmIWalk } from "./components";
import { makeWhereAmICh1, makeWhereAmICh2, makeWhereAmICh3 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#f97316";

export default function WhereAmIApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeWhereAmICh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeWhereAmICh2(lang === "en"));
  const [ch3Q, setCh3Q] = useState(() => makeWhereAmICh3(lang === "en", "py"));

  useEffect(() => {
    setCh3Q(prev => makeWhereAmICh3(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeWhereAmICh1, 1: makeWhereAmICh2, 2: (e) => makeWhereAmICh3(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <WhereAmIProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "whereami-codewalk") return <CodeWalk E={E} lang="py" {...getWhereAmIWalk(E)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <WhereAmIProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "whereami-codewalk") return <CodeWalk E={E} lang="py" {...getWhereAmIWalk(E)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadWhereAmIPDF(E, getWhereAmISections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`whereami-${tab}-${cur}-${lang}`} text={step.narr} />}
