...
import { SumKProgressiveCode, downloadSumKPDF, getSumKSections } from "./components";
...
export default function SumKApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeSumKCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeSumKCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeSumKCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeSumKCh1, 1: (e) => makeSumKCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <SumKProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <SumKProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadSumKPDF(E, getSumKSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`sumk-${tab}-${cur}-${lang}`} text={step.narr} />}
