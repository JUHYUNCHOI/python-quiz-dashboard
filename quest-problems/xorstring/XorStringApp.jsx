...
import { XorStringProgressiveCode, downloadXorStringPDF, getXorStringSections, getXorStringWalk } from "./components";
...
export default function XorStringApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeXorStringCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeXorStringCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeXorStringCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeXorStringCh1, 1: (e) => makeXorStringCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <XorStringProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "xorstring-walk") return <CodeWalk E={E} lang={codeLang} {...getXorStringWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <XorStringProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "xorstring-walk") return <CodeWalk E={E} lang={codeLang} {...getXorStringWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadXorStringPDF(E, getXorStringSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`xors-${tab}-${cur}-${lang}`} text={step.narr} />}
