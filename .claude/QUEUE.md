...
| **CodeWalk 가 아니다** | **⬇️ 41** (USACO 27 · MCC 4 · LeetCode 5 · 그 밖) | 2026-09-25 밤 실측. **오늘 USACO 45 → 87** (+42). 선생님(2026-07-14) *"앞으로 코드는 모두 이런식으로"* |
...
| CodeWalk **1스탑 나머지 18개** | frontend-engineer ×3 | **DONE** — 셋 다 「이미 끝난 것」이었다(목록이 낡았다) |
| CodeWalk **L·M·N·O조 24개** | frontend-engineer ×4 | **DONE** `7524d217` — USACO 45 → 74 |
| CodeWalk **P·Q·R조 18개** | frontend-engineer ×3 | **DONE** `202c543c` `719fdbb0` `d9f7fd8f` — USACO → **87** |
| 안 쓰는 선수개념 정정 | 교육+감사 판정 → 내가 반영 | **DONE** `83ff2641` — 게이트가 「숨김」이었다 |
| 🔒 배지 없는 정답 코드 20개 걸쇠 | backend-engineer | **DONE** `4d24a9ac` — 회귀 시험 둘 |
...
> ⭐ **단위는 quest 다.** 목표는 여전히 **모든 USACO 와 MCC**.
> 오늘 새로 연 축: **「과정을 보여주는 시뮬이 없는 quest」**.
> 전수 재측정(검사기 고친 뒤) = **A 95 · B 71 · C 4 · 판정보류 10**.
> `moolang` 은 C 가 아니었다 — 검사기가 공유 훅(`useTraceStep`)을 못 봐서
> 「시뮬 없음」으로 나왔다. 고쳤다(`63267706`). **목록이 줄어든 쪽이다.**

| 무엇 | 상태 |
|---|---|
| `mcc20citytour` — 검토 넷 반영 (스포일러·29→20걸음·이웃 네 방향·말풍선 가르기) | DONE `79b964fc` |
...
| `walkhome` **프리셋 B(3×3)** | DONE `0e1b686b` — 2×2 엔 없던 **진짜 안쪽 칸**이 여기서 처음 나온다. DP 값을 나·PM 둘이 따로 검산 |
...
| `swaptowin` **과정 시뮬** | DONE(판정) `be9e4dec` `abe96249` — `/decide` 3라운드 **만장일치로 「안 만든다」**. 대신 **글에 중간 상태**를 박았고 닫는 학생이 *«되돌아가지 않고 따라갈 수 있었다»* 로 통과. 「조용히 낡는다」는 `check-quest-samples.mjs` 등록으로 막았다 |
...
| ⭐ 시뮬 진행이 탭 전환으로 리셋 | DONE(판정) — 프리셋 왕복은 `0d457941` 로 고쳤고, 탭 왕복은 위 줄에서 진행 중 |
| 「20걸음 피로」 — **리셋과 별개 신호** | BLOCKED (설계 — 걸음 수를 줄이는 건 `feedback_sim_design_needs_a_pass` 절차가 먼저다) |

### 축 현황 — **C 4 → 1** (2026-09-26 재측정)
A **98** · B 71 · **C 1**(`swaptowin` 하나만 남았고 그건 BLOCKED) · 판정보류 10.
오늘 셋을 넣었다 — `walkhome`(A 프리셋) · `feedcows` · `photoshoot`.
⭐ **셋 다 만들기 전에 예제를 🔒 정답 코드로 돌려 검증했다.** 둘은 통과, `swaptowin` 은 떨어졌다.
   그 검증이 없었으면 `swaptowin` 시뮬은 **틀린 길을 가르치며 완성**됐을 것이다.
   (`photoshoot` 이 정확히 그 사고의 잔해였다 — 다른 문제의 알고리즘을 그리다 통째로 버려졌다.)

### 오늘의 함정 — 적어 둔다
- **걸음 수 상한**이 아직 안 정해졌다. pedagogy 는 12, `citytour` 는 20(+「끝까지 ▶▶」).
  ux 판정이 오면 **그 수를 여기 박고** 새 시뮬 넷이 다 따른다.
- `check-quest-has-process-sim.py` 의 「라벨은 sim 인데 실제는 즉시-결과형」 **12개**는
  아직 안 건드렸다 — C 4개가 끝나면 그쪽이 다음 축이다.

### 배포
오늘(09-26) 몫은 **이미 썼다.** 미푸시가 9개다 — 내일 창에 한 번에.
**커밋은 계속 쌓는다. 문제는 푸시 횟수지 커밋 수가 아니다.**
