...
import { BuyMilkProgressiveCode, downloadBuyMilkPDF, getBuyMilkSections } from "./components";
import { makeBuyMilkCh1, makeBuyMilkCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";

const A = "#d97706";

export default function BuyMilkApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeBuyMilkCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeBuyMilkCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeBuyMilkCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeBuyMilkCh1, 1: (e) => makeBuyMilkCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <BuyMilkProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <BuyMilkProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadBuyMilkPDF(E, getBuyMilkSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`buymilk-${tab}-${cur}-${lang}`} text={step.narr} />}
