...
import { ProductivityProgressiveCode, downloadProductivityPDF, getProductivitySections, getProductivityWalk, ProductivitySim } from "./components";
import { makeProdCh1, makeProdCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";

const A = "#f97316";

export default function ProductivityApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeProdCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeProdCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeProdCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeProdCh1, 1: (e) => makeProdCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "productivity-walk") return <CodeWalk E={E} lang={codeLang} {...getProductivityWalk(E, codeLang)} accent={A} />;
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "productivity-walk") return <CodeWalk E={E} lang={codeLang} {...getProductivityWalk(E, codeLang)} accent={A} />;
  ...
  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadProductivityPDF(E, getProductivitySections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`prod-${tab}-${cur}-${lang}`} text={step.narr} />}
