...
export default function BillboardApp(props = {}) {
  ...
  const [ch3Q, setCh3Q] = useState(() => makeBillboardCh3(false));

  const TABS = E
    ? ["📋 Problem", "🔍 Overlap", "⚡ Code"]
    : ["📋 문제", "🔍 겹침", "⚡ 코드"];

  const setters = { 0: setCh1Q, 1: setCh2Q, 2: setCh3Q };
  const states  = { 0: ch1Q,    1: ch2Q,    2: ch3Q };
  const makers  = { 0: makeBillboardCh1, 1: makeBillboardCh2, 2: makeBillboardCh3 };
  ...
  return (
    <div style={{ maxWidth: "min(820px, 100%)", margin: "0 auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
        <h1 style={{ fontSize: 16, fontWeight: 800, color: A, margin: 0, fontFamily: "'Jua',sans-serif" }}>🪧 Blocked Billboard</h1>
      ...
      {step.narr && <Narration key={`bbd-${tab}-${cur}-${lang}`} text={step.narr} />}
