...
/**
 * 정렬문제 풀이 — /algo/unionfind/practice
 *
...
const SOLVED_KEY = "algo-unionfind-contest-solved"
...
export default function SortingPracticePage() {
  ...
  const problem = problemId ? unionFindContestCluster.problems.find(p => p.id === problemId) : null
  ...
  // 단일 문제 풀이 화면
  if (problem) {
    const problemIdx = unionFindContestCluster.problems.findIndex(p => p.id === problem.id) + 1
    return (
      <div className="min-h-screen bg-gray-50 pb-24">
        ...
        <main className="max-w-5xl mx-auto px-4 pt-4">
          <div className="mb-3 flex items-center justify-between gap-2">
            <button
              onClick={() => router.push("/algo/unionfind")}
        ...
        <LessonPanel open={showLesson} onClose={() => setShowLesson(false)} title={t("유니온 파인드 수업", "Union Find lesson")}>
          <UnionFindLesson />
        </LessonPanel>
        <BottomNav />
      </div>
  ...
  // 리스트 화면
  const totalSolved = unionFindContestCluster.problems.filter(p => solvedSet.has(p.id)).length
  const total = unionFindContestCluster.problems.length
      ...
      <main className="max-w-2xl mx-auto px-4 pt-4">
        <div className="mb-4">
          <JourneyBreadcrumb items={[
            ...
            { label: "유니온 파인드", labelEn: "Union Find", href: "/algo/unionfind", emoji: "📊" },
            { label: "문제 풀이", labelEn: "Practice" },
          ]} />
          <div className="flex items-center gap-2 mb-2">
            <span className="text-3xl">{unionFindContestCluster.emoji}</span>
            <h1 className="text-xl sm:text-2xl font-black text-gray-900">
              {t(unionFindContestCluster.title, unionFindContestCluster.en?.title ?? unionFindContestCluster.title)}
            </h1>
          </div>
          <p className="text-sm text-gray-600 mb-3">
            {t(unionFindContestCluster.description, unionFindContestCluster.en?.description ?? unionFindContestCluster.description)}
          </p>
        ...
        {(() => {
          const warmup = unionFindContestCluster.problems.filter(p => p.difficulty === "쉬움")
          const easy = unionFindContestCluster.problems.filter(p => p.difficulty === "보통")
          const hard = unionFindContestCluster.problems.filter(p => p.difficulty === "어려움")
          const renderItem = (p: any) => {
            const isSolved = solvedSet.has(p.id)
            const globalIdx = unionFindContestCluster.problems.findIndex(pp => pp.id === p.id) + 1
            return (
              <Link
                key={p.id}
                href={`/algo/unionfind/practice?p=${p.id}`}
                className={cn(
