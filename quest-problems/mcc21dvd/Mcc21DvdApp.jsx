...
import { Mcc21DvdProgressiveCode, Mcc21DvdBounceSim, downloadMcc21DvdPDF, getMcc21DvdSections } from "./components";
...
export default function Mcc21DvdApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeMcc21DvdCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeMcc21DvdCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeMcc21DvdCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeMcc21DvdCh1, 1: (e) => makeMcc21DvdCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <Mcc21DvdProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "sim") return <Mcc21DvdBounceSim E={E} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <Mcc21DvdProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "sim") return <Mcc21DvdBounceSim E={E} />;
    return null;
  };
  ...
  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      ...
      <button onClick={() => downloadMcc21DvdPDF(E, getMcc21DvdSections(E), codeLang)} style={{
