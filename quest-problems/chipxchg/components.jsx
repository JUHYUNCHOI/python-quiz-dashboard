...
export function getChipXchgWalk(E, lang = "py") {
  ...
  if (lang === "cpp") {
    const code = [
      ...
      "    ll deficit = (fA - init) - 1;       // " + c("shortfall − 1 (last red added back at +1)", "부족분 − 1 (마지막 1개는 아래 +1)"),
      "    ll waste = (cB - 1) - (B % cB);     // " + c("trickster step 1: waste blue to the max", "심술쟁이 1: 파랑을 최대로 버림 (자투리 cB−1)"),
      "    ll need;",
      "    if (cA >= cB) need = deficit;                        // " + c("swap pays → red (1 each)", "환전 이득 → 빨강으로 (1개씩)"),
      "    else need = (deficit / cA) * cB + (deficit % cA);    // " + c("swap loses → blue in groups", "환전 손해 → 파랑 그룹으로"),
      "    return waste + need + 1;            // " + c("wasted + filled + last red", "버린 + 채운 + 마지막 1개"),
      "}",
      "",
      "int main() {",
      "    int T; cin >> T;",
  ...
  const code = [
    ...
    "    deficit = (fA - init) - 1         # " + c("shortfall − 1 (last red added back at +1)", "부족분 − 1 (마지막 1개는 아래 +1)"),
