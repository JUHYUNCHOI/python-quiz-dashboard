...
const FULL_CPP = [
  ...
  "    cin >> N;",
  "    vector<int> g(N), s(N);",
  "    for (int i = 0; i < N; i++) cin >> g[i] >> s[i];",
  "",
  "    // Try each cow having b items: profit = sum(g_i - max(0, s_i - b)) for b chosen items",
  ...
  "    long long total = 0;",
  "    for (int i = 0; i < N; i++) total += min(g[i], s[i]);",
  "    cout << total << \"\n\";",
  "    return 0;",
  "}",
];
...
export function GiftsProgressiveCode({ E, lang = "py", sections }) {
