...
import { SqPastureProgressiveCode, downloadSqPasturePDF, getSqPastureSections, getSqPastureWalk } from "./components";
import { makeSqPastureCh1, makeSqPastureCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#d97706";

export default function SqPastureApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeSqPastureCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeSqPastureCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeSqPastureCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeSqPastureCh1, 1: (e) => makeSqPastureCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <SqPastureProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "sqp-codewalk") return <CodeWalk E={E} lang={codeLang} {...getSqPastureWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <SqPastureProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "sqp-codewalk") return <CodeWalk E={E} lang={codeLang} {...getSqPastureWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadSqPasturePDF(E, getSqPastureSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`sqpa-${tab}-${cur}-${lang}`} text={step.narr} />}
