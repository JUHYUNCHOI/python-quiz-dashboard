...
/**
 * 정렬문제 풀이 — /algo/backtracking/practice
 *
...
const SOLVED_KEY = "algo-backtracking-contest-solved"
...
export default function SortingPracticePage() {
  ...
  const problem = problemId ? backtrackingContestCluster.problems.find(p => p.id === problemId) : null
  ...
  // 단일 문제 풀이 화면
  if (problem) {
    const problemIdx = backtrackingContestCluster.problems.findIndex(p => p.id === problem.id) + 1
    return (
      <div className="min-h-screen bg-gray-50 pb-24">
        ...
        <main className="max-w-5xl mx-auto px-4 pt-4">
          <div className="mb-3 flex items-center justify-between gap-2">
            <button
              onClick={() => router.push("/algo/backtracking")}
        ...
        <LessonPanel open={showLesson} onClose={() => setShowLesson(false)} title={t("백트래킹 수업", "Backtracking lesson")}>
          <BacktrackingLesson />
        </LessonPanel>
        <BottomNav />
      </div>
  ...
  // 리스트 화면
  const totalSolved = backtrackingContestCluster.problems.filter(p => solvedSet.has(p.id)).length
  const total = backtrackingContestCluster.problems.length
      ...
      <main className="max-w-2xl mx-auto px-4 pt-4">
        <div className="mb-4">
          <JourneyBreadcrumb items={[
            ...
            { label: "백트래킹", labelEn: "Backtracking", href: "/algo/backtracking", emoji: "📊" },
            { label: "문제 풀이", labelEn: "Practice" },
          ]} />
          <div className="flex items-center gap-2 mb-2">
            <span className="text-3xl">{backtrackingContestCluster.emoji}</span>
            <h1 className="text-xl sm:text-2xl font-black text-gray-900">
              {t(backtrackingContestCluster.title, backtrackingContestCluster.en?.title ?? backtrackingContestCluster.title)}
            </h1>
          </div>
          <p className="text-sm text-gray-600 mb-3">
            {t(backtrackingContestCluster.description, backtrackingContestCluster.en?.description ?? backtrackingContestCluster.description)}
          </p>
        ...
        {(() => {
          const warmup = backtrackingContestCluster.problems.filter(p => p.difficulty === "쉬움")
          const easy = backtrackingContestCluster.problems.filter(p => p.difficulty === "보통")
          const hard = backtrackingContestCluster.problems.filter(p => p.difficulty === "어려움")
          const renderItem = (p: any) => {
            const isSolved = solvedSet.has(p.id)
            const globalIdx = backtrackingContestCluster.problems.findIndex(pp => pp.id === p.id) + 1
            return (
              <Link
                key={p.id}
                href={`/algo/backtracking/practice?p=${p.id}`}
                className={cn(
