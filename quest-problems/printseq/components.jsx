...
export function getPrintseqWalk(E, lang = "py") {
  ...
  if (lang === "cpp") {
    return {
      ...
      marks: [
        ...
        { from: 94, to: 95, color: "#818cf8", ko: "↺ 재귀!", en: "↺ recursion!" },
      ],
      beats: [
        { hi: [105, 114], bubble: t(E,
        ...
        { hi: [74, 77], bubble: t(E,
          "Trick ①: all same? (helper checks) → one PRINT wrapped in REP → true. Done!",
          "요령 ①: 다 같아? (도우미가 검사) → PRINT 1 개 + REP → true. 끝!") },
        { hi: [79, 89], bubble: t(E,
          "Trick ②: a repeated block (is_repeat_of checks the shape)?\nThen ↺ can(block, budget) — the SAME question, smaller. That's the recursion!\nWorks → return true on the spot.",
          "요령 ②: 블록의 반복이면 (모양은 is_repeat_of 가 검사)\n↺ can(블록, 예산) — 같은 질문, 더 작게. 이게 재귀!\n되면 그 자리서 return true.") },
        { hi: [91, 100], bubble: t(E,
          "Trick ③: cut in two, split the budget —\n↺ can(left) AND ↺ can(right): recursion twice!\nBoth yes → return true. Try every cut × every split.",
          "요령 ③: 둘로 자르고 예산도 나눠서 —\n↺ can(왼쪽) 그리고 ↺ can(오른쪽): 재귀 두 번!\n둘 다 되면 → return true. 모든 자르기 × 나누기 시도.") },
        { hi: [102, 102], bubble: t(E,
  ...
  return {
    ...
    marks: [
      ...
      { from: 46, to: 46, color: "#818cf8", ko: "↺ 재귀!", en: "↺ recursion!" },
      { from: 52, to: 53, color: "#818cf8", ko: "↺ 재귀!", en: "↺ recursion!" },
    ],
    beats: [
      { hi: [0, 0], bubble: t(E,
      ...
      { hi: [37, 39], bubble: t(E,
        ...
        "요령 ①: 다 같아? (도우미가 검사) → PRINT 1 개 + REP → True. 끝!") },
      { hi: [41, 47], bubble: t(E,
        "Trick ②: a repeated block (is_repeat_of checks the shape)?\nThen ↺ can(seq[:block_len], budget) — the SAME question, smaller. Recursion!\nWorks → return True on the spot.",
        "요령 ②: 블록의 반복이면 (모양은 is_repeat_of 가 검사)\n↺ can(seq[:block_len], budget) — 같은 질문, 더 작게. 재귀!\n되면 그 자리서 return True.") },
      { hi: [49, 54], bubble: t(E,
        "Trick ③: cut in two, split the budget —\n↺ can(left) AND ↺ can(right): recursion twice!\nBoth yes → return True. Try every cut × every split.",
        "요령 ③: 둘로 자르고 예산도 나눠서 —\n↺ can(왼쪽) 그리고 ↺ can(오른쪽): 재귀 두 번!\n둘 다 되면 → return True. 모든 자르기 × 나누기 시도.") },
      { hi: [56, 56], bubble: t(E,
