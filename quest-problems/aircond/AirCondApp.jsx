...
import { AirCondProgressiveCode, downloadAirCondPDF, getAirCondSections, getAirCondWalk } from "./components";
import { makeAirCondCh1, makeAirCondCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";

const A = "#2563eb";

export default function AirCondApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeAirCondCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeAirCondCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeAirCondCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeAirCondCh1, 1: (e) => makeAirCondCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <AirCondProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getAirCondWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <AirCondProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getAirCondWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadAirCondPDF(E, getAirCondSections(E), codeLang)} style={{
