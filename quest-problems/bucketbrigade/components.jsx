// 🔒 USACO_VERIFIED — cpid=939, bucketbrigade (2019 Open Bronze #1)
...
const FULL_PY = [
  ...
  "base = abs(barn_r - lake_r) + abs(barn_c - lake_c) - 1",
  "answer = base",
  "# 같은 행: L, B, R 모두 같은 행이고 R 이 L 과 B 사이에 있음",
  "if barn_r == lake_r and rock_r == barn_r:",
  "    if min(lake_c, barn_c) < rock_c < max(lake_c, barn_c):",
  "        answer = base + 2",
  "# 같은 열",
  "if barn_c == lake_c and rock_c == barn_c:",
  "    if min(lake_r, barn_r) < rock_r < max(lake_r, barn_r):",
  "        answer = base + 2",
  "",
  "with open('buckets.out', 'w') as file:",
...
const FULL_CPP = [
  ...
  "    }",
  "    // 10x10 grid 에서 B (barn), L (lake), R (rock) 위치 찾기",
  "    int barn_r = 0, barn_c = 0;",
  "    int lake_r = 0, lake_c = 0;",
  "    int rock_r = 0, rock_c = 0;",
  "    for (int r = 0; r < 10; r++) {",
  "        for (int c = 0; c < 10; c++) {",
  "            if (grid[r][c] == 'B') { barn_r = r; barn_c = c; }",
  "            else if (grid[r][c] == 'L') { lake_r = r; lake_c = c; }",
  "            else if (grid[r][c] == 'R') { rock_r = r; rock_c = c; }",
  "        }",
  "    }",
  "    // 최단 거리 = 맨해튼 - 1 (양 끝 제외)",
  "    // rock 이 L 과 B 의 같은 행/열에 끼어 있으면 우회 → +2",
  "    int base = abs_int(barn_r - lake_r) + abs_int(barn_c - lake_c) - 1;",
  "    int answer = base;",
  "    // 같은 행",
  "    if (barn_r == lake_r && rock_r == barn_r) {",
  "        int lo = lake_c < barn_c ? lake_c : barn_c;",
  "        int hi = lake_c > barn_c ? lake_c : barn_c;",
  "        if (lo < rock_c && rock_c < hi) answer = base + 2;",
  "    }",
  "    // 같은 열",
  "    if (barn_c == lake_c && rock_c == barn_c) {",
  "        int lo = lake_r < barn_r ? lake_r : barn_r;",
  "        int hi = lake_r > barn_r ? lake_r : barn_r;",
  "        if (lo < rock_r && rock_r < hi) answer = base + 2;",
  "    }",
  "    fout << answer << \"\\n\";",
  "    return 0;",
  "}",
