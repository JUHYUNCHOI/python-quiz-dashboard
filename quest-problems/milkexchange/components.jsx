...
const FULL_CPP = [
  ...
  "int main() {",
  "    int N, M; cin >> N >> M;",
  "    vector<long long> cap(N), milk(N);",
  "    for (auto& x : cap) cin >> x;",
  "    for (auto& x : milk) cin >> x;",
  "    vector<char> dir(N);",
  "    for (auto& x : dir) cin >> x;",
  "    int steps = min((long long)M, 2LL * N);",
  "    while (steps--) {",
  "        vector<long long> nm(N, 0);",
  "        for (int i = 0; i < N; i++) {",
  "            long long give = min(1LL, milk[i]);",
  "            int target = (dir[i] == 'L') ? (i - 1 + N) % N : (i + 1) % N;",
  "            nm[i] += milk[i] - give;",
  "            nm[target] += give;",
  "        }",
  "        for (int i = 0; i < N; i++) milk[i] = min(nm[i], cap[i]);",
  "    }",
  "    for (int i = 0; i < N; i++) cout << milk[i] << \"\n\";",
  "    return 0;",
  "}",
];
