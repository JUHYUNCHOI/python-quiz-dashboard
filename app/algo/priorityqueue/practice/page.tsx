...
/**
 * 정렬문제 풀이 — /algo/priorityqueue/practice
 *
...
const SOLVED_KEY = "algo-priorityqueue-contest-solved"
...
export default function SortingPracticePage() {
  ...
  const problem = problemId ? priorityQueueContestCluster.problems.find(p => p.id === problemId) : null
  ...
  // 단일 문제 풀이 화면
  if (problem) {
    const problemIdx = priorityQueueContestCluster.problems.findIndex(p => p.id === problem.id) + 1
    return (
      <div className="min-h-screen bg-gray-50 pb-24">
        ...
        <main className="max-w-5xl mx-auto px-4 pt-4">
          <div className="mb-3 flex items-center justify-between gap-2">
            <button
              onClick={() => router.push("/algo/priorityqueue")}
        ...
        <LessonPanel open={showLesson} onClose={() => setShowLesson(false)} title={t("우선순위 큐 수업", "Priority Queue lesson")}>
          <PriorityQueueLesson />
        </LessonPanel>
        <BottomNav />
      </div>
  ...
  // 리스트 화면
  const totalSolved = priorityQueueContestCluster.problems.filter(p => solvedSet.has(p.id)).length
  const total = priorityQueueContestCluster.problems.length
      ...
      <main className="max-w-2xl mx-auto px-4 pt-4">
        <div className="mb-4">
          <JourneyBreadcrumb items={[
            ...
            { label: "우선순위 큐", labelEn: "Priority Queue", href: "/algo/priorityqueue", emoji: "📊" },
            { label: "문제 풀이", labelEn: "Practice" },
          ]} />
          <div className="flex items-center gap-2 mb-2">
            <span className="text-3xl">{priorityQueueContestCluster.emoji}</span>
            <h1 className="text-xl sm:text-2xl font-black text-gray-900">
              {t(priorityQueueContestCluster.title, priorityQueueContestCluster.en?.title ?? priorityQueueContestCluster.title)}
            </h1>
          </div>
          <p className="text-sm text-gray-600 mb-3">
            {t(priorityQueueContestCluster.description, priorityQueueContestCluster.en?.description ?? priorityQueueContestCluster.description)}
          </p>
        ...
        {(() => {
          const warmup = priorityQueueContestCluster.problems.filter(p => p.difficulty === "쉬움")
          const easy = priorityQueueContestCluster.problems.filter(p => p.difficulty === "보통")
          const hard = priorityQueueContestCluster.problems.filter(p => p.difficulty === "어려움")
          const renderItem = (p: any) => {
            const isSolved = solvedSet.has(p.id)
            const globalIdx = priorityQueueContestCluster.problems.findIndex(pp => pp.id === p.id) + 1
            return (
              <Link
                key={p.id}
                href={`/algo/priorityqueue/practice?p=${p.id}`}
                className={cn(
