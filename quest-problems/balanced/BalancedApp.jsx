...
export default function BalancedApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeBalancedCh1(false));
  const [ch2Q, setCh2Q] = useState(() => makeBalancedCh2(false));
  const [ch3Q, setCh3Q] = useState(() => makeBalancedCh3(false, "py"));

  useEffect(() => {
    setCh3Q(prev => makeBalancedCh3(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeBalancedCh1, 1: makeBalancedCh2, 2: (e) => makeBalancedCh3(e, codeLang) };
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(1300px, 100%)", margin: "0 auto", padding: "0 12px" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 12, alignItems: "center" }}>
            ...
            {TABS.map((label, i) => {
              ...
              return (
                <button key={i} onClick={() => changeTab(i)} style={{
                  ...
                  background: isCurrent ? A : (isVisited ? "#fff7ed" : "transparent"),
                  border: `1.5px solid ${isCurrent ? A : (isVisited ? "#fdba74" : C.border)}`,
                  color: isCurrent ? "#fff" : (isVisited ? "#9a3412" : C.dim),
          ...
          {showCodeControls && (
            <div style={{ display: "flex", gap: 0, alignItems: "stretch", flexShrink: 0 }}>
              ...
              <button onClick={() => downloadBalancedPDF(E, getBalancedSections(E), codeLang)} style={{
        ...
        {step.narr && <Narration key={`bal-${tab}-${cur}-${lang}`} text={step.narr} />}
