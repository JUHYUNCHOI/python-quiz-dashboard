...
import { ClockFenceProgressiveCode, downloadClockFencePDF, getClockFenceSections, getClockFenceWalk } from "./components";
import { makeClockCh1, makeClockCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#8b5cf6";

export default function ClockFenceApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeClockCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeClockCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeClockCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeClockCh1, 1: (e) => makeClockCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <ClockFenceProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "clockfence-walk") return <CodeWalk E={E} lang={codeLang} {...getClockFenceWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <ClockFenceProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "clockfence-walk") return <CodeWalk E={E} lang={codeLang} {...getClockFenceWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadClockFencePDF(E, getClockFenceSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`cloc-${tab}-${cur}-${lang}`} text={step.narr} />}
