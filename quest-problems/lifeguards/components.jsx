// 🔒 USACO_VERIFIED — cpid=784, lifeguards (2018 Jan Bronze #2)
...
const FULL_PY = [
  ...
  "with open('lifeguards.in', 'r') as file:",
  ...
  "        best = cov",
  "",
  "with open('lifeguards.out', 'w') as file:",
  "    file.write(str(best) + '\\n')",
];
...
const FULL_CPP = [
  ...
  "        for (int i = 0; i < N; i++) {",
  "            if (i == skip) continue;",
  "            events.push_back(make_pair(starts[i], 1));",
  "            events.push_back(make_pair(ends[i], -1));",
  "        }",
  "        sort(events.begin(), events.end());",
  "        int cov = 0, active = 0, prev = 0;",
  "        for (int k = 0; k < (int)events.size(); k++) {",
  "            int time = events[k].first;",
  "            int delta = events[k].second;",
  "            if (active > 0) cov += time - prev;",
  "            active += delta;",
  "            prev = time;",
  "        }",
...
export function getLifeguardsSections(E) {
  return [
    {
...
export function LifeguardsProgressiveCode(props) {
...
export function downloadLifeguardsPDF(E, sections, lang = "py") {
  const win = window.open("", "_blank");
  if (!win) { alert(t(E, "Pop-up blocked.", "팝업 차단됨.")); return; }
  const esc = (s) => String(s).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
  const langLabel = lang === "py" ? "🐍 Python" : "💻 C++";
  const fileTitle = t(E, "Lifeguards — Full Study Guide", "Lifeguards — 종합 풀이 노트");
