...
import { GiftsProgressiveCode, downloadGiftsPDF, getGiftsSections } from "./components";
import { makeGiftsCh1, makeGiftsCh2 } from "./chapters";
...
export default function GiftsApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeGiftsCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeGiftsCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeGiftsCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeGiftsCh1, 1: (e) => makeGiftsCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <GiftsProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <GiftsProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadGiftsPDF(E, getGiftsSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`gift-${tab}-${cur}-${lang}`} text={step.narr} />}
