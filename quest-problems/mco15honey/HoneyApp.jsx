...
import { HoneyProgressiveCode, HoneySim, downloadHoneyPDF, getHoneySections, getHoneyWalk } from "./components";
import { makeHoneyCh1, makeHoneyCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";

const A = "#d97706";

export default function HoneyApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeHoneyCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeHoneyCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeHoneyCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeHoneyCh1, 1: (e) => makeHoneyCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <HoneyProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getHoneyWalk(E, codeLang)} accent={A} />;
    if (step.type === "sim") return <HoneySim E={E} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <HoneyProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getHoneyWalk(E, codeLang)} accent={A} />;
    if (s.type === "sim") return <HoneySim E={E} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadHoneyPDF(E, getHoneySections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`hone-${tab}-${cur}-${lang}`} text={step.narr} />}
