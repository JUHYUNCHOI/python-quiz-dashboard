...
**IF...ELSE**는 둘 중 하나는 **반드시** 실행돼요!`
        },
        {
          id: "ch2-mod",
          type: "explain",
          title: "🔢 MOD와 DIV — 나눗셈의 두 가지 결과",
          content: `조건문에서 자주 쓰이는 **두 가지 산술 연산자**를 먼저 알아봐요!

**나눗셈을 하면 두 가지 결과가 나와요:**

\`\`\`
17 ÷ 5 = 3 ... 나머지 2
\`\`\`

{!blue} **DIV** = 나눗셈의 **몫** (정수 부분만!)
{!pink} **MOD** = 나눗셈의 **나머지**

| 연산 | 의미 | 예시 | 결과 |
|------|------|------|------|
| \`17 DIV 5\` | 몫 | 17 ÷ 5의 정수 부분 | **3** |
| \`17 MOD 5\` | 나머지 | 17 ÷ 5의 나머지 | **2** |
| \`10 DIV 3\` | 몫 | 10 ÷ 3의 정수 부분 | **3** |
| \`10 MOD 3\` | 나머지 | 10 ÷ 3의 나머지 | **1** |

### 💡 MOD는 언제 쓸까?

{!green} **짝수/홀수 판별**: \`number MOD 2\` → 0이면 짝수, 1이면 홀수!
{!orange} **배수 확인**: \`number MOD 3 = 0\`이면 3의 배수!
...
@핵심: **DIV = 몫, MOD = 나머지**. IGCSE 시험에 자주 나와요!`
        },
        {
          id: "ch2-predict1",
          type: "predict",
