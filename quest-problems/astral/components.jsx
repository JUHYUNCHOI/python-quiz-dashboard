...
const AST_S3_PY = [
  ...
  "        break",
  "    for c0 in range(N):",
  "        if visited[r0][c0]:       # 이미 처리한 칸이면 패스",
  "            continue",
...
const AST_GREEDY_CORE_CPP = [
  ...
  "                } else {",
  "                    impossible = true;   // 직전 W → 못 만듦",
  "                    break;",
  "                }",
  "            } else {",
  "                impossible = true;   // 직전 밖 → 못 만듦",
  "                break;",
  "            }",
  "        }",
  "        else if (star == 'G') {      // 한 사진에만 별",
  "            if (possibles.count({r, c})) {",
  "                continue;   // 이미 별 → 끝",
  "            }",
...
// English-comment display copies of the verified backward-greedy code.
...
const AST_GREEDY_FULL_PY_EN = [
  ...
  "        impossible = False",
  "        for r in range(N - 1, -1, -1):       # scan from the end, backward",
  "            if impossible:",
  "                break",
  "            for c in range(N - 1, -1, -1):",
  "                star = grid[r][c]",
  "                if star == 'B':              # star in BOTH photos",
  "                    possibles.add((r, c))",
  "                    if r - down >= 0 and c - right >= 0:",
  "                        aa = grid[r - down][c - right]",
  "                        if aa == 'G' or aa == 'B':",
  ...
  "                        impossible = True",
  "                        break",
  "                elif star == 'G':            # star in ONE photo only",
  "                    if (r, c) in possibles:  # already a star here",
  "                        continue",
  "                    if r - down < 0 or c - right < 0:",
  "                        possibles.add((r, c))",
  "                    elif grid[r - down][c - right] in ('G', 'B'):",
...
const AST_GREEDY_FULL_CPP_EN = [
  ...
  "    cin >> T;",
  "    for (int t = 0; t < T; t++) {",
  "        int N, right, down;   // right = shift right, down = shift down",
  "        cin >> N >> right >> down;",
  "        vector<string> grid(N);",
  "        for (int r = 0; r < N; r++) {",
  "            cin >> grid[r];",
  "        }",
  "",
  "        set<pair<int,int>> possibles;   // cells that must have had a star",
  "        bool impossible = false;",
  "        for (int r = N - 1; r >= 0 && !impossible; r--) {",
  "            for (int c = N - 1; c >= 0; c--) {",
  "                char star = grid[r][c];",
  ...
  "                    }",
  "                }",
  "                else if (star == 'G') {         // star in ONE photo only",
  "                    if (possibles.count({r, c})) {",
  "                        continue;",
  "                    }",
  "                    if (r - down < 0 || c - right < 0)",
...
// getAstralWalk — CodeWalk 재료(코드 + 변수 범례 + beats). 검증된 뒤→앞 그리디를
// '짜는 순서대로 한 줄씩 밝히며' 걷기 (선생님 2026-07-16: astral 에 이게 없었음).
// 🔒 코드는 검증본 그대로(comment 만 언어별) — 로직 안 건드림.
export function getAstralWalk(E, lang = "py") {
  const vars = [
    { v: "grid", ko: "지금 격자 (W/G/B)", en: "the grid (W/G/B)" },
    { v: "right·down", ko: "별 이동 (오른·아래)", en: "star move (right·down)" },
    { v: "possibles", ko: "원래 별 있어야 하는 칸", en: "cells that must be stars" },
    { v: "impossible", ko: "불가능 표시", en: "impossible flag" },
  ];
  if (lang === "cpp") {
    return {
      code: E ? AST_GREEDY_FULL_CPP_EN : AST_GREEDY_FULL_CPP,
      vars,
      beats: [
        { hi: [6, 15], bubble: t(E,
