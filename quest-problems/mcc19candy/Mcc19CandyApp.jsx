...
import { Mcc19CandyProgressiveCode, downloadMcc19CandyPDF, getMcc19CandySections } from "./components";
import { makeMcc19CandyCh1, makeMcc19CandyCh2 } from "./chapters";
...
export default function Mcc19CandyApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeMcc19CandyCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeMcc19CandyCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeMcc19CandyCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeMcc19CandyCh1, 1: (e) => makeMcc19CandyCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <Mcc19CandyProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <Mcc19CandyProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadMcc19CandyPDF(E, getMcc19CandySections(E), codeLang)} style={{
