...
const FULL_CPP = [
  ...
  "        // positions.extend(range(a, b + 1))",
  "",
  "    auto occupied = [positions[i] for i in range(len(positions)) if i < M];",
  "    // Actually: read occupied positions separately",
  "",
  "    // Binary search on minimum distance D",
  "    auto can_place = [&](D, positions, occupied, cows_to_place) {   // TODO: type args",
  "        auto all_pos = sorted(set(positions));",
  "        auto occ = set(occupied);",
  "        auto placed = sorted(occupied);",
  "        auto need = cows_to_place;",
  "        // for p in all_pos:",
  "            if (p in occ) {",
  "                continue;",
  "            // Check if p is at least D from all placed",
...
export function SocDist1ProgressiveCode({ E, lang = "py", sections }) {
