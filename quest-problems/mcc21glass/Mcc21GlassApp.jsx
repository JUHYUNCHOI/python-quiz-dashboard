...
import { Mcc21GlassProgressiveCode, downloadMcc21GlassPDF, getMcc21GlassSections } from "./components";
...
export default function Mcc21GlassApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeMcc21GlassCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeMcc21GlassCh2(lang === "en"));
  const [ch3Q, setCh3Q] = useState(() => makeMcc21GlassCh3(lang === "en", "py"));

  useEffect(() => {
    setCh3Q(prev => makeMcc21GlassCh3(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeMcc21GlassCh1, 1: makeMcc21GlassCh2, 2: (e) => makeMcc21GlassCh3(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <Mcc21GlassProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <Mcc21GlassProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadMcc21GlassPDF(E, getMcc21GlassSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`mcc21gla-${tab}-${cur}-${lang}`} text={step.narr} />}
