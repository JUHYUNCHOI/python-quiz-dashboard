...
- **pair 정렬** — 두 값 묶어서 한 기준 또는 두 기준으로
...
정렬 한 번 했더니 갑자기 가능해지는 일들 — \`binary_search\`, \`lower_bound\`, \`upper_bound\`. "x 가 있나?", "x 이상 첫 거는?", "x 가 몇 개?" 모든 검색 질문을 **O(log n)** 으로 풀어요.

### 그 다음 (선택, 심화) — **sort 응용 패턴** (cpp-26)

\`sort + unique + erase\` 로 중복 제거, \`stable_sort\` 동점 보존, \`count_if\` / \`find_if\` / \`accumulate\` 같은 STL 일반 알고리즘. **Bronze 합격에 필수 아님** — 시간 여유 있을 때 봐도 OK.

---

> 💡 한 lesson 에 너무 많이 담지 않으려고 sort 마스터를 셋으로 나눴어요. **사실은 같은 흐름.** "정렬한다 → 정렬된 걸로 빠르게 검색한다 → 정렬 응용한다." 차근차근 가도 충분해요.`
        }
      ]
    }
