...
export default function ShellGameApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeShellCh1(false));
  const [ch2Q, setCh2Q] = useState(() => makeShellCh2(false));
  const [ch3Q, setCh3Q] = useState(() => makeShellCh3(false, "py"));

  useEffect(() => {
    setCh3Q(prev => makeShellCh3(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeShellCh1, 1: makeShellCh2, 2: (e) => makeShellCh3(e, codeLang) };
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(1300px, 100%)", margin: "0 auto", padding: "0 12px" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 12, alignItems: "center" }}>
          ...
          {showCodeControls && (
            <div style={{ display: "flex", gap: 0, alignItems: "stretch", flexShrink: 0 }}>
              ...
              <button onClick={() => downloadShellGamePDF(E, getShellGameSections(E), codeLang)} style={{
        ...
        {step.narr && <Narration key={`she-${tab}-${cur}-${lang}`} text={step.narr} />}
