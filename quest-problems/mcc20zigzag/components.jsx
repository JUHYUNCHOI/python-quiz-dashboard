...
const FULL_CPP = [
  ...
  "        // DP: dp[i][j] = number of zig-zag subsequences",
  "        // of length j ending at index i",
  "        auto dp = [[0] * (K + 1) for _ in range(N)];",
  "",
  ...
  "                for (long long p = j - 2; p < i; p++) {",
  "                    // if (j % 2 == 0 and s[p] < s[i]) or \\\\",
  "                    // (j % 2 == 1 and s[p] > s[i]):",
  "                        // dp[i][j] += dp[p][j - 1]",
  "",
  "        auto ans = sum(dp[i][K] for i in range(N));",
  "        cout << ans << \"\\n\";",
  "",
  "    return 0;",
  "}",
...
export function Mcc20ZigzagProgressiveCode({ E, lang = "py", sections }) {
