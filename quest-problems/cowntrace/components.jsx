...
const FULL_PY = [
  ...
  "with open('tracing.in', 'r') as file:",
  ...
  "                max_k = K",
  "",
  "with open('tracing.out', 'w') as file:",
  "    if max_k >= T:",
  "        file.write(str(len(valid_pz)) + ' ' + str(min_k) + ' Infinity\\n')",
  "    else:",
  "        file.write(str(len(valid_pz)) + ' ' + str(min_k) + ' ' + str(max_k) + '\\n')",
];
...
const FULL_CPP = [
  ...
  "using namespace std;",
  "",
  "struct Event { int t, a, b; };",
  "",
  "bool byTime(const Event& x, const Event& y) {",
  "    return x.t < y.t;",
  "}",
  "",
  "int main() {",
  ...
  "    set<int> infectedEnd;",
  "    for (int i = 0; i < N; i++) {",
  "        if (s[i] == '1') infectedEnd.insert(i + 1);",
  "    }",
  "",
  "    vector<Event> events(T);",
  "    for (int i = 0; i < T; i++) {",
  "        fin >> events[i].t >> events[i].a >> events[i].b;",
  "    }",
  ...
  "                int a = events[i].a;",
  "                int b = events[i].b;",
  "                bool aSick = sick.count(a) > 0;",
  "                bool bSick = sick.count(b) > 0;",
  "                if (aSick && bSick) {",
  "                    count[a]++;",
  "                    count[b]++;",
  "                } else if (aSick) {",
  ...
  "        }",
  "    }",
  "    fout << validPz.size() << \" \" << minK << \" \";",
  "    if (maxK >= T) fout << \"Infinity\";",
  "    else fout << maxK;",
  "    fout << \"\\n\";",
  "    return 0;",
  "}",
