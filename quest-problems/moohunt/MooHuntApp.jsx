...
import { MooHuntProgressiveCode, downloadMooHuntPDF, getMooHuntSections } from "./components";
import { makeMooHuntCh1, makeMooHuntCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";

const A = "#dc2626";

export default function MooHuntApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeMooHuntCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeMooHuntCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeMooHuntCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeMooHuntCh1, 1: (e) => makeMooHuntCh2(e, codeLang) };
  ...
  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadMooHuntPDF(E, getMooHuntSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`moohunt-${tab}-${cur}-${lang}`} text={step.narr} />}
