...
import { StampGridProgressiveCode, downloadStampGridPDF, getStampGridSections, getStampGridWalk } from "./components";
import { makeStampCh1, makeStampCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#059669";

export default function StampGridApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeStampCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeStampCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeStampCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeStampCh1, 1: (e) => makeStampCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <StampGridProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getStampGridWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <StampGridProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getStampGridWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadStampGridPDF(E, getStampGridSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`stam-${tab}-${cur}-${lang}`} text={step.narr} />}
