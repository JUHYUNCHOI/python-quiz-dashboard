...
import { makeCowSplitsCh1, makeCowSplitsCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";

const A = "#059669";

export default function CowSplitsApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeCowSplitsCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeCowSplitsCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeCowSplitsCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeCowSplitsCh1, 1: (e) => makeCowSplitsCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <CowSplitsProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <CowSplitsProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadCowSplitsPDF(E, getCowSplitsSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`cowsplits-${tab}-${cur}-${lang}`} text={step.narr} />}
