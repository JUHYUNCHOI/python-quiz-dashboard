...
import { BovGenomicsProgressiveCode, downloadBovGenomicsPDF, getBovGenomicsSections, getBovGenomicsWalk } from "./components";
import { makeGenomicsCh1, makeGenomicsCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#2563eb";

export default function BovGenomicsApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeGenomicsCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeGenomicsCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeGenomicsCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeGenomicsCh1, 1: (e) => makeGenomicsCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <BovGenomicsProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "bovgenomics-codewalk") return <CodeWalk E={E} lang={codeLang} {...getBovGenomicsWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <BovGenomicsProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "bovgenomics-codewalk") return <CodeWalk E={E} lang={codeLang} {...getBovGenomicsWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadBovGenomicsPDF(E, getBovGenomicsSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`bovg-${tab}-${cur}-${lang}`} text={step.narr} />}
