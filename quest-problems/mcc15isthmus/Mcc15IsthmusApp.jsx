...
import { Mcc15IsthmusProgressiveCode, downloadMcc15IsthmusPDF, getMcc15IsthmusSections, getMcc15IsthmusWalk } from "./components";
...
export default function Mcc15IsthmusApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeMcc15IsthmusCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeMcc15IsthmusCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeMcc15IsthmusCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeMcc15IsthmusCh1, 1: (e) => makeMcc15IsthmusCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <Mcc15IsthmusProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "isthmus-walk") return <CodeWalk E={E} lang={codeLang} {...getMcc15IsthmusWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <Mcc15IsthmusProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "isthmus-walk") return <CodeWalk E={E} lang={codeLang} {...getMcc15IsthmusWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadMcc15IsthmusPDF(E, getMcc15IsthmusSections(E), codeLang)} style={{
