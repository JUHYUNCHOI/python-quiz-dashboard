...
import { MeasTrafficProgressiveCode, downloadMeasTrafficPDF, getMeasTrafficSections } from "./components";
import { makeTrafficCh1, makeTrafficCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";

const A = "#8b5cf6";

export default function MeasTrafficApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeTrafficCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeTrafficCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeTrafficCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeTrafficCh1, 1: (e) => makeTrafficCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <MeasTrafficProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <MeasTrafficProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadMeasTrafficPDF(E, getMeasTrafficSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`meas-${tab}-${cur}-${lang}`} text={step.narr} />}
