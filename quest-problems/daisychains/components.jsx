...
const FULL_CPP = [
  ...
  "",
  "    long long N; cin >> N;",
  "    vector<long long> p; { long long _x; while (cin >> _x) p.push_back(_x); if (!cin) cin.clear(); } // adapt: read N values",
  "",
  "    auto count = 0;",
...
export function getDaisyChainsSections(E) {
  return [
    {
...
export function DaisyChainsProgressiveCode({ E, lang = "py", sections }) {
...
export function downloadDaisyChainsPDF(E, sections, lang = "py") {
  const win = window.open("", "_blank");
  if (!win) { alert(t(E, "Pop-up blocked.", "팝업 차단됨.")); return; }
  const esc = (s) => String(s).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
  const langLabel = lang === "py" ? "🐍 Python" : "💻 C++";
  const fileTitle = t(E, "DaisyChains — Full Study Guide", "DaisyChains — 종합 풀이 노트");
