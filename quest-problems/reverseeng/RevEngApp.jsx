...
export default function RevEngApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeRevEngCh1(false));
  const [ch2Q, setCh2Q] = useState(() => makeRevEngCh2(false, "py"));

  useEffect(() => {
    setCh2Q(prev => makeRevEngCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeRevEngCh1, 1: (e) => makeRevEngCh2(e, codeLang) };
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 12, alignItems: "center" }}>
          ...
          {showCodeControls && (
            <div style={{ display: "flex", gap: 0, alignItems: "stretch", flexShrink: 0 }}>
              ...
              <button onClick={() => downloadRevEngPDF(E, getRevEngSections(E), codeLang)} style={{
