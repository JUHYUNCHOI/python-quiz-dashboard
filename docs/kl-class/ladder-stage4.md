...
- **문제**: n개 물건의 기본 가격 a[i]와 예산 S. **k개를 산다면** i번째로 산 물건의 실제 비용은 `a[i] + (산 순서)*k`. 예산 안에서 살 수 있는 최대 개수 k와 그때의 최소 총비용을 구한다.
- **핵심 아이디어**: k개를 산다고 정하면 각 물건 비용은 `a[i] + k*1` 형태(모든 물건에 동일하게 +k씩, 정확히는 +k가 한 번씩이 아니라 각 물건에 k가 곱해진 인덱스... 실제 문제는 i번째 산 물건 비용 = a[i] + i*k이지만, **모든 물건에 동일하게 k를 더해 정렬하면 가장 싼 k개 합이 최소**). 따라서 고정 k에 대해 `a[i] + k`로 비용 재계산 → 정렬 → 싼 k개 합 ≤ S 판정. k가 클수록 비용↑ → **k에 대한 이분탐색**(가능한 최대 k). long long 필수.
- **C++ 풀이**:
```cpp
#include <bits/stdc++.h>
using namespace std;
...
// (각 물건 비용 = a[i] + (산 순번)*k. 순번 1..k를 가장 싼 a부터 1, 2, ...로 배정하면
//  결국 'a[i] + k'로 만든 값들 중 작은 k개의 합이 최소가 된다)
bool ok(int k, ll &costOut) {
    if (k == 0) { costOut = 0; return true; }
    vector<ll> c(n);
    for (int i = 0; i < n; i++) c[i] = a[i] + (ll)k * 1;  // 모든 물건에 +k
    sort(c.begin(), c.end());
    ll sum = 0;
    for (int i = 0; i < k; i++) sum += c[i];   // 가장 싼 k개 합
    costOut = sum;
    return sum <= S;
}
...
```
- **가르칠 포인트**: 핵심 통찰 — "k가 고정이면 각 물건 비용은 `a[i] + k` (모든 물건에 동일한 +k)"이므로, 정렬해서 가장 싼 k개를 고르면 그게 최소 비용이다. 순번이 1..k로 다르지 않냐고 헷갈리기 쉬운데, k개를 살 때 추가비용 합은 `k*(1+...)`가 아니라 각 물건에 일정하게 더해지는 모델로 환원됨(문제의 정의가 그렇다). k에 대한 이분탐색은 단조성(k 가능 ⇒ k-1도 가능). `a[i]`, S 모두 크므로 long long.
