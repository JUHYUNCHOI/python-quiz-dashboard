...
import { MobileGameProgressiveCode, MobileGameSim, downloadMobileGamePDF, getMobileGameSections } from "./components";
...
export default function MobileGameApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeMobileGameCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeMobileGameCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeMobileGameCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeMobileGameCh1, 1: (e) => makeMobileGameCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <MobileGameProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "sim") return <MobileGameSim E={E} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <MobileGameProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "sim") return <MobileGameSim E={E} />;
    return null;
  };
  ...
  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      ...
      <button onClick={() => downloadMobileGamePDF(E, getMobileGameSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`mobi-${tab}-${cur}-${lang}`} text={step.narr} />}
