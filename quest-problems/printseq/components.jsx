...
export function getPrintseqWalk(E, lang = "py") {
  if (lang === "cpp") {
    return {
      ...
      beats: [
        ...
        { hi: [72, 82], bubble: t(E,
          "Trick ② failed too → last resort (if (!result) again).\nTrick ③ — CUT at position `cut`: left piece + right piece, and split the budget.\n(This is the [1 1 2 2] → [1 1] + [2 2] from the plan!)",
          "요령 ②도 안 됐으니 마지막 수단 (또 if (!result)).\n요령 ③ — cut 위치에서 둘로: 왼쪽 조각 + 오른쪽 조각, 예산도 나눠요.\n(계획에서 본 [1 1 2 2] → [1 1] + [2 2] 가 이거예요!)") },
        { hi: [83, 89], bubble: t(E,
          "If the LEFT piece works AND the RIGHT piece works → the whole thing works!\nEach half is solved by can() again (recursion).\nTry every cut × every budget split until one works.",
          "왼쪽 조각도 되고 오른쪽 조각도 되면 → 전체가 돼요!\n각 반쪽은 can() 을 또 불러서 풀어요 (재귀).\n하나 될 때까지 모든 자르기 × 예산 나누기 시도.") },
        { hi: [6, 27], bubble: t(E,
  ...
  return {
    ...
    beats: [
      ...
      { hi: [47, 53], bubble: t(E,
        "Trick ② failed too → last resort (if not result again).\nTrick ③ — CUT at position `cut`: left piece + right piece, and split the budget.\n(This is the [1 1 2 2] → [1 1] + [2 2] from the plan!)",
        "요령 ②도 안 됐으니 마지막 수단 (또 if not result).\n요령 ③ — cut 위치에서 둘로: 왼쪽 조각 + 오른쪽 조각, 예산도 나눠요.\n(계획에서 본 [1 1 2 2] → [1 1] + [2 2] 가 이거예요!)") },
      { hi: [54, 59], bubble: t(E,
        "If the LEFT piece works AND the RIGHT piece works → the whole thing works!\nEach half is solved by can() again (recursion).\nTry every cut position × every budget split until one works.",
        "왼쪽 조각도 되고 오른쪽 조각도 되면 → 전체가 돼요!\n각 반쪽은 can() 을 또 불러서 풀어요 (재귀).\n하나 될 때까지 모든 자르기 × 모든 예산 나누기 시도.") },
      { hi: [2, 9], bubble: t(E,
