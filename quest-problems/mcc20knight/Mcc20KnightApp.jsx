...
import { Mcc20KnightProgressiveCode, downloadMcc20KnightPDF, getMcc20KnightSections } from "./components";
...
export default function Mcc20KnightApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeMcc20KnightCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeMcc20KnightCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeMcc20KnightCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeMcc20KnightCh1, 1: (e) => makeMcc20KnightCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <Mcc20KnightProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <Mcc20KnightProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadMcc20KnightPDF(E, getMcc20KnightSections(E), codeLang)} style={{
