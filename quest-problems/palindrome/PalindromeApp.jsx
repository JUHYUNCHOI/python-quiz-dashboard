...
export default function PalindromeApp(props = {}) {
  ...
  const [ch2Q, setCh2Q] = useState(() => makePalindromeCh2(false));
  const TABS = E ? ["📋 Problem", "⚡ Code"] : ["📋 문제", "⚡ 코드"];
  const setters = { 0: setCh1Q, 1: setCh2Q }, states = { 0: ch1Q, 1: ch2Q }, makers = { 0: makePalindromeCh1, 1: makePalindromeCh2 };
  ...
  return (
    <div style={{ maxWidth: "min(820px, 100%)", margin: "0 auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
        <h1 style={{ fontSize: 16, fontWeight: 800, color: A, margin: 0, fontFamily: "'Jua',sans-serif" }}>🎲 Palindrome Game</h1>
        <div style={{ display: "flex", gap: 2, background: C.card, borderRadius: 8, border: `1.5px solid ${C.border}`, padding: 2 }}>
          { false && [["ko", "🇰🇷"], ["en", "🇺🇸"]].map(([v, flag]) => <button key={v} onClick={() => switchLang(v)} style={{ background: lang === v ? C.accent : "transparent", border: "none", borderRadius: 6, padding: "4px 8px", cursor: "pointer", fontSize: 14, color: lang === v ? "#fff" : C.dim }}>{flag}</button>)}
        </div>
      </div>
