...
export function FunctionOrderAnimation({ lang = "ko" }: { lang?: "ko" | "en" }) {
  const isEn = lang === "en"
    ...
    <div className="bg-slate-900 rounded-2xl p-4 sm:p-5 text-white font-mono text-sm">
      <div className="flex items-center justify-between mb-3">
        <span className="text-slate-400 text-xs">
          {!isStarted
            ? (isEn ? "▶ Press button to follow the compiler reading order" : "▶ 버튼을 눌러 컴파일러가 읽는 순서를 따라가봐요")
            : showError
            ? (isEn ? "💥 Compile error!" : "💥 컴파일 에러 발생!")
            : (isEn ? `⚡ Compiler reading... (line ${currentLine + 1})` : `⚡ 컴파일러가 읽는 중... (${currentLine + 1}번째 줄)`)}
        </span>
        {isStarted && (
      ...
      {showError && (
        <div className="mb-4 rounded-xl border border-red-500/50 bg-red-950/40 p-3 text-sm">
          <p className="text-red-300 font-bold mb-1">{isEn ? "💥 Compile error!" : "💥 컴파일 에러!"}</p>
          <p className="text-slate-300 text-xs leading-relaxed">
            {isEn ? (
              <>The compiler encountered <code className="text-red-300">add(3, 5)</code> on line 5, but <code className="text-red-300">add</code> hasn't been defined yet.<br />C++ reads code <strong className="text-white">top to bottom</strong>, so functions must be declared before use!</>
            ) : (
              <>컴파일러가 5번째 줄 <code className="text-red-300">add(3, 5)</code>를 만났는데, 아직 <code className="text-red-300">add</code>가 뭔지 정의된 적이 없어요.<br />C++은 코드를 <strong className="text-white">위에서 아래로</strong> 읽기 때문에, 함수를 쓰기 전에 먼저 알려줘야 해요!</>
            )}
          </p>
        </div>
      )}
