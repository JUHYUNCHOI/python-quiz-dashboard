...
export function makeCrossRd3Ch1(E) {
  return [
    ...
    // 1-1b: 입출력 형식 + 제약 (USACO 원문) — 시즌 표준 (photoshoot25 참조)
    {
      type: "reveal",
      narr: t(E,
        "How does the input arrive? N, then N lines of arrival and duration.",
        "입력은 N 다음에 N줄의 도착 시각과 통과 시간이 이어져요."),
      content: (
        <div style={{ padding: 16, wordBreak: "keep-all" }}>
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            <div style={{ background: "#fffbeb", border: "2px solid #fde68a", borderRadius: 10, padding: "10px 14px", fontFamily: "'JetBrains Mono',monospace", fontSize: 13, lineHeight: 1.8 }}>
              ...
              <div style={{ marginTop: 4, paddingLeft: 10, borderLeft: "2px solid #fde68a" }}>
                <div><span style={{ color: "#92400e", fontWeight: 800 }}>a[i] d[i]</span> <span style={{ color: C.dim, fontSize: 11 }}>{t(E, "— arrival time, passage duration", "— 도착 시각, 통과 시간")}</span></div>
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            <div style={{ background: "#ecfdf5", border: "2px solid #6ee7b7", borderRadius: 10, padding: "10px 14px", fontSize: 13, lineHeight: 1.7 }}>
              {t(E, "The earliest time at which all cows finish passing through the gate — a single integer.",
                  "모든 소가 문을 통과해 끝나는 가장 이른 시각을 한 줄로 출력해요.")}
            </div>
          </div>
          {/* 제약 */}
          <div>
            ...
            <div style={{ background: "#fff", border: `1.5px solid ${C.border}`, borderRadius: 10, padding: "10px 14px", fontFamily: "'JetBrains Mono',monospace", fontSize: 12, lineHeight: 1.9 }}>
              <div>1 ≤ N ≤ 100</div>
              <div>1 ≤ a[i], d[i] ≤ 1,000,000 (= 10⁶)</div>
            </div>
          </div>
        </div>),
    },
