...
export function getMooHuntFastWalk(E, lang = "py") {
  if (lang === "cpp") {
    return { code: FAST_CPP, vars: _FAST_VARS, beats: [
      ...
      { hi: [54, 64], bubble: t(E, "This board is done. What is the next one?\nTreat cell 1 as the ones place and add 1.\nTurn Ms back to O until you meet an O, then make that one an M.",
                                   "이 보드는 끝났어요. 다음 보드는 무엇일까요?\n1번 칸을 일의 자리로 보고 1 을 더해요.\nM 이면 O 로 되돌리다가, O 를 만나면 그 자리를 M 으로 바꿔요.") },
  ...
  return { code: FAST_PY, vars: _FAST_VARS, beats: [
    ...
    { hi: [40, 44], bubble: t(E, "This board has a score. Compare it with the best so far.\nSame score? Then one more board reaches it.",
                                 "이 보드의 점수가 나왔어요. 지금까지 제일 높은 값과 견줘요.\n같으면 그 점수인 보드가 하나 늘어요.") },
    { hi: [46, 55], bubble: t(E, "This board is done. What is the next one?\nTreat cell 1 as the ones place and add 1.\nTurn Ms back to O until you meet an O, then make that one an M.\nAll M already? Then there is nowhere left to go — stop.",
