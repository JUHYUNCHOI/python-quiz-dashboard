...
const SOLVED_KEY = "algo-greedy-contest-solved"
...
export default function SortingPracticePage() {
  ...
  const problem = problemId ? greedyContestCluster.problems.find(p => p.id === problemId) : null
  ...
  // 단일 문제 풀이 화면
  if (problem) {
    const problemIdx = greedyContestCluster.problems.findIndex(p => p.id === problem.id) + 1
    return (
      <div className="min-h-screen bg-gray-50 pb-24">
        ...
        <main className="max-w-5xl mx-auto px-4 pt-4">
          <div className="mb-3 flex items-center justify-between gap-2">
            <button
              onClick={() => router.push("/algo/greedy")}
        ...
        <LessonPanel open={showLesson} onClose={() => setShowLesson(false)} title={t("그리디 수업", "Greedy lesson")}>
          <GreedyLesson />
        </LessonPanel>
        <BottomNav />
      </div>
  ...
  // 리스트 화면
  const totalSolved = greedyContestCluster.problems.filter(p => solvedSet.has(p.id)).length
  const total = greedyContestCluster.problems.length
      ...
      <main className="max-w-2xl mx-auto px-4 pt-4">
        <div className="mb-4">
          <JourneyBreadcrumb items={[
            ...
            { label: "그리디", labelEn: "Greedy", href: "/algo/greedy", emoji: "📊" },
            { label: "문제 풀이", labelEn: "Practice" },
          ]} />
          <div className="flex items-center gap-2 mb-2">
            <span className="text-3xl">{greedyContestCluster.emoji}</span>
            <h1 className="text-xl sm:text-2xl font-black text-gray-900">
              {t(greedyContestCluster.title, greedyContestCluster.en?.title ?? greedyContestCluster.title)}
            </h1>
          </div>
          <p className="text-sm text-gray-600 mb-3">
            {t(greedyContestCluster.description, greedyContestCluster.en?.description ?? greedyContestCluster.description)}
          </p>
        ...
        {(() => {
          const warmup = greedyContestCluster.problems.filter(p => p.difficulty === "쉬움")
          const easy = greedyContestCluster.problems.filter(p => p.difficulty === "보통")
          const hard = greedyContestCluster.problems.filter(p => p.difficulty === "어려움")
          const renderItem = (p: any) => {
            const isSolved = solvedSet.has(p.id)
            const globalIdx = greedyContestCluster.problems.findIndex(pp => pp.id === p.id) + 1
            return (
              <Link
                key={p.id}
                href={`/algo/greedy/practice?p=${p.id}`}
                className={cn(
