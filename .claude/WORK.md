...
### ⛔ 보류 — 레슨1 `try-empty-print`
"빈 줄 만들기" 미션. `mission` 으로 올려도 **못 잡는다.**
`components/python/python-runner.tsx:248` 의 `normalize` 가
`s.trim().toLowerCase().replace(/\s+/g, " ")` 라 `'안녕\n\n반가워'` 와 `'안녕\n반가워'` 가 같아진다.

**normalize 를 고치면 안 된다** — 전수 측정 결과 빈칸 없는 `tryit` **82곳**이
공백 차이를 normalize 가 봐줘서 통과 중이다. 고치면 그 82곳이 한꺼번에 오답이 된다.
별도 작업: normalize 를 줄바꿈 보존 + 줄 안 공백만 뭉개는 방식으로 바꾸고 82곳 전수 재검증.
아니면 이 스텝의 **문제 자체를 바꾼다**(빈 줄 말고 normalize 가 안 뭉개는 것을 묻게).
...
| | 무엇 | 근거 |
|---|---|---|
| 1 | `normalize` 줄바꿈 보존 + 82곳 재검증 | 레슨1 보류 사유. 이걸 풀어야 "빈 줄" 미션이 산다 |
| 2 | 빈칸 없는 `tryit` ~79개 재분류 | 09-05 표 4번 |
| 3 | 안 가르치고 쓰는 문법 (한 줄 if 등) | 09-05 표 5번 |
