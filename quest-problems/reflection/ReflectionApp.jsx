...
export default function ReflectionApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeReflectionCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeReflectionCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeReflectionCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeReflectionCh1, 1: (e) => makeReflectionCh2(e, codeLang) };
  ...
  const prev = () => {
    if (cur > 0) {
      ...
      const pt = tab - 1;           // 코드 탭 첫 쪽에서 이전 → 문제 탭 마지막 쪽으로
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <ReflectionProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "sim") return <ReflectionSim E={E} />;
    if (step.type === "runner") return <ReflectionRunner E={E} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <ReflectionProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "sim") return <ReflectionSim E={E} />;
    if (s.type === "runner") return <ReflectionRunner E={E} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadReflectionPDF(E, getReflectionSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`refl-${tab}-${cur}-${lang}`} text={step.narr} />}
