...
import { AirCond1ProgressiveCode, downloadAirCond1PDF, getAirCond1Sections, getAirCond1Walk } from "./components";
import { makeAirCond1Ch1, makeAirCond1Ch2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#f97316";

export default function AirCond1App(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeAirCond1Ch1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeAirCond1Ch2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeAirCond1Ch2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeAirCond1Ch1, 1: (e) => makeAirCond1Ch2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <AirCond1ProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getAirCond1Walk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <AirCond1ProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getAirCond1Walk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadAirCond1PDF(E, getAirCond1Sections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`airc-${tab}-${cur}-${lang}`} text={step.narr} />}
