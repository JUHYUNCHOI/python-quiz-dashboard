...
\`\`\`
Python 의 \`self.x = y\` 와 거의 똑같이 생겼어요 — 익숙하죠.
...
\`\`\`
콜론 \`:\` 뒤에 \`멤버(값)\` 형태. \`{ }\` 는 비워둬요.

---

> 📖 **둘 다 C++ 에서 합법**. 문법상 어느 쪽 써도 OK. 다만 **실무 C++ 은 거의 방법 2 만** 씀.

🤔 **Python 에 없는 낯선 문법인데, 왜 실무는 방법 2 를 쓸까?** 이유가 있어요. 다음 페이지 시뮬레이터에서 직접 봐요.`
        },
        {
