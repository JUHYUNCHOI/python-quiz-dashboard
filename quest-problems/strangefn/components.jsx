...
export function getStrangeFnWalk(E, lang = "py") {
  if (lang === "cpp") {
    return { code: FULL_CPP, vars: _SF_VARS, beats: [
      ...
      { hi: [42, 43], bubble: t(E, "Step 3 — we need floor(3n/2).\nMultiplying an even number by anything keeps it even, so if n\nis even, 3n is even too and last = 0.\nOdd times odd is always odd, so if n\nis odd, 3n is odd too, and we subtract last = 1.\nEither way, 3n − last is always even.", "3단계 — floor(3n/2) 를 구해야 해요.\n짝수에 무엇을 곱해도 짝수라서,\nn 이 짝수면 3n 도 짝수이고 last = 0이에요.\n홀수끼리 곱하면 홀수라서,\nn 이 홀수면 3n 도 홀수인데 last = 1을 빼요.\n그래서 3n − last 는 늘 짝수예요.") },
      { hi: [44, 44], bubble: t(E, "3n − last is always even, remember?\nBut there's no decimal point in the world of\nremainders — 13 ÷ 2 = 6.5 just doesn't work here.\nSo instead of dividing, C++ multiplies by\nINV2 = 500000004, a precomputed constant.\nThat's the \"modular inverse\" the code's comment names.\nIt's a value to memorize — why it works comes later.", "3n − last 는 늘 짝수였죠.\n근데 나머지만 남기는 계산에서는 소수점이 없어서,\n13 ÷ 2 = 6.5 처럼은 안 돼요.\n그래서 C++은 나눗셈 대신,\nINV2 = 500000004 라는 미리 계산해 둔 수를 곱해요.\n이 수가 '모듈러 역원'이에요 (코드 주석의 그 말이에요).\n외워서 쓰는 값이에요 — 까닭은 나중에 배워요.") },
  ...
  return { code: FULL_PY, vars: _SF_VARS, beats: [
    ...
    { hi: [24, 24], bubble: t(E, "3n − last is always even, remember?\nBut there's no decimal point in the world of\nremainders — 13 ÷ 2 = 6.5 just doesn't work here.\nInstead, we multiply by pow(2, MOD-2, MOD).\nINV2 is short for \"inverse of 2,\"\nthe same \"modular inverse\" the code's comment names.\nIt's a formula to memorize — why it works comes later.", "3n − last 는 늘 짝수였죠.\n근데 나머지만 남기는 계산에서는 소수점이 없어서,\n13 ÷ 2 = 6.5 처럼은 안 돼요.\n대신 pow(2, MOD-2, MOD) 를 곱해요.\nINV2 는 '2 의 역원'을 줄인 이름이고,\n코드 주석의 '모듈러 역원'이 바로 이거예요.\n외워서 쓰는 공식이에요 — 까닭은 나중에 배워요.") },
