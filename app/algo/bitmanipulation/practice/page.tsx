...
/**
 * 정렬문제 풀이 — /algo/bitmanipulation/practice
 *
...
const SOLVED_KEY = "algo-bitmanipulation-contest-solved"
...
export default function SortingPracticePage() {
  ...
  const problem = problemId ? bitManipulationContestCluster.problems.find(p => p.id === problemId) : null
  ...
  // 단일 문제 풀이 화면
  if (problem) {
    const problemIdx = bitManipulationContestCluster.problems.findIndex(p => p.id === problem.id) + 1
    return (
      <div className="min-h-screen bg-gray-50 pb-24">
        ...
        <main className="max-w-5xl mx-auto px-4 pt-4">
          <div className="mb-3 flex items-center justify-between gap-2">
            <button
              onClick={() => router.push("/algo/bitmanipulation")}
        ...
        <LessonPanel open={showLesson} onClose={() => setShowLesson(false)} title={t("비트 조작 수업", "Bit Manipulation lesson")}>
  ...
  // 리스트 화면
  const totalSolved = bitManipulationContestCluster.problems.filter(p => solvedSet.has(p.id)).length
  const total = bitManipulationContestCluster.problems.length
      ...
      <main className="max-w-2xl mx-auto px-4 pt-4">
        <div className="mb-4">
          <JourneyBreadcrumb items={[
            ...
            { label: "비트 조작", labelEn: "Bit Manipulation", href: "/algo/bitmanipulation", emoji: "📊" },
            { label: "문제 풀이", labelEn: "Practice" },
          ]} />
          <div className="flex items-center gap-2 mb-2">
            <span className="text-3xl">{bitManipulationContestCluster.emoji}</span>
            <h1 className="text-xl sm:text-2xl font-black text-gray-900">
              {t(bitManipulationContestCluster.title, bitManipulationContestCluster.en?.title ?? bitManipulationContestCluster.title)}
            </h1>
          </div>
          <p className="text-sm text-gray-600 mb-3">
            {t(bitManipulationContestCluster.description, bitManipulationContestCluster.en?.description ?? bitManipulationContestCluster.description)}
          </p>
        ...
        {(() => {
          const warmup = bitManipulationContestCluster.problems.filter(p => p.difficulty === "쉬움")
          const easy = bitManipulationContestCluster.problems.filter(p => p.difficulty === "보통")
          const hard = bitManipulationContestCluster.problems.filter(p => p.difficulty === "어려움")
          const renderItem = (p: any) => {
            const isSolved = solvedSet.has(p.id)
            const globalIdx = bitManipulationContestCluster.problems.findIndex(pp => pp.id === p.id) + 1
            return (
              <Link
                key={p.id}
                href={`/algo/bitmanipulation/practice?p=${p.id}`}
                className={cn(
