...
import { TeamTttProgressiveCode, downloadTeamTttPDF, getTeamTttSections, getTeamTttWalk } from "./components";
import { makeTttCh1, makeTttCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#dc2626";

export default function TeamTttApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeTttCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeTttCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeTttCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeTttCh1, 1: (e) => makeTttCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <TeamTttProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "teamttt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getTeamTttWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <TeamTttProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "teamttt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getTeamTttWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadTeamTttPDF(E, getTeamTttSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`team-${tab}-${cur}-${lang}`} text={step.narr} />}
