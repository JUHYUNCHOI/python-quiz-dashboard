...
# ⚠️ 2026-09-25: **오늘 이 검사기의 오탐을 다섯 번 만났다. 전부 같은 원인이다** —
#   `1..m` · `(.)` · `짝-홀-짝-홀…` · `a.back()`·`8!` · `0..25`.
#   담당들이 그때마다 「표기만 바꿔서」 0건으로 만들었는데, 그건 **뜻이 아니라 도구에
#   맞춰 글을 고친 것**이다. 여섯 번째가 나오기 전에 검사기를 고친다.
#   ⭕ 문장 끝은 **`[.!?]` 뒤에 빈칸이나 끝**이 오는 자리다. 코드 안의 마침표는
#     `a.back()` 처럼 뒤에 글자가 붙으므로 이 규칙 하나로 대부분 걸러진다.
#   ⭕ 그래도 남는 것 둘을 따로 뺀다 — **코드 호출 꼴**(`chr(...)`·`a.back()`)과
#     **범위 표기**(`0..25`·`1..m`), 그리고 **말줄임표**(`…`·`...`).
#   ⛔ 「숫자 + !」(`8!` 팩토리얼)는 뒤에 빈칸이 와서 규칙으로 안 걸러진다 —
#     숫자 뒤의 `!` 는 문장 끝으로 세지 않는다.
#   ⚠️ 이렇게 고쳐도 **줄 수(`\n`) 쪽 잣대는 그대로다** — 둘 중 큰 쪽을 쓰므로
#     일부러 줄을 많이 나눈 글은 여전히 걸린다(그게 `astral` 사례였다).
#   ⚠️ 첫 판에서 `vals.begin()` 이 안 잡혔다 — `begin()` 만 지우고 **앞의 `vals.` 가
#     남아** 그 점이 문장 끝으로 세어졌다. **점이 붙은 이름까지 함께** 지운다.
_CODEISH = re.compile(r"`[^`]*`|[\w.]+\s*\([^)]*\)|\d+\.\.\w+|\.{2,}|…")
_SENT_END = re.compile(r"(?<![0-9])[.!?]+(?=\s|$)")


def _sentences(ko):
    """문장 개수 — **코드 기호를 문장 끝으로 세지 않는다.**"""
    text = _CODEISH.sub(" ", ko)
    return len([x for x in _SENT_END.split(text) if x.strip()])
