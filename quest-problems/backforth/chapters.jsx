...
export function makeBackForthCh1(E) {
  return [
    // 1-1: Title reveal
    {
      type: "reveal",
      narr: t(E,
        "2 barns with 10 buckets each (1000 total milk per barn). Over 4 days, alternate carrying one bucket between barns. Count distinct possible final amounts in barn 1.",
        "각 10개 양동이를 가진 2개의 헛간 (헛간당 총 우유 1000). 4일 동안 번갈아 양동이 하나를 옮겨. 헛간 1의 가능한 최종 우유량의 수를 구해!"),
      content: (
    ...
    // 1-2: Quiz
    {
      type: "quiz",
      narr: t(E,
        "If all 10 buckets in each barn have the same amount s, then every day we transfer exactly s. After 4 days (2 out, 2 back), barn 1 = 1000 - s + s - s + s = 1000. Only 1 outcome!",
        "각 헛간의 10개 양동이가 모두 같은 양 s이면, 매일 정확히 s를 이동해. 4일 후 (2번 보내고 2번 받으면), 헛간1 = 1000 - s + s - s + s = 1000. 결과 1개!"),
      question: t(E,
    ...
    // 1-3: Input
    {
      type: "input",
      narr: t(E,
        "All buckets have the same amount. How many distinct final amounts for barn 1?",
        "모든 양동이가 같은 양이야. 헛간 1의 고유 최종 우유량은 몇 가지?"),
      question: t(E,
...
export function makeBackForthCh2(E, lang = "py") {
  return [
    // 2-1: Complexity reveal
    {
      type: "reveal",
      narr: t(E,
        "Brute force all 10 * 11 * 11 * 11 possibilities (bucket count grows each day). Use a set to collect distinct barn 1 totals.",
        "10 * 11 * 11 * 11가지 가능성을 브루트 포스. 매일 양동이 수가 증가. set으로 헛간 1 총량의 고유값 수집."),
      content: (
        <div style={{ padding: 16, textAlign: "center" }}>
