...
const FULL_CPP = [
  ...
  "    // Find cycle length by repeated application",
  "    // (apply perm until we get identity)",
  "    auto cur = list(range(N));",
  "    auto cycle = 0;",
  "    while (True) {",
  "        auto cur = [cur[perm[i]] for i in range(N)];",
  "        cycle += 1;",
  "        if (cur == list(range(N))) {",
  "            break;",
  "",
  "    // K mod cycle gives effective rounds",
  "    auto eff = K % cycle;",
  "    auto result = list(range(N));",
  "    for (long long _ = 0; _ < eff; _++) {",
  "        auto result = [result[perm[i]] for i in range(N)];",
  "",
  "    // Output 1-indexed",
...
export function SwapityProgressiveCode({ E, lang = "py", sections }) {
