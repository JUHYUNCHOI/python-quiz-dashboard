...
import { CollatzProgressiveCode, downloadCollatzPDF, getCollatzSections } from "./components";
...
export default function CollatzApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeCollatzCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeCollatzCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeCollatzCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeCollatzCh1, 1: (e) => makeCollatzCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <CollatzProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <CollatzProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadCollatzPDF(E, getCollatzSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`coll-${tab}-${cur}-${lang}`} text={step.narr} />}
