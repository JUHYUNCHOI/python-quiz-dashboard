...
import { CowCollegeProgressiveCode, downloadCowCollegePDF, getCowCollegeSections, getCowCollegeWalk } from "./components";
import { makeCowCollegeCh1, makeCowCollegeCh2, makeCowCollegeCh3 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#d97706";

export default function CowCollegeApp(props = {}) {
  ...
  useEffect(() => {
    ...
    /* ⭐ 2026-09-26: `quest-algohint` 배선 — 코드 탭(tab >= 2)에서만
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <CowCollegeProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "cowcollege-walk") return <CodeWalk E={E} lang={codeLang} {...getCowCollegeWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <CowCollegeProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "cowcollege-walk") return <CodeWalk E={E} lang={codeLang} {...getCowCollegeWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadCowCollegePDF(E, getCowCollegeSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`cowcolle-${tab}-${cur}-${lang}`} text={step.narr} />}
