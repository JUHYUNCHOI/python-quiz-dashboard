...
import { MooOpsProgressiveCode, downloadMooOpsPDF, getMooOpsSections, getMooOpsWalk } from "./components";
import { makeMooOpsCh1, makeMooOpsCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#059669";

export default function MooOpsApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeMooOpsCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeMooOpsCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeMooOpsCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeMooOpsCh1, 1: (e) => makeMooOpsCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <MooOpsProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "mooops-walk") return <CodeWalk E={E} lang={codeLang} {...getMooOpsWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <MooOpsProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "mooops-walk") return <CodeWalk E={E} lang={codeLang} {...getMooOpsWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadMooOpsPDF(E, getMooOpsSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`mooo-${tab}-${cur}-${lang}`} text={step.narr} />}
