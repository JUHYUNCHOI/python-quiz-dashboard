...
import { PalindromeProgressiveCode, downloadPalindromePDF, getPalindromeSections, PalindromeSim, PalindromeRunner } from "./components";
...
export default function PalindromeApp(props = {}) {
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(1300px, 100%)", margin: "0 auto", padding: "0 12px" }}>
            ...
            {TABS.map((label, i) => {
              ...
              return (
                <button key={i} onClick={() => changeTab(i)} style={{
                  ...
                  background: isCurrent ? A : (isVisited ? "#fee2e2" : "transparent"),
                  border: `1.5px solid ${isCurrent ? A : (isVisited ? "#fca5a5" : C.border)}`,
                  color: isCurrent ? "#fff" : (isVisited ? "#7f1d1d" : C.dim),
        ...
        {step.narr && <Narration key={`pal-${tab}-${cur}-${lang}`} text={step.narr} />}
