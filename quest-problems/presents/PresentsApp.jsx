...
export default function PresentsApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makePresentsCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makePresentsCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makePresentsCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makePresentsCh1, 1: (e) => makePresentsCh2(e, codeLang) };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <PresentsProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "sim") return <PresentsSim E={E} />;
    if (s.type === "runner") return <PresentsRunner E={E} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadPresentsPDF(E, getPresentsSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`pres-${tab}-${cur}-${lang}`} text={step.narr} />}
