...
**컨테이너 다룰 땐 거의 안 써요.** 이터레이터 (또는 \`range-for\`) 가 표준:

| 상황 | 요즘 쓰는 게 |
|---|---|
| vector / map / set 순회 | \`for (auto x : v)\` (이터레이터 호출됨) |
| STL 함수 (\`sort\`, \`find\` …) | 이터레이터 페어가 기본 |
| C 함수 / OS API / 저수준 메모리 | 포인터 |

이번 챕터부터 계속 쓰는 \`sort(v.begin(), v.end())\` — 그 \`v.begin()\` 이 바로 이터레이터예요.
