...
import { SleepyHerdProgressiveCode, downloadSleepyHerdPDF, getSleepyHerdSections } from "./components";
import { makeSleepyHerdCh1, makeSleepyHerdCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";

const A = "#d97706";

export default function SleepyHerdApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeSleepyHerdCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeSleepyHerdCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeSleepyHerdCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeSleepyHerdCh1, 1: (e) => makeSleepyHerdCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <SleepyHerdProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <SleepyHerdProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadSleepyHerdPDF(E, getSleepyHerdSections(E), codeLang)} style={{
