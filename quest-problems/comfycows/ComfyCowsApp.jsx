...
import { ComfyCowsProgressiveCode, downloadComfyCowsPDF, getComfyCowsSections, getComfyCowsWalk } from "./components";
import { makeComfyCh1, makeComfyCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#f97316";

export default function ComfyCowsApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeComfyCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeComfyCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeComfyCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeComfyCh1, 1: (e) => makeComfyCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <ComfyCowsProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "comfycows-walk") return <CodeWalk E={E} lang={codeLang} {...getComfyCowsWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <ComfyCowsProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "comfycows-walk") return <CodeWalk E={E} lang={codeLang} {...getComfyCowsWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadComfyCowsPDF(E, getComfyCowsSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`comf-${tab}-${cur}-${lang}`} text={step.narr} />}
