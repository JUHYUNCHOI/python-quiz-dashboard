...
import { BillboardProgressiveCode, downloadBillboardPDF, getBillboardSections, getBillboardWalk } from "./components";
import { makeBillboardCh1, makeBillboardCh2, makeBillboardCh3 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#d97706";

export default function BillboardApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeBillboardCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeBillboardCh2(lang === "en"));
  const [ch3Q, setCh3Q] = useState(() => makeBillboardCh3(lang === "en", "py"));

  useEffect(() => {
    setCh3Q(prev => makeBillboardCh3(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const TABS = E ? ["📋 Ch1", "📋 Ch2", "⚡ Code"] : ["📋 Ch1", "📋 Ch2", "⚡ 코드"];
  const setters = { 0: setCh1Q, 1: setCh2Q, 2: setCh3Q };
  const states  = { 0: ch1Q,    1: ch2Q,    2: ch3Q };
  const makers  = { 0: makeBillboardCh1, 1: makeBillboardCh2, 2: (e) => makeBillboardCh3(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <BillboardProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "billboard-codewalk") return <CodeWalk E={E} lang={codeLang} {...getBillboardWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <BillboardProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "billboard-codewalk") return <CodeWalk E={E} lang={codeLang} {...getBillboardWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadBillboardPDF(E, getBillboardSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`billboar-${tab}-${cur}-${lang}`} text={step.narr} />}
