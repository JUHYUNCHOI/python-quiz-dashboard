...
import { Mcc15EqProgressiveCode, downloadMcc15EqPDF, getMcc15EqSections } from "./components";
...
export default function Mcc15EqApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeMcc15EqCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeMcc15EqCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeMcc15EqCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeMcc15EqCh1, 1: (e) => makeMcc15EqCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <Mcc15EqProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <Mcc15EqProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadMcc15EqPDF(E, getMcc15EqSections(E), codeLang)} style={{
