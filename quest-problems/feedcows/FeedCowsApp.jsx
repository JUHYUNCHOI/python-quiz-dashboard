...
import { FeedCowsProgressiveCode, downloadFeedCowsPDF, getFeedCowsSections, getFeedCowsWalk } from "./components";
import { makeFeedCh1, makeFeedCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#059669";

export default function FeedCowsApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeFeedCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeFeedCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeFeedCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeFeedCh1, 1: (e) => makeFeedCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <FeedCowsProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "feedcows-walk") return <CodeWalk E={E} lang={codeLang} {...getFeedCowsWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <FeedCowsProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "feedcows-walk") return <CodeWalk E={E} lang={codeLang} {...getFeedCowsWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadFeedCowsPDF(E, getFeedCowsSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`feed-${tab}-${cur}-${lang}`} text={step.narr} />}
