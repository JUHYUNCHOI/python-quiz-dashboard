...
import { LeadersProgressiveCode, downloadLeadersPDF, getLeadersSections, getLeadersWalk } from "./components";
import { makeLeadersCh1, makeLeadersCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#dc2626";

export default function LeadersApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeLeadersCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeLeadersCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeLeadersCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeLeadersCh1, 1: (e) => makeLeadersCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <LeadersProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getLeadersWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <LeadersProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getLeadersWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadLeadersPDF(E, getLeadersSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`lead-${tab}-${cur}-${lang}`} text={step.narr} />}
