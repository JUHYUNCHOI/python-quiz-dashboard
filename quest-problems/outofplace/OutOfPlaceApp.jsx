...
import { makeOutOfPlaceCh1, makeOutOfPlaceCh2 } from "./chapters";

const A = "#059669";

export default function OutOfPlaceApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeOutOfPlaceCh1(false));
  const [ch2Q, setCh2Q] = useState(() => makeOutOfPlaceCh2(false, "py"));

  useEffect(() => {
    setCh2Q(prev => makeOutOfPlaceCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeOutOfPlaceCh1, 1: (e) => makeOutOfPlaceCh2(e, codeLang) };
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(1300px, 100%)", margin: "0 auto", padding: "0 12px" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 12, alignItems: "center" }}>
          ...
          {showCodeControls && (
            <div style={{ display: "flex", gap: 0, alignItems: "stretch", flexShrink: 0 }}>
              ...
              <button onClick={() => downloadOutOfPlacePDF(E, getOutOfPlaceSections(E), codeLang)} style={{
        ...
        {step.narr && <Narration key={`out-${tab}-${cur}-${lang}`} text={step.narr} />}
