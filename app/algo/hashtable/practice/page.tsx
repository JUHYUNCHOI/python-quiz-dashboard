...
/**
 * 해시테이블문제 풀이 — /algo/hashtable/practice
 *
...
const SOLVED_KEY = "algo-hashtable-contest-solved"
...
export default function SortingPracticePage() {
  ...
  const problem = problemId ? hashTableContestCluster.problems.find(p => p.id === problemId) : null
  ...
  // 단일 문제 풀이 화면
  if (problem) {
    const problemIdx = hashTableContestCluster.problems.findIndex(p => p.id === problem.id) + 1
    return (
      <div className="min-h-screen bg-gray-50 pb-24">
        ...
        <LessonPanel open={showLesson} onClose={() => setShowLesson(false)} title={t("해시테이블 수업", "Hash Table lesson")}>
          <HashTableLesson />
        </LessonPanel>
        <BottomNav />
      </div>
  ...
  // 리스트 화면
  const totalSolved = hashTableContestCluster.problems.filter(p => solvedSet.has(p.id)).length
  const total = hashTableContestCluster.problems.length
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-orange-50 via-white to-purple-50 pb-24">
      ...
      <main className="max-w-2xl mx-auto px-4 pt-4">
        <div className="mb-4">
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            <h1 className="text-xl sm:text-2xl font-black text-gray-900">
              {t(hashTableContestCluster.title, hashTableContestCluster.en?.title ?? hashTableContestCluster.title)}
            </h1>
          </div>
          <p className="text-sm text-gray-600 mb-3">
            {t(hashTableContestCluster.description, hashTableContestCluster.en?.description ?? hashTableContestCluster.description)}
          </p>
        ...
        {(() => {
          const warmup = hashTableContestCluster.problems.filter(p => p.difficulty === "쉬움")
          const easy = hashTableContestCluster.problems.filter(p => p.difficulty === "보통")
          const hard = hashTableContestCluster.problems.filter(p => p.difficulty === "어려움")
          const renderItem = (p: any) => {
            const isSolved = solvedSet.has(p.id)
            const globalIdx = hashTableContestCluster.problems.findIndex(pp => pp.id === p.id) + 1
            return (
              <Link
                key={p.id}
                href={`/algo/hashtable/practice?p=${p.id}`}
                className={cn(
