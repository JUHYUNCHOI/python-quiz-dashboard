...
export default function StallingApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeStallingCh1(false));
  const [ch2Q, setCh2Q] = useState(() => makeStallingCh2(false, "py"));

  useEffect(() => {
    setCh2Q(prev => makeStallingCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeStallingCh1, 1: (e) => makeStallingCh2(e, codeLang) };
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 12, alignItems: "center" }}>
          ...
          {showCodeControls && (
            <div style={{ display: "flex", gap: 0, alignItems: "stretch", flexShrink: 0 }}>
              ...
              <button onClick={() => downloadStallingPDF(E, getStallingSections(E), codeLang)} style={{
