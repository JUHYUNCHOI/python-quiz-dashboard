...
import { Feb23ProgressiveCode, downloadFeb23PDF, getFeb23Sections } from "./components";
import { makeFebCh1, makeFebCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";

const A = "#dc2626";

export default function Feb23App(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeFebCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeFebCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeFebCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <Feb23ProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <Feb23ProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadFeb23PDF(E, getFeb23Sections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`feb2-${tab}-${cur}-${lang}`} text={step.narr} />}
