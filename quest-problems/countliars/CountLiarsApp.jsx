...
import { CountLiarsProgressiveCode, downloadCountLiarsPDF, getCountLiarsSections, getCountLiarsWalk } from "./components";
import { makeLiarsCh1, makeLiarsCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#2563eb";

export default function CountLiarsApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeLiarsCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeLiarsCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeLiarsCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeLiarsCh1, 1: (e) => makeLiarsCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <CountLiarsProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "countliars-walk") return <CodeWalk E={E} lang={codeLang} {...getCountLiarsWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <CountLiarsProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "countliars-walk") return <CodeWalk E={E} lang={codeLang} {...getCountLiarsWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadCountLiarsPDF(E, getCountLiarsSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`coun-${tab}-${cur}-${lang}`} text={step.narr} />}
