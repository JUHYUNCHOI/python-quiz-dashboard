...
export default function AstralApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeAstralCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeAstralCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeAstralCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeAstralCh1, 1: (e) => makeAstralCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <AstralProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "sim") return <AstralSim E={E} />;
    if (step.type === "runner") return <AstralRunner E={E} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <AstralProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "sim") return <AstralSim E={E} />;
    if (s.type === "runner") return <AstralRunner E={E} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadAstralPDF(E, getAstralSections(E), codeLang)} style={{
