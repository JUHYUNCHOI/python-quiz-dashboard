...
컴퓨터가 소수를 저장하는 방식 때문에 아주 작은 오차가 생겨요. **정수 비교는 안전, 소수 \`==\` 비교는 조심.** 지금은 "이런 게 있구나" 만 알아두면 OK.`
        ...
        },
        {
          ...
          type: "quiz",
          title: "🤔 어떤 부호?",
          content: "**비밀번호가 *맞는지 비교* 하려면?**",
          options: ["= 로 저장", "== 로 비교", "!= 로 다름 확인"],
          answer: 1,
          explanation: "*비교* 는 `==` (두 개!). `=` (한 개) 는 *저장* 이라 비교 아님."
        },
        {
          id: "try-equal",
          type: "tryit",
...
⚠️ 지금 단계에선 **있다는 것만** 알아두면 OK. 자세한 사용은 **lesson 11 (조건문)** 에서 \`if\` 와 함께 자연스럽게 배워요.`
        },
        {
          ...
          type: "quiz",
          title: "🤔 어떤 부호?",
          content: "**'60점 *이상*' 이라고 말할 때 어떤 부호?**",
          options: ["> (초과)", ">= (이상)", "== (같다)", "= (저장)"],
          answer: 1,
          explanation: "'이상' 은 같은 값도 포함 → `>=`. '*초과*' (>) 와 헷갈리지 말기!"
        },
        {
          id: "pre-try-chained-eq",
          type: "quiz",
          title: "🤔 어떤 부호?",
          content: "**'정확히 100점' 이라고 말할 때 어떤 부호?**",
          options: ["= (저장)", "== (같다)", ">= (이상)", "!= (다름)"],
          answer: 1,
          explanation: "'정확히 같다' = `==`. `=` 는 *저장* 이에요, 비교가 아님!"
        },
        {
          id: "try-chained",
          type: "tryit",
...
> 💡 \`+=\` 는 **카운터 (점수, 횟수)** 와 **체력/자원 변경** 에 가장 많이 등장. 다음 레슨들에서 계속 만날 거예요.`
        ...
        },
        {
          ...
          type: "quiz",
          title: "🤔 어떤 복합 대입?",
          content: "**'데미지를 받았다 = 빼서 저장' — 어떤 복합 대입?**",
          options: ["+= (더하기 저장)", "-= (빼서 저장)", "*= (곱하기 저장)", "= (그냥 저장)"],
          answer: 1,
          explanation: "*빼서 저장* = `-=`. 같은 패턴: `*=`, `+=`, `/=` 도 똑같이 작동."
        },
        {
          id: "try5",
          type: "tryit",
