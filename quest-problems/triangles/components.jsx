...
const FULL_CPP = [
  ...
  "",
  "    // Group points by x-coordinate and y-coordinate",
  "    auto by_x = defaultdict(list);",
  "    auto by_y = defaultdict(list);",
  "    // for x, y in points:",
  "        // by_x[x].append(y)",
  "        // by_y[y].append(x)",
  "",
  ...
  "    // for x, y in points:",
  "        // vertical neighbors (same x, different y)",
  "        auto heights = [abs(y2 - y) for y2 in by_x[x] if y2 != y];",
  "        // horizontal neighbors (same y, different x)",
  "        auto widths = [abs(x2 - x) for x2 in by_y[y] if x2 != x];",
  "        // for h in heights:",
  "            // for w in widths:",
  "                auto ans = max(ans, h * w);",
  "",
  "    cout << ans << \"\\n\";",
  "",
  "    return 0;",
...
export function TrianglesProgressiveCode({ E, lang = "py", sections }) {
