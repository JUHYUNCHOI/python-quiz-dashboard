...
export function getMakeDistinctWalk(E, lang = "py") {
  if (lang === "cpp") {
    return { code: FULL_CPP, vars: _MD_VARS, beats: [
      { hi: [7, 17],  bubble: t(E, "What are we solving for? One number per test — the fewest operations needed.\ncin >> T reads the test count, then the loop runs once per test. n and k are long long — values can pass 2 billion, too big for int.\nLike the constraint on page 2, N can be up to 200,000 — if many numbers pile up, one of them may be pushed almost that far, and with many such numbers the total easily passes 2 billion.\nvector<long long> a(n) makes the array, and cin >> a[i] fills it one by one.\nllabs(k) is just abs() for long long — we need it because k can be negative.", "무엇을 구해야 하나요? 테스트마다 최소 횟수 하나예요.\ncin >> T 로 테스트 개수를 받고, 테스트마다 돌아요. n, k 는 long long 타입이에요 — 값이 20억을 넘을 수 있어 int 로는 부족해요.\n2쪽 제약처럼 N 이 최대 200,000 인데, 같은 수가 몰리면 하나가 그 근처까지 밀릴 수 있고 그런 수가 여럿이면 답이 20억을 훌쩍 넘어요.\nvector<long long> a(n) 으로 배열을 만들고 cin >> a[i] 로 하나씩 채워요.\nllabs(k) 는 long long 용 abs() 예요 — k 가 음수일 수 있어서 절댓값이 필요해요.") },
  ...
  return { code: FULL_PY, vars: _MD_VARS, beats: [
    ...
    { hi: [4, 7],   bubble: t(E, "solve() handles ONE test: read n, k, the array.\nK can be negative, so the grouping size takes its absolute value: m = abs(k).", "solve() 는 테스트 하나를 맡아요. n, k, 배열을 읽어요.\nK 가 음수일 수도 있어서, 묶을 때 쓸 크기는 절댓값으로 잡아요: m = abs(k)") },
    ...
    { hi: [23, 25], bubble: t(E, "Otherwise it collides, so push it to cur + K — the next free spot.\nBut cur isn't guaranteed to sit right before vals[i].\nEarlier values may have already pushed cur far ahead.\nSo vals[i] jumps straight from its own spot to cur, in one leap.\nRemember page 5? The third 3 did this — 5 was already taken too, so it went to 7.\nDistance 4, moved by K=2 each time: (7-3) / 2 = 2.\nSame remainder means every gap in the group is a multiple of K — that's why we grouped by it.\ncur is built by adding K to a group value, so it's in the group too.\nSo cur - vals[i] is a multiple of K, and dividing it gives exactly the push count.", "아니면 부딪히니까 cur + K, 즉 다음 빈 자리로 밀어요.\n그런데 cur 가 vals[i] 바로 앞이라는 보장은 없어요.\n앞선 값들이 이미 cur 를 저만치 밀어 놨을 수 있어요.\n그래서 vals[i] 는 자기 자리에서 cur 까지 한 번에 건너뛰어요.\n5쪽에서 셋째 3 이 그랬죠? 5도 이미 차서 7까지 갔어요.\n거리 4 를 K=2 씩 가니 (7-3) ÷ 2 = 2 회예요.\n나머지가 같으면 묶음 안 모든 차이가 K 의 배수예요 — 그래서 나머지로 묶은 거였죠.\ncur 도 묶음의 값에 K 를 더해 만든 거라 같은 묶음이에요.\n그래서 cur - vals[i] 도 K 의 배수, 나누면 딱 민 횟수가 나와요.") },
