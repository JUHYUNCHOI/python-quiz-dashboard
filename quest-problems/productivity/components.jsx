...
const FULL_CPP = [
  ...
  "    auto c = list(map(int, input().split()))  # closing times;",
  "    auto ti = list(map(int, input().split()))  # travel times;",
  "",
  "    // d[i] = c[i] - t[i]: max wake-up time to visit farm i",
  "    auto d = sorted([c[i] - ti[i] for i in range(N)]);",
  "",
  "    for (long long _ = 0; _ < Q; _++) {",
  "        long long S, V; cin >> S >> V;",
  "        // Count farms where d[i] > S",
  "        // bisect_right(d, S) gives index of first element > S",
  "        auto reachable = N - bisect_right(d, S);",
  "        cout << 'YES' if reachable >= V else 'NO' << \"\\n\";",
  "",
  "    return 0;",
  "}",
];
...
export function ProductivityProgressiveCode({ E, lang = "py", sections }) {
