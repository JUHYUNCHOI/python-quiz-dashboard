...
다음 레슨 (*sort & 이진탐색*) 에서 \`sort\` 를 본격적으로 다뤄요. 거기서 "근데 점수는 **내림차순**, 이름은 오름차순으로 동시에 하려면?" 같은 좀 더 복잡한 정렬을 lambda 와 함께 배워요. 오늘 본 pair + sort 자동 정렬이 그 출발점이에요.`
        },
        {
          id: "ch1-question",
...
🚀 **다음 레슨(*sort & 이진탐색*)**에서는 **sort 를 본격적으로** 배울 거예요! 커스텀 정렬, 람다, 구조체 정렬까지!`
        }
      ]
    }
  ]
