...
앞 페이지에서 본 **이진 탐색** (가운데 보고 절반 버리기) — 그 한 가지 아이디어 위에 이 챕터의 도구들이 다 얹혀 있어요.

---

### 📊 같은 일, 정렬 전후 — 100 만 개 데이터에서

| 하고 싶은 일 | 정렬 안 됨 | 정렬 후 (이 챕터에서) |
|---|---|---|
| "x 가 있나?" | 100 만 번 (\`find\` — 처음부터 훑기) | **~20 번** (\`binary_search\`) |
| "x 가 몇 개?" | 100 만 번 (\`std::count\`) | **~40 번** (\`upper - lower\`) |
| "x 이상 첫 거?" | 100 만 번 (직접 루프) | **~20 번** (\`lower_bound\`) |
| "중복 제거" | 직접 짜야 함 | **한 줄** (\`sort + unique + erase\`) |

> 💡 함수 이름이 낯설어도 OK — **다음 페이지부터 하나씩 만나요.** 지금은 "정렬해두면 이만큼 빨라진다" 만 머리에 담아 가요.

---

### 🔑 학생용 한 줄

> "**정렬은 끝이 아니라 시작.** 정렬 한 번 = 검색이 폭발적으로 빨라짐. 각 도구마다 '어디 쓰는지' 가 분명해서 외울 게 줄어들 거예요."`
        },
        {
          id: "s23-ch2-iter",
          type: "explain",
