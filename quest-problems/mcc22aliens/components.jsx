...
const FULL_CPP = [
  ...
  "    vector<int> claimP(N);",
  "    vector<string> claimB(N);",
  "    for (int i = 0; i < N; i++) {",
  "        string ti, bi;",
  "        int pi;",
  "        cin >> ti >> pi >> bi;",
  "        types[i] = ti;",
  "        claimP[i] = pi - 1;   // 0-indexed target",
  "        claimB[i] = bi;",
  "    }",
  "",
  "    // Check consistency:",
  ...
  "        string bi = claimB[i];",
  "        if (types[i] == \"T\") {",
  "            if (types[pi] != bi) { consistent = false; break; }",
  "        } else {",
  "            if (types[pi] == bi) { consistent = false; break; }",
  "        }",
  "    }",
  ...
  return [
    {
      ...
      why: [
        ...
        t(E, "The C++ mirrors the Python line by line: read each alien's (type, target, claimed-type), then verify T-types match and F-types mismatch.",
            "C++ 는 Python 을 한 줄씩 그대로 옮긴 거예요: 각 외계인의 (타입, 지목 대상, 주장한 타입) 을 읽고, T 는 일치·F 는 불일치를 확인해요."),
      ],
      pyOnly: [
