...
export default function SortingPracticePage() {
  ...
  // 단일 문제 풀이 화면
  if (problem) {
    const problemIdx = sortingContestCluster.problems.findIndex(p => p.id === problem.id) + 1
    return (
  ...
  // 리스트 화면
  const totalSolved = sortingContestCluster.problems.filter(p => solvedSet.has(p.id)).length
  const total = sortingContestCluster.problems.length
      ...
      <main className="max-w-2xl mx-auto px-4 pt-4">
        <div className="mb-4">
          <JourneyBreadcrumb items={[
            ...
            { label: "정렬", labelEn: "Sorting", href: "/algo/sorting", emoji: "📊" },
            { label: "문제 풀이", labelEn: "Practice" },
          ]} />
          <div className="flex items-center gap-2 mb-2">
            <span className="text-3xl">{sortingContestCluster.emoji}</span>
            <h1 className="text-xl sm:text-2xl font-black text-gray-900">
              {t(sortingContestCluster.title, sortingContestCluster.en?.title ?? sortingContestCluster.title)}
            </h1>
          </div>
          <p className="text-sm text-gray-600 mb-3">
            {t(sortingContestCluster.description, sortingContestCluster.en?.description ?? sortingContestCluster.description)}
          </p>
        ...
        {(() => {
          const warmup = sortingContestCluster.problems.filter(p => p.difficulty === "쉬움")
          const easy = sortingContestCluster.problems.filter(p => p.difficulty === "보통")
          const hard = sortingContestCluster.problems.filter(p => p.difficulty === "어려움")
          const renderItem = (p: any) => {
            const isSolved = solvedSet.has(p.id)
            const globalIdx = sortingContestCluster.problems.findIndex(pp => pp.id === p.id) + 1
            return (
              <Link
                key={p.id}
