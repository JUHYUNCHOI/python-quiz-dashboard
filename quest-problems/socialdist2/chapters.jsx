...
export function makeSocDist2Ch1(E) {
  return [
    ...
    // 1-1b: 입출력 형식 + 제약 (USACO 원문 socdist2.in/.out, usaco.org cpid=1036 직접 확인) —
    // 시즌 표준화 (photoshoot25 형태). 샘플은 원문 그대로, FULL_PY 로 직접 돌려 "3" 확인함.
    {
      type: "reveal",
      narr: t(E,
        "How does the data arrive?  N, then N lines of position and sick/healthy.",
        "입력은 N, 그다음 위치와 상태가 적힌 N 줄이에요."),
      content: (
        <div style={{ padding: 16, wordBreak: "keep-all" }}>
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            <div style={{ background: "#fffbeb", border: "2px solid #fde68a", borderRadius: 10, padding: "10px 14px", fontFamily: "'JetBrains Mono',monospace", fontSize: 13, lineHeight: 1.8 }}>
              ...
              <div style={{ marginTop: 4, paddingLeft: 10, borderLeft: "2px solid #fde68a" }}>
                <div><span style={{ color: "#92400e", fontWeight: 800 }}>x s</span> <span style={{ color: C.dim, fontSize: 11 }}>{t(E, "— position x, and s: 0=healthy, 1=sick", "— 위치 x, 상태 s: 0=건강, 1=아픔")}</span></div>
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            <div style={{ background: "#ecfdf5", border: "2px solid #6ee7b7", borderRadius: 10, padding: "10px 14px", fontSize: 13, lineHeight: 1.7 }}>
              {t(E, "One line: the minimum number of cows that could have started sick.",
                  "한 줄: 처음 아팠던 소의 최소 수.")}
            </div>
          </div>
          {/* 제약 (USACO 원문) */}
          <div>
            ...
            <div style={{ background: "#fff", border: `1.5px solid ${C.border}`, borderRadius: 10, padding: "10px 14px", fontFamily: "'JetBrains Mono',monospace", fontSize: 12, lineHeight: 1.9 }}>
              <div>1 ≤ N ≤ 1,000</div>
              <div>0 ≤ x ≤ 10⁶ (one million)</div>
              <div style={{ color: C.dim, fontSize: 11, marginTop: 2 }}>{t(E, "positions are distinct  ·  at least one cow is sick", "위치는 서로 겹치지 않음  ·  아픈 소가 최소 1 마리는 있음")}</div>
            </div>
          </div>
        </div>),
    },
