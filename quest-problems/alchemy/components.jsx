...
//   Python: 0/1 (RTE - wrong input parsing)
//   C++:    0/1 (WA - scaffold only (just checks initial count))
...
const FULL_CPP = [
  ...
  "    int M; cin >> M;",
  "    for (int _m = 0; _m < M; _m++) { int x; cin >> x; count[x]++; }",
  "    int K; cin >> K;",
  "    map<int, vector<pair<int,int>>> recipes;   // out -> list of (qty, ingr)",
  "    for (int _k = 0; _k < K; _k++) {",
  "        int out, c; cin >> out >> c;",
  "        vector<pair<int,int>> ings;",
  "        for (int _c = 0; _c < c; _c++) { int q, i; cin >> q >> i; ings.push_back({q, i}); }",
  "        recipes[out] = ings;",
  "    }",
