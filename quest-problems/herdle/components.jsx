...
//   Python: 0/1 (RTE - wrong input parsing (no T))
//   C++:    0/1 (WA - cpp expects T but problem has single test case)
...
const FULL_CPP = [
  ...
  "        vector<vector<string>> gue(3, vector<string>(3));",
  "        for (int r = 0; r < 3; r++) {",
  "            for (int c = 0; c < 3; c++) {",
  "                cin >> ans[r][c];",
  "            }",
  ...
  return [
    {
      ...
      cppOnly: [
        t(E, "vector<vector<string>> stores each 3x3 grid of breeds.",
            "vector<vector<string>>로 3x3 품종 격자를 저장."),
        t(E, "map<string,int> counts breeds left over after the green pass.",
            "map<string,int>로 green 처리 후 남은 품종 수를 셈."),
      ],
    },
  ];
}
