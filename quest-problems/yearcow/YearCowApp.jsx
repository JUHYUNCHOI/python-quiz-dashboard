...
import { YearCowProgressiveCode, downloadYearCowPDF, getYearCowSections, getYearCowWalk } from "./components";
import { makeYearCowCh1, makeYearCowCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#d97706";

export default function YearCowApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeYearCowCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeYearCowCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeYearCowCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeYearCowCh1, 1: (e) => makeYearCowCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <YearCowProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "yearcow-walk") return <CodeWalk E={E} lang={codeLang} {...getYearCowWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <YearCowProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "yearcow-walk") return <CodeWalk E={E} lang={codeLang} {...getYearCowWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadYearCowPDF(E, getYearCowSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`year-${tab}-${cur}-${lang}`} text={step.narr} />}
