...
const FULL_CPP = [
  ...
  "        vector<int> h(N - 1);",
  "        for (auto& x : h) cin >> x;",
  "        // Try each starting value, build perm via deque (insert at front or back)",
  "        // (Simplified to brute attempt)",
  "        for (int start = 1; start <= N; start++) {",
  "            deque<int> perm = {start};",
  ...
  "                    next = prev - h[i];",
  "                    if (next < 1 || next > N || used[next]) { ok = false; break; }",
  "                }",
  "                perm.push_back(next); used[next] = true;",
  "            }",
  "            if (ok) {",
