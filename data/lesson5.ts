...
- **마지막 글자 인덱스** — \`text[len(text)-1]\` (그래도 \`text[-1]\` 이 더 짧음!)
...
\`len\` 은 "여러 개 모인 것" 의 개수를 세는 도구예요. 숫자 12345 는 한 개의 값일 뿐. (글자수 5 를 알고 싶으면 lesson 9 에서 배워요.)
...
문자열은 **글자 단위 비교**. \`"10"\` 의 첫 글자는 \`'1'\`, \`"9"\` 는 \`'9'\` — 사전상 \`'1'\` 이 \`'9'\` 보다 앞이라 결과는 \`True\`. 숫자처럼 비교하고 싶다면 lesson 9 에서 배울 변환이 필요해요.`
        },
        {
          id: "try-in-compare",
          type: "tryit",
...
\`print()\` 안에서 콤마 \`,\` 로 나누면 종류가 달라도 한 줄에 출력할 수 있어요. \`print\` 가 알아서 각각을 글자로 바꿔서 보여줘요.
