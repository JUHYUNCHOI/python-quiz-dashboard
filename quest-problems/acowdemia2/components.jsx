...
const FULL_CPP = [
  ...
  "        int K; cin >> K;",
  "        vector<int> auth(K);",
  "        for (auto& x : auth) cin >> x;",
  "        for (int i = 0; i < K; i++) for (int j = i + 1; j < K; j++) before[auth[i]][auth[j]] = true;",
  "    }",
  "    int ans = 0;",
  "    for (int a = 0; a < N; a++) {",
  "        bool always = true;",
  "        for (int b = 0; b < N; b++) if (b != a && !before[a][b]) { always = false; break; }",
  "        if (always) ans++;",
  "    }",
  "    cout << ans << \"\n\";",
  "    return 0;",
...
export function Acowdemia2ProgressiveCode({ E, lang = "py", sections }) {
