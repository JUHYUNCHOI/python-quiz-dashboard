...
import { downloadCowEvolutionPDF, getCowEvolutionSections, getCowEvolutionWalk, CowEvolutionSim } from "./components";
...
export default function CowEvolutionApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeEvolutionCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeEvolutionCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeEvolutionCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeEvolutionCh1, 1: (e) => makeEvolutionCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "codewalk") return <CodeWalk E={E} lang={codeLang} {...getCowEvolutionWalk(E, codeLang)} accent={A} />;
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "codewalk") return <CodeWalk E={E} lang={codeLang} {...getCowEvolutionWalk(E, codeLang)} accent={A} />;
  ...
  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadCowEvolutionPDF(E, getCowEvolutionSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`cowe-${tab}-${cur}-${lang}`} text={step.narr} />}
