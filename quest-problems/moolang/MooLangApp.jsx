...
import { MooLangProgressiveCode, downloadMooLangPDF, getMooLangSections, getMooLangWalk } from "./components";
import { makeMooLangCh1, makeMooLangCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#2563eb";

export default function MooLangApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeMooLangCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeMooLangCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeMooLangCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeMooLangCh1, 1: (e) => makeMooLangCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <MooLangProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getMooLangWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <MooLangProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getMooLangWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadMooLangPDF(E, getMooLangSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`mool-${tab}-${cur}-${lang}`} text={step.narr} />}
