    ...
    <div className="bg-indigo-50/60 rounded-2xl overflow-hidden border border-indigo-200 shadow-md">
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-indigo-100/60 border-b border-indigo-200">
        <div className="flex items-center gap-2">
          ...
          <span className="text-slate-600 text-xs font-mono ml-2">{title ?? lbl.title}</span>
        </div>
          ...
          <div className="flex-shrink-0">
            ...
            <div className="flex gap-3 mb-3 text-xs">
              <span className="flex items-center gap-1">
                ...
                <span className="text-slate-600 font-mono">i ({lbl.iLabel})</span>
              </span>
              <span className="flex items-center gap-1">
                <span className="inline-block w-2.5 h-2.5 rounded-sm bg-blue-400" />
                <span className="text-slate-600 font-mono">j ({lbl.jLabel})</span>
              </span>
              <span className="flex items-center gap-1">
                <span className="inline-block w-2.5 h-2.5 rounded-sm bg-emerald-400" />
                <span className="text-slate-600 font-mono">{isEn ? "current" : "현재"}</span>
              </span>
            </div>
            ...
            <div className="flex gap-1 mb-1 ml-8">
              {Array.from({ length: cols }, (_, c) => (
                <div key={c} className={cn(
                  ...
                  isVisiting && j === c ? "text-blue-600" : "text-slate-400"
                )}>
                  j={c}
                </div>
              ))}
            ...
            {defaultGrid.map((row, r) => (
              <div key={r} className="flex items-center gap-1 mb-1">
                ...
                <div className={cn(
                  ...
                  (isEnteringRow || isVisiting) && i === r ? "text-amber-600" : "text-slate-400"
                )}>
                  i={r}
                </div>
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        ...
                        isCurrentCell
                          ? "bg-emerald-100 border-2 border-emerald-500 text-emerald-700 scale-110 shadow-lg shadow-emerald-200"
                          : isRowActive
            ...
            {phase !== "idle" && phase !== "done" && (
              <div className="mt-3 flex gap-3">
                <div className="flex items-center gap-1.5 bg-amber-50 border border-amber-300 rounded-lg px-2.5 py-1.5">
                  <span className="text-amber-600 font-mono font-bold text-sm">i</span>
                  <span className="text-slate-400 text-xs">=</span>
                  <span className="text-amber-700 font-mono font-bold text-sm">{i >= 0 ? i : "?"}</span>
                </div>
                <div className="flex items-center gap-1.5 bg-blue-50 border border-blue-300 rounded-lg px-2.5 py-1.5">
                  <span className="text-blue-600 font-mono font-bold text-sm">j</span>
                  <span className="text-slate-400 text-xs">=</span>
                  <span className="text-blue-700 font-mono font-bold text-sm">{j >= 0 ? j : "—"}</span>
                </div>
                {isVisiting && (
                  <div className="flex items-center gap-1.5 bg-emerald-50 border border-emerald-300 rounded-lg px-2.5 py-1.5">
                    <span className="text-slate-500 font-mono text-xs">grid[i][j]</span>
                    <span className="text-slate-400 text-xs">=</span>
                    <span className="text-emerald-700 font-mono font-bold text-sm">{defaultGrid[i][j]}</span>
                  </div>
                )}
              </div>
            )}
            ...
            <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
              ...
              <div className="p-3 font-mono text-xs sm:text-sm space-y-0.5">
                ...
                <div className={cn(
                  ...
                  isEnteringRow ? "bg-amber-100 border border-amber-300" : "border border-transparent"
                )}>
                  <span className="text-purple-600">for</span>
                  <span className="text-slate-500">(int</span>
                  <span className={cn("font-bold", i >= 0 ? "text-amber-600" : "text-slate-700")}> i</span>
                  <span className="text-slate-500">= 0; i &lt;</span>
                  <span className="text-amber-600 font-bold"> {rows}</span>
                  <span className="text-slate-500">; i++) &#123;</span>
                  {isEnteringRow && (
                    <span className="ml-1 text-amber-600 text-xs font-semibold animate-pulse">↵ i={i}</span>
                  )}
                </div>
                ...
                <div className={cn(
                  ...
                  isVisiting ? "bg-blue-100 border border-blue-300" : "border border-transparent"
                )}>
                  <span className="text-purple-600">for</span>
                  <span className="text-slate-500">(int</span>
                  <span className={cn("font-bold", j >= 0 ? "text-blue-600" : "text-slate-700")}> j</span>
                  <span className="text-slate-500">= 0; j &lt;</span>
                  <span className="text-blue-600 font-bold"> {cols}</span>
                  <span className="text-slate-500">; j++) &#123;</span>
                  {isVisiting && (
                    <span className="ml-1 text-blue-600 text-xs font-semibold animate-pulse">↵ j={j}</span>
                  )}
                </div>
                ...
                <div className={cn(
                  ...
                  isVisiting ? "bg-emerald-50 border border-emerald-300" : "border border-transparent"
                )}>
                  <span className="text-cyan-600">cout</span>
                  <span className="text-slate-500">&lt;&lt;</span>
                  <span className={cn(
                    "transition-all duration-200",
                    isVisiting ? "text-emerald-700" : "text-slate-700"
                  )}>
                    grid[
                    <span className={cn("font-bold", i >= 0 ? "text-amber-600" : "text-slate-400")}>
                      {i >= 0 ? i : "i"}
                    </span>
                    ][
                    <span className={cn("font-bold", j >= 0 ? "text-blue-600" : "text-slate-400")}>
                      {j >= 0 ? j : "j"}
                    </span>
                    ]
                  </span>
                  {isVisiting && (
                    <span className="text-slate-400 ml-1">
                      ...
                      <span className="text-emerald-600 font-bold ml-1">{defaultGrid[i][j]}</span>
                    </span>
                  )}
                </div>

                <div className="ml-4 text-slate-400 px-2">&#125;</div>
              ...
              <div className="min-h-[28px] flex flex-wrap gap-1.5">
                {visited.map((key, idx) => {
                  ...
                  return (
                    <span
                      key={key}
                      className="inline-flex items-center gap-1 px-1.5 py-0.5 bg-white rounded text-xs font-mono text-slate-600 border border-slate-300"
                    >
                      <span className="text-slate-400 text-[10px]">[{r}][{c}]</span>
                      <span className="text-emerald-600 font-bold">{defaultGrid[r][c]}</span>
                    </span>
                  )
                })}
                {isVisiting && (
                  <span className="inline-flex items-center gap-1 px-1.5 py-0.5 bg-emerald-50 rounded text-xs font-mono border border-emerald-400 animate-pulse">
                    <span className="text-slate-400 text-[10px]">[{i}][{j}]</span>
                    <span className="text-emerald-600 font-bold">{defaultGrid[i][j]}</span>
                  </span>
                )}
                ...
                {isDone && (
                  <span className="ml-1 text-emerald-600 text-xs font-mono">✓</span>
                )}
              </div>
            </div>
          </div>
      ...
      <div className="px-4 pb-4 flex items-center gap-2">
        ...
        <button
          onClick={reset}
          className="flex items-center gap-1.5 px-3 py-2 bg-slate-100 hover:bg-slate-200 border border-slate-300 rounded-xl text-sm text-slate-500 hover:text-slate-700 transition-all duration-150"
          title={lbl.restart}
        >
