...
/**
 * 정렬문제 풀이 — /algo/shortestpath/practice
 *
...
const SOLVED_KEY = "algo-shortestpath-contest-solved"
...
export default function SortingPracticePage() {
  ...
  const problem = problemId ? shortestPathContestCluster.problems.find(p => p.id === problemId) : null
  ...
  // 단일 문제 풀이 화면
  if (problem) {
    const problemIdx = shortestPathContestCluster.problems.findIndex(p => p.id === problem.id) + 1
    return (
      <div className="min-h-screen bg-gray-50 pb-24">
        ...
        <main className="max-w-5xl mx-auto px-4 pt-4">
          <div className="mb-3 flex items-center justify-between gap-2">
            <button
              onClick={() => router.push("/algo/shortestpath")}
        ...
        <LessonPanel open={showLesson} onClose={() => setShowLesson(false)} title={t("최단 경로 수업", "Shortest Path lesson")}>
          <ShortestPathLesson />
        </LessonPanel>
        <BottomNav />
      </div>
  ...
  // 리스트 화면
  const totalSolved = shortestPathContestCluster.problems.filter(p => solvedSet.has(p.id)).length
  const total = shortestPathContestCluster.problems.length
      ...
      <main className="max-w-2xl mx-auto px-4 pt-4">
        <div className="mb-4">
          <JourneyBreadcrumb items={[
            ...
            { label: "최단 경로", labelEn: "Shortest Path", href: "/algo/shortestpath", emoji: "📊" },
            { label: "문제 풀이", labelEn: "Practice" },
          ]} />
          <div className="flex items-center gap-2 mb-2">
            <span className="text-3xl">{shortestPathContestCluster.emoji}</span>
            <h1 className="text-xl sm:text-2xl font-black text-gray-900">
              {t(shortestPathContestCluster.title, shortestPathContestCluster.en?.title ?? shortestPathContestCluster.title)}
            </h1>
          </div>
          <p className="text-sm text-gray-600 mb-3">
            {t(shortestPathContestCluster.description, shortestPathContestCluster.en?.description ?? shortestPathContestCluster.description)}
          </p>
        ...
        {(() => {
          ...
          const renderItem = (p: any) => {
            ...
            const globalIdx = shortestPathContestCluster.problems.findIndex(pp => pp.id === p.id) + 1
            return (
              <Link
                key={p.id}
                href={`/algo/shortestpath/practice?p=${p.id}`}
                className={cn(
