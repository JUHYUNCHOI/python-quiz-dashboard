...
while (lo <= hi) {
    ...
    if (a[mid] == target) { /* 찾음 */ break; }
      ...
      <section>
        ...
        <Code code={`sort(v.begin(), v.end());
auto it = lower_bound(v.begin(), v.end(), x);  // x 이상 첫 위치
int idx = it - v.begin();
// upper_bound: x 초과 첫 위치 → 개수 세기에 유용`} />
        <Code lang="python" code={`import bisect
bisect.bisect_left(a, x)   # x 이상 첫 위치 (lower_bound)
bisect.bisect_right(a, x)  # x 초과 첫 위치 (upper_bound)`} />
      </section>
      ...
      <section>
        ...
        <Code code={`int lo = 1, hi = MAX, ans = -1;
while (lo <= hi) {
    int mid = (lo + hi) / 2;
