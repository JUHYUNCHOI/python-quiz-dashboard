...
import { BackForthProgressiveCode, downloadBackForthPDF, getBackForthSections, getBackForthWalk, BackForthStateTreeSim } from "./components";
import { makeBackForthCh1, makeBackForthCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";

const A = "#8b5cf6";

export default function BackForthApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeBackForthCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeBackForthCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeBackForthCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeBackForthCh1, 1: (e) => makeBackForthCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <BackForthProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getBackForthWalk(E, codeLang)} accent={A} />;
    if (step.type === "tree") return <BackForthStateTreeSim E={E} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <BackForthProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getBackForthWalk(E, codeLang)} accent={A} />;
    if (s.type === "tree") return <BackForthStateTreeSim E={E} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadBackForthPDF(E, getBackForthSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`back-${tab}-${cur}-${lang}`} text={step.narr} />}
