...
const FULL_CPP = [
  ...
  "        cin >> xs[i] >> ys[i] >> ds[i];",
  "    }",
  "    // Bronze: simulate chain reactions",
  "    map<pair<int,int>, char> posMap;",
  "    for (int i = 0; i < N; i++) {",
  "        posMap[make_pair(xs[i], ys[i])] = ds[i];",
  "    }",
  "    cout << posMap.size() << \"\\n\";   // placeholder",
  "    return 0;",
  "}",
  ...
  return [
    {
      ...
      cppOnly: [
        t(E, "Three parallel vectors store x, y, and direction for each arrow.",
            "세 개의 평행 벡터로 화살표의 x, y, 방향을 저장."),
        t(E, "map<pair<int,int>,char> looks up an arrow by its (x, y) position.",
            "map<pair<int,int>,char>로 (x, y) 좌표로 화살표를 찾음."),
      ],
    },
  ];
}
