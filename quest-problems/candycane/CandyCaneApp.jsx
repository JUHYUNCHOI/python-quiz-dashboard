...
import { CandyCaneProgressiveCode, downloadCandyCanePDF, getCandyCaneSections, getCandyCaneWalk } from "./components";
import { makeCandyCh1, makeCandyCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";

const A = "#dc2626";

export default function CandyCaneApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeCandyCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeCandyCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeCandyCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeCandyCh1, 1: (e) => makeCandyCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <CandyCaneProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getCandyCaneWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <CandyCaneProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getCandyCaneWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadCandyCanePDF(E, getCandyCaneSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`cand-${tab}-${cur}-${lang}`} text={step.narr} />}
