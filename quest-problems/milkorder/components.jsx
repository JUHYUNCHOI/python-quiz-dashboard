...
const FULL_CPP = [
  ...
  "    cin >> N >> M >> K;",
  "    vector<int> hier(M);",
  "    for (int i = 0; i < M; i++) cin >> hier[i];",
  "    map<int,int> pos;   // cow -> position",
  "    for (int i = 0; i < K; i++) {",
  "        int c, p; cin >> c >> p;",
  "        pos[c] = p;",
  "    }",
  "",
  "    // Try every starting position for hier[0]",
  "    int answer = -1;",
  "    for (int start = 1; start <= N; start++) {",
  "        // Place hierarchy starting from \"start\"",
  "        bool ok = true;",
  "        for (int i = 0; i < M && ok; i++) {",
  "            if (pos.count(hier[i]) && pos[hier[i]] != start + i) ok = false;",
  "            if (start + i > N) ok = false;",
  "        }",
  "        if (ok) {",
  "            // Find Bessie's position (cow #1)",
  "            int bessieIdx = -1;",
  "            for (int i = 0; i < M; i++) if (hier[i] == 1) { bessieIdx = i; break; }",
  "            if (bessieIdx >= 0) { answer = start + bessieIdx; break; }",
  "            // else: Bessie not in hierarchy, take first available slot",
  "            // Bessie position = first slot not in hier",
  "            for (int p = 1; p <= N; p++) {",
  "                bool used = false;",
  "                for (int i = 0; i < M; i++) if (start + i == p) { used = true; break; }",
  "                if (!used && (!pos.count(1) || pos[1] == p)) { answer = p; break; }",
  "            }",
  "            if (answer != -1) break;",
  "        }",
  "    }",
...
export function MilkOrderProgressiveCode({ E, lang = "py", sections }) {
