...
import { HungryCowProgressiveCode, downloadHungryCowPDF, getHungryCowSections, getHungryCowWalk } from "./components";
...
export default function HungryCowApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeHungryCowCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeHungryCowCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeHungryCowCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeHungryCowCh1, 1: (e) => makeHungryCowCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <HungryCowProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getHungryCowWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <HungryCowProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getHungryCowWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadHungryCowPDF(E, getHungryCowSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`hung-${tab}-${cur}-${lang}`} text={step.narr} />}
