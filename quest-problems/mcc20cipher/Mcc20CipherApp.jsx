...
import { Mcc20CipherProgressiveCode, downloadMcc20CipherPDF, getMcc20CipherSections, getMcc20CipherWalk } from "./components";
...
export default function Mcc20CipherApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeMcc20CipherCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeMcc20CipherCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeMcc20CipherCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeMcc20CipherCh1, 1: (e) => makeMcc20CipherCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <Mcc20CipherProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getMcc20CipherWalk(E)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <Mcc20CipherProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getMcc20CipherWalk(E)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadMcc20CipherPDF(E, getMcc20CipherSections(E), codeLang)} style={{
