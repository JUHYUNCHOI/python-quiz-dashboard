...
버튼을 눌러서 직접 따라가 봐요!`,
        },
        {
          ...
          content: `## 🚀 이번 챕터의 큰 그림 한 줄

> **🎯 "정렬해두면 갑자기 가능해지는 일들" 모음집**

이번 챕터에서 \`binary_search\`, \`lower_bound\`, \`upper_bound\`, \`unique\` 등 함수가 한꺼번에 등장해요. **하나씩 따로 외우면 머리 터져요.** 사실은 **모두 같은 아이디어 (정렬이 만들어주는 마법) 위에 얹힌 도구들** 이에요.

---

### 📊 정렬 안 됨 vs 정렬 됨 — 같은 질문, 다른 도구

| 하고 싶은 일 | 정렬 안 됨 (기존) | 정렬 후 (이번 챕터) |
|---|---|---|
| "x 가 있나?" | \`find\` — O(N) 훑기 | **\`binary_search\`** — O(log N) |
| "x 가 몇 개?" | \`std::count\` — O(N) | **\`upper - lower\`** — O(log N) |
| **"x 이상 첫 거?"** | \`find_if\` 로 가능하지만 O(N) | **\`lower_bound(x)\`** — O(log N) |
| **"정렬 유지하며 끼워넣기"** | 어려움 | **\`insert(lower_bound(x), x)\`** |
| **"중복 제거 (vector 유지)"** | 직접 짜야 함 | **\`sort + unique + erase\`** 한 줄 |

100 만 개 데이터에서 차이: 100 만 번 비교 vs ~20 번 비교. **정렬 한 번이 이만큼 풀어줘요.**

---

### ❓ 자주 묻는 질문

**Q1. "70 점 이상 첫 학생" 은 \`find\` 로도 되지 않아?**

\`find(v, 70)\` 은 **정확히 70 인 거** 만 찾아요. "이상" 같은 **조건** 은 못 받아요. \`find_if(v, [](int x){ return x >= 70; })\` 로는 되는데, 그것도 **처음부터 차례로 훑어** O(N). \`lower_bound\` 만 가운데 잘라가며 **O(log N) 으로 빠르게** 같은 답을 줘요.

**Q2. "중복 제거" 는 그냥 \`set\` 쓰면 안 돼?**

set 도 dedupe 자동이에요 — 단, 결과가 **set** 이 됨. set 은 트리라 **메모리 더 먹고 \`v[i]\` 같은 인덱스 접근 불가**. **vector 를 유지하면서** 빠르게 dedupe 하고 싶을 때 \`sort + unique + erase\` — 그게 표준 패턴이에요. (간단한 경우엔 set 도 OK.)

---

### 🔑 왜 정렬이 이렇게 강력해?

정렬돼 있으면 **"중간을 봤더니 X 면, 답은 한쪽에만 있다"** 가 가능해져요. 한 번 비교에 절반을 버려요. 이게 이진 탐색이고, 위 도구들이 다 그 위에 얹혀 있어요.

> 💡 **학생용 한 줄:** "정렬은 끝이 아니라 **시작**. 정렬 한 번 = 이 도구들이 한꺼번에 열림."

다음 페이지부터 하나씩 보지만 "왜 굳이 또?" 라는 생각이 들면 이 표로 돌아오세요.`
        },
        {
          id: "s23-ch2-iter",
          type: "explain",
          title: "📌 이터레이터 — 다음 페이지 들어가기 전 1 분 정리",
          content: `다음 페이지에 \`lower_bound\` 가 나오는데, 결과로 숫자가 아니라 **이터레이터 (iterator)** 라는 게 나와요. 처음 보면 낯서니까 1 분만 정리하고 가요.

---

### 이터레이터 = "위치를 가리키는 손가락"

sort 할 때마다 쓰던 \`v.begin()\`, \`v.end()\` — 그게 이터레이터예요. **벡터 안의 한 자리를 가리키는 손가락.**
...
vector 에서는 이터레이터를 **포인터처럼** 생각해도 돼요. 메모리에 값들이 옆에 줄지어 있으니까 손가락 ++ 하면 다음 값. 문법까지 똑같아요:
...
> ⚠️ 차이는 나중에 — map / set / list 같은 **메모리가 흩어진** 컨테이너에선 포인터는 못 쓰고 이터레이터만 통해요. 그건 그때 가서. 지금은 **"vector 에 쓰는 포인터 비슷한 거"** 면 충분.

---

### 🎯 외울 공식 딱 두 줄

다음 페이지부터 \`lower_bound\` 같은 함수가 이터레이터를 돌려줄 때, 학생이 다룰 일은 이 둘 뿐:
...
> 💡 \`*it\` = 값, \`it - v.begin()\` = 인덱스. **이 두 줄만 외우면** 다음 페이지부터 막힘 없음.`
        },
        {
          id: "s23-ch2-lb",
          type: "explain",
...
> 🎯 **USACO 에서 진짜 자주 나오는 lower_bound 활용:**
> • **"기준 이상 첫 거"** — 정렬된 가격에서 "5만원 이상 가장 싼 상품" / 정렬된 시간에서 "지금 이후 첫 약속".
> • **"기준 이하 가장 큰 거"** — <code>lower_bound(x+1) - 1</code> 로 "예산 안에서 가장 비싼 거" 찾기.
> • **"이 구간에 몇 개?"** — <code>upper_bound(b) - lower_bound(a)</code> 로 "[a, b] 범위 안의 점 개수".
> • **좌표 압축** (Silver+) — 값을 0, 1, 2 같은 작은 번호로 재매김할 때 <code>lower_bound</code> 가 핵심.
