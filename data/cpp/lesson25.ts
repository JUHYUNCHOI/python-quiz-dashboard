...
지금까지 우리는 sort 함수 자체를 배웠어요. 그런데 sort 가 정말 강력한 이유는 다른 데 있어요 — **정렬해두면 이전엔 느렸던 작업들이 갑자기 엄청 빨라져요.**

앞 페이지의 **이진 탐색** 떠올려봐요. "가운데부터 펼치기" 가 가능했던 단 하나의 이유 — *전화번호부가 정렬돼 있어서* 였잖아요? **이번 챕터에서 배울 모든 함수가 그 원리를 써먹어요.**

---

### 📊 정렬 한 번이 풀어주는 것들 — 100 만 개 데이터

| 하고 싶은 일 | 정렬 **안** 됐을 때 | 정렬해두면 (이 챕터에서 배움) |
|---|---|---|
...
| "최댓값 / 최솟값?" | 전체 스캔 | **즉시** (양 끝 \`v.front()\`, \`v.back()\`) |

> 💡 함수 이름이 처음 보여도 OK — **곧 하나씩** 배워요. 지금은 *정렬해두면 이렇게 많은 게 빨라진다* 만 느끼면 됨.

---

### 그래서 알고리즘 문제 풀 때 자동 반사

USACO 같은 문제를 보면 거의 모든 풀이가 이렇게 시작해요:

\`\`\`cpp
cin >> n;
for (int i = 0; i < n; i++) cin >> v[i];
sort(v.begin(), v.end());   // ← 일단 sort 부터, 거의 반사적으로
// ↓ 진짜 풀이는 여기부터
\`\`\`

왜? **정렬해두면 위 표의 모든 도구가 다 풀려서**, 어떤 문제가 나와도 한 발 앞서 출발할 수 있거든요.

> ✨ **한 줄 요약: 정렬 한 번 해두면 검색이 거의 5 만 배 빨라져요.** 그래서 "일단 sort" 가 습관이에요.

---

### ⚠️ 이번 챕터의 약속 — 모든 함수는 *정렬된 배열* 전제

이 챕터에서 배울 \`binary_search\`, \`lower_bound\`, \`upper_bound\` — **셋 다 정렬된 배열에서만 정확히 동작해요.**

정렬 안 된 배열에 쓰면 *에러도 안 나고* 엉뚱한 답이 슬쩍 나와요 — 더 무서움. 그러니까 함수 쓰기 전엔 항상 \`sort(v.begin(), v.end())\` 가 먼저.

---

### 🪧 다음 페이지 미리 안내

위 함수들이 결과를 좀 낯선 형태로 돌려줘요 — **숫자 (인덱스) 가 아니라 "위치를 가리키는 손가락"**. 그 손가락 이름이 **이터레이터 (iterator)** 예요.

이름 무섭게 들리지만 별 거 아니에요. 다음 페이지에서 1 분만 친해지고 본격적으로 함수 배우러 가요!`
        },
        {
          id: "s23-ch2-iter",
          type: "explain",
          title: "📌 이터레이터 — 위치를 가리키는 손가락",
          content: `방금 본 표 (👈) 의 \`lower_bound\`, \`binary_search\`, \`upper_bound\` 가 결과로 **숫자가 아니라 위치 (= 손가락)** 를 돌려준다고 했죠? 그 손가락의 진짜 이름이 **이터레이터 (iterator)** 예요.

**이름은 무섭지만 진짜 별 거 아니에요** — 딱 손가락 하나만 생각하면 끝!

---

### 이터레이터 = 벡터 안 한 자리를 가리키는 손가락 🫵

벡터에 손가락을 살짝 올려놓는다고 상상해봐요:
...
손가락이 지금 첫 자리 (10) 를 가리키고 있어요. **이게 이터레이터예요.** 끝. 변수에 담으면:

\`\`\`cpp
auto it = v.begin();   // 손가락을 첫 자리에 올림
\`\`\`

\`v.begin()\` 이 "벡터 첫 자리 가리키는 손가락" 을 만들어줘요.
...
\`++\` 누를 때마다 옆 자리로 *스윽*. 그게 전부예요.

---

### 두 동작을 합치면 → 전체 훑기

\`*it\` 로 읽고 \`++it\` 로 옆 가는 걸 반복하면 벡터 전체를 훑을 수 있어요:
...
여기서 새로 나온 건 **\`v.end()\` 딱 하나** 예요. 끝을 알리는 표지:
...
> 💡 평소엔 \`for (int x : v)\` (range-for) 가 훨씬 짧아서 이걸 더 자주 써요. 위 풀-패턴은 **이터레이터가 어떻게 움직이는지** 보여주기 위한 거예요 — 다음 페이지에서 \`lower_bound\` 가 손가락을 정확히 어디에 놔주는지 따라가려면 이 그림이 필요해요.

---

### 혹시 "포인터" 들어봤다면?

이터레이터는 **포인터랑 거의 똑같아요** — \`*\` 로 값 보고 \`++\` 로 옆 가는 게 다 똑같음. **다음 페이지에서 차이를 눈으로 보여줄게요** — 지금은 "아, 손가락 같은 거구나" 면 충분!`
        },
        {
          id: "s23-ch2-iter-sim",
          type: "animation" as const,
