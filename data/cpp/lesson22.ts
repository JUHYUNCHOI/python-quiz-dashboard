      ...
      steps: [
        {
          ...
          type: "explain",
          title: "🚗 class란?",
          content: `컴퓨터 세상에는 자동차, 오디오 같은 건 없어요. **다 만들어야 해요.**
...
그런데 class를 만들기 전에 **먼저 생각해야 해요.**

자동차 하면 뭐가 떠오르나요? 바퀴, 핸들, 엔진, 색깔, 속도, 연료, 문...

이것들 중에 **"내 프로그램에서 무엇이 필요한지"** 생각하는 게 중요해요.

레이싱 게임을 만든다면? → 색깔, 속도, 앞으로/뒤로 가기가 필요해요.
주차 관리 시스템이라면? → 번호판, 크기, 입차 시간이 필요하고요.

**목적에 맞게 필요한 것을 골라 넣는 것** — 이게 class 설계의 첫 번째 단계예요!

> 💡 친구한테 "김밥 만들려면 뭐가 필요해?" 물어봤더니 밥, 단무지, 소고기, 계란은 다 얘기하면서 **김**을 빠뜨렸어요.
> 이렇게 **필요한 게 빠지지 않도록 꼼꼼히 생각하는 연습** — 이게 class를 잘 만드는 핵심이에요.
...
class Car {
public:
    string color;   // 기억해야 할 것
    double speed;

    void forward()  { speed += 10; }  // 해야 할 것
...
이렇게 틀(class)을 만들면, 틀로 찍어낸 **객체**를 여러 개 만들 수 있어요:
...
> 🍩 **붕어빵틀 = class, 붕어빵 = 객체**
> 틀 하나로 붕어빵을 여러 개 찍듯, class 하나로 객체를 여러 개 만들 수 있어요.
