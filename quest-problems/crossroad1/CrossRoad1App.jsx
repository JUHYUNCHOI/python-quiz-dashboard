...
import { CrossRoad1ProgressiveCode, downloadCrossRoad1PDF, getCrossRoad1Sections, getCrossRoad1Walk } from "./components";
import { makeCrossRd1Ch1, makeCrossRd1Ch2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#d97706";

export default function CrossRoad1App(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeCrossRd1Ch1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeCrossRd1Ch2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeCrossRd1Ch2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeCrossRd1Ch1, 1: (e) => makeCrossRd1Ch2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <CrossRoad1ProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "crossroad1-codewalk") return <CodeWalk E={E} lang={codeLang} {...getCrossRoad1Walk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <CrossRoad1ProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "crossroad1-codewalk") return <CodeWalk E={E} lang={codeLang} {...getCrossRoad1Walk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadCrossRoad1PDF(E, getCrossRoad1Sections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`cros-${tab}-${cur}-${lang}`} text={step.narr} />}
