...
import { Mcc22BirthdayProgressiveCode, downloadMcc22BirthdayPDF, getMcc22BirthdaySections } from "./components";
...
export default function Mcc22BirthdayApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeMcc22BirthdayCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeMcc22BirthdayCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeMcc22BirthdayCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeMcc22BirthdayCh1, 1: (e) => makeMcc22BirthdayCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <Mcc22BirthdayProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <Mcc22BirthdayProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadMcc22BirthdayPDF(E, getMcc22BirthdaySections(E), codeLang)} style={{
