...
import { DroughtProgressiveCode, downloadDroughtPDF, getDroughtSections, getDroughtWalk } from "./components";
import { makeDroughtCh1, makeDroughtCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";

const A = "#d97706";

export default function DroughtApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeDroughtCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeDroughtCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeDroughtCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeDroughtCh1, 1: (e) => makeDroughtCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <DroughtProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "drought-walk") return <CodeWalk E={E} lang={codeLang} {...getDroughtWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <DroughtProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "drought-walk") return <CodeWalk E={E} lang={codeLang} {...getDroughtWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadDroughtPDF(E, getDroughtSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`drou-${tab}-${cur}-${lang}`} text={step.narr} />}
