...
\`\`\`
👉 객체를 **먼저 만들고**, 그 다음 \`{ }\` 안에서 \`=\` 로 값을 넣어요.

**방법 2: 만들면서 넣기** (새로 배우는 방식)
\`\`\`cpp
BankAccount(string name, double initial)
...
> 📖 프로그래밍 용어로는 방법 1 을 **"바디에서 대입"**, 방법 2 를 **"이니셜라이저 리스트 (initializer list)"** 라고 불러요. 나중에 책이나 인터넷에서 이 단어 만나면 "아 그 두 방식이구나" 연결하면 돼요.

---

**결과는 똑같아 보여요** — 둘 다 \`owner\`, \`balance\` 에 값이 들어가니까요.

🤔 **그런데 C++은 "만들면서 넣기" 를 권장해요.** 왜 그럴까요? 단순한 선호가 아니라 **내부 동작이 다르기 때문**이에요. 다음 페이지에서 시뮬레이션으로 봐요.`
        },
        {
          ...
          content: `아래 시뮬레이터의 **상단 탭**(생성자 없음 / 있음)과 **1 → 2 → 3** 단계 버튼을 눌러 비교해봐요.

- **생성자 없음**: 멤버가 쓰레기값으로 태어나서 **영원히** 그 상태 (위험!)
- **만들고 넣기** (바디 대입): 멤버가 먼저 **빈값/쓰레기값** 으로 태어난 뒤 (1단계), 바디에서 **덮어써요** (2단계) → 일이 **2번**
- **만들면서 넣기** (이니셜라이저 리스트): 멤버가 **처음부터 원하는 값으로** 태어나요 → 일이 **1번**
...
단순 타입(\`int\`, \`double\`)에선 속도 차이 미미. **진짜 중요한 차이는** — 어떤 멤버들은 **"나중에 덮어쓰기" 가 아예 불가능** 해요. 다음 페이지에서 그 주인공들(\`const\`, \`reference\`)을 만나봐요.`
        },
        {
          ...
          type: "explain",
          title: "⚠️ const와 reference — \"만들면서 넣기\" 만 가능",
          content: `**const 멤버** 와 **reference 멤버** 는 **"만들면서 넣기"** (이니셜라이저 리스트) 만 가능해요. **"만들고 넣기"** (바디 대입) 는 컴파일 에러가 나요.
...
→ const·reference·기본생성자없는클래스 가 멤버로 있다면 **"만들면서 넣기" 선택지 밖에 없어요**.`
        },
        {
          id: "ch3-initlist-realworld",
          type: "explain",
          title: "🌍 실무에서는 뭘 더 많이 쓰나요?",
          content: `**답: "만들면서 넣기"** (이니셜라이저 리스트). 실제 C++ 프로젝트·오픈소스 라이브러리 코드는 대부분 이 방식으로 써요.
...
**그럼 왜 이 레슨은 "만들고 넣기" 를 먼저 가르쳤을까?**

읽기 쉽고 Python 의 \`__init__\` 과 모양이 비슷해서 **처음 배울 때 직관적** 이에요. 개념부터 잡고 문법은 살짝 바꾸는 순서.

이제 개념은 잡았으니 **앞으로는 "만들면서 넣기" 를 기본** 으로 쓰는 습관을 들여보세요. 실전 코드, 오픈소스 읽을 때 훨씬 편해져요.
...
\`\`\`cpp
// 이 레슨에서 처음 배운 방식 (만들고 넣기)
BankAccount(string name, double initial) {
    owner = name;
    balance = initial;
}

// 실무에서 더 자주 보는 방식 (만들면서 넣기)
BankAccount(string name, double initial)
    : owner(name), balance(initial) {}
\`\`\`
...
| | 만들고 넣기 (바디) | 만들면서 넣기 (리스트) |
|---|---|---|
| 가독성 (초보 기준) | 쉬움 | 익숙해지면 더 짧음 |
| const·reference 지원 | ❌ 불가능 | ✅ 가능 |
| 효율 | 약간 느릴 수 있음 | 한 번에 끝 |
| 실무 선호도 | △ 가끔 | ⭐ 대부분 |
| 이 레슨 이후 권장 | — | ✅ 기본으로 |

앞으로 \`Player(string n, int h) : name(n), health(h) {}\` 같은 코드를 봐도 **"아, 만들면서 넣기 (이니셜라이저 리스트) 구나"** 바로 알아보세요.`
        },
        {
          id: "ch3-pred1",
          type: "predict" as const,
