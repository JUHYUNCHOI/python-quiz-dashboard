...
import { CowGymProgressiveCode, downloadCowGymPDF, getCowGymSections, getCowGymWalk } from "./components";
import { makeCowGymCh1, makeCowGymCh2, makeCowGymCh3 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#d97706";

export default function CowGymApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeCowGymCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeCowGymCh2(lang === "en"));
  const [ch3Q, setCh3Q] = useState(() => makeCowGymCh3(lang === "en", "py"));

  useEffect(() => {
    setCh3Q(prev => makeCowGymCh3(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeCowGymCh1, 1: makeCowGymCh2, 2: (e) => makeCowGymCh3(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <CowGymProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "cowgym-codewalk") return <CodeWalk E={E} lang={codeLang} {...getCowGymWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <CowGymProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "cowgym-codewalk") return <CodeWalk E={E} lang={codeLang} {...getCowGymWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadCowGymPDF(E, getCowGymSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`cowgym-${tab}-${cur}-${lang}`} text={step.narr} />}
