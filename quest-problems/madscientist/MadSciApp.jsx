...
import { MadSciProgressiveCode, MismatchRunsSim, downloadMadSciPDF, getMadSciSections, getMadSciWalk } from "./components";
import { makeMadSciCh1, makeMadSciCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#f97316";

export default function MadSciApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeMadSciCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeMadSciCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeMadSciCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeMadSciCh1, 1: (e) => makeMadSciCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <MadSciProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "madsci-walk") return <CodeWalk E={E} lang={codeLang} {...getMadSciWalk(E, codeLang)} accent={A} />;
    if (step.type === "sim") return <MismatchRunsSim E={E} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <MadSciProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "madsci-walk") return <CodeWalk E={E} lang={codeLang} {...getMadSciWalk(E, codeLang)} accent={A} />;
    if (s.type === "sim") return <MismatchRunsSim E={E} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadMadSciPDF(E, getMadSciSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`mads-${tab}-${cur}-${lang}`} text={step.narr} />}
