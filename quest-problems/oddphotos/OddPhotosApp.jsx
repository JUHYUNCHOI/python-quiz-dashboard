...
import { OddPhotosProgressiveCode, downloadOddPhotosPDF, getOddPhotosSections, getOddPhotosWalk } from "./components";
import { makeOddPhotosCh1, makeOddPhotosCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";

const A = "#2563eb";

export default function OddPhotosApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeOddPhotosCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeOddPhotosCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeOddPhotosCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeOddPhotosCh1, 1: (e) => makeOddPhotosCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <OddPhotosProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "oddphotos-walk") return <CodeWalk E={E} lang={codeLang} {...getOddPhotosWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <OddPhotosProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "oddphotos-walk") return <CodeWalk E={E} lang={codeLang} {...getOddPhotosWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadOddPhotosPDF(E, getOddPhotosSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`oddp-${tab}-${cur}-${lang}`} text={step.narr} />}
