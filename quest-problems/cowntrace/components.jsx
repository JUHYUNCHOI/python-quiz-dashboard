...
const FULL_CPP = [
  ...
  "    // events.sort()",
  "",
  "    auto infected_end = set();",
  "    auto x = list(map(int, input().split()))  # final infected;",
  "    auto infected_end = set(x);",
  "",
  "    // Try each cow as patient zero",
  "    // Try each K from 0 to T",
  "    auto results = [];",
  "    for (long long pz = 1; pz < N + 1; pz++) {",
  "        auto min_k = None;",
  "        auto max_k = None;",
  "        // for K in range(T + 1):",
  "            auto sick = {pz};",
  "            auto count = {pz: 0}  # handshakes used;",
  "            // for t_i, a, b in events:",
  "                auto a_sick = a in sick;",
  "                auto b_sick = b in sick;",
  "                if (a_sick and not b_sick) {",
  "                    if (count.get(a, 0) < K) {",
  "                        // sick.add(b)",
  "                        // count[b] = 0",
  "                        // count[a] = count.get(a, 0) + 1",
  "                else if (b_sick and not a_sick) {",
  "                    if (count.get(b, 0) < K) {",
  "                        // sick.add(a)",
  "                        // count[a] = 0",
  "                        // count[b] = count.get(b, 0) + 1",
  "                else if (a_sick and b_sick) {",
  "                    // count[a] = count.get(a, 0) + 1",
  "                    // count[b] = count.get(b, 0) + 1",
  "            if (sick == infected_end) {",
  "                if (min_k is None) {",
  ...
  "    cout << len(results) << \"\\n\";",
  "    // for pz, mn, mx in results:",
  "        auto inf = 'Infinity' if mx >= T else str(mx);",
  "        cout << pz, mn, inf << \"\\n\";",
  "",
  "    return 0;",
  "}",
...
export function CowntraceProgressiveCode({ E, lang = "py", sections }) {
