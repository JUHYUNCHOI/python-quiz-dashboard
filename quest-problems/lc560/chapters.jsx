          ...
          sections={[
            {
              ...
              py: [
                "count = 0",
                "for i in range(len(nums)):          # 토막 시작",
                "    total = 0",
                "    for j in range(i, len(nums)):   # 토막 끝",
                "        total += nums[j]",
                "        if total == k:",
                "            count += 1",
                "",
              ...
              cpp: [
                "int count = 0;",
                "int n = nums.size();",
                "for (int i = 0; i < n; i++) {           // 토막 시작",
                "    long long total = 0;",
                "    for (int j = i; j < n; j++) {       // 토막 끝",
                "        total += nums[j];",
                "        if (total == k) {",
                "            count++;",
                "        }",
        ...
        <CodeJourney
          ...
          sections={[
            ...
            },
            {
              ...
              py: [
                "    count = 0",
                "    seen = defaultdict(int)       # {누적합 값 : 몇 번}",
                "    for p in prefix:",
                "        count += seen[p - k]      # 없는 키 → 0",
                "        seen[p] += 1",
                "    return count",
              ],
              cpp: [
