...
export function getMakeDistinctWalk(E, lang = "py") {
  if (lang === "cpp") {
    return { code: FULL_CPP, vars: _MD_VARS_CPP, beats: [
      ...
      { hi: [19, 24], bubble: t(E, "Key insight: adding K never changes a value's remainder when divided by m.\nAdd 2 to 3 again and again — 3, 5, 7 … remainder is always 1, and 4 stays remainder 0.\nSame remainder means the gap is always a multiple of K — 3 and 7 share remainder 1, and 7 − 3 = 4 is a multiple of 2.\nSo grouping by remainder means groups never collide.\nC++'s % can return a negative number for negative input, so ((x % m) + m) % m nudges it back to 0..m-1.", "핵심: K 를 더해도 m 으로 나눈 나머지는 안 바뀌어요.\n3 에 2 를 계속 더하면 3, 5, 7 … 나머지가 늘 1, 4 는 늘 0 이에요.\n나머지가 같으면 차이도 K 의 배수예요.\n3 과 7 은 나머지가 둘 다 1 이고, 7−3=4 는 2 의 배수죠.\n그래서 나머지가 같은 값끼리 묶으면 서로 절대 안 부딪혀요.\nC++ 의 % 는 음수가 들어오면 음수를 돌려줄 수 있어요.\n((x % m) + m) % m 으로 한 번 더 보정해 0 이상으로 맞춰요.") },
  ...
  return { code: FULL_PY, vars: _MD_VARS, beats: [
    ...
    { hi: [9, 12],  bubble: t(E, "Key insight: adding K never changes a value's remainder when divided by m.\nAdd 2 to 3 again and again — 3, 5, 7 … remainder is always 1, and 4 stays remainder 0.\nSame remainder means the gap is always a multiple of K — 3 and 7 share remainder 1, and 7 − 3 = 4 is a multiple of 2.\nSo grouping by remainder means groups never collide.\ndefaultdict(list) makes an empty list on its own for each new remainder, so groups[x % m].append(x) just works.", "핵심: K 를 더해도 m 으로 나눈 나머지는 안 바뀌어요.\n3 에 2 를 계속 더하면 3, 5, 7 … 나머지가 늘 1, 4 는 늘 0 이에요.\n나머지가 같으면 차이도 K 의 배수예요.\n3 과 7 은 나머지가 둘 다 1 이고, 7−3=4 는 2 의 배수죠.\n그래서 나머지가 같은 값끼리 묶으면 서로 절대 안 부딪혀요.\ndefaultdict(list) 는 새 나머지마다 빈 리스트를 스스로 만들어요.\n그래서 groups[x % m].append(x) 를 바로 쓸 수 있어요.") },
