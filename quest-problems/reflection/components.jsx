// 🔒 USACO_VERIFIED (2026-05-13, C++ 재작성 2026-07-13, Python 2D배열 재작성 2026-07-24 — USACO 재제출 권장)
//   Python: 원본 3/16 (dict TLE) → 2026-07-24 RFL_FAST_PY 를 2D 배열(painted[gr][gc])로 교체.
//           로컬: 브루트 800/800, N=2000·U=10만 1.22s (기존 2.40s). USACO 재제출로 통과 확인 필요.
//   C++:    15/15 PASS — 원본 검증. **2026-07-13 lambda → regular function 로 재작성
//           (RFL_S2_CPP, RFL_S3_CPP, RFL_BRUTE_CPP, RFL_FAST_CPP). 알고리즘 동일하나
...
const RFL_S2_PY = [
  ...
  "# 칸 (r, c) 의 묶음 좌표 = (min(r, N+1-r), min(c, N+1-c)),  1 ~ H(=N/2)",
  "H = N // 2",
  "",
  "# painted[gr][gc] = 그 묶음 4칸 중 칠해진(#) 칸 수 — dict 아닌 2D 배열 (빠름)",
  "painted = [[0] * (H + 1) for _ in range(H + 1)]",
  "for r in range(1, N + 1):",
  "    gr = min(r, N + 1 - r)             # r 이 속한 묶음 행",
  "    for c in range(1, N + 1):",
  "        gc = min(c, N + 1 - c)         # c 이 속한 묶음 열",
  "        if grid[r - 1][c - 1] == '#':",
  "            painted[gr][gc] += 1",
];
const RFL_S2_CPP = [
  "    // count[gr][gc] = 묶음(gr, gc) 에 있는 # 개수 (0~4)",
  "    // gr, gc 는 1 부터 H(=N/2) 까지",
  "    int H = N / 2;",
  "    vector<vector<int>> count(H + 1, vector<int>(H + 1, 0));",
  "",
  "    for (int r = 1; r <= N; r++) {",
  "        for (int c = 1; c <= N; c++) {",
  "            int gr = min(r, N + 1 - r);         // r 이 속한 묶음 행",
  "            int gc = min(c, N + 1 - c);         // c 이 속한 묶음 열",
  "            if (grid[r-1][c-1] == '#') {",
  "                count[gr][gc]++;",
  "            }",
  "        }",
...
const RFL_S3_PY = [
  ...
  "total = 0",
  "for gr in range(1, H + 1):           # gr = 묶음 행",
  "    for gc in range(1, H + 1):       # gc = 묶음 열",
  "        total += min_flips(painted[gr][gc])",
  "",
  "answers = [total]",
];
const RFL_S3_CPP = [
  "    // 처음 답 = 모든 묶음의 최소 뒤집기 합",
  "    int total = 0;",
  "    for (int gr = 1; gr <= H; gr++) {           // gr = 묶음 행",
  "        for (int gc = 1; gc <= H; gc++) {       // gc = 묶음 열",
  "            int cnt = count[gr][gc];            // 이 묶음의 # 개수",
  "            total += min(cnt, 4 - cnt);         // 적은 쪽만 뒤집기",
  "        }",
  "    }",
  "    cout << total << '\\n';",
];
...
const RFL_BRUTE_CPP = [
  "// 지금 count[][] 상태에서 총 뒤집기 개수 세기",
  "// = 모든 묶음의 min(# 개수, 4 - # 개수) 를 더한 값",
  "int total_flips(vector<vector<int>>& count, int H) {",
  "    int total = 0;",
  "    for (int gr = 1; gr <= H; gr++) {           // gr = 묶음 행",
  "        for (int gc = 1; gc <= H; gc++) {       // gc = 묶음 열",
  "            int cnt = count[gr][gc];            // 이 묶음의 # 개수",
  ...
  "",
  "int main() {",
  "    // (입력 + count[][] 초기화 — 앞 섹션 그대로)",
  "",
  "    cout << total_flips(count, H) << '\\n';     // 처음 답",
  "",
  "    for (int i = 0; i < U; i++) {",
  "        int r, c;",
  "        cin >> r >> c;",
  ...
  "        }",
  "",
  "        // count[][] 를 처음부터 다시 세기 — 여기가 느린 부분!",
  "        // ... (앞 섹션의 count 초기화 코드 그대로 반복) ...",
  "",
  "        cout << total_flips(count, H) << '\\n';",
  "    }",
  "}",
...
const RFL_FAST_PY = [
  ...
  "grid = []",
  "for r in range(N):",
  "    grid.append(bytearray(data[pos])); pos += 1",
  "",
  "H = N // 2",
  "",
  "# painted[gr][gc] = 그 묶음에 칠해진(#) 칸 수 — 2D 배열 (dict 아님 → 빠름)",
  "painted = [[0] * (H + 1) for _ in range(H + 1)]",
  "for r in range(1, N + 1):",
  "    gr = min(r, N + 1 - r)             # r 이 속한 묶음 행",
  "    row = grid[r - 1]",
  ...
  "    gr = min(r, N + 1 - r)",
  "    gc = min(c, N + 1 - c)",
  "    before = min_flips(painted[gr][gc])",
  "",
  "    if grid[r - 1][c - 1] == 35:       # '#' → '.'",
  "        grid[r - 1][c - 1] = 46",
  "        painted[gr][gc] -= 1",
...
const RFL_FAST_CPP = [
  ...
  "using namespace std;",
  "",
  "// 한 묶음의 최소 뒤집기 = 적은 쪽 (# 개수 vs · 개수 중 작은 것)",
  "int flip_cost(int cnt) {",
  "    return min(cnt, 4 - cnt);",
  "}",
  "",
  "int main() {",
  ...
  "    }",
  "",
  "    // 처음 답 = 모든 묶음의 flip_cost 합",
  "    int total = 0;",
  "    for (int gr = 1; gr <= H; gr++) {",
  "        for (int gc = 1; gc <= H; gc++) {",
  "            total += flip_cost(count[gr][gc]);",
  "        }",
  "    }",
  "    cout << total << '\\n';",
  "",
  "    // 뒤집기마다 그 묶음 하나만 갱신 → O(1)",
  "    for (int i = 0; i < U; i++) {",
  "        int r, c;",
  "        cin >> r >> c;",
  "",
