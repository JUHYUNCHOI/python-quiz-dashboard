...
import { RectanglesProgressiveCode, downloadRectanglesPDF, getRectanglesSections } from "./components";
...
export default function RectanglesApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeRectanglesCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeRectanglesCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeRectanglesCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeRectanglesCh1, 1: (e) => makeRectanglesCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <RectanglesProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <RectanglesProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    return null;
  };
  ...
  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      ...
      <button onClick={() => downloadRectanglesPDF(E, getRectanglesSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`rect-${tab}-${cur}-${lang}`} text={step.narr} />}
