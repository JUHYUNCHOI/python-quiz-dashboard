...
if (navMisclick.size) {
  ...
  console.log('      아무 일도 안 나는 게 아니라 **엉뚱한 버튼이 눌릴 수 있다.**')
  /* ⚠️ 2026-09-25 정정 — 여기 「**눌려 화면이 넘어간다**」고 단정해 놓았는데
     **실제로 눌러 보니 안 그랬다.** 화면 담당이 브라우저로 진짜 클릭을 4건
     (moohunt·buymilk × 데스크탑·모바일) 했고 **전부 복사됐다** — 진행 표시가
     안 바뀌어 쪽 넘김이 없었고, 클립보드에 코드 전문이 들어간 것까지 확인했다.
     이유: `QuestNavBar.jsx`(`fee8617c`, 2026-09-25)가 바 전체에 `pointerEvents:"none"`
     을 걸고 **Prev/Next 두 버튼에만** `"auto"` 를 준다. 이 검사기의 `elementFromPoint`
     는 `pointer-events` 를 존중하므로 기제 자체는 옳다 — 다만 **버튼 한가운데 점
     하나**만 보기 때문에, 그 점이 하필 작은 Prev/Next 위에 얹힐 때만 걸린다.
     즉 이 신고는 **「그럴 수 있다」이고 「그렇다」가 아니다.**
     ⛔ 단정을 그대로 두면 학생·검토자가 이걸 계속 결함으로 올리고(오늘 그랬다)
        이미 고쳐진 것을 또 고치게 된다. **말의 세기를 줄이는 게 맞다.**
     ⚠️ 그렇다고 이 신고를 빼지는 마라 — `socialdist1` 모바일에서 복사 버튼 자리를
        누르면 `다음 →` 이 눌린 건 **실제로 있었던 일**이다. */
  console.log('   ⚠️ **단, 이건 「그럴 수 있다」이고 「그렇다」가 아니다.**')
  console.log('      2026-09-25 실측: 브라우저로 진짜 클릭한 4건은 **전부 정상 동작했다** —')
  console.log('      `QuestNavBar` 가 바 전체에 `pointerEvents:"none"` 을 걸어 클릭이 통과한다.')
  console.log('      이 검사기는 **버튼 한가운데 점 하나**만 본다. 결함으로 올리기 전에')
  console.log('      **브라우저로 그 자리를 실제로 눌러 보고** 무엇이 일어나는지 확인해라.')
}
...
if (coveredAtTop.length) {
  ...
  /* ⚠️ 2026-09-25: 처음 여기 「고정 바 높이만큼 여백을 줘라」고 적었는데 **그게 틀렸다.**
     실측: 바 68px · `QuestNavBar` 가 문서 끝에 두는 스페이서 78px → **여백은 이미 남는다.**
     여백을 더하면 그 요소가 **더 아래로 밀려 오히려 나빠진다**(실제로 담당이 한 번 그렇게 갔다). */
  console.log('   ⛔ 여백(margin-bottom)을 더하지 마라 — 바 68px 에 스페이서가 이미 78px 이다.')
  console.log('      **더 밀리면 더 나빠진다.** 그 요소를 위로 올려야 한다.')
  console.log('   ⭕ 그 쪽의 **주된 조작기**라면(슬라이더·입력칸) 그림·표보다 **위로 올려라** —')
  console.log('      학생이 조작기를 먼저 보고 결과가 아래에서 바뀌는 순서가 되어 오히려 낫다.')
  console.log('   ⭕ 보조 버튼(복사 등)이면 스크롤로 닿으니 **판정이 필요하다** — 자동으로 고치지 마라.')
} else {
