...
import { ExplodingArrowProgressiveCode, downloadExplodingArrowPDF, getExplodingArrowSections, getExplodingArrowWalk } from "./components";
...
export default function ExplodingArrowApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeExplodingArrowCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeExplodingArrowCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeExplodingArrowCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeExplodingArrowCh1, 1: (e) => makeExplodingArrowCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <ExplodingArrowProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "explodingarrow-walk") return <CodeWalk E={E} lang={codeLang} {...getExplodingArrowWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <ExplodingArrowProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "explodingarrow-walk") return <CodeWalk E={E} lang={codeLang} {...getExplodingArrowWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadExplodingArrowPDF(E, getExplodingArrowSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`expl-${tab}-${cur}-${lang}`} text={step.narr} />}
