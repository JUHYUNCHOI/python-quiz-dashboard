...
export const QUEST_CONCEPT_META: Record<string, QuestConceptMeta> = {
  ...
  moolang: {
    type: "brute-force",
    concepts_taught: ["complete-search", "sentence-construction", "comma-conj-budgeting"],
    concepts_required: ["loop", "list-basics", "string-basics"],
    difficulty: 4,
    supported_languages: ["py", "cpp"],
    validate_io: [
      // Deterministic mini-case: 1 noun + 1 intrans verb → exactly one type-1 sentence.
      // The official samples have multiple valid outputs (paragraph order/structure
      // is not unique), so they're checked against a fixed reference here that
      // matches THIS solution's particular choice — any correct count is accepted
      // by the USACO judge, but for regression we pick a deterministic case.
      {
    ...
    solution_py: [
      ...
      "",
      "    # Try every type-2 count; for each, derive the best type-1 count.",
      "    ans = (0, 0, 0, 0)",
  ...
  stampgrid: {
    ...
    supported_languages: ["py", "cpp"],
    validate_io: [
      // Official USACO Feb 2023 Bronze #2 sample 1 (4 test cases)
      {
        input: "4\n2\n**\n*.\n1\n*\n3\n.**\n.**\n***\n2\n.*\n**\n3\n...\n.*.\n...\n3\n.*.\n...\n...\n3\n**.\n.**\n..*\n2\n.*\n*.\n",
        expected: "YES\nYES\nNO\nYES",
      },
  ...
  candycane: {
    ...
    solution_py: [
      ...
      "M = int(data[p])",
      "p += 1",
      "h = [int(data[p + i]) for i in range(N)]",
      "p += N",
