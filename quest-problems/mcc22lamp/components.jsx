...
const FULL_CPP = [
  ...
  "",
  "int main() {",
  "    int N, M;                 // N lamps, M toggle ops",
  "    cin >> N >> M;",
  "",
  "    // Difference array: for toggle(L, R) add +1 at L, -1 at R+1",
  "    vector<int> toggles(N + 2, 0);",
  ...
  "    }",
  "",
  "    // Prefix sum gives the toggle count for each lamp",
  "    int count = 0;",
  "    int ans = 0;",
  "    for (int i = 1; i <= N; i++) {",
  "        count += toggles[i];",
  ...
  return [
    {
      ...
      why: [
        ...
        t(E, "The C++ version reads N, M and the ranges with cin, and uses a vector<int> difference array — same +1 at L, -1 at R+1 trick, then a prefix sum counting odd (ON) lamps.",
            "C++ 버전은 cin 으로 N, M 과 범위를 읽고 vector<int> 차분 배열을 써요 — L 에 +1, R+1 에 -1 같은 방식, 그다음 누적합으로 홀수(켜짐) 램프를 세요."),
      ],
      pyOnly: [
