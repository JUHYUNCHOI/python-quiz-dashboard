...
export default function Mooin3App(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeMooin3Ch1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeMooin3Ch2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeMooin3Ch2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeMooin3Ch1, 1: (e) => makeMooin3Ch2(e, codeLang) };
  ...
  // Hover preview body — same logic as renderContent but with no-op
  // handlers (preview is read-only).
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <Mooin3ProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "sim") return <Mooin3Sim E={E} />;
    return null;
  };
  ...
  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadMooin3PDF(E, getMooin3Sections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`m3-${tab}-${cur}-${lang}`} text={step.narr} />}
