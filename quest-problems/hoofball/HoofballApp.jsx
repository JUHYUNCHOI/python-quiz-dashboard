...
import { HoofballProgressiveCode, downloadHoofballPDF, getHoofballSections, getHoofballWalk } from "./components";
import { makeHoofballCh1, makeHoofballCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#f97316";

export default function HoofballApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeHoofballCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeHoofballCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeHoofballCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeHoofballCh1, 1: (e) => makeHoofballCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <HoofballProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "hoofball-codewalk") return <CodeWalk E={E} lang={codeLang} {...getHoofballWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <HoofballProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "hoofball-codewalk") return <CodeWalk E={E} lang={codeLang} {...getHoofballWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadHoofballPDF(E, getHoofballSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`hoof-${tab}-${cur}-${lang}`} text={step.narr} />}
