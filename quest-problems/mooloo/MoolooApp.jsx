...
import { MoolooProgressiveCode, downloadMoolooPDF, getMoolooSections, getMoolooWalk } from "./components";
...
export default function MoolooApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeMoolooCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeMoolooCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeMoolooCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeMoolooCh1, 1: (e) => makeMoolooCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <MoolooProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getMoolooWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <MoolooProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getMoolooWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadMoolooPDF(E, getMoolooSections(E), codeLang)} style={{
