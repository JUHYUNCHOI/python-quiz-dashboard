...
export function getChipXchgWalk(E, lang = "py") {
  ...
  if (lang === "cpp") {
    const code = [
      ...
      "        need = missing;                 //   " + c("1 red = 1 chip", "빨강 1개 = 칩 1개"),
      "    } else if (missing % cA == 0) {     // " + c("blue groups divide it exactly", "파랑 묶음으로 딱 떨어질 때"),
      "        need = (missing / cA - 1) * cB + cA;      // " + c("skip the last group, take cA singles", "마지막 묶음은 안 사고 낱개 cA개로"),
      "    } else {                            // " + c("does not divide exactly", "딱 안 떨어질 때"),
      "        need = missing / cA * cB + missing % cA;  // " + c("groups + the rest as singles", "묶음들 + 남는 건 낱개로"),
      "    }",
      "    return waste + need;                // " + c("wasted blue + chips to fill", "버린 파랑 + 채우는 데 든 칩"),
      "}",
      "",
      "int main() {",
      "    int T; cin >> T;",
    ...
    return { code, vars: _CX_VARS, beats: [
      ...
      { hi: [18, 18], bubble: t(E, "answer = wasted blue + chips to fill. Check (0 0 2 3 5): missing 5, waste 2, 5 % 2 = 1 → case 3 → 5/2*3 + 1 = 7, so 2 + 7 = 9. One calculation, O(1).", "답 = 버린 파랑 + 채우는 데 든 칩. (0 0 2 3 5) 로 확인: missing 5, waste 2, 5 % 2 = 1 이라 경우 3 → 5/2*3 + 1 = 7, 그래서 2 + 7 = 9. 계산 한 번, O(1).") },
    ] };
  }
  const code = [
    "import sys",
  ...
  return { code, vars: _CX_VARS, beats: [
    ...
    { hi: [24, 24], bubble: t(E, "answer = wasted blue + chips to fill. Check (0 0 2 3 5): missing 5, waste 2, 5 % 2 = 1 → case 3 → 5//2*3 + 1 = 7, so 2 + 7 = 9. One calculation, O(1).", "답 = 버린 파랑 + 채우는 데 든 칩. (0 0 2 3 5) 로 확인: missing 5, waste 2, 5 % 2 = 1 이라 경우 3 → 5//2*3 + 1 = 7, 그래서 2 + 7 = 9. 계산 한 번, O(1).") },
  ] };
}
...
export function getChipXchgFormulaWalk(E, lang = "py") {
  ...
  const code = lang === "cpp" ? [
    ...
    "ll need;                          // " + c("trickster step 2: fill with the least-helpful color", "심술쟁이 2단계: 가장 덜 도와주는 색으로 채움"),
    "if (cA >= cB) need = missing;                         //   " + c("swap pays → red (1 each)", "환전 이득 → 빨강 (1개당 1)"),
    "else if (missing % cA == 0)                           //   " + c("groups divide it exactly", "묶음으로 딱 떨어짐"),
    "    need = (missing / cA - 1) * cB + cA;              //     " + c("skip the last group → cA singles", "마지막 묶음 대신 낱개 cA개"),
    "else need = missing / cA * cB + missing % cA;         //   " + c("groups + the rest as singles", "묶음들 + 남는 건 낱개로"),
    "ll answer = waste + need;",
  ] : [
    "# " + c("first: can I reach the goal with what I have now?", "먼저: 지금 가진 걸로 목표에 닿나?"),
    ...
    "    need = missing                #   " + c("red (1 each)", "빨강으로 (1개당 1)"),
