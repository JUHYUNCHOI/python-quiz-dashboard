...
import { UdderedProgressiveCode, downloadUdderedPDF, getUdderedSections, getUdderedWalk } from "./components";
import { makeUdderedCh1, makeUdderedCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#dc2626";

export default function UdderedApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeUdderedCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeUdderedCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeUdderedCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeUdderedCh1, 1: (e) => makeUdderedCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <UdderedProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "uddered-walk") return <CodeWalk E={E} lang={codeLang} {...getUdderedWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <UdderedProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "uddered-walk") return <CodeWalk E={E} lang={codeLang} {...getUdderedWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadUdderedPDF(E, getUdderedSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`udde-${tab}-${cur}-${lang}`} text={step.narr} />}
