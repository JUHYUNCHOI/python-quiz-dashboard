...
export default function CowTipApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeCowTipCh1(false));
  const [ch2Q, setCh2Q] = useState(() => makeCowTipCh2(false, "py"));

  useEffect(() => {
    setCh2Q(prev => makeCowTipCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeCowTipCh1, 1: (e) => makeCowTipCh2(e, codeLang) };
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 12, alignItems: "center" }}>
          ...
          {showCodeControls && (
            <div style={{ display: "flex", gap: 0, alignItems: "stretch", flexShrink: 0 }}>
              ...
              <button onClick={() => downloadCowTipPDF(E, getCowTipSections(E), codeLang)} style={{
