...
import { SimpleGameProgressiveCode, downloadSimpleGamePDF, getSimpleGameSections } from "./components";
...
export default function SimpleGameApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeSimpleGameCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeSimpleGameCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeSimpleGameCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeSimpleGameCh1, 1: (e) => makeSimpleGameCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <SimpleGameProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <SimpleGameProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    return null;
  };
  ...
  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      ...
      <button onClick={() => downloadSimpleGamePDF(E, getSimpleGameSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`simp-${tab}-${cur}-${lang}`} text={step.narr} />}
