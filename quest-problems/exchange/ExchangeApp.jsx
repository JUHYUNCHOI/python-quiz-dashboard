...
import { ExchangeProgressiveCode, downloadExchangePDF, getExchangeSections, getExchangeWalk, ExchangeSim, ExchangeRunner } from "./components";
...
export default function ExchangeApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeExchangeCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeExchangeCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeExchangeCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeExchangeCh1, 1: (e) => makeExchangeCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "exchange-walk") return <CodeWalk E={E} lang={codeLang} {...getExchangeWalk(E, codeLang)} accent={A} />;
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <ExchangeProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "exchange-walk") return <CodeWalk E={E} lang={codeLang} {...getExchangeWalk(E, codeLang)} accent={A} />;
    if (s.type === "sim") return <ExchangeSim E={E} />;
    if (s.type === "runner") return <ExchangeRunner E={E} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadExchangePDF(E, getExchangeSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`exch-${tab}-${cur}-${lang}`} text={step.narr} />}
