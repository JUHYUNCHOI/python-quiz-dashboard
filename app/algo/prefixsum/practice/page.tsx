...
const SOLVED_KEY = "algo-prefixsum-contest-solved"
...
export default function SortingPracticePage() {
  ...
  const problem = problemId ? prefixSumContestCluster.problems.find(p => p.id === problemId) : null
  ...
  // 단일 문제 풀이 화면
  if (problem) {
    const problemIdx = prefixSumContestCluster.problems.findIndex(p => p.id === problem.id) + 1
    return (
      <div className="min-h-screen bg-gray-50 pb-24">
        ...
        <main className="max-w-5xl mx-auto px-4 pt-4">
          <div className="mb-3 flex items-center justify-between gap-2">
            <button
              onClick={() => router.push("/algo/prefixsum")}
        ...
        <LessonPanel open={showLesson} onClose={() => setShowLesson(false)} title={t("누적합 수업", "Prefix Sum lesson")}>
          <PrefixSumLesson />
        </LessonPanel>
        <BottomNav />
      </div>
  ...
  // 리스트 화면
  const totalSolved = prefixSumContestCluster.problems.filter(p => solvedSet.has(p.id)).length
  const total = prefixSumContestCluster.problems.length
      ...
      <main className="max-w-2xl mx-auto px-4 pt-4">
        <div className="mb-4">
          <JourneyBreadcrumb items={[
            ...
            { label: "누적합", labelEn: "Prefix Sum", href: "/algo/prefixsum", emoji: "📊" },
            { label: "문제 풀이", labelEn: "Practice" },
          ]} />
          <div className="flex items-center gap-2 mb-2">
            <span className="text-3xl">{prefixSumContestCluster.emoji}</span>
            <h1 className="text-xl sm:text-2xl font-black text-gray-900">
              {t(prefixSumContestCluster.title, prefixSumContestCluster.en?.title ?? prefixSumContestCluster.title)}
            </h1>
          </div>
          <p className="text-sm text-gray-600 mb-3">
            {t(prefixSumContestCluster.description, prefixSumContestCluster.en?.description ?? prefixSumContestCluster.description)}
          </p>
        ...
        {(() => {
          const warmup = prefixSumContestCluster.problems.filter(p => p.difficulty === "쉬움")
          ...
          const renderItem = (p: any) => {
            ...
            return (
              <Link
                key={p.id}
                href={`/algo/prefixsum/practice?p=${p.id}`}
                className={cn(
