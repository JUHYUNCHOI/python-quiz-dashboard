...
import { AlchemyProgressiveCode, downloadAlchemyPDF, getAlchemySections } from "./components";
import { makeAlchemyCh1, makeAlchemyCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";

const A = "#d97706";

export default function AlchemyApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeAlchemyCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeAlchemyCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeAlchemyCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeAlchemyCh1, 1: (e) => makeAlchemyCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <AlchemyProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <AlchemyProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    return null;
  };
  ...
  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      ...
      <button onClick={() => downloadAlchemyPDF(E, getAlchemySections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`alch-${tab}-${cur}-${lang}`} text={step.narr} />}
