...
> **정렬 한 번 해두면 검색이 거의 5 만 배 빨라져요.** 그래서 USACO 문제는 거의 "일단 sort 부터" 하고 시작.`
        },
        {
          ...
          content: `다음 페이지의 \`lower_bound\` 가 결과로 숫자가 아니라 **이터레이터 (iterator)** 라는 낯선 걸 돌려줘요. 미리 친해지고 가요 — 별 거 아니에요!

---

### 이터레이터 = "벡터를 가리키는 손가락" 🫵

벡터 \`{10, 20, 30, 40, 50}\` 에 손가락 하나를 올려놨다고 상상해봐요. 그 손가락이 바로 **이터레이터** 예요. 첫 자리에 올려두면 10 을 가리키고, 한 칸 옮기면 20 을 가리키고… 그런 식.
...
사실 우리가 \`sort(v.begin(), v.end())\` 쓸 때마다 이미 손가락 두 개를 넘긴 거였어요 — 첫 손가락 (\`begin()\`) 부터 끝 표지 (\`end()\`) 까지 정렬해 달라는 뜻.

> 💡 \`end()\` 가 *마지막* 이 아니라 *마지막 다음* 인 게 살짝 어색한데, "[begin, end) 사이만 진짜 데이터" 라고 STL 전체가 약속해 둔 거예요. 익숙해지면 편함.

---

### 손가락으로 할 줄 알아야 하는 건 딱 두 가지

| 문법 | 손가락이 하는 일 |
|---|---|
| \`*it\` | **손가락이 가리키는 자리의 값을 봐요** (\`*\` = "여기 뭐 있어?") |
| \`++it\` | **손가락을 옆 칸으로 한 칸 옮겨요** |

그래서 벡터를 처음부터 끝까지 훑는 코드는 이렇게 생겼어요:
...
한 줄씩 풀어보면:
- \`auto it = v.begin()\` → 손가락을 **첫 자리** 에 올려요 (10 가리킴)
- \`it != v.end()\` → 손가락이 **끝 표지에 닿기 전까지만** 반복 (끝 표지 자체는 값 없으니까 안 읽음)
- \`++it\` → 한 칸 오른쪽으로
- \`*it\` → 지금 손가락이 보고 있는 값을 출력

> 💡 사실 평소엔 \`for (int x : v)\` (range-for) 가 훨씬 짧고 흔히 써요. 위 패턴은 **이터레이터가 어떻게 움직이는지 머릿속에 그려보는 용도** — 다음 페이지에서 \`lower_bound\` 가 손가락을 어디에 올려주는지 이해하려면 필요해요.

---

### 혹시 포인터 들어본 적 있다면?

이터레이터는 **포인터의 똑똑한 사촌** 이에요. \`*\` 로 값 보고 \`++\` 로 다음 가는 게 똑같음. 차이는 vector / list / map 어디서나 같은 코드로 돌아간다는 점이에요. **다음 페이지에서 직접 눈으로 보여줄게요** — 그러니까 지금은 "아, 손가락이구나" 정도면 충분!`
        },
        {
          id: "s23-ch2-iter-sim",
          type: "animation" as const,
