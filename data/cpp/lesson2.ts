...
학교에 **민수**가 여러 명이면 어떻게 구분하나요?
→ "**1반** 민수", "**2반** 민수" 이렇게 **소속(반)을 앞에** 붙이죠!

C++도 똑같아요. 도구 이름이 겹칠 수 있어서 **그룹 이름을 앞에** 붙여요.

![std 네임스페이스 구조](/images/cpp/std-namespace.svg)

여기서 **std** = **standard** (표준) — C++이 기본으로 제공하는 도구 그룹이에요.

이 "그룹"의 공식 이름이 **namespace**예요.
→ \`std\`는 C++의 **표준 namespace**, 그 안에 \`cout\`, \`endl\`, \`string\` 등이 들어있어요.
...
@핵심: 규칙: **그룹이름** + **::** + **도구이름** → \`std::cout\`, \`std::endl\``
        },
        {
          ...
          content: `이 "그룹"의 공식 이름이 **namespace(네임스페이스)**예요!
...
→ C++이 기본으로 제공하는 모든 도구가 여기 들어있어요!

💡 그래서 계속 \`std::cout\`이라고 썼던 거예요 — "성(姓)"을 붙여서 정확히 지정하는 거죠!`
        },
        {
          id: "ch2-pred1",
          type: "predict" as const,
