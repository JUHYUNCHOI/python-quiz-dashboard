...
import { Acowdemia2ProgressiveCode, downloadAcowdemia2PDF, getAcowdemia2Sections, getAcowdemia2Walk } from "./components";
import { makeAcow2Ch1, makeAcow2Ch2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";

const A = "#2563eb";

export default function Acowdemia2App(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeAcow2Ch1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeAcow2Ch2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeAcow2Ch2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeAcow2Ch1, 1: (e) => makeAcow2Ch2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <Acowdemia2ProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getAcowdemia2Walk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <Acowdemia2ProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getAcowdemia2Walk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadAcowdemia2PDF(E, getAcowdemia2Sections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`acow-${tab}-${cur}-${lang}`} text={step.narr} />}
