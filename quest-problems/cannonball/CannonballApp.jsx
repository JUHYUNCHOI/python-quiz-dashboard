...
import { CannonballProgressiveCode, downloadCannonballPDF, getCannonballSections, getCannonballWalk } from "./components";
import { makeCannonCh1, makeCannonCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";

const A = "#f97316";

export default function CannonballApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeCannonCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeCannonCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeCannonCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeCannonCh1, 1: (e) => makeCannonCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <CannonballProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getCannonballWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <CannonballProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getCannonballWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadCannonballPDF(E, getCannonballSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`cann-${tab}-${cur}-${lang}`} text={step.narr} />}
