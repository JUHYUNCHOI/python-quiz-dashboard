...
const FULL_CPP = [
  ...
  "    vector<long long> t; { long long _x; while (cin >> _x) t.push_back(_x); if (!cin) cin.clear(); } // adapt: read N values",
  "",
  "    // lo, hi = 0, 10**18",
  "    while (lo < hi) {",
  "        auto mid = (lo + hi) // 2;",
  "        auto cur = [h[i] + a[i] * mid for i in range(N)];",
  "        auto order = sorted(range(N), key=lambda i: cur[i]);",
  "        if (order == t) {",
  "            auto hi = mid;",
  "        else {",
  "            auto lo = mid + 1;",
...
export function FjFarmsProgressiveCode({ E, lang = "py", sections }) {
