...
const PSQ_FAST_PY = [
  ...
  "# ─── ⭐ 재귀는 전부 여기 ───",
  "",
  "# can(쪽지): budget 개 이하로 만들 수 있나?",
  "def can(seq, budget):",
  "    if (seq, budget) in memo:         # 이미 푼 쪽지면",
  "        return memo[(seq, budget)]    #   답 꺼내기",
  "",
  "    # ✋ 베이스 케이스 — 여기서 멈춤!",
  "    if len(seq) == 0 or budget <= 0:",
  "        return False",
  ...
  "                if can(seq[:block_len], budget):   # ↺ 재귀!",
  "                    result = True",
  "                    break",
  "",
  "    # 요령 ③: 둘로 잘라서 예산 나눠 갖기",
  ...
  "            for left_budget in range(1, budget):",
  "                right_budget = budget - left_budget",
  "                if can(seq[:cut], left_budget):        # ↺ 왼쪽 되나?",
  "                    if can(seq[cut:], right_budget):   # ↺ 오른쪽도?",
  "                        result = True",
  "                        break",
  "            if result:",
  "                break",
  "",
  "    memo[(seq, budget)] = result      # 답 적어두기",
  "    return result",
  "",
  "for _ in range(T):",
  "    # 첫 줄: N 과 K",
...
const PSQ_FAST_CPP = [
  ...
  "    if (memo.count(key) > 0) {       // 이미 푼 쪽지면",
  "        return memo[key];            //   답 꺼내기",
  "    }",
  "",
  "    // ✋ 베이스 케이스 — 여기서 멈춤!",
  ...
  "            if (is_repeat_of(seq, block_len)) {",
  "                vector<int> block = slice(seq, 0, block_len);",
  "                if (can(block, budget)) {   // ↺ 재귀!",
  "                    result = true;",
  "                    break;",
  "                }",
  "            }",
  ...
  "                vector<int> left  = slice(seq, 0, cut);",
  "                vector<int> right = slice(seq, cut, n);",
  "                int right_budget = budget - left_budget;",
  "                if (can(left, left_budget)) {        // ↺ 왼쪽 되나?",
  "                    if (can(right, right_budget)) {  // ↺ 오른쪽도?",
  "                        result = true;",
  "                        break;",
  "                    }",
  "                }",
...
const PSQ_FAST_PY_EN = [
  ...
  "    return True",
  "",
  "# ─── ⭐ ALL the recursion lives here ───",
  "",
  "# can(note): makeable with <= budget PRINTs?",
  "def can(seq, budget):",
  "    if (seq, budget) in memo:         # solved before?",
  "        return memo[(seq, budget)]    #   reuse the answer",
  "",
  ...
  "            block_len = n // copies",
  "            if is_repeat_of(seq, block_len):",
  "                if can(seq[:block_len], budget):   # ↺ recursion!",
  "                    result = True",
  "                    break",
  "",
  ...
  "            for left_budget in range(1, budget):",
  "                right_budget = budget - left_budget",
  "                if can(seq[:cut], left_budget):        # ↺ left works?",
  "                    if can(seq[cut:], right_budget):   # ↺ right too?",
  "                        result = True",
  "                        break",
  "            if result:",
  "                break",
  "",
  "    memo[(seq, budget)] = result      # write it down",
  "    return result",
  "",
  "for _ in range(T):",
  "    # first line: N and K",
...
const PSQ_FAST_CPP_EN = [
  ...
  "bool can(vector<int> seq, int budget) {",
  "    string key = make_key(seq, budget);",
  "    if (memo.count(key) > 0) {       // solved before?",
  "        return memo[key];            //   reuse the answer",
  "    }",
  "",
  "    // ✋ BASE CASE — recursion stops here!",
  "    if ((int)seq.size() == 0 || budget <= 0) {",
...
export function getPrintseqWalk(E, lang = "py") {
  ...
  if (lang === "cpp") {
    return {
      ...
      vars,
      // ✋ 베이스(장미) + ↺ 재귀 호출(남보라) 상시 강조 — 재귀는 전부 can 안 (선생님 2026-07-17)
      marks: [
        { from: 62, to: 65, ko: "✋ 베이스 케이스", en: "✋ base case" },
        { from: 84, to: 84, color: "#818cf8", ko: "↺ 재귀!", en: "↺ recursion!" },
        { from: 99, to: 100, color: "#818cf8", ko: "↺ 재귀!", en: "↺ recursion!" },
      ],
      beats: [
        { hi: [113, 122], bubble: t(E,
        ...
        { hi: [124, 128], bubble: t(E,
          "What do we DO with that input?\nCall can(target, K) → print YES / NO. That's the goal.\nNow let's read can — ALL the recursion lives in there ⭐",
          "이 입력으로 뭘 하지?\ncan(목표, K) 를 불러 YES / NO 출력. 이게 목표.\n이제 can 을 읽어요 — 재귀는 전부 그 안에 있어요 ⭐") },
        { hi: [55, 60], bubble: t(E,
          "can — the question-note. First: check the memo.\nSolved this exact note before? → hand back the saved answer.\n(key = the string make_key builds, lines 12~21)",
          "can — '질문 쪽지'. 받자마자 먼저: 메모 확인.\n똑같은 쪽지를 푼 적 있으면 → 적어둔 답을 바로 꺼내요.\n(열쇠 = 위 make_key(12~21행)가 만든 문자열)") },
        { hi: [6, 8], bubble: t(E,
        ...
        { hi: [67, 73], bubble: t(E,
          "Trick ①: \"all the same?\" — we ask the helper is_all_same (a worker, no recursion).\nIf yes → one PRINT wrapped in REP finishes it → true.",
          "요령 ①: \"다 같아?\" — 도우미 is_all_same 에게 물어봐요 (재귀 없는 일꾼).\n맞으면 → PRINT 1 개를 REP 로 감싸면 끝 → true.") },
        { hi: [75, 90], bubble: t(E,
          "Trick ②: if it's a repeated block (is_repeat_of checks the shape),\ncut out ONE block and — ↺ can(block, budget)!\ncan calls ITSELF: the same question, just smaller. That's the recursion.",
          "요령 ②: 블록의 반복이면 (모양 확인은 is_repeat_of 도우미),\n블록 하나만 잘라서 — ↺ can(block, budget)!\ncan 이 '자기 자신'을 불러요: 같은 질문, 더 작게. 이게 재귀.") },
        { hi: [92, 107], bubble: t(E,
          "Trick ③: cut in two, split the budget —\nthen ↺ can(left) AND ↺ can(right): recursion TWICE!\nBoth yes → glue the two programs → success. Try every cut × every split.",
          "요령 ③: 둘로 자르고 예산도 나눠서 —\n↺ can(왼쪽) 그리고 ↺ can(오른쪽): 재귀 두 번!\n둘 다 되면 → 두 프로그램 이어 붙여 성공. 모든 자르기 × 나누기 시도.") },
        { hi: [109, 110], bubble: t(E,
          "Finally: write the answer into memo, then return it. Done!",
          "마지막: 답을 memo 에 적고 돌려줘요. 끝!") },
        { hi: [22, 32], bubble: t(E,
          "Now the helpers — NO recursion here, just simple checks.\nis_all_same: any number different from the first → false.",
          "이제 도우미들 — 여긴 재귀 ❌, 그냥 단순 검사예요.\nis_all_same: 첫 숫자와 다른 게 하나라도 있으면 false.") },
        { hi: [34, 42], bubble: t(E,
          "is_repeat_of: does a block of block_len repeat all the way?\nJust a shape check — the recursion stays inside can!",
          "is_repeat_of: block_len 짜리 블록이 끝까지 반복되는 모양인지만 확인.\n그냥 모양 검사 — 재귀는 can 안에만 있어요!") },
        { hi: [44, 51], bubble: t(E,
          "slice: cut out the [from, to) piece — C++'s version of Python's seq[from:to].\nAlso no recursion. Workers work, can thinks.",
          "slice: [from, to) 조각 잘라내기 — 파이썬 seq[from:to] 의 C++ 판.\n역시 재귀 없음. 일꾼은 일하고, 생각(재귀)은 can 이 해요.") },
      ],
    };
  }
  return {
    ...
    // ✋ 베이스(장미) + ↺ 재귀 호출(남보라) 상시 강조 — 재귀는 전부 can 안 (선생님 2026-07-17)
    marks: [
      { from: 29, to: 31, ko: "✋ 베이스 케이스", en: "✋ base case" },
      { from: 47, to: 47, color: "#818cf8", ko: "↺ 재귀!", en: "↺ recursion!" },
      { from: 56, to: 57, color: "#818cf8", ko: "↺ 재귀!", en: "↺ recursion!" },
    ],
    beats: [
      { hi: [0, 0], bubble: t(E,
      ...
      { hi: [33, 38], bubble: t(E,
        ...
        "요령 ①: \"다 같아?\" — 도우미 is_all_same 에게 물어봐요 (재귀 없는 일꾼).\n맞으면 → PRINT 1 개를 REP 로 감싸면 끝 → True.") },
      { hi: [40, 49], bubble: t(E,
        "Trick ②: if it's a repeated block (is_repeat_of checks the shape),\nthen — ↺ can(seq[:block_len], budget)!\ncan calls ITSELF: the same question, just smaller. That's the recursion.",
        "요령 ②: 블록의 반복이면 (모양 확인은 is_repeat_of 도우미),\n그럼 — ↺ can(seq[:block_len], budget)!\ncan 이 '자기 자신'을 불러요: 같은 질문, 더 작게. 이게 재귀.") },
      { hi: [51, 61], bubble: t(E,
        "Trick ③: cut in two, split the budget —\nthen ↺ can(left piece) AND ↺ can(right piece): recursion TWICE!\nBoth yes → glue the two programs → success. Try every cut × every split.",
        "요령 ③: 둘로 자르고 예산도 나눠서 —\n↺ can(왼쪽 조각) 그리고 ↺ can(오른쪽 조각): 재귀 두 번!\n둘 다 되면 → 두 프로그램 이어 붙여 성공. 모든 자르기 × 나누기 시도.") },
      { hi: [63, 64], bubble: t(E,
        "Finally: write the answer into memo, then return it. Done!",
        "마지막: 답을 memo 에 적고 돌려줘요. 끝!") },
      { hi: [6, 13], bubble: t(E,
        ...
        "이제 도우미들 — 여긴 재귀 ❌, 그냥 단순 검사예요.\nis_all_same: 첫 숫자와 다른 게 하나라도 있으면 False.") },
      { hi: [15, 20], bubble: t(E,
        "is_repeat_of: does a block of block_len repeat all the way?\nJust a shape check — the recursion stays inside can!\nWorkers work, can thinks.",
        "is_repeat_of: block_len 짜리 블록이 끝까지 반복되는 모양인지만 확인.\n그냥 모양 검사 — 재귀는 can 안에만!\n일꾼은 일하고, 생각(재귀)은 can 이 해요.") },
    ],
  };
}
