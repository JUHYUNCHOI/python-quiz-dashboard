...
const FULL_CPP = [
  ...
  "    map<string, long long> milk;",
  "    for (auto& c : cows) milk[c] = 0;",
  "    int N; cin >> N;",
  "    while (N--) {",
  "        string name; long long m; cin >> name >> m;",
  "        milk[name] += m;",
  "    }",
  "    // Find second-lowest non-zero, or lowest with at least one tie",
  "    vector<pair<long long, string>> sorted_;",
  "    for (auto& c : cows) sorted_.push_back({milk[c], c});",
  "    sort(sorted_.begin(), sorted_.end());",
  "    if (sorted_[0].first == sorted_[1].first) cout << \"Tie\n\";",
  "    else cout << sorted_[1].second << \"\n\";",
  "    return 0;",
  "}",
];
...
export function DontBeLastProgressiveCode({ E, lang = "py", sections }) {
