...
import { PermProgressiveCode, downloadPermPDF, getPermSections, getPermWalk, PermSim, PermRunner } from "./components";
import { makePermCh1, makePermCh2, makePermCh3 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";

const A = "#7c5cfc";

export default function PermutationApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makePermCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makePermCh2(lang === "en"));
  const [ch3Q, setCh3Q] = useState(() => makePermCh3(lang === "en", "py"));

  useEffect(() => {
    setCh3Q(prev => makePermCh3(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makePermCh1, 1: makePermCh2, 2: (e) => makePermCh3(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "perm-walk") return <CodeWalk E={E} lang={codeLang} {...getPermWalk(E, codeLang)} accent={A} />;
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <PermProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "perm-walk") return <CodeWalk E={E} lang={codeLang} {...getPermWalk(E, codeLang)} accent={A} />;
    if (s.type === "sim") return <PermSim E={E} />;
    if (s.type === "runner") return <PermRunner E={E} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadPermPDF(E, getPermSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`perm-${tab}-${cur}-${lang}`} text={step.narr} />}
