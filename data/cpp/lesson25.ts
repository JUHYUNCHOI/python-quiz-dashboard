...
> 💡 \`+4\` 는 \`int\` (4바이트) 일 때. \`double\` 면 \`+8\`, \`char\` 면 \`+1\` — 컴파일러가 타입 보고 \`sizeof\` 만큼 알아서 점프해요.

쉽게 말하면:

> 🧭 **포인터 = 종이 쪽지에 적힌 주소** ("ABC 아파트 301호"). 다음 집 = 302호 (그냥 +1).
> 🧭 **이터레이터 = GPS 내비게이션**. 주소 알고, "다음으로" 누르면 알아서 가요.
...
**컨테이너 다룰 땐 거의 안 써요.** 이터레이터 (또는 \`range-for\`) 가 표준이에요.

| 상황 | 요즘 쓰는 게 |
|---|---|
| vector / map / set 순회 | \`for (auto x : v)\` ← 내부적으로 이터레이터 호출 |
| STL 함수 (\`sort\`, \`find\`, \`lower_bound\` …) | 이터레이터 페어가 기본 인터페이스 |
| C 라이브러리 호출 / OS API | 포인터 (\`memcpy\`, \`fread\` …) |
| 동적 메모리 관리 | \`unique_ptr\`, \`shared_ptr\` (smart pointer) |

---

#### 왜 이터레이터가 표준이 됐어요?

**한 가지 인터페이스로 모든 컨테이너를 다룰 수 있어요.** 컨테이너가 vector 든 list 든 map 든:

\`\`\`cpp
for (auto it = c.begin(); it != c.end(); ++it) {
    cout << *it << " ";   // 어떤 컨테이너든 똑같이 작동
}
\`\`\`

- \`*it\` → 값 보기 (포인터 \`*p\` 랑 똑같음)
- \`++it\` → 다음 칸 (vector 면 +4, list 면 다음 노드, map 이면 트리 다음 노드)
- \`it != c.end()\` → 끝났나 확인

**\`sort\` 같은 STL 함수도 이걸 그대로 받아요:**

\`\`\`cpp
sort(v.begin(), v.end());                          // vector
sort(arr, arr + 5);                                // C 배열 (포인터도 이터레이터로 동작)
list<int> l = {...}; l.sort();                     // list 는 자체 멤버 (메모리 흩어져서)
\`\`\`

이렇게 **시작 위치 + 끝 위치만 주면** 컨테이너 종류 안 가리고 정렬·검색이 됨. 이게 STL 의 핵심 디자인이에요.

---

#### 포인터는 언제 보여요?

요즘 코드에서 raw 포인터 (\`int*\` 같은 거) 직접 쓰는 경우는:

1. **C 함수 호출** — \`memcpy(dst, src, n)\`, 파일 I/O, OS API
2. **함수 인자에서 "null 일 수 있는 참조"** — \`void log(const string* msg)\` (msg 가 없을 수도 있음)
3. **저수준 메모리 직접 다루기** — 임베디드, 게임 엔진 내부 등

**메모리 할당** (예전엔 \`new\`/\`delete\`) 은 거의 다 smart pointer (\`unique_ptr\`, \`shared_ptr\`) 로 대체됐어요 — raw \`new\` 직접 쓰는 건 모던 C++ 에서 안티패턴.

> 💡 정리: 이번 챕터부터 \`sort(v.begin(), v.end())\` 같은 거 계속 쓰는데, 그 \`v.begin()\` 이 바로 이터레이터예요. 학생이 STL 을 본격적으로 쓰기 시작했단 뜻 — 자연스럽게 이터레이터가 친숙해질 거예요.
