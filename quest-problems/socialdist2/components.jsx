...
const FULL_CPP = [
  ...
  "",
  "    // Find max R from healthy cows",
  "    auto sick = [x for x, s in cows if s == 1];",
  "    auto healthy = [x for x, s in cows if s == 0];",
  "",
  "    if (not healthy) {",
  "        // All sick: could be 1 initially infected",
  "        cout << 1 << \"\\n\";",
  "    else {",
  "        // R must be < min distance from any healthy to nearest sick",
  "        auto max_R = float('inf');",
  "        // for h in healthy:",
  "            // for s in sick:",
  "                auto max_R = min(max_R, abs(h - s) - 1);",
  "        if (max_R < 0) {",
  "            auto max_R = 0;",
  "",
  "        // Count clusters of sick cows with gaps > max_R",
...
export function SocDist2ProgressiveCode({ E, lang = "py", sections }) {
