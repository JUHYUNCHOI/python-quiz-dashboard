...
const FULL_CPP = [
  ...
  "    for (int i = 0; i < N; i++) {",
  "        cin >> dels[i].first >> dels[i].second;",
  "    }",
  ...
  "    long long stock = 0;",
  "    long long prev = 0;",
  "    for (auto& [day, bales] : dels) {",
  "        long long gap = day - prev - 1;",
  "        long long eat = min(stock, gap);",
  "        eaten += eat;",
  "        stock -= eat;",
  "        stock += bales;",
  "        prev = day - 1;",
...
export function getHungryCowSections(E) {
  return [
    {
      ...
      cppOnly: [
        t(E, "T can reach 10^14, so days and accumulated stock need long long.",
            "T 가 10^14 까지 가니 날짜와 쌓아 둔 건초는 long long 으로 담아야 해요."),
        t(E, "Structured binding for (auto& [day, bales] : dels) unpacks pairs cleanly.",
            "for (auto& [day, bales] : dels) 라고 쓰면 pair 의 두 값을 한 번에 꺼낼 수 있어요."),
      ],
    },
  ];
}

export function HungryCowProgressiveCode(props) {
...
export function getHungryCowWalk(E, lang = "py") {
  if (lang === "cpp") {
    ...
    return {
      ...
      beats: [
        { hi: [6, 12], bubble: t(E,
          "What do we hand back? The number of days Bessie actually eats by day T (T can reach 10^14, so use long long). Read every (day, bales) delivery.",
          "무엇을 내놓아야 하나요? T 일까지 Bessie 가 실제로 먹은 날의 수예요 (T 가 10^14 까지 가서 long long 을 써요).\n배달마다 (day, bales) 를 읽어요.") },
        { hi: [13, 13], bubble: t(E,
...
export function downloadHungryCowPDF(E, sections, lang = "py") {
  const win = window.open("", "_blank");
  if (!win) { alert(t(E, "Pop-up blocked.", "팝업이 막혀 있어요.")); return; }
  const esc = (s) => String(s).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
  const langLabel = lang === "py" ? "🐍 Python" : "💻 C++";
  const fileTitle = t(E, "HungryCow — Full Study Guide", "HungryCow — 종합 풀이 노트");
