...
export default function CheckupsApp() {
  ...
  const [ch2Q, setCh2Q] = useState(() => makeCheckupsCh2(false));

  const TABS = E ? ["📋 Problem", "⚡ Code"] : ["📋 문제", "⚡ 코드"];

  const setters = { 0: setCh1Q, 1: setCh2Q };
  const states  = { 0: ch1Q,    1: ch2Q };
  const makers  = { 0: makeCheckupsCh1, 1: makeCheckupsCh2 };
  ...
  return (
    <div style={{ maxWidth: 440, margin: "0 auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
        <h1 style={{ fontSize: 16, fontWeight: 800, color: A, margin: 0, fontFamily: "'Jua',sans-serif" }}>🐮 Cow Checkups</h1>
      ...
      {step.narr && <Narration key={`chk-${tab}-${cur}-${lang}`} text={step.narr} />}
