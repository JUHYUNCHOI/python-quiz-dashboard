...
import { WordProcProgressiveCode, downloadWordProcPDF, getWordProcSections, getWordProcWalk } from "./components";
import { makeWordProcCh1, makeWordProcCh2, makeWordProcCh3 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";

const A = "#dc2626";

export default function WordProcApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeWordProcCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeWordProcCh2(lang === "en"));
  const [ch3Q, setCh3Q] = useState(() => makeWordProcCh3(lang === "en", "py"));

  useEffect(() => {
    setCh3Q(prev => makeWordProcCh3(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeWordProcCh1, 1: makeWordProcCh2, 2: (e) => makeWordProcCh3(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <WordProcProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "wordproc-walk") return <CodeWalk E={E} lang={codeLang} {...getWordProcWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <WordProcProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "wordproc-walk") return <CodeWalk E={E} lang={codeLang} {...getWordProcWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadWordProcPDF(E, getWordProcSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`wordproc-${tab}-${cur}-${lang}`} text={step.narr} />}
