...
export default function CowPhotosApp(props = {}) {
  ...
  const [ch2Q, setCh2Q] = useState(() => makeCowPhotosCh2(false));
  const TABS = E ? ["📋 Problem", "⚡ Code"] : ["📋 문제", "⚡ 코드"];
  const setters = { 0: setCh1Q, 1: setCh2Q }, states = { 0: ch1Q, 1: ch2Q }, makers = { 0: makeCowPhotosCh1, 1: makeCowPhotosCh2 };
  ...
  return (
    <div style={{ maxWidth: "min(820px, 100%)", margin: "0 auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
        <h1 style={{ fontSize: 16, fontWeight: 800, color: A, margin: 0, fontFamily: "'Jua',sans-serif" }}>📸 More Cow Photos</h1>
      ...
      {step.narr && <Narration key={`cp-${tab}-${cur}-${lang}`} text={step.narr} />}
