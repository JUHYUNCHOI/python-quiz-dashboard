...
import { TichuProgressiveCode, downloadTichuPDF, getTichuSections } from "./components";
import { makeTichuCh1, makeTichuCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";

const A = "#dc2626";

export default function TichuApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeTichuCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeTichuCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeTichuCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeTichuCh1, 1: (e) => makeTichuCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <TichuProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <TichuProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadTichuPDF(E, getTichuSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`tich-${tab}-${cur}-${lang}`} text={step.narr} />}
