      ...
      content: (
        <div style={{ padding: 16, textAlign: "center" }}>
          ...
          <div style={{ marginTop: 12, background: "#f5f3ff", border: "2px solid #c4b5fd", borderRadius: 12, padding: 12, fontSize: 13, color: C.text, lineHeight: 1.8 }}>
            {t(E,
              "Key: DP where dp[i][j] = count of zig-zag subsequences of length j ending at position i. Transition depends on zig-zag direction.",
              "핵심: dp[i][j] = 위치 i에서 끝나는 길이 j의 지그재그 부분수열 수. 전이는 지그재그 방향에 따라.")}
          </div>
        </div>),
    },
    {
  ...
  return [
    {
      ...
      content: (
        <div style={{ padding: 16, textAlign: "center" }}>
          ...
          <div style={{ marginTop: 12, background: "#f5f3ff", border: "2px solid #c4b5fd", borderRadius: 12, padding: 12, fontSize: 13, color: C.text, lineHeight: 1.8 }}>
            {t(E,
              "dp[i][j] counts zig-zag subsequences of length j ending at i. Check direction (up/down) based on j parity. Sum dp[i][K] for answer.",
              "dp[i][j]는 i에서 끝나는 길이 j의 지그재그 부분수열 수. j의 홀짝에 따라 방향(상승/하강) 확인. dp[i][K] 합이 답.")}
          </div>
        </div>),
    },
    {
