...
const SOLVED_KEY = "algo-string-contest-solved"
...
export default function SortingPracticePage() {
  ...
  const problem = problemId ? stringContestCluster.problems.find(p => p.id === problemId) : null
  ...
  // 단일 문제 풀이 화면
  if (problem) {
    const problemIdx = stringContestCluster.problems.findIndex(p => p.id === problem.id) + 1
    return (
      <div className="min-h-screen bg-gray-50 pb-24">
        ...
        <main className="max-w-5xl mx-auto px-4 pt-4">
          <div className="mb-3 flex items-center justify-between gap-2">
            <button
              onClick={() => router.push("/algo/string")}
        ...
        <LessonPanel open={showLesson} onClose={() => setShowLesson(false)} title={t("문자열 수업", "String lesson")}>
          <StringLesson />
        </LessonPanel>
        <BottomNav />
      </div>
  ...
  // 리스트 화면
  const totalSolved = stringContestCluster.problems.filter(p => solvedSet.has(p.id)).length
  const total = stringContestCluster.problems.length
      ...
      <main className="max-w-2xl mx-auto px-4 pt-4">
        <div className="mb-4">
          <JourneyBreadcrumb items={[
            ...
            { label: "문자열", labelEn: "String", href: "/algo/string", emoji: "📊" },
            { label: "문제 풀이", labelEn: "Practice" },
          ]} />
          <div className="flex items-center gap-2 mb-2">
            <span className="text-3xl">{stringContestCluster.emoji}</span>
            <h1 className="text-xl sm:text-2xl font-black text-gray-900">
              {t(stringContestCluster.title, stringContestCluster.en?.title ?? stringContestCluster.title)}
            </h1>
          </div>
          <p className="text-sm text-gray-600 mb-3">
            {t(stringContestCluster.description, stringContestCluster.en?.description ?? stringContestCluster.description)}
          </p>
        ...
        {(() => {
          const warmup = stringContestCluster.problems.filter(p => p.difficulty === "쉬움")
          const easy = stringContestCluster.problems.filter(p => p.difficulty === "보통")
          const hard = stringContestCluster.problems.filter(p => p.difficulty === "어려움")
          const renderItem = (p: any) => {
            const isSolved = solvedSet.has(p.id)
            const globalIdx = stringContestCluster.problems.findIndex(pp => pp.id === p.id) + 1
            return (
              <Link
                key={p.id}
                href={`/algo/string/practice?p=${p.id}`}
                className={cn(
