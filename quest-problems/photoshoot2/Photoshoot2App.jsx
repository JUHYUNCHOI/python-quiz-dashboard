...
import { Photoshoot2ProgressiveCode, downloadPhotoshoot2PDF, getPhotoshoot2Sections, getPhotoshoot2Walk } from "./components";
import { makePhoto2Ch1, makePhoto2Ch2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#f97316";

export default function Photoshoot2App(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makePhoto2Ch1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makePhoto2Ch2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makePhoto2Ch2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makePhoto2Ch1, 1: (e) => makePhoto2Ch2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <Photoshoot2ProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getPhotoshoot2Walk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <Photoshoot2ProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getPhotoshoot2Walk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadPhotoshoot2PDF(E, getPhotoshoot2Sections(E), codeLang)} style={{
