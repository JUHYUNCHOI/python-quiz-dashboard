...
const SOLVED_KEY = "algo-dp-contest-solved"
...
export default function SortingPracticePage() {
  ...
  const problem = problemId ? dpContestCluster.problems.find(p => p.id === problemId) : null
  ...
  // 단일 문제 풀이 화면
  if (problem) {
    const problemIdx = dpContestCluster.problems.findIndex(p => p.id === problem.id) + 1
    return (
      <div className="min-h-screen bg-gray-50 pb-24">
        ...
        <LessonPanel open={showLesson} onClose={() => setShowLesson(false)} title={t("동적 프로그래밍 수업", "Dynamic Programming lesson")}>
          <DpLesson />
        </LessonPanel>
        <BottomNav />
      </div>
  ...
  // 리스트 화면
  const totalSolved = dpContestCluster.problems.filter(p => solvedSet.has(p.id)).length
  const total = dpContestCluster.problems.length
      ...
      <main className="max-w-2xl mx-auto px-4 pt-4">
        <div className="mb-4">
          <JourneyBreadcrumb items={[
            ...
            { label: "동적 프로그래밍", labelEn: "Dynamic Programming", href: "/algo/dp", emoji: "📊" },
            { label: "문제 풀이", labelEn: "Practice" },
          ]} />
          <div className="flex items-center gap-2 mb-2">
            <span className="text-3xl">{dpContestCluster.emoji}</span>
            <h1 className="text-xl sm:text-2xl font-black text-gray-900">
              {t(dpContestCluster.title, dpContestCluster.en?.title ?? dpContestCluster.title)}
            </h1>
          </div>
          <p className="text-sm text-gray-600 mb-3">
            {t(dpContestCluster.description, dpContestCluster.en?.description ?? dpContestCluster.description)}
          </p>
        ...
        {(() => {
          const warmup = dpContestCluster.problems.filter(p => p.difficulty === "쉬움")
          const easy = dpContestCluster.problems.filter(p => p.difficulty === "보통")
          const hard = dpContestCluster.problems.filter(p => p.difficulty === "어려움")
          const renderItem = (p: any) => {
            const isSolved = solvedSet.has(p.id)
            const globalIdx = dpContestCluster.problems.findIndex(pp => pp.id === p.id) + 1
            return (
              <Link
                key={p.id}
                href={`/algo/dp/practice?p=${p.id}`}
                className={cn(
