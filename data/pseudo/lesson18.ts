...
**정렬 완료!** [1, 2, 4, 7]`
        },
        {
          ...
          type: "quiz",
          title: "🧠 퀴즈!",
          content: '삽입 정렬에서 "key"란 무엇인가요?',
          options: [
            '정렬되지 않은 부분에서 꺼내서 올바른 위치에 삽입할 값',
            '배열에서 가장 작은 값',
            '배열에서 가장 큰 값',
            '배열의 첫 번째 값'
          ],
          answer: 0,
          explanation: 'key는 아직 정렬되지 않은 부분에서 꺼낸 값이에요. 이 key를 이미 정렬된 부분에서 **올바른 위치를 찾아 삽입**하는 것이 삽입 정렬의 핵심이에요!'
        },
        {
          id: "ch1-quiz2",
          type: "quiz",
          title: "🧠 퀴즈!",
          content: '배열 [3, 8, 5, 1]을 삽입 정렬할 때, i = 2 (key = 8)일 때 무슨 일이 일어나나요?',
          options: [
            '8 > 3이므로 아무 변화가 없다 (이미 올바른 위치)',
            '8과 3을 교환한다',
            '8을 배열의 맨 뒤로 보낸다',
            '8을 배열의 맨 앞에 삽입한다'
          ],
          answer: 0,
          explanation: 'key = 8이고, 왼쪽의 정렬된 부분에는 3만 있어요. 8 > 3이므로 이미 올바른 위치에 있어요! **아무것도 밀 필요가 없죠.** 삽입 정렬에서 key가 이미 제자리에 있으면 아무 일도 일어나지 않아요.'
        }
      ]
    },
    {
...
\`\`\``,
        ...
        },
        {
          ...
          codeTemplate: 'WHILE j >= 1 AND arr[j] > key\n  arr[j + 1] ← arr[j]\n  j ← j - 1\nENDWHILE\narr[___] ← key',
          fillBlanks: [
