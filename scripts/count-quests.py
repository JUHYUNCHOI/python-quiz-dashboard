...
#   위 UNTAUGHT 사전에 그 항목이 없어서 `mooin3` 는 **0건으로 나왔다.**
#   학생이 "💻 코드 6/9 부터 **완전히 막혔다**" 고 한 지점이 정확히 거기였다.
#
# ⚠️⚠️ **"안 가르친" 이라는 이름에 속지 마라 — 층이 둘이다.** (2026-09-24 정정)
#   처음엔 «`data/lesson*.ts` 에 0건이니 커리큘럼에 아예 없다» 고 적었는데 **틀렸다.**
#   pedagogy 검토자가 잡았다 — `data/algorithm/topics/string.ts:103·140·158·203` 이
#   `idx = ord(s[i]) - ord('a')` 를 **a→0, b→1, z→25 그림까지 붙여 가르치고 있다.**
#   `prefixsum.ts:521` 에는 `[[0]*(N+1) for _ in range(26)]` 이 **똑같은 모양**으로 있다.
#   → **파이썬 레슨에는 없고 Algorithm Lab(`/algo`)에는 있다.**
#   `learning_tracks.md` 상 알고리즘(4단계)이 대회(5단계)보다 앞이므로, 여기 걸렸다고
#   «트랙이 안 맞는다» 로 바로 읽으면 안 된다. 이 목록은 **«확인해야 할 자리»** 지
#   «안 배운 게 확실한 자리» 가 아니다 — `data/algorithm/topics/` 도 같이 grep 해라.
#
