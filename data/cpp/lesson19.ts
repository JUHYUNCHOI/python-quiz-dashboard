      ...
      steps: [
        {
          id: "ch0-notice",
          type: "explain",
          title: "📌 이 레슨은 **참고용** — 지금 안 봐도 OK",
          content: `이 레슨은 **참고용** 이에요. 알고리즘 시작 전 필수 셋업 (\`bits/stdc++.h\` + Fast I/O 두 줄) 은 별도 미니 레슨 **cpp-24** 에서 짧게 다뤄요. 거기서 핵심만 외우고 넘어가면 돼요.

### 이 레슨은 언제 봐야 하나?

| 상황 | 어디 보기 |
|---|---|
| 알고리즘 시작 전 필수 셋업 (3줄 외우기) | → **cpp-24** (5분) |
| 옛날 USACO 처럼 \`.in\` / \`.out\` 파일 다뤄야 함 | → 이 레슨 (파일 I/O) |
| Fast I/O 더 자세히 알고 싶음 | → 이 레슨 (Fast I/O) |

요즘 백준 / Codeforces / USACO 본선 다 표준 입출력 (\`cin\`/\`cout\`) 만 써요. 파일 I/O 쓸 일은 거의 없으니 **건너뛰어도 됨**.
