...
const FULL_CPP = [
  ...
  "    ll k = x / size;",
  "    ll r = x - k * size;",
  "    ll opt1 = a[i] * k + (r > 0 ? solve(r, i - 1) : 0);",
  "    ll opt2 = a[i] * (k + 1);",
  "    ll opt3 = solve(x, i - 1);",
  "    return min({opt1, opt2, opt3});",
  "}",
  ...
  "        ll big = INF;",
  "        for (int i = 0; i < N; i++) {",
  "            if ((1LL << i) >= x) { big = a[i]; break; }",
  "        }",
  "        int top = min((ll)N - 1, (ll)(64 - __builtin_clzll(x)));",
  ...
  return [
    {
      ...
      cppOnly: [
        ...
        t(E, "ios::sync_with_stdio(false) + cin.tie(nullptr) keeps Q = 10^4 queries fast.",
            "ios::sync_with_stdio(false) + cin.tie(nullptr) 로 Q=10^4 쿼리 가속."),
      ],
    },
  ];
}
