...
export default function MexesApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeMexesCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeMexesCh2(lang === "en", "py"));
  ...
  const makers  = { 0: makeMexesCh1, 1: (e) => makeMexesCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <MexesProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "sim") return <MexesSim E={E} />;
    if (step.type === "runner") return <MexesRunner E={E} />;
    return null;
  };
