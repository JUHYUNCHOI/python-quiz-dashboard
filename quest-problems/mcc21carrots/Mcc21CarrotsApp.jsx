...
import { Mcc21CarrotsProgressiveCode, downloadMcc21CarrotsPDF, getMcc21CarrotsSections } from "./components";
...
export default function Mcc21CarrotsApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeMcc21CarrotsCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeMcc21CarrotsCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeMcc21CarrotsCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeMcc21CarrotsCh1, 1: (e) => makeMcc21CarrotsCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <Mcc21CarrotsProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <Mcc21CarrotsProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadMcc21CarrotsPDF(E, getMcc21CarrotsSections(E), codeLang)} style={{
