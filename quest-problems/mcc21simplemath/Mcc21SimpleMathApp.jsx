...
import { Mcc21SimpleMathProgressiveCode, Mcc21SimpleMathOpSim, downloadMcc21SimpleMathPDF, getMcc21SimpleMathSections } from "./components";
...
export default function Mcc21SimpleMathApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeMcc21SimpleMathCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeMcc21SimpleMathCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeMcc21SimpleMathCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeMcc21SimpleMathCh1, 1: (e) => makeMcc21SimpleMathCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <Mcc21SimpleMathProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "opsim") return <Mcc21SimpleMathOpSim E={E} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <Mcc21SimpleMathProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "opsim") return <Mcc21SimpleMathOpSim E={E} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadMcc21SimpleMathPDF(E, getMcc21SimpleMathSections(E), codeLang)} style={{
