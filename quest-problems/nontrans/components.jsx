...
//   Python: passes official sample (yes/no). USACO re-submit pending.
//   C++:    passes official sample (yes/no). USACO re-submit pending.
...
const FULL_CPP = [
  ...
  "        bool found = false;",
  "        if (beats(A, B)) {",
  "            // Try every die C: 4 faces, values 1..10 (sorted to skip duplicates)",
  "            for (int c1 = 1; c1 <= 10 && !found; c1++)",
  "            for (int c2 = c1; c2 <= 10 && !found; c2++)",
  "            for (int c3 = c2; c3 <= 10 && !found; c3++)",
  "            for (int c4 = c3; c4 <= 10 && !found; c4++) {",
  "                vector<int> C_die = {c1, c2, c3, c4};",
  "                if (beats(B, C_die) && beats(C_die, A)) found = true;",
  "            }",
  "        }",
  "        cout << (found ? \"yes\" : \"no\") << \"\\n\";",
  "    }",
