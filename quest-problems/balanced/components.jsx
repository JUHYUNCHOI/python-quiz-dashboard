    ...
    <div style={{ padding: 14 }}>
      ...
      <div style={{ textAlign: "center", fontSize: 11, color: C.dim, marginBottom: 12 }}>
        {t(E, "Coloured = matched (counts toward answer). Greyed = leftover.",
              "색깔이 있으면 짝을 찾은 괄호예요. 회색은 남은 괄호예요.")}
      </div>
    ...
    <div style={{ padding: 14 }}>
      ...
      <div style={{ textAlign: "center", fontSize: 11, color: C.dim, marginBottom: 8 }}>
        {t(E, "Tap a bracket to skip / keep. Build a subsequence and see if it's balanced.",
              "괄호를 누르면 건너뛰거나 다시 넣을 수 있어요. 만든 부분수열이 균형인지 바로 알려 줘요.")}
      </div>
...
export function BalancedRunner({ E }) {
  ...
  const run = () => {
    ...
    if (!T || T < 1 || lines.length < T + 1) {
      setResults([{ error: t(E, "Invalid: line 1 = T, then T lines of 'N M'.", "입력 모양이 달라요. 첫 줄에 T, 그 아래 T 줄에 'N M' 을 적어 주세요.") }]);
      return;
    }
  ...
  return [
    {
      ...
      why: [
        t(E, "All '(' come before all ')', so any '(' can pair with any ')' — limit is min(N, M) pairs.",
            "'(' 가 모두 ')' 보다 앞에 있어서 어떤 '(' 든 어떤 ')' 와도 짝이 돼요. 그래서 적은 쪽 개수인 min(N, M) 쌍까지 만들 수 있어요."),
        t(E, "Each pair = 2 characters → answer = 2 × min(N, M). O(1) per test case.",
            "한 쌍이 글자 2 개라서 답은 2 × min(N, M) 이에요. 문제 하나를 푸는 데 계산 한 번이면 돼요."),
      ],
      pyOnly: [
        t(E, "min() built-in for the formula. sys.stdin.readline still useful for many T.",
            "min() 은 파이썬에 이미 있어요. 문제 수가 많으면 sys.stdin.readline 이 더 빨라요."),
      ],
      cppOnly: [
        t(E, "long long for N, M — could be up to 10^18.", "N 과 M 이 10^18 까지 갈 수 있어서 long long 으로 받아요."),
        t(E, "min(N, M) needs #include <algorithm>.", "min(N, M) 을 쓰려면 #include <algorithm> 이 있어야 해요."),
      ],
    },
  ];
}

export function BalancedProgressiveCode(props) {
...
<p>${t(E, "Given a string of N '(' followed by M ')', find the LONGEST balanced bracket subsequence.", "'(' N 개 뒤에 ')' M 개가 붙은 문자열이에요. 여기서 가장 긴 균형 부분수열의 길이를 구해요.")}</p>
...
<div class="box">
  ...
  ${t(E, "Since ALL '(' come BEFORE ALL ')', every '(' can pair with every ')'. The bottleneck is whichever has fewer characters.",
        "'(' 가 모두 ')' 보다 앞에 있어서 어떤 '(' 든 어떤 ')' 와도 짝이 돼요. 그래서 적은 쪽 개수가 한계예요.")}
</div>
<p>${t(E, "Answer = 2 × min(N, M). Each pair = 2 chars.", "답은 2 × min(N, M) 이에요. 한 쌍이 글자 2 개거든요.")}</p>
<h2>3. ${t(E, "Code", "코드")}</h2>
${sections.map(s => `
