...
/**
 * 알고리즘 토픽 연습 화면 — 20 개 토픽 공용.
 *
 * 예전엔 app/algo/<topic>/practice/page.tsx 가 336 줄씩 **20 개 복사본**(총 6,720 줄)
 * 이었다. 버튼 하나 추가가 20 파일 수정이 되어서, 여기로 합쳤다.
 * (components/algo/topic-problems-page.tsx 가 쓰는 패턴과 동일)
 *
 * 토픽별로 다른 건 3 개뿐: topicId · cluster · lesson.
 * 나머지(제목·아이콘·SOLVED_KEY·URL)는 topicId 와 ALGO_TOPIC_MAP 에서 유도한다.
 */
...
import { ALGO_TOPIC_MAP } from "@/data/algo/topics"
import type { PracticeCluster } from "@/data/practice/types"
import React from "react"

interface TopicPracticePageProps {
  /** algo 토픽 id (예: "recursion") — 제목/아이콘/URL/저장키를 여기서 유도 */
  topicId: string
...
export function TopicPracticePage({ topicId, cluster, lesson }: TopicPracticePageProps) {
  const { t } = useLanguage()
  const router = useRouter()
  const searchParams = useSearchParams()
  const { user } = useAuth()
  const meta = ALGO_TOPIC_MAP[topicId]
  const SOLVED_KEY = `algo-${topicId}-contest-solved`
  const getSolvedSet = (): Set<string> => {
  ...
  const problem = problemId ? cluster.problems.find(p => p.id === problemId) : null
  ...
  // 단일 문제 풀이 화면
  if (problem) {
    const problemIdx = cluster.problems.findIndex(p => p.id === problem.id) + 1
    return (
      <div className="min-h-screen bg-gray-50 pb-24">
        ...
        <main className="max-w-5xl mx-auto px-4 pt-4">
          <div className="mb-3 flex items-center justify-between gap-2">
            <button
              onClick={() => router.push(`/algo/${topicId}`)}
          ...
          <PracticeRunner
            ...
            stuckLadder
            onSkip={() => {
              // 막혔을 때 멈추지 않게 — 다음 문제, 없으면 토픽 목록으로.
              const idx = cluster.problems.findIndex(p => p.id === problem.id)
              const nextP = cluster.problems[idx + 1]
              router.push(nextP ? `/algo/${topicId}/practice?p=${nextP.id}` : `/algo/${topicId}`)
            }}
          />
        </main>

        {/* 📖 수업 — 이동 없이 옆에서 슬라이드 (in-context). 실제 수업 본문이 패널 안에. */}
        <LessonPanel open={showLesson} onClose={() => setShowLesson(false)} title={t(`${meta.title} 수업`, `${meta.titleEn} lesson`)}>
          {lesson}
        </LessonPanel>
        <BottomNav />
      </div>
  ...
  // 리스트 화면
  const totalSolved = cluster.problems.filter(p => solvedSet.has(p.id)).length
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-orange-50 via-white to-purple-50 pb-24">
      ...
      <main className="max-w-2xl mx-auto px-4 pt-4">
        <div className="mb-4">
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            <h1 className="text-xl sm:text-2xl font-black text-gray-900">
              {t(cluster.title, cluster.en?.title ?? cluster.title)}
            </h1>
          </div>
          <p className="text-sm text-gray-600 mb-3">
            {t(cluster.description, cluster.en?.description ?? cluster.description)}
          </p>
        ...
        {(() => {
          const warmup = cluster.problems.filter(p => p.difficulty === "쉬움")
          const easy = cluster.problems.filter(p => p.difficulty === "보통")
          const hard = cluster.problems.filter(p => p.difficulty === "어려움")
          const renderItem = (p: any) => {
            const isSolved = solvedSet.has(p.id)
            const globalIdx = cluster.problems.findIndex(pp => pp.id === p.id) + 1
            return (
              <Link
                key={p.id}
