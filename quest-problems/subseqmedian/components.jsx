...
const FULL_CPP = [
  ...
  "        // Count increasing subseqs of length k",
  "        // from elements < v to the left",
  "        auto left = []  # left[k] = count of length-k inc subseqs;",
  "        for (long long j = 0; j < i; j++) {",
  "            if (A[j] < v) {",
  "                // extend existing subsequences",
  "                auto new_left = [0] * (len(left) + 1);",
  "                // new_left[0] = 1  # empty subseq",
  "                // for k in range(len(left)):",
  "                    // new_left[k] = (new_left[k] + left[k]) % MOD",
  "                    // new_left[k+1] = (new_left[k+1] + left[k]) % MOD",
  ...
  "            // (skip elements >= v)",
  "        if (not left) {",
  "            auto left = [1]  # just the empty subsequence;",
  "",
  "        // Similarly count on the right",
  "        auto right = [];",
  "        for (long long j = N - 1; j < i, -1; j++) {",
  "            if (A[j] > v) {",
  "                auto new_right = [0] * (len(right) + 1);",
  "                // new_right[0] = 1",
  "                // for k in range(len(right)):",
  "                    // new_right[k] = (new_right[k] + right[k]) % MOD",
  "                    // new_right[k+1] = (new_right[k+1] + right[k]) % MOD",
  "                auto right = new_right;",
  "        if (not right) {",
  "            auto right = [1];",
  "",
  "        // Match: sum over k where left has k and right has k",
  "        auto max_k = min(len(left), len(right));",
  "        auto contrib = 0;",
  "        for (long long k = 0; k < max_k; k++) {",
  "            auto contrib = (contrib + left[k] * right[k]) % MOD;",
  "",
  "        auto ans = (ans + v * contrib) % MOD;",
  "",
  "    cout << ans << \"\\n\";",
  "",
  "    return 0;",
...
export function SubseqMedianProgressiveCode({ E, lang = "py", sections }) {
