...
import { Mcc15RectProgressiveCode, downloadMcc15RectPDF, getMcc15RectSections } from "./components";
import { makeMcc15RectCh1, makeMcc15RectCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";

const A = "#059669";

export default function Mcc15RectApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeMcc15RectCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeMcc15RectCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeMcc15RectCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeMcc15RectCh1, 1: (e) => makeMcc15RectCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <Mcc15RectProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <Mcc15RectProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadMcc15RectPDF(E, getMcc15RectSections(E), codeLang)} style={{
