...
const FULL_CPP = [
  ...
  "    for (int i = 0; i < N; i++) cin >> guards[i].first >> guards[i].second;",
  "",
  "    // Try removing each guard, compute total covered by union",
  "    int best = 0;",
  "    for (int skip = 0; skip < N; skip++) {",
  "        // Sort remaining by start, sweep to compute union",
  "        vector<pair<int,int>> rest;",
  "        for (int i = 0; i < N; i++) if (i != skip) rest.push_back(guards[i]);",
  "        sort(rest.begin(), rest.end());",
  "",
  "        int covered = 0;",
  "        int curEnd = 0;",
  "        for (auto& [s, e] : rest) {",
  "            int start = max(s, curEnd);",
...
export function LifeguardsProgressiveCode({ E, lang = "py", sections }) {
