...
import { Mcc21MenuProgressiveCode, downloadMcc21MenuPDF, getMcc21MenuSections } from "./components";
...
export default function Mcc21MenuApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeMcc21MenuCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeMcc21MenuCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeMcc21MenuCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeMcc21MenuCh1, 1: (e) => makeMcc21MenuCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <Mcc21MenuProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <Mcc21MenuProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadMcc21MenuPDF(E, getMcc21MenuSections(E), codeLang)} style={{
