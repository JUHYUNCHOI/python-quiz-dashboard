...
import { RoundingProgressiveCode, downloadRoundingPDF, getRoundingSections, RoundingSim, RoundingRunner } from "./components";
import { makeRoundingCh1, makeRoundingCh2 } from "./chapters";
...
export default function RoundingApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeRoundingCh1(false));
  const [ch2Q, setCh2Q] = useState(() => makeRoundingCh2(false, "py"));

  useEffect(() => {
    setCh2Q(prev => makeRoundingCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeRoundingCh1, 1: (e) => makeRoundingCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <RoundingProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "sim") return <RoundingSim E={E} />;
    if (step.type === "runner") return <RoundingRunner E={E} />;
    return null;
  };
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 12, alignItems: "center" }}>
            ...
            {TABS.map((label, i) => {
              ...
              return (
                <button key={i} onClick={() => changeTab(i)} style={{
                  ...
                  background: isCurrent ? A : (isVisited ? "#e0f2fe" : "transparent"),
                  border: `1.5px solid ${isCurrent ? A : (isVisited ? "#7dd3fc" : C.border)}`,
                  color: isCurrent ? "#fff" : (isVisited ? "#0c4a6e" : C.dim),
          ...
          {showCodeControls && (
            <div style={{ display: "flex", gap: 0, alignItems: "stretch", flexShrink: 0 }}>
              ...
              <button onClick={() => downloadRoundingPDF(E, getRoundingSections(E), codeLang)} style={{
        ...
        {step.narr && <Narration key={`rnd-${tab}-${cur}-${lang}`} text={step.narr} />}
