// 🔒 USACO_VERIFIED — cpid=737, modernart (2017 Open Bronze #3)
...
const FULL_PY = [
  ...
  "    row_arr = []",
  "    for c in range(N):",
  "        row_arr.append(int(row[c]))",
  "    canvas.append(row_arr)",
  "",
  ...
  "    if maxr[col] == -1:  # canvas 에 안 보임",
  "        continue",
  "    is_first = True",
  "    for r in range(N):",
  "        if not is_first: break",
  "        for c in range(N):",
  "            if canvas[r][c] != col: continue",
  "            # 이 C 셀이 다른 색깔의 bbox 에 있는지 확인",
  "            for other in range(1, 10):",
  "                if other == col: continue",
  "                if maxr[other] == -1: continue",
  "                if minr[other] <= r <= maxr[other] and minc[other] <= c <= maxc[other]:",
  "                    is_first = False",
  "                    break",
  "            if not is_first: break",
  "    if is_first:",
...
const FULL_CPP = [
  ...
  "    }",
  "    // 각 색깔의 bounding box",
  "    vector<int> minr(10, N), maxr(10, -1), minc(10, N), maxc(10, -1);",
  "    for (int r = 0; r < N; r++) {",
  "        for (int c = 0; c < N; c++) {",
  "            int col = canvas[r][c];",
  "            if (col == 0) continue;",
  ...
  "        if (maxr[col] == -1) continue;",
  "        bool is_first = true;",
  "        for (int r = 0; r < N && is_first; r++) {",
  "            for (int c = 0; c < N && is_first; c++) {",
  "                if (canvas[r][c] != col) continue;",
  "                for (int other = 1; other <= 9; other++) {",
  "                    if (other == col) continue;",
  "                    if (maxr[other] == -1) continue;",
  "                    if (minr[other] <= r && r <= maxr[other] && minc[other] <= c && c <= maxc[other]) {",
  "                        is_first = false;",
  "                        break;",
  "                    }",
  "                }",
