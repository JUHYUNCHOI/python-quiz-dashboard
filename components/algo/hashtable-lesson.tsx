  ...
  return (
    <div className="space-y-6 text-sm text-gray-700 leading-relaxed max-w-2xl">
      ...
      <section>
        ...
        <Code code={`unordered_map<int,int> cnt;
for (int x : a) cnt[x]++;     // 없으면 0에서 시작 → +1
// cnt[7] == 7이 나온 횟수`} />
        <Code lang="python" code={`from collections import Counter
cnt = Counter(a)        # {값: 횟수}
print(cnt[7])           # 7이 나온 횟수`} />
      </section>
      ...
      <section>
        ...
        <Code code={`unordered_set<int> seen;
for (int x : a) {
    if (seen.count(x)) { /* 중복! */ }
    seen.insert(x);
}`} />
      ...
      <section>
        ...
        <Code code={`unordered_map<int,int> idx;   // 값 → 인덱스
for (int i = 0; i < n; i++) {
    int need = target - a[i];
