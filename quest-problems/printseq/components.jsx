...
const PSQ_BU_PY = [
  ...
  "    return True",
  "",
  "# ─── 🎁 재귀 없이: '답 표'를 작은 조각부터 채우기 ───",
  "",
  "# made[(i, j, k)] = 조각 target[i:j] 를 PRINT k 개 이하로 만들 수 있나",
  "made = {}",
  "",
  "# 조각 하나의 답 — 표를 '읽기만' 해요 (자기 호출 ❌)",
  "def piece_answer(i, j, k):",
  "    piece = target[i:j]",
  "",
  "    # 요령 ①: 다 같음 → PRINT 1 개면 끝",
  "    if is_all_same(piece):",
  "        return True",
  "",
  "    # 요령 ②: 블록의 반복 → 블록 답은 이미 표에!",
  "    length = j - i",
  "    for block_len in range(1, length):",
  "        if length % block_len == 0:",
  "            if is_repeat_of(piece, block_len):",
  "                if made[(i, i + block_len, k)]:",
  "                    return True",
  "",
  "    # 요령 ③: 둘로 자르기 → 양쪽 답도 이미 표에!",
  ...
  "    made.clear()",
  "    # ⭐ 작은 조각부터! (길이 1 → N)",
  "    # 큰 조각이 찾는 '더 작은 답'은 항상 이미 표에 적혀 있다",
  "    for length in range(1, N + 1):",
  "        for i in range(N - length + 1):",
  "            for k in range(1, K + 1):",
  "                made[(i, i + length, k)] = piece_answer(i, i + length, k)",
  "",
  "    if made[(0, N, K)]:   # 표의 마지막 칸 = 전체 답!",
  "        print('YES')",
  "    else:",
  "        print('NO')",
];
...
const PSQ_BU_PY_EN = [
  "T = int(input())          # number of test cases",
  "",
  "# ─── helpers (same as the main version — no recursion) ───",
  "",
  "# are all numbers the same?",
  "def is_all_same(seq):",
  "    for x in seq:",
  ...
  "    return True",
  "",
  "# ─── 🎁 NO recursion: fill an answer TABLE, small pieces first ───",
  "",
  "# made[(i, j, k)] = can piece target[i:j] be made with <= k PRINTs?",
  "made = {}",
  "",
  "# answer for ONE piece — only READS the table (never calls itself!)",
  "def piece_answer(i, j, k):",
  "    piece = target[i:j]",
  "",
  "    # trick 1: all same -> one PRINT",
  ...
  "        return True",
  "",
  "    # trick 2: repeated block -> the block's answer is already in the table!",
  "    length = j - i",
  "    for block_len in range(1, length):",
  "        if length % block_len == 0:",
  "            if is_repeat_of(piece, block_len):",
  ...
  "                    return True",
  "",
  "    # trick 3: cut in two -> both answers are already in the table!",
  "    for cut in range(i + 1, j):",
  "        for k1 in range(1, k):",
  "            if made[(i, cut, k1)]:",
  "                if made[(cut, j, k - k1)]:",
  ...
  "",
  "    made.clear()",
  "    # ⭐ small pieces FIRST! (length 1 -> N)",
  "    # whatever a bigger piece needs is always already written down",
  "    for length in range(1, N + 1):",
  "        for i in range(N - length + 1):",
  "            for k in range(1, K + 1):",
  "                made[(i, i + length, k)] = piece_answer(i, i + length, k)",
  "",
  "    if made[(0, N, K)]:   # the table's last cell = the full answer!",
  "        print('YES')",
  "    else:",
  "        print('NO')",
];
...
const PSQ_BU_CPP = [
  ...
  "using namespace std;",
  "",
  "vector<int> target_seq;        // 지금 케이스의 목표 수열",
  "// made[i][j][k] = 조각 target[i:j] 를 PRINT k 개 이하로 만들 수 있나",
  "bool made[101][101][4];",
  "",
  "// ─── 도우미 (본편과 동일 — 재귀 ❌) ───",
  ...
  "",
  "// 조각 하나의 답 — 표를 '읽기만' 해요 (자기 호출 ❌)",
  "bool piece_answer(int i, int j, int k) {",
  "    vector<int> piece = slice(target_seq, i, j);",
  "",
  "    // 요령 ①: 다 같음 → PRINT 1 개면 끝",
  "    if (is_all_same(piece)) {",
  "        return true;",
  ...
  "    // 요령 ②: 블록의 반복 → 블록 답은 이미 표에!",
  "    int length = j - i;",
  "    for (int block_len = 1; block_len < length; block_len++) {",
  "        if (length % block_len == 0) {",
  "            if (is_repeat_of(piece, block_len)) {",
  "                if (made[i][i + block_len][k]) {",
  "                    return true;",
...
const PSQ_BU_CPP_EN = [
  ...
  "using namespace std;",
  "",
  "vector<int> target_seq;        // current case's target sequence",
  "// made[i][j][k] = can piece target[i:j] be made with <= k PRINTs?",
  "bool made[101][101][4];",
  "",
  "// ─── helpers (same as the main version — no recursion) ───",
...
export function getPrintseqBuWalk(E, lang = "py") {
  const vars = [
    { v: "made", ko: "답 표", en: "the answer table" },
    { v: "piece", ko: "지금 보는 조각", en: "current piece" },
