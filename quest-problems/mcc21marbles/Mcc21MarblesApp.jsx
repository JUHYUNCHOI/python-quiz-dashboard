...
export default function Mcc21MarblesApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeMcc21MarblesCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeMcc21MarblesCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeMcc21MarblesCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeMcc21MarblesCh1, 1: (e) => makeMcc21MarblesCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <Mcc21MarblesProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <Mcc21MarblesProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadMcc21MarblesPDF(E, getMcc21MarblesSections(E), codeLang)} style={{
