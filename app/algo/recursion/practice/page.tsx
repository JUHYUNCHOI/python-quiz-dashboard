...
/**
 * 정렬문제 풀이 — /algo/recursion/practice
 *
...
const SOLVED_KEY = "algo-recursion-contest-solved"
...
export default function SortingPracticePage() {
  ...
  const problem = problemId ? recursionContestCluster.problems.find(p => p.id === problemId) : null
  ...
  // 단일 문제 풀이 화면
  if (problem) {
    const problemIdx = recursionContestCluster.problems.findIndex(p => p.id === problem.id) + 1
    return (
      <div className="min-h-screen bg-gray-50 pb-24">
        ...
        <main className="max-w-5xl mx-auto px-4 pt-4">
          <div className="mb-3 flex items-center justify-between gap-2">
            <button
              onClick={() => router.push("/algo/recursion")}
        ...
        <LessonPanel open={showLesson} onClose={() => setShowLesson(false)} title={t("재귀 수업", "Recursion lesson")}>
          <RecursionLesson />
        </LessonPanel>
        <BottomNav />
      </div>
  ...
  // 리스트 화면
  const totalSolved = recursionContestCluster.problems.filter(p => solvedSet.has(p.id)).length
  const total = recursionContestCluster.problems.length
      ...
      <main className="max-w-2xl mx-auto px-4 pt-4">
        <div className="mb-4">
          <JourneyBreadcrumb items={[
            ...
            { label: "재귀", labelEn: "Recursion", href: "/algo/recursion", emoji: "📊" },
            { label: "문제 풀이", labelEn: "Practice" },
          ]} />
          <div className="flex items-center gap-2 mb-2">
            <span className="text-3xl">{recursionContestCluster.emoji}</span>
            <h1 className="text-xl sm:text-2xl font-black text-gray-900">
              {t(recursionContestCluster.title, recursionContestCluster.en?.title ?? recursionContestCluster.title)}
            </h1>
          </div>
          <p className="text-sm text-gray-600 mb-3">
            {t(recursionContestCluster.description, recursionContestCluster.en?.description ?? recursionContestCluster.description)}
          </p>
        ...
        {(() => {
          const warmup = recursionContestCluster.problems.filter(p => p.difficulty === "쉬움")
          const easy = recursionContestCluster.problems.filter(p => p.difficulty === "보통")
          const hard = recursionContestCluster.problems.filter(p => p.difficulty === "어려움")
          const renderItem = (p: any) => {
            const isSolved = solvedSet.has(p.id)
            const globalIdx = recursionContestCluster.problems.findIndex(pp => pp.id === p.id) + 1
            return (
              <Link
                key={p.id}
                href={`/algo/recursion/practice?p=${p.id}`}
                className={cn(
