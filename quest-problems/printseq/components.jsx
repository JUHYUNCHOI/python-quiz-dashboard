...
const PSQ_STK_PY = [
  ...
  "",
  "    answer = {}            # 답 공책: (start, end, budget) → True / False",
  ...
  "",
  "        if piece in answer:              # 이미 푼 조각이면 넘어가기",
  "            todo.pop()",
  "            continue",
  ...
  "            continue",
  "",
  "        answer[piece] = can_make         # 세 요령 다 해봤으면 답 확정",
  "        del block_lens[piece], block_tried[piece], cut_at[piece], give_left[piece]",
  "        todo.pop()",
  "",
  "    print('YES' if answer[(0, N, K)] else 'NO')",
];
...
const PSQ_STK_PY_EN = [
  ...
  "            divisors[length].append(block_len)",
  "",
  "    answer = {}            # answer notebook: (start, end, budget) -> True / False",
  ...
  "        start, end, budget = piece",
  "",
  "        if piece in answer:              # already solved — skip",
  "            todo.pop()",
  "            continue",
  ...
  "            continue",
  "",
  "        answer[piece] = can_make         # all three tricks tried -> answer settled",
  "        del block_lens[piece], block_tried[piece], cut_at[piece], give_left[piece]",
  "        todo.pop()",
  "",
  "    print('YES' if answer[(0, N, K)] else 'NO')",
...
const PSQ_STK_CPP = [
  ...
  "vector<int> target;",
  "",
  "int  answerOf[101][101][4];      // -1 = 아직 모름, 0 = 못 만듦, 1 = 만들 수 있음",
  ...
  "                for (int c = 0; c <= K; c++) { answerOf[a][b][c] = -1; opened[a][b][c] = false; }",
  "            }",
  "",
  ...
  "            if (answerOf[start][end][budget] != -1) { todo.pop_back(); continue; }",
  "            if (start >= end || budget <= 0) { answerOf[start][end][budget] = 0; todo.pop_back(); continue; }",
  "",
  "            if (!opened[start][end][budget]) {    // 이 조각을 처음 꺼냈을 때만",
  "                opened[start][end][budget] = true;",
  "                if (all_same(start, end)) { answerOf[start][end][budget] = 1; todo.pop_back(); continue; }",
  ...
  "                if (answerOf[start][mid][budget] == -1) { todo.push_back({start, mid, budget}); unknown = true; break; }",
  "                if (answerOf[start][mid][budget] == 1) { canMake = true; break; }",
  ...
  "                    if (answerOf[start][cut][toLeft] == -1) { todo.push_back({start, cut, toLeft}); unknown = true; break; }",
  "                    if (answerOf[start][cut][toLeft] == 1) {",
  "                        if (answerOf[cut][end][budget - toLeft] == -1) { todo.push_back({cut, end, budget - toLeft}); unknown = true; break; }",
  "                        if (answerOf[cut][end][budget - toLeft] == 1) { canMake = true; break; }",
  "                    }",
  "                    giveLeft[start][end][budget]++;",
  "                }",
  "            }",
  "            if (unknown) continue;                // 모르는 조각부터 먼저 (할 일 목록에 올라감)",
  "            answerOf[start][end][budget] = canMake ? 1 : 0;",
  "            todo.pop_back();",
  "        }",
  "        cout << (answerOf[0][N][K] == 1 ? \"YES\" : \"NO\") << '\\n';",
  "    }",
  "    return 0;",
  "}",
];
...
const PSQ_STK_CPP_EN = [
  ...
  "vector<int> target;",
  "",
  "int  answerOf[101][101][4];      // -1 = unknown, 0 = cannot, 1 = can",
...
export function getPrintseqStkWalk(E, lang = "py") {
  ...
  if (lang === "cpp") {
    return {
      ...
      vars,
      marks: [
        { from: 40, to: 41, color: "#0d9488", ko: "📋 재귀 대신 이것!", en: "📋 instead of recursion!" },
      ],
      beats: [
        { hi: [28, 33], bubble: t(E,
          "Start where we always start — the INPUT.\nmain reads T, then each case: N, K, and the target sequence.",
  ...
  return {
    ...
    vars,
    marks: [
      { from: 94, to: 96, color: "#0d9488", ko: "📋 재귀 대신 이것!", en: "📋 instead of recursion!" },
    ],
    beats: [
      { hi: [0, 7], bubble: t(E,
