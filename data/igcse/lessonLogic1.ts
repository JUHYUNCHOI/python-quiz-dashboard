...
Logic Gate는 1(True)과 0(False)을 입력받아 하나의 출력을 내보내는 장치예요.

### AND Gate
두 입력이 **모두 1**일 때만 출력이 1이에요.
...
이 3가지가 모든 논리 회로의 기본이에요!`
        },
        {
          ...
          answer: 0,
          explanation: `AND gate는 두 입력이 **모두 1**이어야 출력이 1이에요.
...
A=1, B=0 → 둘 다 1이 아니므로 출력은 **0**`
        },
        {
          ...
          answer: 0,
          explanation: `OR gate는 입력 중 **하나라도 1**이면 출력이 1이에요.
...
A=1, B=1일 때 출력은?`,
          ...
          answer: 0,
          explanation: `AND gate는 두 입력이 **모두 1**일 때 출력이 1이에요.
...
기본 게이트를 조합하면 새로운 게이트를 만들 수 있어요!

### NAND Gate (NOT AND)
AND의 **반대**입니다. AND 결과를 NOT 해요.
...
두 입력이 모두 1이면 AND는 1이고, 이걸 반전하면 **0**`
        },
        {
          id: "ch2-q2",
          type: "quiz",
