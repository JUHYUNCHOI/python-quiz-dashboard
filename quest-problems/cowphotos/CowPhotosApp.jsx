...
export default function CowPhotosApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeCowPhotosCh1(false));
  const [ch2Q, setCh2Q] = useState(() => makeCowPhotosCh2(false, "py"));

  useEffect(() => {
    setCh2Q(prev => makeCowPhotosCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeCowPhotosCh1, 1: (e) => makeCowPhotosCh2(e, codeLang) };
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 12, alignItems: "center" }}>
            ...
            {TABS.map((label, i) => {
              ...
              return (
                <button key={i} onClick={() => changeTab(i)} style={{
                  ...
                  background: isCurrent ? A : (isVisited ? "#fef3c7" : "transparent"),
                  border: `1.5px solid ${isCurrent ? A : (isVisited ? "#fcd34d" : C.border)}`,
                  color: isCurrent ? "#fff" : (isVisited ? "#92400e" : C.dim),
          ...
          {showCodeControls && (
            <div style={{ display: "flex", gap: 0, alignItems: "stretch", flexShrink: 0 }}>
              ...
              <button onClick={() => downloadCowPhotosPDF(E, getCowPhotosSections(E), codeLang)} style={{
        ...
        {step.narr && <Narration key={`cph-${tab}-${cur}-${lang}`} text={step.narr} />}
        ...
        <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
          ...
          <div style={{ display: "flex", gap: 8, justifyContent: "center", alignItems: "center" }}>
            ...
            <div style={{ position: "relative" }}>
              ...
              {stepListOpen && (
                <>
                  <div onClick={() => setStepListOpen(false)} style={{ position: "fixed", inset: 0, zIndex: 200 }} />
                  <div style={{
                    position: "absolute", bottom: "calc(100% + 8px)", left: "50%", transform: "translateX(-50%)",
                    zIndex: 201, background: "#fff", border: `1.5px solid ${C.border}`,
                    ...
                    {steps.map((s, i) => {
                      ...
                      const label = narrText
                        ...
                        : (s.type === "quiz" ? t(E, "Quiz", "퀴즈")
                          ...
                          : s.type === "sim" ? t(E, "Sim", "시뮬")
                          : s.type === "runner" ? t(E, "Runner", "실행기")
                          : t(E, `Step ${i + 1}`, `${i + 1} 단계`));
                      ...
                      return (
                        ...
                        >
                          <span style={{
                            ...
                            color: isCurrent ? A : C.dim, minWidth: 22, textAlign: "right", fontSize: 11,
                          }}>{i + 1}.</span>
