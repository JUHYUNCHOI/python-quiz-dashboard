      ...
      steps: [
        {
          ...
          type: "explain",
          title: "📚 스택은 쌓아놓은 책!",
          content: `책상에 책을 한 권씩 위로 쌓는다고 생각해 봐요. 📚

맨 위에 *마지막으로* 올린 책이, 꺼낼 땐 *제일 먼저* 손에 잡히죠? 맨 아래 책을 꺼내려면 위에 쌓인 걸 다 들어내야 하고요.

\`\`\`
    [책 3]  ← 마지막에 넣음 → 가장 먼저 꺼냄
    [책 2]
    [책 1]  ← 처음 넣음 → 가장 나중에 꺼냄
   =========
\`\`\`

컴퓨터에도 이렇게 **"마지막에 넣은 게 먼저 나오는"** 상자가 있어요. 그게 바로 **스택(Stack)** 이에요. 어렵게 생각할 거 없이 — 방금 떠올린 책 더미가 스택의 전부랍니다.

> 💡 이 규칙을 영어로 **LIFO** (Last In, First Out) — "마지막에 들어간 게 먼저 나간다" 라고 불러요. 시험에도 자주 나오는 말이지만, 뜻은 방금 본 책 더미 그대로예요.`
        },
        {
          id: "realworld",
          type: "explain",
...
> 💡 왜 전부 "최근 것부터"일까요? 실행취소는 *방금 한 실수* 를 되돌리고 싶은 거고, 뒤로가기는 *방금 떠난 페이지* 로 돌아가고 싶은 거예요. 사람이 "되돌리고 싶은 것" 은 거의 항상 *가장 최근에 한 것*. 그래서 스택의 "마지막에 넣은 게 먼저 나온다" 가 이렇게 자주 쓰여요.`
        },
        {
          ...
          type: "explain",
          title: "⚙️ 스택 연산",
          content: `스택으로 하는 일은 딱 **두 가지**, 그것도 전부 **맨 위에서만** 일어나요. 접시 더미를 떠올리면 쉬워요 — 올릴 때도 맨 위, 집을 때도 맨 위죠. 🍽️

- **push** — 맨 위에 하나 *얹기* 📥
- **pop** — 맨 위 하나 *집어내기* 📤  (집어낸 걸 우리한테 돌려줘요)

**왜 맨 위에서만 할까요?** 더미 한가운데 접시를 쏙 빼면 위에 쌓인 게 와르르 무너지잖아요. 그래서 스택은 *일부러* 맨 위만 만질 수 있게 막아둔 거예요. 덕분에 push·pop 이 아주 빠르고, "마지막에 넣은 게 먼저" 순서도 저절로 지켜져요.

> 💡 가끔 쓰는 보조 동작도 있어요 — **peek**(맨 위를 *보기만*), **isEmpty**(비었나?), **size**(몇 개?).
>
> 말로 들으면 헷갈리죠? 바로 아래에서 **직접 눌러보면** 한눈에 들어와요 👇`
        },
        {
          id: "stack-viz",
          type: "interactive",
          title: "🎬 스택 직접 움직여보기",
          description: "push 로 맨 위에 쌓고, pop 으로 맨 위를 빼고, peek 로 맨 위만 살짝 봐요. '마지막에 넣은 게 먼저 나온다'를 눈으로 확인해보세요.",
