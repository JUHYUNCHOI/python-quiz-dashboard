...
const SOLVED_KEY = "algo-tree-contest-solved"
...
export default function SortingPracticePage() {
  ...
  const problem = problemId ? treeContestCluster.problems.find(p => p.id === problemId) : null
  ...
  // 단일 문제 풀이 화면
  if (problem) {
    const problemIdx = treeContestCluster.problems.findIndex(p => p.id === problem.id) + 1
    return (
  ...
  // 리스트 화면
  const totalSolved = treeContestCluster.problems.filter(p => solvedSet.has(p.id)).length
  const total = treeContestCluster.problems.length
      ...
      <main className="max-w-2xl mx-auto px-4 pt-4">
        <div className="mb-4">
          <JourneyBreadcrumb items={[
            ...
            { label: "트리", labelEn: "Tree", href: "/algo/tree", emoji: "📊" },
            { label: "문제 풀이", labelEn: "Practice" },
          ]} />
          <div className="flex items-center gap-2 mb-2">
            <span className="text-3xl">{treeContestCluster.emoji}</span>
            <h1 className="text-xl sm:text-2xl font-black text-gray-900">
              {t(treeContestCluster.title, treeContestCluster.en?.title ?? treeContestCluster.title)}
            </h1>
          </div>
          <p className="text-sm text-gray-600 mb-3">
            {t(treeContestCluster.description, treeContestCluster.en?.description ?? treeContestCluster.description)}
          </p>
        ...
        {(() => {
          const warmup = treeContestCluster.problems.filter(p => p.difficulty === "쉬움")
          const easy = treeContestCluster.problems.filter(p => p.difficulty === "보통")
          const hard = treeContestCluster.problems.filter(p => p.difficulty === "어려움")
          const renderItem = (p: any) => {
            const isSolved = solvedSet.has(p.id)
            const globalIdx = treeContestCluster.problems.findIndex(pp => pp.id === p.id) + 1
            return (
              <Link
                key={p.id}
                href={`/algo/tree/practice?p=${p.id}`}
                className={cn(
