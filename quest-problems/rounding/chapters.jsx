...
export function makePatternSteps(E) {
  return [
    ...
    { type: "quiz",
      ...
      options: t(E,
        ["First digit always ≥ 5", "First digit always 4", "All divisible by 8", "Even number of digits"],
        ["첫째 자리가 5 이상", "첫째 자리가 전부 4", "전부 8의 배수", "자릿수가 짝수"]),
      correct: 1,
...
const OPT_INPUT_CPP = (E) => [
  ...
  E ? "// long long: defensive — d=10 would overflow int" : "// long long: 안전용 — d=10 이면 int 오버플로",
  "long long smallest_for(int digits) {",
  "    long long value = 0;",
  ...
  "long long largest_for(int digits) {",
  "    long long value = 4;",
  "    for (int i = 0; i < digits - 1; i++) {",
...
const OPT_LOOP_CPP = (E) => [
  ...
  "            long long smallest = smallest_for(digits);",
  "            long long largest  = largest_for(digits);",
  "            if (smallest > N) break;",
  E ? "            long long right_end = (N < largest) ? N : largest;  // min(N, largest)" : "            long long right_end = (N < largest) ? N : largest;  // min(N, largest)",
