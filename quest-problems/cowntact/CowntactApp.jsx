...
import { CowntactProgressiveCode, downloadCowntactPDF, getCowntactSections, getCowntactWalk } from "./components";
import { makeCowntactCh1, makeCowntactCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#7c5cfc";

export default function CowntactApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeCowntactCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeCowntactCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeCowntactCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeCowntactCh1, 1: (e) => makeCowntactCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <CowntactProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getCowntactWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <CowntactProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getCowntactWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadCowntactPDF(E, getCowntactSections(E), codeLang)} style={{
