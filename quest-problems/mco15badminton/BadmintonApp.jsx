...
import { BadmintonProgressiveCode, downloadBadmintonPDF, getBadmintonSections, getBadmintonWalk } from "./components";
import { makeBadmintonCh1, makeBadmintonCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";

const A = "#059669";

export default function BadmintonApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeBadmintonCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeBadmintonCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeBadmintonCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeBadmintonCh1, 1: (e) => makeBadmintonCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <BadmintonProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "badminton-walk") return <CodeWalk E={E} lang={codeLang} {...getBadmintonWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <BadmintonProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "badminton-walk") return <CodeWalk E={E} lang={codeLang} {...getBadmintonWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadBadmintonPDF(E, getBadmintonSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`badm-${tab}-${cur}-${lang}`} text={step.narr} />}
