...
import { TameHerdProgressiveCode, TameHerdSim, downloadTameHerdPDF, getTameHerdSections, getTameHerdWalk } from "./components";
import { makeTameHerdCh1, makeTameHerdCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#8b5cf6";

export default function TameHerdApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeTameHerdCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeTameHerdCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeTameHerdCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeTameHerdCh1, 1: (e) => makeTameHerdCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <TameHerdProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "tameherd-codewalk") return <CodeWalk E={E} lang={codeLang} {...getTameHerdWalk(E, codeLang)} accent={A} />;
    if (step.type === "sim") return <TameHerdSim E={E} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <TameHerdProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "tameherd-codewalk") return <CodeWalk E={E} lang={codeLang} {...getTameHerdWalk(E, codeLang)} accent={A} />;
    if (s.type === "sim") return <TameHerdSim E={E} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadTameHerdPDF(E, getTameHerdSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`tame-${tab}-${cur}-${lang}`} text={step.narr} />}
