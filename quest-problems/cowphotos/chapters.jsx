...
import { getCowPhotosSections, FreqWalkSimulator } from "./components";
...
export function makeCowPhotosCh1(E) {
  return [
    ...
    // 1-2: Interactive frequency walk-through (was a static table)
    {
      type: "reveal",
      narr: t(E,
        "Walk a small example one height at a time. Count how many cows of each height, take pairs out (each pair = symmetric left+right slot), and keep 1 extra as the peak if any height has an odd count.",
        "작은 예시를 키 하나씩 따라가요. 각 키의 소가 몇 마리인지 세고, 쌍씩 빼내서 (각 쌍 = 좌+우 대칭 자리), 어떤 키든 홀수가 있으면 1 마리를 가운데 정점으로 남겨요."),
      content: (<FreqWalkSimulator E={E} />),
    },
    {
      type: "quiz",
      narr: t(E,
