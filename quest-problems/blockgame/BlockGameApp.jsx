...
import { BlockGameProgressiveCode, downloadBlockGamePDF, getBlockGameSections, getBlockGameWalk } from "./components";
import { makeBlockGameCh1, makeBlockGameCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#f97316";

export default function BlockGameApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeBlockGameCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeBlockGameCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeBlockGameCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeBlockGameCh1, 1: (e) => makeBlockGameCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <BlockGameProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "bg-codewalk") return <CodeWalk E={E} lang={codeLang} {...getBlockGameWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <BlockGameProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "bg-codewalk") return <CodeWalk E={E} lang={codeLang} {...getBlockGameWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadBlockGamePDF(E, getBlockGameSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`bloc-${tab}-${cur}-${lang}`} text={step.narr} />}
