...
import { LogicalMoosProgressiveCode, downloadLogicalMoosPDF, getLogicalMoosSections, getLogicalMoosWalk, LogicalMoosSim } from "./components";
...
export default function LogicalMoosApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeLogicalCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeLogicalCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeLogicalCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeLogicalCh1, 1: (e) => makeLogicalCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "logicalmoos-walk") return <CodeWalk E={E} lang={codeLang} {...getLogicalMoosWalk(E, codeLang)} accent={A} />;
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <LogicalMoosProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "logicalmoos-walk") return <CodeWalk E={E} lang={codeLang} {...getLogicalMoosWalk(E, codeLang)} accent={A} />;
    if (s.type === "sim") return <LogicalMoosSim E={E} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadLogicalMoosPDF(E, getLogicalMoosSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`logi-${tab}-${cur}-${lang}`} text={step.narr} />}
