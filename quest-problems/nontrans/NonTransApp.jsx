...
import { NonTransProgressiveCode, downloadNonTransPDF, getNonTransSections, getNonTransWalk } from "./components";
import { makeNonTransCh1, makeNonTransCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#dc2626";

export default function NonTransApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeNonTransCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeNonTransCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeNonTransCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeNonTransCh1, 1: (e) => makeNonTransCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <NonTransProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getNonTransWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <NonTransProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getNonTransWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadNonTransPDF(E, getNonTransSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`nont-${tab}-${cur}-${lang}`} text={step.narr} />}
