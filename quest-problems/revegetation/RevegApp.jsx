...
import { RevegProgressiveCode, RevegSim, downloadRevegPDF, getRevegSections } from "./components";
import { makeRevegCh1, makeRevegCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";

const A = "#f97316";

export default function RevegApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeRevegCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeRevegCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeRevegCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeRevegCh1, 1: (e) => makeRevegCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <RevegProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "sim") return <RevegSim E={E} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <RevegProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "sim") return <RevegSim E={E} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadRevegPDF(E, getRevegSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`reve-${tab}-${cur}-${lang}`} text={step.narr} />}
