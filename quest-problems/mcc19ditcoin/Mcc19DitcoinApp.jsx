...
import { Mcc19DitcoinProgressiveCode, downloadMcc19DitcoinPDF, getMcc19DitcoinSections } from "./components";
import { makeMcc19DitcoinCh1, makeMcc19DitcoinCh2 } from "./chapters";
...
export default function Mcc19DitcoinApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeMcc19DitcoinCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeMcc19DitcoinCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeMcc19DitcoinCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeMcc19DitcoinCh1, 1: (e) => makeMcc19DitcoinCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <Mcc19DitcoinProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <Mcc19DitcoinProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadMcc19DitcoinPDF(E, getMcc19DitcoinSections(E), codeLang)} style={{
