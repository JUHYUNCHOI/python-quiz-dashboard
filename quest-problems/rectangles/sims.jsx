...
function RectStage({ groups = null }) {
        ...
        {groups && groups.map((g, gi) => {
          ...
          return (
            <div key={gi} style={{ position: "absolute",
              ...
              width: sw * UNIT, height: mh * UNIT, boxSizing: "border-box",
              background: BLUBG, border: `2.5px solid ${BLU}`, borderRadius: 5 }}>
              <span style={{ position: "absolute", top: -19, left: "50%", transform: "translateX(-50%)",
                fontSize: 11, fontWeight: 800, color: BLU, whiteSpace: "nowrap", fontFamily: "'JetBrains Mono',monospace" }}>
                {mh}×{sw}={area}
              </span>
            </div>
          );
