...
const FULL_CPP = [
  ...
  "",
  "    int firstG = N, lastG = -1, firstH = N, lastH = -1;",
  "    for (int i = 0; i < N; i++) {",
  "        if (breed[i] == 'G') { firstG = min(firstG, i); lastG = max(lastG, i); }",
  "        else { firstH = min(firstH, i); lastH = max(lastH, i); }",
  "    }",
  "",
  "    vector<int> Gleads, Hleads;",
  "    for (int i = 0; i < N; i++) {",
  "        if (breed[i] == 'G' && E[i] - 1 >= lastG && i <= firstG) Gleads.push_back(i);",
  "        if (breed[i] == 'H' && E[i] - 1 >= lastH && i <= firstH) Hleads.push_back(i);",
  "    }",
  "",
  "    long long ans = 0;",
  "    for (int g : Gleads) for (int h : Hleads) {",
  "        if (g + E[g] - 1 >= h || h + E[h] - 1 >= g) ans++;",
  "    }",
  "    cout << ans << \"\n\";",
  "    return 0;",
  "}",
...
export function LeadersProgressiveCode({ E, lang = "py", sections }) {
