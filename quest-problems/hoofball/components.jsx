...
const FULL_CPP = [
  ...
  "    if (N == 1) { cout << 1 << \"\n\"; return 0; }",
  "",
  "    // Each cow passes to its nearest neighbor (left or right)",
  "    // Count cows that don't receive a pass — those need a ball.",
  "    vector<int> receives(N, 0);",
  "    for (int i = 0; i < N; i++) {",
  "        int leftD  = (i == 0)     ? INT_MAX : x[i] - x[i-1];",
  "        int rightD = (i == N - 1) ? INT_MAX : x[i+1] - x[i];",
  "        if (rightD <= leftD) receives[i+1]++;   // tie → right",
  "        else receives[i-1]++;",
  "    }",
  "    int balls = 0;",
  "    for (int i = 0; i < N; i++) if (receives[i] == 0) balls++;",
  "    cout << balls << \"\n\";",
  "    return 0;",
  "}",
];
...
export function HoofballProgressiveCode({ E, lang = "py", sections }) {
