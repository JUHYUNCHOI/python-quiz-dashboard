...
const M3_STAGE_A_CPP = [
  ...
  "                if (s[idx] != ch) { left_i = idx; break; }",
  "            }",
  "            if (left_i == -1) continue;",
  "            // 가장 오른쪽 k (s[k] == ch)",
  "            int right_k = -1;",
  "            for (int idx = r; idx > left_i; idx--) {",
  "                if (s[idx] == ch) { right_k = idx; break; }",
  "            }",
  "            if (right_k <= left_i) continue;",
  "            // 그 사이의 j 후보들",
  "            for (int j : positions_of[c]) {",
  "                if (j <= left_i) continue;",
  "                if (j >= right_k) break;",
