...
export default function ChipXchgApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeChipXchgCh1(false));
  const [ch2Q, setCh2Q] = useState(() => makeChipXchgCh2(false, "py"));
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <ChipXchgProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <ChipXchgProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    return null;
  };
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`chipxchg-${tab}-${cur}-${lang}`} text={step.narr} />}
