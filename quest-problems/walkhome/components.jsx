...
const FULL_PY = [
  ...
  "",
  "    # dp(r, c, direction, changes) = paths from (r, c) to (N-1, N-1)",
  "    # direction: 0 = arrived moving right, 1 = arrived moving down, 2 = start",
  "    # changes: number of direction changes used so far",
  ...
  "            new_changes = changes + (1 if direction == 1 else 0)",
  "            if new_changes <= K:",
  "                total += dp(r, c+1, 0, new_changes)",
  "        # Move down",
  "        if r + 1 < N and grid[r+1][c] != 'H':",
  "            new_changes = changes + (1 if direction == 0 else 0)",
  "            if new_changes <= K:",
...
const FULL_CPP = [
  ...
  "int N, K;",
  "vector<string> grid;",
  "long long memo[50][50][3][4];",
  "bool seen[50][50][3][4];",
  "",
  "// direction: 0 = arrived moving right, 1 = arrived moving down, 2 = start",
  "long long dp(int r, int c, int direction, int changes) {",
  "    if (changes > K) return 0;",
  "    if (r == N - 1 && c == N - 1) return 1;",
  "    if (seen[r][c][direction][changes]) return memo[r][c][direction][changes];",
  "    seen[r][c][direction][changes] = true;",
  "    long long total = 0;",
  "    // Move right",
  "    if (c + 1 < N && grid[r][c + 1] != 'H') {",
  "        int nc = changes + (direction == 1 ? 1 : 0);",
  "        if (nc <= K) total += dp(r, c + 1, 0, nc);",
  "    }",
  "    // Move down",
  "    if (r + 1 < N && grid[r + 1][c] != 'H') {",
  "        int nc = changes + (direction == 0 ? 1 : 0);",
  "        if (nc <= K) total += dp(r + 1, c, 1, nc);",
  "    }",
  "    memo[r][c][direction][changes] = total;",
  "    return total;",
  "}",
  "",
  "int main() {",
