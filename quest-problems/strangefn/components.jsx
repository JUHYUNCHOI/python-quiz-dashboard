...
export function getStrangeFnWalk(E, lang = "py") {
  if (lang === "cpp") {
    return { code: FULL_CPP, vars: _SF_VARS, beats: [
      ...
      { hi: [44, 44], bubble: t(E, "3n − last is always even as a whole number.\nBut it's already reduced mod 10⁹+7,\nso it may not be even now (1000000008 becomes 1).\nSo we can't just divide by 2 here.\nInstead, C++ multiplies by INV2 = 500000004 —\nthe \"modular inverse\" the comment names.\nThe (... + MOD) guards against C++'s % returning a negative.\nIt's a value to memorize — why it works comes later.", "3n − last 는 정수로는 늘 짝수예요.\n그런데 이미 10⁹+7 로 줄인 값이라\n짝수가 아닐 수도 있어요 (1000000008 → 1).\n그래서 2 로 그냥 못 나눠요.\nC++은 대신 INV2 = 500000004 를 곱해요 —\n주석의 '모듈러 역원'이 이거예요.\n앞의 (... + MOD)는 C++ 의 % 가 음수를 줄 수 있어서예요.\n외워서 쓰는 값이에요 — 까닭은 나중에 배워요.") },
  ...
  return { code: FULL_PY, vars: _SF_VARS, beats: [
    ...
    { hi: [24, 24], bubble: t(E, "3n − last is always even as a whole number.\nBut it's already reduced mod 10⁹+7,\nso it may not be even now (1000000008 becomes 1).\nSo we can't just divide by 2 here.\nInstead, we multiply by pow(2, MOD-2, MOD).\nINV2 is short for \"inverse of 2\" — the \"modular inverse\" the comment names.\nIt's a formula to memorize — why it works comes later.", "3n − last 는 정수로는 늘 짝수예요.\n그런데 이미 10⁹+7 로 줄인 값이라\n짝수가 아닐 수도 있어요 (1000000008 → 1).\n그래서 2 로 그냥 못 나눠요.\n대신 pow(2, MOD-2, MOD) 를 곱해요.\nINV2 는 '2 의 역원', 주석의 '모듈러 역원'이 이거예요.\n외워서 쓰는 공식이에요 — 까닭은 나중에 배워요.") },
