...
import { SecretProgressiveCode, downloadSecretPDF, getSecretSections, getSecretWalk } from "./components";
import { makeSecretCh1, makeSecretCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";

const A = "#8b5cf6";

export default function SecretApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeSecretCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeSecretCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeSecretCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeSecretCh1, 1: (e) => makeSecretCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <SecretProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "secret-walk") return <CodeWalk E={E} lang={codeLang} {...getSecretWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <SecretProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "secret-walk") return <CodeWalk E={E} lang={codeLang} {...getSecretWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadSecretPDF(E, getSecretSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`secr-${tab}-${cur}-${lang}`} text={step.narr} />}
