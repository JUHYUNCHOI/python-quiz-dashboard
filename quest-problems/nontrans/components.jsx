...
const FULL_CPP = [
  ...
  "        vector<long long> B; { long long _x; while (cin >> _x) B.push_back(_x); if (!cin) cin.clear(); } // adapt: read N values",
  "",
  "        auto beats = [&](X, Y) {   // TODO: type args",
  "            auto win = sum(1 for x in X for y in Y if x > y);",
  "            auto lose = sum(1 for x in X for y in Y if x < y);",
  "            return win > lose;",
  "",
  "        if (not beats(A, B)) {",
  "            cout << 'no' << \"\\n\";",
  ...
  "        auto found = False;",
  "        // Brute force all possible die C (4 sides, values 1-10)",
  "        for (long long c1 = 1; c1 < 11; c1++) {",
  "            for (long long c2 = c1; c2 < 11; c2++) {",
  "                for (long long c3 = c2; c3 < 11; c3++) {",
  "                    for (long long c4 = c3; c4 < 11; c4++) {",
  "                        auto C_die = [c1, c2, c3, c4];",
  "                        if (beats(B, C_die) and beats(C_die, A)) {",
  "                            auto found = True;",
  "                            break;",
  "                    // if found: break",
...
export function NonTransProgressiveCode({ E, lang = "py", sections }) {
