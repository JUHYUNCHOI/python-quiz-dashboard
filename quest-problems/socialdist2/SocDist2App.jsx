...
import { SocDist2ProgressiveCode, downloadSocDist2PDF, getSocDist2Sections, getSocDist2Walk } from "./components";
import { makeSocDist2Ch1, makeSocDist2Ch2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#2563eb";

export default function SocDist2App(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeSocDist2Ch1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeSocDist2Ch2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeSocDist2Ch2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeSocDist2Ch1, 1: (e) => makeSocDist2Ch2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <SocDist2ProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "socdist2-walk") return <CodeWalk E={E} lang={codeLang} {...getSocDist2Walk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <SocDist2ProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "socdist2-walk") return <CodeWalk E={E} lang={codeLang} {...getSocDist2Walk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadSocDist2PDF(E, getSocDist2Sections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`socd-${tab}-${cur}-${lang}`} text={step.narr} />}
