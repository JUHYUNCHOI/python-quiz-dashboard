...
import { CowntraceProgressiveCode, CowntraceSim, downloadCowntracePDF, getCowntraceSections, getCowntraceWalk } from "./components";
import { makeCowntraceCh1, makeCowntraceCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#059669";

export default function CowntraceApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeCowntraceCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeCowntraceCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeCowntraceCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeCowntraceCh1, 1: (e) => makeCowntraceCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <CowntraceProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "cowntrace-walk") return <CodeWalk E={E} lang={codeLang} {...getCowntraceWalk(E, codeLang)} accent={A} />;
    if (step.type === "sim") return <CowntraceSim E={E} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <CowntraceProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "cowntrace-walk") return <CodeWalk E={E} lang={codeLang} {...getCowntraceWalk(E, codeLang)} accent={A} />;
    if (s.type === "sim") return <CowntraceSim E={E} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadCowntracePDF(E, getCowntraceSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`cown-${tab}-${cur}-${lang}`} text={step.narr} />}
