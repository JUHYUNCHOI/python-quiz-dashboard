      ...
      steps: [
        {
          ...
          content: `이 레슨은 **참고용** 이에요. 지금 시간 들여 외우지 않아도 돼요.

### 왜?

이 레슨이 다루는 두 가지 — **파일 I/O** 와 **Fast I/O** — 는 *언제 쓰는지* 가 핵심인데, 그 *언제* 가 다음 단계로 가서 실제 문제 풀 때예요.

| 도구 | 언제 진짜 필요? |
|---|---|
| **Fast I/O** (\`sync_with_stdio(false)\`) | 알고리즘 문제 코드가 *시간 초과* 났을 때 |
| **파일 I/O** (\`ifstream\`/\`ofstream\`/\`freopen\`) | 옛날 USACO 문제처럼 \`.in\` / \`.out\` 파일을 다뤄야 할 때 |

지금 미리 배워봐야 *그때 가면 까먹어 있어요.* 컨텍스트 없이 본 한 줄짜리 셋업 코드는 잘 안 남아요.

### 그럼 언제 봐야 하나?

- 알고리즘 랩에서 코드가 시간 초과 나면 → "잠깐 Fast I/O 한 줄 추가해야 했지" 떠올리고 *돌아오기*
- 옛날 USACO 문제 만나면 → "freopen 쓰는 거였지" 떠올리고 *돌아오기*

그 순간 이 레슨으로 돌아와서 *해당 페이지만* 짧게 보면 돼요.

### 그래서 지금?

- **건너뛰어도 됨** — 다음 레슨 잠금 안 됨
- **궁금하면 둘러봐도 됨** — 콘텐츠는 그대로 남겨둠
- **외우려고 애쓰지 마세요** — 컨텍스트 없으니 비효율

> 다음 단계 (알고리즘) 로 그냥 진행해도 OK. 필요해지면 그때 *돌아오는 게* 정답.`
        }
      ]
    },
    // ============================================
