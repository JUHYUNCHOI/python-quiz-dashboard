...
const PSQ_FAST_CPP = [
  ...
  "using namespace std;",
  "",
  "// memo = 답 공책 (메모이제이션) (메모이제이션) — 열쇠 = \"수열#예산\" 문자열",
...
const PSQ_FAST_PY_EN = [
  ...
  "",
  "# memo = the answer notebook (memoization) (memoization)",
  "memo = {}",
  "",
...
const PSQ_STK_CPP = [
  ...
  "        for (int a = 0; a <= N; a++)",
  "            for (int b = 0; b <= N; b++) {",
  "                blockLensReady[a][b] = false; blockLens[a][b].clear();",
  "                for (int c = 0; c <= K; c++) { memo[a][b][c] = -1; opened[a][b][c] = false; }",
  "            }",
  "",
  "        vector<vector<int>> todo;                 // 할 일 목록 (재귀 대신)",
  "        todo.push_back({0, N, K});",
  "        while (!todo.empty()) {",
  "            int start = todo.back()[0], end = todo.back()[1], budget = todo.back()[2];",
  "            if (memo[start][end][budget] != -1) { todo.pop_back(); continue; }",
  "            if (start >= end || budget <= 0) { memo[start][end][budget] = 0; todo.pop_back(); continue; }",
  "",
  "            if (!opened[start][end][budget]) {    // 이 조각을 처음 꺼냈을 때만",
  "                opened[start][end][budget] = true;",
  "                if (all_same(start, end)) { memo[start][end][budget] = 1; todo.pop_back(); continue; }",
  ...
  "            }",
  "",
  "            bool unknown = false, canMake = false;",
  ...
  "                if (memo[start][mid][budget] == -1) { todo.push_back({start, mid, budget}); unknown = true; break; }",
  "                if (memo[start][mid][budget] == 1) { canMake = true; break; }",
  "                blockTried[start][end][budget]++;",
  "            }",
  "            if (!unknown && !canMake) {",
  "                while (cutAt[start][end][budget] < end) {                                   // 요령 ③",
  "                    int cut = cutAt[start][end][budget], toLeft = giveLeft[start][end][budget];",
  "                    if (toLeft >= budget) { cutAt[start][end][budget]++; giveLeft[start][end][budget] = 1; continue; }",
  "                    if (memo[start][cut][toLeft] == -1) { todo.push_back({start, cut, toLeft}); unknown = true; break; }",
  "                    if (memo[start][cut][toLeft] == 1) {",
  "                        if (memo[cut][end][budget - toLeft] == -1) { todo.push_back({cut, end, budget - toLeft}); unknown = true; break; }",
  "                        if (memo[cut][end][budget - toLeft] == 1) { canMake = true; break; }",
  "                    }",
  "                    giveLeft[start][end][budget]++;",
  "                }",
  "            }",
  "            if (unknown) continue;                // 모르는 조각부터 먼저 (할 일 목록에 올라감)",
...
const PSQ_STK_CPP_EN = [
  ...
  "                }",
  "            }",
  "            if (unknown) continue;                // solve the unknown piece first (already on the list)",
  "            memo[start][end][budget] = canMake ? 1 : 0;",
  "            todo.pop_back();",
  "        }",
  "        cout << (memo[0][N][K] == 1 ? \"YES\" : \"NO\") << '\\n';",
...
export function getPrintseqStkWalk(E, lang = "py") {
  ...
  if (lang === "cpp") {
    return {
      ...
      vars,
      marks: [
        { from: 41, to: 42, color: "#0d9488", ko: "📋 재귀 대신 이것!", en: "📋 instead of recursion!" },
      ],
      beats: [
        { hi: [29, 34], bubble: t(E,
          "Start where we always start — the INPUT.\nmain reads T, then each case: N, K, and the target sequence.",
