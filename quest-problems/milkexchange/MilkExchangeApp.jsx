...
import { MilkExchangeProgressiveCode, downloadMilkExchangePDF, getMilkExchangeSections, getMilkExchangeWalk } from "./components";
import { makeMilkExCh1, makeMilkExCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";

const A = "#059669";

export default function MilkExchangeApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeMilkExCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeMilkExCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeMilkExCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeMilkExCh1, 1: (e) => makeMilkExCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <MilkExchangeProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getMilkExchangeWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <MilkExchangeProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getMilkExchangeWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadMilkExchangePDF(E, getMilkExchangeSections(E), codeLang)} style={{
