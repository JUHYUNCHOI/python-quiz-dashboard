...
import { Hps17ProgressiveCode, downloadHps17PDF, getHps17Sections } from "./components";
import { makeHps17Ch1, makeHps17Ch2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";

const A = "#2563eb";

export default function Hps17App(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeHps17Ch1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeHps17Ch2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeHps17Ch2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeHps17Ch1, 1: (e) => makeHps17Ch2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <Hps17ProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <Hps17ProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadHps17PDF(E, getHps17Sections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`hps1-${tab}-${cur}-${lang}`} text={step.narr} />}
