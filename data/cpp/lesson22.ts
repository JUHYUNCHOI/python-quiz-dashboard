...
**방법 1: Python 스타일 — 바디 대입** (지금까지 배운 방식)
\`\`\`cpp
...
\`\`\`
👉 객체를 **먼저 만들고**, 그 다음 \`{ }\` 안에서 \`=\` 로 값을 넣어요. Python 의 \`self.x = y\` 와 **거의 똑같이** 생겼죠.

**방법 2: C++ 표준 — 이니셜라이저 리스트** ⭐ (실무 표준)
\`\`\`cpp
BankAccount(string name, double initial)
...
> 📖 왜 "C++ 표준" 이냐면 — 실제 C++ 프로젝트·라이브러리·스타일 가이드 (Google, C++ Core Guidelines 등) 가 대부분 이 방식을 기본으로 써요. 학생이 오픈소스 보면 거의 이 방식만 나와요.

---

**결과는 똑같아 보여요** — 둘 다 \`owner\`, \`balance\` 에 값이 들어가니까요.

🤔 **그런데 왜 C++ 표준은 방법 2 일까요?** 단순한 선호가 아니라 **내부 동작이 다르기 때문**이에요. 다음 페이지에서 시뮬레이션으로 봐요.`
        },
        {
          ...
          content: `아래 시뮬레이터의 **상단 탭**(생성자 없음 / Python 스타일 / C++ 표준)과 단계 버튼을 눌러 비교해봐요.

- **생성자 없음**: 멤버가 쓰레기값으로 태어나서 **영원히** 그 상태 (위험!)
- **Python 스타일** (바디 대입): 멤버가 먼저 **빈값/쓰레기값** 으로 태어난 뒤, 바디에서 **덮어써요** → 일이 **2번**
- **C++ 표준** (이니셜라이저 리스트): 멤버가 **처음부터 원하는 값으로** 태어나요 → 일이 **1번**

단순 타입(\`int\`, \`double\`)에선 속도 차이 미미. **진짜 중요한 차이는** — 어떤 멤버들은 **"나중에 덮어쓰기" 가 아예 불가능** 해요. 다음 페이지에서 그 주인공들(\`const\`, \`reference\`)을 만나봐요.`
        },
        {
          id: "ch3-initlist-const-ref",
          type: "explain",
          title: "⚠️ const와 reference — C++ 표준 방식만 가능",
          content: `**const 멤버** 와 **reference 멤버** 는 **C++ 표준 방식** (이니셜라이저 리스트) 만 가능해요. **Python 스타일** (바디 대입) 은 컴파일 에러가 나요.
...
→ const·reference·기본생성자없는클래스 가 멤버로 있다면 **C++ 표준 방식 선택지 밖에 없어요**.`
        },
        {
          id: "ch3-initlist-realworld",
          type: "explain",
          title: "🌍 실무에서는 뭘 더 많이 쓰나요?",
          content: `**답: C++ 표준 방식** (이니셜라이저 리스트). 실제 C++ 프로젝트·오픈소스 라이브러리 코드는 대부분 이 방식으로 써요.
...
이제 개념은 잡았으니 **앞으로는 C++ 표준 (이니셜라이저 리스트) 를 기본** 으로 쓰는 습관을 들여보세요. 실전 코드, 오픈소스 읽을 때 훨씬 편해져요.
...
// 실무에서 더 자주 보는 방식 — C++ 표준 (이니셜라이저 리스트) ⭐
BankAccount(string name, double initial)
    : owner(name), balance(initial) {}
\`\`\`
...
앞으로 \`Player(string n, int h) : name(n), health(h) {}\` 같은 코드를 봐도 **"아, C++ 표준 방식 (이니셜라이저 리스트) 구나"** 바로 알아보세요.`
        },
        {
          id: "ch3-pred1",
          type: "predict" as const,
