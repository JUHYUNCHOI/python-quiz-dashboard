...
const FULL_CPP = [
  ...
  "    for (auto& s : spotted) cin >> s;",
  "    for (auto& s : plain) cin >> s;",
  "    int ans = 0;",
  "    // For each subset of positions (size 1, 2, or 3) check if it explains spotted-ness",
  "    for (int i = 0; i < M; i++) {",
  "        set<string> spp, pln;",
  "        for (auto& s : spotted) spp.insert(string(1, s[i]));",
  "        for (auto& s : plain) pln.insert(string(1, s[i]));",
  "        bool ok = true;",
  "        for (auto& a : spp) if (pln.count(a)) { ok = false; break; }",
  "        if (ok) ans++;",
  "    }",
  "    cout << ans << \"\n\";",
  "    return 0;",
...
export function BovGenomicsProgressiveCode({ E, lang = "py", sections }) {
