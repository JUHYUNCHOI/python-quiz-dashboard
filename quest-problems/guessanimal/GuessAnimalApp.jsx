...
import { GuessAnimalProgressiveCode, downloadGuessAnimalPDF, getGuessAnimalSections, getGuessAnimalWalk } from "./components";
import { makeGuessAnimalCh1, makeGuessAnimalCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#059669";

export default function GuessAnimalApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeGuessAnimalCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeGuessAnimalCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeGuessAnimalCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeGuessAnimalCh1, 1: (e) => makeGuessAnimalCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <GuessAnimalProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "guessanimal-codewalk") return <CodeWalk E={E} lang={codeLang} {...getGuessAnimalWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <GuessAnimalProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "guessanimal-codewalk") return <CodeWalk E={E} lang={codeLang} {...getGuessAnimalWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadGuessAnimalPDF(E, getGuessAnimalSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`gues-${tab}-${cur}-${lang}`} text={step.narr} />}
