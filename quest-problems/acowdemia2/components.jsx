...
const FULL_CPP = [
  ...
  "    int N, M;",
  "    cin >> N >> M;",
  "    vector<vector<bool>> before(N, vector<bool>(N, false));",
  "    for (int p = 0; p < M; p++) {",
  "        int K;",
  "        cin >> K;",
  "        vector<int> auth(K);",
  "        for (int i = 0; i < K; i++) cin >> auth[i];",
  "        for (int i = 0; i < K; i++) {",
  "            for (int j = i + 1; j < K; j++) {",
  ...
  return [
    {
      ...
      cppOnly: [
        t(E, "Split #include into specific headers (iostream, vector) — what we've been using.",
            "#include 는 배운 헤더들로 (iostream, vector) 나눠 적어."),
        t(E, "Use vector<vector<bool>> for 2D flags. int counts fit fine here.",
            "2D 플래그는 vector<vector<bool>>. 카운트는 int 로 충분."),
      ],
    },
  ];
}
