...
import { TrianglesProgressiveCode, TrianglesSim, downloadTrianglesPDF, getTrianglesSections, getTrianglesWalk } from "./components";
...
export default function TrianglesApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeTrianglesCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeTrianglesCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeTrianglesCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeTrianglesCh1, 1: (e) => makeTrianglesCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "triangles-walk") return <CodeWalk E={E} lang={codeLang} {...getTrianglesWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "triangles-walk") return <CodeWalk E={E} lang={codeLang} {...getTrianglesWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadTrianglesPDF(E, getTrianglesSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`tria-${tab}-${cur}-${lang}`} text={step.narr} />}
