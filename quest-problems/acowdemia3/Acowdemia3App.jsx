...
import { Acowdemia3ProgressiveCode, downloadAcowdemia3PDF, getAcowdemia3Sections, getAcowdemia3Walk } from "./components";
import { makeAcow3Ch1, makeAcow3Ch2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#059669";

export default function Acowdemia3App(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeAcow3Ch1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeAcow3Ch2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeAcow3Ch2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeAcow3Ch1, 1: (e) => makeAcow3Ch2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <Acowdemia3ProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getAcowdemia3Walk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <Acowdemia3ProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getAcowdemia3Walk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadAcowdemia3PDF(E, getAcowdemia3Sections(E), codeLang)} style={{
