...
const FULL_CPP = [
  ...
  "    cin >> s;",
  "",
  "    // Count G's and H's. We need odd group sum: (G + H) odd.",
  "    int G = 0, H = 0;",
  "    for (char c : s) {",
  "        if (c == 'G') {",
  "            G++;",
  ...
  "        }",
  "    }",
  "    // Pair up cows of different breeds. Each pair = 1 photo,",
  "    // leftover singles also become 1-cow photos.",
  "    int pairs = min(G, H);",
  "    int singles = abs(G - H);",
  "    cout << (pairs + singles) << \"\\n\";   // total photos",
  "    return 0;",
  ...
  return [
    {
      ...
      cppOnly: [
        t(E, "Range-for `for (char c : s)` walks the whole string — cpp-10 form.",
            "범위 for `for (char c : s)` 로 문자열 전체 순회 — cpp-10 형태."),
        t(E, "abs(G - H) gives the leftover count after pairing G with H.",
            "abs(G - H) 가 짝짓고 남은 개수."),
      ],
    },
  ];
