...
const FULL_CPP = [
  ...
  "    for (int i = 0; i < N; i++) for (int j = 0; j < N; j++) grid[i][j] = rows[i][j] - '0';",
  "    int ans = 0;",
  "    for (int i = N - 1; i >= 0; i--) {",
  "        for (int j = N - 1; j >= 0; j--) {",
  "            if (grid[i][j] == 1) {",
  "                ans++;",
  "                for (int x = 0; x <= i; x++) for (int y = 0; y <= j; y++) grid[x][y] ^= 1;",
  "            }",
  "        }",
  "    }",
  "    cout << ans << \"\n\";",
...
export function CowTipProgressiveCode({ E, lang = "py", sections }) {
