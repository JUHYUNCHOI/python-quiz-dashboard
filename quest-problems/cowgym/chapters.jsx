...
export function makeCowGymCh1(E) {
  return [
    // 1-1: Title
    {
      type: "reveal",
      narr: t(E,
        "K gymnastics sessions! In each, N cows are ranked from best to worst. We need to find pairs of cows where one ALWAYS beats the other in EVERY session.",
        "K번의 체조 세션! 각 세션에서 N마리 소가 순위를 매겨져. 모든 세션에서 한 소가 항상 다른 소를 이기는 쌍을 찾아야 해."),
      content: (
    ...
    // 1-2: Ranking table visual
    {
      type: "reveal",
      narr: t(E,
        "Here's an example with 3 cows and 2 sessions. The table shows each cow's rank (position) in each session. Lower rank = better performance!",
        "3마리 소와 2개 세션 예시야. 표는 각 소의 세션별 순위(위치)를 보여줘. 낮은 순위 = 더 좋은 성적!"),
      content: (
    ...
    // 1-3: What is a consistent pair?
    {
      type: "reveal",
      narr: t(E,
        "A consistent pair means the relative order NEVER changes across sessions. Cow A always beats cow B, or cow B always beats cow A.",
        "일관된 쌍은 상대적 순서가 세션 간에 절대 바뀌지 않는 것. 소 A가 항상 소 B를 이기거나, 소 B가 항상 소 A를 이기거나."),
      content: (
        <div style={{ padding: 16 }}>
          <div style={{ fontSize: 13, fontWeight: 800, color: "#d97706", marginBottom: 10 }}>
    ...
    // 1-4: Quiz — is (A,B) consistent?
    {
      type: "quiz",
      narr: t(E,
        "Sessions: [A>B>C] and [A>C>B]. Is pair (A,B) consistent? A is above B in both sessions, so yes!",
        "세션: [A>B>C]과 [A>C>B]. 쌍 (A,B)는 일관된가? A가 두 세션 모두에서 B보다 위이니 맞아!"),
      question: t(E,
    ...
    // 1-5: Quiz — count consistent pairs
    {
      type: "quiz",
      narr: t(E,
        "Sessions: [1,2,3] and [1,3,2]. Check all pairs: (1,2)✓ always 1 beats 2, (1,3)✓ always 1 beats 3, (2,3)✗ order changes. Total: 2!",
        "세션: [1,2,3]과 [1,3,2]. 모든 쌍 확인: (1,2)✓ 항상 1이 2를 이김, (1,3)✓ 항상 1이 3을 이김, (2,3)✗ 순서 변경. 총: 2!"),
      question: t(E,
    ...
    // 1-6: Input practice
    {
      type: "input",
      narr: t(E,
        "With N=3 cows, how many total pairs do we need to check? We check every unique pair (i, j) where i < j.",
        "N=3마리 소가 있으면, 총 몇 개의 쌍을 확인해야 할까? i < j인 모든 유일한 쌍 (i, j)을 확인해."),
      question: t(E,
...
export function makeCowGymCh2(E) {
  return [
    // 2-1: Convert order to rank
    {
      type: "reveal",
      narr: t(E,
        "First key insight: the input gives ORDER (who came 1st, 2nd, ...) but we need RANK (what position was cow X?). We need to convert!",
        "첫 번째 핵심: 입력은 순서(누가 1등, 2등...)를 주지만 우리는 순위(소 X의 위치?)가 필요해. 변환해야 해!"),
      content: (
        <div style={{ padding: 16 }}>
          <div style={{ fontSize: 13, fontWeight: 800, color: "#d97706", marginBottom: 10 }}>
    ...
    // 2-2: Check all pairs visually
    {
      type: "reveal",
      narr: t(E,
        "Then for each pair (i, j), check ALL K sessions: is rank[s][i] < rank[s][j] for every s? Or the reverse? If either holds, it's consistent!",
        "그 다음 각 쌍 (i, j)에 대해 모든 K 세션을 확인: 모든 s에서 rank[s][i] < rank[s][j]인가? 또는 반대? 둘 중 하나가 성립하면 일관!"),
      content: (
        <div style={{ padding: 16 }}>
    ...
    // 2-3: Quiz
    {
      type: "quiz",
      narr: t(E,
        "For N cows, how many unique pairs are there? This determines our algorithm's complexity!",
        "N마리 소에 대해 유일한 쌍은 몇 개? 이것이 알고리즘의 복잡도를 결정해!"),
      question: t(E,
    ...
    // 2-4: Complexity
    {
      type: "reveal",
      narr: t(E,
        "For each of the N*(N-1)/2 pairs, we check all K sessions. Total: O(K * N^2). With N<=20 and K<=10 from constraints, this is at most 10*400 = 4000 operations. Very fast!",
        "N*(N-1)/2개 쌍 각각에 대해 K개 세션을 확인. 총: O(K * N^2). 제약조건 N<=20, K<=10이면 최대 10*400 = 4000 연산. 매우 빨라!"),
      content: (
        <div style={{ padding: 16, textAlign: "center" }}>
    ...
    // 2-5: Input practice
    {
      type: "input",
      narr: t(E,
        "With N=5 cows, how many unique pairs must we check?",
        "N=5마리 소에 대해 몇 개의 유일한 쌍을 확인해야 할까?"),
      question: t(E,
        "N=5 cows. N*(N-1)/2 = ?",
        "N=5마리 소. N*(N-1)/2 = ?"),
      hint: t(E,
...
export function makeCowGymCh3(E) {
  return [
    // 3-1: Read input and build rank array
    {
      type: "reveal",
      narr: t(E,
        "Step 1: Read K sessions. For each, convert the order list into a rank array. rank[s][cow] = position of cow in session s.",
        "1단계: K개 세션 읽기. 각각 순서 리스트를 순위 배열로 변환. rank[s][cow] = 세션 s에서 소의 위치."),
      content: (
        <div style={{ padding: 16 }}>
    ...
    // 3-2: Double loop over pairs
    {
      type: "reveal",
      narr: t(E,
        "Step 2: Check all pairs (i, j) where i < j. For each pair, verify if the relative order is consistent across all K sessions.",
        "2단계: i < j인 모든 쌍 (i, j) 확인. 각 쌍에 대해 상대적 순서가 모든 K세션에서 일관되는지 검증."),
      content: (
        <div style={{ padding: 16 }}>
          <div style={{ fontSize: 13, fontWeight: 800, color: "#d97706", marginBottom: 6 }}>
    ...
    // 3-3: Quiz — why all()?
    {
      type: "quiz",
      narr: t(E,
        "We use all() to check every session. Why not any()? Think about what 'consistent' means!",
        "모든 세션을 확인하기 위해 all()을 사용해. 왜 any()가 아닐까? '일관'의 의미를 생각해봐!"),
      question: t(E,
    ...
    // 3-4: Why range starts at 1
    {
      type: "quiz",
      narr: t(E,
        "Look at the loop: range(1, N+1). Why start at 1, not 0? And why j starts at i+1?",
        "루프를 봐: range(1, N+1). 왜 0이 아니라 1에서 시작? j는 왜 i+1부터?"),
      question: t(E,
