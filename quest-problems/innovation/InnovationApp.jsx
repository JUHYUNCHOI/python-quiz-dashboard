...
import { InnovationProgressiveCode, InnovationSim, downloadInnovationPDF, getInnovationSections } from "./components";
...
export default function InnovationApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeInnovationCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeInnovationCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeInnovationCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeInnovationCh1, 1: (e) => makeInnovationCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "sim") return <InnovationSim E={E} />;
    if (step.type === "progressive") return <InnovationProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "sim") return <InnovationSim E={E} />;
    if (s.type === "progressive") return <InnovationProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadInnovationPDF(E, getInnovationSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`inno-${tab}-${cur}-${lang}`} text={step.narr} />}
