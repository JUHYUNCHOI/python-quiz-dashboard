...
import { Mcc20MissingProgressiveCode, downloadMcc20MissingPDF, getMcc20MissingSections, getMcc20MissingWalk } from "./components";
...
export default function Mcc20MissingApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeMcc20MissingCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeMcc20MissingCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeMcc20MissingCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeMcc20MissingCh1, 1: (e) => makeMcc20MissingCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <Mcc20MissingProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getMcc20MissingWalk(E)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <Mcc20MissingProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getMcc20MissingWalk(E)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadMcc20MissingPDF(E, getMcc20MissingSections(E), codeLang)} style={{
