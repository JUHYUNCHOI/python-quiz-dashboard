...
**왜 그래요?** 컴퓨터는 글자마다 숨은 번호를 매겨놨어요. 대문자 \`A\` 가 소문자 \`a\` 보다 **번호가 작아요**. 그래서 \`"Apple" < "apple"\` 이 \`True\`.

> 💡 정확한 번호 (예: \`A\` 는 65 번) 는 **lesson 9** 에서 자세히. 지금은 "대문자 < 소문자" 만 기억!
...
따옴표 안에 있으면 숫자처럼 보여도 **문자열**. 진짜 숫자로 비교하는 법은 나중에 (lesson 9) 따로 배워요.`
        },
        {
          id: "try-in-compare",
          type: "tryit",
...
> 💡 이번 레슨에서 미뤘던 것들 (대소문자 무시 비교, \`+\` 로 숫자 합치기, \`int("10")\` 같은 변환) 은 다음 레슨들에서 차근차근 배워요. 지금은 위 도구들 손에 익히는 데 집중!`
        ...
        },
        {
          ...
          type: "mission",
          title: "🏆 미션 3 — 내 이니셜 카드",
          task: "내 영어 이름의 이니셜 카드를 만들어요! 두 빈칸 모두 인덱스 번호 (숫자) — 첫 글자를 가리키는 번호를 넣으세요.\n\n💡 first / last 값은 자기 이름으로 바꿔도 OK! (단, 첫 글자가 대문자여야 깔끔)",
