...
import { Mcc19BakeryProgressiveCode, downloadMcc19BakeryPDF, getMcc19BakerySections } from "./components";
import { makeMcc19BakeryCh1, makeMcc19BakeryCh2 } from "./chapters";
...
export default function Mcc19BakeryApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeMcc19BakeryCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeMcc19BakeryCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeMcc19BakeryCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeMcc19BakeryCh1, 1: (e) => makeMcc19BakeryCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <Mcc19BakeryProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <Mcc19BakeryProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadMcc19BakeryPDF(E, getMcc19BakerySections(E), codeLang)} style={{
