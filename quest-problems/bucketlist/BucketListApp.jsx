...
import { BucketListProgressiveCode, BucketListSim, downloadBucketListPDF, getBucketListSections, getBucketListWalk } from "./components";
import { makeBucketListCh1, makeBucketListCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#f97316";

export default function BucketListApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeBucketListCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeBucketListCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeBucketListCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeBucketListCh1, 1: (e) => makeBucketListCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <BucketListProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "bucketlist-codewalk") return <CodeWalk E={E} lang={codeLang} {...getBucketListWalk(E, codeLang)} accent={A} />;
    if (step.type === "sim") return <BucketListSim E={E} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <BucketListProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "bucketlist-codewalk") return <CodeWalk E={E} lang={codeLang} {...getBucketListWalk(E, codeLang)} accent={A} />;
    if (s.type === "sim") return <BucketListSim E={E} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadBucketListPDF(E, getBucketListSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`buck-${tab}-${cur}-${lang}`} text={step.narr} />}
