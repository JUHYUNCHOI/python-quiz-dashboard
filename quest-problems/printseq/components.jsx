...
const PSQ_FAST_PY = [
  ...
  "    # 요령 ① → ② → ③ 순서대로",
  "    result = False",
  "    if is_all_same(seq):              # ① 다 같음",
  "        result = True",
  "    elif try_repeat(seq, budget):     # ② 블록 반복 (↺재귀)",
  "        result = True",
  "    elif try_cut(seq, budget):        # ③ 둘로 자르기 (↺재귀)",
  "        result = True",
  "",
...
const PSQ_FAST_CPP = [
  ...
  "    if (is_all_same(seq)) {                 // ① 다 같음",
  "        result = true;",
  "    } else if (try_repeat(seq, budget)) {   // ② 블록 반복 (↺재귀)",
  "        result = true;",
  "    } else if (try_cut(seq, budget)) {      // ③ 둘로 자르기 (↺재귀)",
  "        result = true;",
  "    }",
  "",
...
const PSQ_FAST_PY_EN = [
  ...
  "def can(seq, budget):",
  "    # solved before? -> reuse the answer",
  "    if (seq, budget) in memo:",
  "        return memo[(seq, budget)]",
  "",
  ...
  "    # try trick 1 -> 2 -> 3",
  "    result = False",
  "    if is_all_same(seq):              # 1) all same",
  "        result = True",
  "    elif try_repeat(seq, budget):     # 2) repeating block (↺ recursion)",
  "        result = True",
  "    elif try_cut(seq, budget):        # 3) cut in two (↺ recursion)",
  "        result = True",
  "",
...
const PSQ_FAST_CPP_EN = [
  ...
  "}",
  "",
  "// declare can first — tricks 2/3 call it (recursion)",
  "bool can(vector<int> seq, int budget);",
  "",
  "// trick 1: are all numbers the same?",
  "bool is_all_same(vector<int>& seq) {",
  ...
  "    if (is_all_same(seq)) {                 // 1) all same",
  "        result = true;",
  "    } else if (try_repeat(seq, budget)) {   // 2) repeating block (↺ recursion)",
  "        result = true;",
  "    } else if (try_cut(seq, budget)) {      // 3) cut in two (↺ recursion)",
  "        result = true;",
  "    }",
  "",
...
export function getPrintseqWalk(E, lang = "py") {
  ...
  if (lang === "cpp") {
    return {
      ...
      marks: [
        { from: 96, to: 99, ko: "✋ 베이스 케이스", en: "✋ base case" },
        { from: 57, to: 57, color: "#818cf8", ko: "↺ 재귀!", en: "↺ recursion!" },
        { from: 78, to: 79, color: "#818cf8", ko: "↺ 재귀!", en: "↺ recursion!" },
      ],
      beats: [
        ...
        { hi: [101, 109], bubble: t(E,
          "Didn't stop? Then try trick ① → ② → ③ in order.\ncan() now reads EXACTLY like our plan!\nThe recursion lives inside ②③ — on the ↺ lines, can calls ITSELF 👇",
          "안 멈췄으면? 요령 ① → ② → ③ 을 차례로 시도.\ncan() 이 우리 계획서랑 똑같이 읽히죠!\n재귀는 ②③ 안에 있어요 — ↺ 표시 줄에서 can 이 '자기 자신'을 불러요 👇") },
        { hi: [25, 33], bubble: t(E,
  ...
  return {
    ...
    // ✋ 베이스 케이스 상시 강조 (선생님 2026-07-17: "base case 가 눈에 잘 보이게")
    marks: [
      { from: 53, to: 55, ko: "✋ 베이스 케이스", en: "✋ base case" },
      { from: 30, to: 30, color: "#818cf8", ko: "↺ 재귀!", en: "↺ recursion!" },
      { from: 42, to: 43, color: "#818cf8", ko: "↺ 재귀!", en: "↺ recursion!" },
    ],
    beats: [
      { hi: [0, 0], bubble: t(E,
