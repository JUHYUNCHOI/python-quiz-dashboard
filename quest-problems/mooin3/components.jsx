...
const M3_STAGE_A_PY = [
  ...
  "        right_pointer = -1",
  "        for idx in range(r, left_pointer, -1):",
  "            if s[idx] == ch:",
  "                right_pointer = idx",
  "                break",
...
const M3_STAGE_A_CPP = [
  ...
  "            // 가장 오른쪽 k (s[k] == ch)",
  "            int right_pointer = -1;",
  "            for (int idx = r; idx > left_pointer; idx--) {",
  "                if (s[idx] == ch) {",
  "                    right_pointer = idx;",
  "                    break;",
  "                }",
...
export function getMooin3Walk(E, lang = "py", mode = "brute") {
  if (mode === "brute") {
    if (lang === "cpp") {
      return { code: M3_FULL_CPP, vars: _M3_VARS, beats: [
        { hi: [4, 8],   bubble: t(E, "One answer per query: the biggest (j−i)×(k−j) we can make.\nSo first, read N, Q and the string s.",
                              "물음마다 답을 하나 내요. (j−i)×(k−j) 중 제일 큰 값이에요.\n그러니 먼저 N 과 Q, 글자 줄 s 를 읽어요.") },
        { hi: [10, 15], bubble: t(E, "A query arrives. l and r count from 1, so shift them to 0.\nNothing found yet, so best starts at -1 — and long long, the product gets big.",
                              "물음이 왔어요. l 과 r 이 1 부터 세니 0 부터로 바꿔요.\n아직 아무것도 못 찾았으니 best 는 -1 이에요. 곱이 커지니 long long 으로 둬요.") },
        { hi: [16, 16], bubble: t(E, "Where do we start looking? Pin the middle spot j.\nOnce j is fixed, the left side and the right side can be looked at separately.",
                              "어디서부터 찾을까요? 가운데 자리 j 를 하나 고정해요.\nj 를 정해 놓으면 왼쪽과 오른쪽을 따로 볼 수 있어요.") },
        { hi: [17, 23], bubble: t(E, "j is fixed, so look left. The smaller i is, the bigger j−i gets.\nSo take the FIRST spot whose letter differs from s[j].",
                              "j 를 정했으니 왼쪽을 봐요. i 가 작을수록 j−i 가 커져요.\n그러니 s[j] 와 다른 글자가 처음 나오는 자리를 골라요.") },
        { hi: [24, 30], bubble: t(E, "Right side, same idea. The bigger k is, the bigger k−j gets.\nSo take the LAST spot whose letter is the same as s[j].",
                              "오른쪽도 같은 생각이에요. k 가 클수록 k−j 가 커져요.\n그러니 s[j] 와 같은 글자가 마지막으로 있는 자리를 골라요.") },
        { hi: [31, 36], bubble: t(E, "Both ends exist, so this j has a score: (j−left_pointer)×(right_pointer−j).\nKeep it if it beats the best. Cast to long long so it does not overflow.",
                              "양쪽 끝이 다 있으면 이 j 의 점수가 나와요.\n(j−left_pointer)×(right_pointer−j) 예요. 더 크면 최댓값을 바꿔요.\nlong long 으로 바꿔 곱해야 값이 안 넘쳐요.") },
        { hi: [38, 38], bubble: t(E, "Every j has been tried. Print this query's answer.",
                              "j 를 다 봤어요. 이 물음의 답을 출력해요.") },
      ] };
    }
    return { code: M3_FULL_PY(E), vars: _M3_VARS, beats: [
      { hi: [0, 1],   bubble: t(E, "One answer per query: the biggest (j−i)×(k−j) we can make.\nSo first, read N, Q and the string s.",
                              "물음마다 답을 하나 내요. (j−i)×(k−j) 중 제일 큰 값이에요.\n그러니 먼저 N 과 Q, 글자 줄 s 를 읽어요.") },
      { hi: [3, 7],   bubble: t(E, "A query arrives. l and r count from 1, so shift them to 0.\nNothing found yet, so best starts at -1.",
                              "물음이 왔어요. l 과 r 이 1 부터 세니 0 부터로 바꿔요.\n아직 아무것도 못 찾았으니 best 는 -1 로 시작해요.") },
      { hi: [8, 9],   bubble: t(E, "Where do we start looking? Pin the middle spot j.\nOnce j is fixed, the left side and the right side can be looked at separately.",
                              "어디서부터 찾을까요? 가운데 자리 j 를 하나 고정해요.\nj 를 정해 놓으면 왼쪽과 오른쪽을 따로 볼 수 있어요.") },
      { hi: [10, 15], bubble: t(E, "j is fixed, so look left. The smaller i is, the bigger j−i gets.\nSo take the FIRST spot whose letter differs from s[j].",
                              "j 를 정했으니 왼쪽을 봐요. i 가 작을수록 j−i 가 커져요.\n그러니 s[j] 와 다른 글자가 처음 나오는 자리를 골라요.") },
      { hi: [16, 21], bubble: t(E, "Right side, same idea. The bigger k is, the bigger k−j gets.\nSo take the LAST spot whose letter is the same as s[j].",
                              "오른쪽도 같은 생각이에요. k 가 클수록 k−j 가 커져요.\n그러니 s[j] 와 같은 글자가 마지막으로 있는 자리를 골라요.") },
      { hi: [22, 25], bubble: t(E, "Both ends exist, so this j has a score: (j−left_pointer)×(right_pointer−j).\nKeep it if it beats the best.",
                              "양쪽 끝이 다 있으면 이 j 의 점수가 나와요.\n(j−left_pointer)×(right_pointer−j) 예요. 더 크면 최댓값을 바꿔요.") },
      { hi: [26, 26], bubble: t(E, "Every j has been tried. Print this query's answer.",
                              "j 를 다 봤어요. 이 물음의 답을 출력해요.") },
    ] };
  }
  ...
  if (lang === "cpp") {
    return { code: M3_FAST_CPP(E), vars: _M3_FAST_VARS, beats: [
      { hi: [5, 9],   bubble: t(E, "The brute force re-scanned the string for every query — far too slow.\nThe string never changes, so write what we need into tables first.\nStart by reading N, Q and s.",
                              "브루트는 물음마다 글자 줄을 다시 훑어서 너무 느렸어요.\n글자 줄은 안 바뀌니 필요한 걸 미리 표에 적어 둬요.\n먼저 N 과 Q, 글자 줄 s 를 읽어요.") },
      { hi: [15, 18], bubble: t(E, "What do we want to look up instantly? Three things, for every letter and every spot.\nMake three empty tables (26 letters × N spots).\n[ ... for _ in range(26)] means: build that same row 26 times and hold them in one list.",
                              "무엇을 바로 꺼내 쓰고 싶은가요? 글자마다·자리마다 세 가지예요.\n빈 표 세 개를 만들어요 (글자 26 × 자리 N).\n[ ... for _ in range(26)] 는 같은 줄을 26 개 만들어 한 리스트에 담는 모양이에요.") },
      { hi: [19, 21], bubble: t(E, "Take one letter at a time and fill its row.",
                              "글자 하나를 골라 그 글자의 줄을 채워요.") },
      { hi: [22, 25], bubble: t(E, "First sweep, left to right. At each spot write down\nwhere the last c was up to here — that is latest_same.",
                              "먼저 왼쪽에서 오른쪽으로 한 번 훑어요.\n자리마다 '여기까지 본 마지막 c 는 어디?' 를 적어요 — latest_same 이에요.") },
      { hi: [26, 32], bubble: t(E, "Second sweep, right to left. At each spot write down\nthe nearest c on the right, and the nearest non-c on the right.",
                              "이번엔 오른쪽에서 왼쪽으로 훑어요.\n자리마다 '오른쪽으로 제일 가까운 c' 와 '제일 가까운 다른 글자' 를 적어요.") },
      { hi: [35, 40], bubble: t(E, "The tables are ready. Now answer the queries.\nShift l and r to 0-based, best starts at -1.",
                              "표가 다 됐어요. 이제 물음에 답할 차례예요.\nl 과 r 을 0 부터로 바꾸고, best 는 -1 로 시작해요.") },
      { hi: [41, 43], bubble: t(E, "We do not know which letter sits in the middle, so try all 26.\nThe left end i comes straight out of the table — no scanning.",
                              "가운데 글자가 무엇인지 모르니 26 글자를 다 해 봐요.\n왼쪽 끝 i 는 표에서 바로 꺼내요 — 훑지 않아요.") },
      { hi: [44, 46], bubble: t(E, "The right end k comes out of the table too.\nWith i and k fixed, (j−i)(k−j) is biggest near the middle, so look at m.",
                              "오른쪽 끝 k 도 표에서 바로 꺼내요.\ni 와 k 가 정해지면 (j−i)(k−j) 는 한가운데에서 제일 커요. 그래서 m 을 봐요.") },
      { hi: [47, 48], bubble: t(E, "But j has to be a c. So take the nearest c on each side of m — two candidates.",
  ...
  return { code: M3_FAST_PY(E), vars: _M3_FAST_VARS, beats: [
    { hi: [0, 1],   bubble: t(E, "30,000 queries arrive, so reading has to be quick. That is all this line is for.",
                              "물음이 3만 개예요. 읽는 것부터 빨라야 해요. 이 줄은 그것뿐이에요.") },
    { hi: [3, 4],   bubble: t(E, "The brute force re-scanned the string for every query — far too slow.\nThe string never changes, so take it in now and build tables next.",
                              "브루트는 물음마다 글자 줄을 다시 훑어서 너무 느렸어요.\n글자 줄은 안 바뀌니, 받아 두고 표를 만들 거예요.") },
    { hi: [10, 13], bubble: t(E, "What do we want to look up instantly? Three things, for every letter and every spot.\nMake three empty tables (26 letters × N spots).\n[ ... for _ in range(26)] means: build that same row 26 times and hold them in one list.",
                              "무엇을 바로 꺼내 쓰고 싶은가요? 글자마다·자리마다 세 가지예요.\n빈 표 세 개를 만들어요 (글자 26 × 자리 N).\n[ ... for _ in range(26)] 는 같은 줄을 26 개 만들어 한 리스트에 담는 모양이에요.") },
    { hi: [14, 17], bubble: t(E, "Take one letter at a time and fill its row.",
                              "글자 하나를 골라 그 글자의 줄을 채워요.") },
    { hi: [18, 21], bubble: t(E, "First sweep, left to right. At each spot write down\nwhere the last c was up to here — that is latest_same.",
                              "먼저 왼쪽에서 오른쪽으로 한 번 훑어요.\n자리마다 '여기까지 본 마지막 c 는 어디?' 를 적어요 — latest_same 이에요.") },
    { hi: [22, 25], bubble: t(E, "Now the right-hand side. Nothing found yet, so start at INF.",
                              "이제 오른쪽 차례예요. 아직 아무것도 못 찾았으니 INF 로 시작해요.") },
    { hi: [26, 32], bubble: t(E, "Second sweep, right to left. At each spot write down\nthe nearest c on the right, and the nearest non-c on the right.",
                              "이번엔 오른쪽에서 왼쪽으로 훑어요.\n자리마다 '오른쪽으로 제일 가까운 c' 와 '제일 가까운 다른 글자' 를 적어요.") },
    { hi: [34, 39], bubble: t(E, "The tables are ready. Now answer the queries.\nShift l and r to 0-based, best starts at -1.",
                              "표가 다 됐어요. 이제 물음에 답할 차례예요.\nl 과 r 을 0 부터로 바꾸고, best 는 -1 로 시작해요.") },
    { hi: [40, 43], bubble: t(E, "We do not know which letter sits in the middle, so try all 26.\nThe left end i comes straight out of the table — no scanning.",
                              "가운데 글자가 무엇인지 모르니 26 글자를 다 해 봐요.\n왼쪽 끝 i 는 표에서 바로 꺼내요 — 훑지 않아요.") },
    { hi: [44, 46], bubble: t(E, "The right end k comes out of the table too. If k is not past i, this letter cannot work.",
                              "오른쪽 끝 k 도 표에서 바로 꺼내요. k 가 i 보다 크지 않으면 이 글자는 안 돼요.") },
    { hi: [47, 50], bubble: t(E, "With i and k fixed, (j−i)(k−j) is biggest near the middle m.\nBut j has to be a c, so take the nearest c on each side of m.",
                              "i 와 k 가 정해지면 (j−i)(k−j) 는 한가운데 m 에서 제일 커요.\n그런데 j 는 c 여야 하니, m 양옆에서 제일 가까운 c 둘을 후보로 잡아요.") },
    { hi: [51, 54], bubble: t(E, "If a candidate sits between i and k, score it: (j−i)(k−j). Keep the max.",
                              "후보가 i 와 k 사이에 있으면 점수를 내요. (j−i)(k−j) 예요. 더 크면 최댓값을 바꿔요.") },
    { hi: [55, 57], bubble: t(E, "Every query is answered. Collect them and print in one go.",
                              "물음에 다 답했어요. 모아서 한 번에 출력해요.") },
  ] };
}
...
export function getMooin3MapWalk(E, lang = "py") {
  if (lang === "cpp") {
    return {
      ...
      beats: [
        { hi: [16, 19], bubble: t(E, "Same idea as the tables, written a different way.\nStart by reading N, Q and the string s.",
                              "표 풀이와 같은 생각인데, 적는 방법만 달라요.\n먼저 N 과 Q, 글자 줄 s 를 읽어요.") },
        { hi: [22, 24], bubble: t(E, "Queries keep asking 'where is the nearest c?'.\nSo list every position of every letter once, before any query — that is what makes it fast.",
                              "물음마다 '제일 가까운 c 가 어디?' 를 묻게 돼요.\n그래서 글자마다 나온 위치를 posOf 에 미리 한 번만 적어 둬요 — 이게 빨라지는 핵심이에요.") },
        { hi: [27, 30], bubble: t(E, "We will also keep asking 'where does the letter change?'.\nWrite that down too, so finding i takes no scanning at all.",
                              "'글자가 어디서 바뀌지?' 도 계속 묻게 돼요.\n그것도 nextDiff 에 미리 적어 두면 i 를 훑지 않고 한 번에 찾아요.") },
        { hi: [33, 38], bubble: t(E, "The lists are ready. Now answer a query.\nShift L and R to 0-based, best starts at -1.",
                              "목록이 다 됐어요. 이제 물음에 답해요.\nL 과 R 을 0 부터로 바꾸고, best 는 -1 로 시작해요.") },
        { hi: [40, 45], bubble: t(E, "We do not know the middle letter, so try all 26.\nk = the rightmost c that is ≤ R — binary search on the list, no scanning.",
                              "가운데 글자를 모르니 26 글자를 다 해 봐요.\nk 는 R 을 넘지 않는 가장 오른쪽 c 예요. 목록에서 이분탐색으로 찾아요.") },
        { hi: [47, 49], bubble: t(E, "i = the leftmost letter that differs from c.\nEither L itself, or nextDiff[L] when s[L] is a c.",
  ...
  return {
    ...
    beats: [
      { hi: [3, 6],   bubble: t(E, "Same idea as the tables, written a different way.\nStart by reading N, Q and the string s.",
                              "표 풀이와 같은 생각인데, 적는 방법만 달라요.\n먼저 N 과 Q, 글자 줄 s 를 읽어요.") },
      { hi: [8, 11],  bubble: t(E, "Queries keep asking 'where is the nearest c?'.\nSo list every position of every letter once, before any query — that is what makes it fast.",
                              "물음마다 '제일 가까운 c 가 어디?' 를 묻게 돼요.\n그래서 글자마다 나온 위치를 pos_of 에 미리 한 번만 적어 둬요 — 이게 빨라지는 핵심이에요.") },
      { hi: [13, 16], bubble: t(E, "We will also keep asking 'where does the letter change?'.\nWrite that down too, so finding i takes no scanning at all.",
                              "'글자가 어디서 바뀌지?' 도 계속 묻게 돼요.\n그것도 next_diff 에 미리 적어 두면 i 를 훑지 않고 한 번에 찾아요.") },
      { hi: [20, 22], bubble: t(E, "The lists are ready. Now answer a query.\nShift L and R to 0-based, best starts at -1.",
                              "목록이 다 됐어요. 이제 물음에 답해요.\nL 과 R 을 0 부터로 바꾸고, best 는 -1 로 시작해요.") },
      { hi: [23, 30], bubble: t(E, "We do not know the middle letter, so try all 26.\nk = the rightmost c that is ≤ R — binary search on the list, no scanning.",
                              "가운데 글자를 모르니 26 글자를 다 해 봐요.\nk 는 R 을 넘지 않는 가장 오른쪽 c 예요. 목록에서 이분탐색으로 찾아요.") },
      { hi: [31, 34], bubble: t(E, "i = the leftmost letter that differs from c.\nEither L itself, or next_diff[L] when s[L] is a c.",
