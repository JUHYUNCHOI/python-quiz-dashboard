...
/**
 * 스택/큐문제 풀이 — /algo/stackqueue/practice
 *
...
const SOLVED_KEY = "algo-stackqueue-contest-solved"
...
export default function SortingPracticePage() {
  ...
  const problem = problemId ? stackQueueContestCluster.problems.find(p => p.id === problemId) : null
  ...
  // 단일 문제 풀이 화면
  if (problem) {
    const problemIdx = stackQueueContestCluster.problems.findIndex(p => p.id === problem.id) + 1
    return (
      <div className="min-h-screen bg-gray-50 pb-24">
        ...
        <main className="max-w-5xl mx-auto px-4 pt-4">
          <div className="mb-3 flex items-center justify-between gap-2">
            <button
              onClick={() => router.push("/algo/stackqueue")}
        ...
        <LessonPanel open={showLesson} onClose={() => setShowLesson(false)} title={t("스택/큐 수업", "Stack & Queue lesson")}>
          <StackQueueLesson />
        </LessonPanel>
        <BottomNav />
      </div>
  ...
  // 리스트 화면
  const totalSolved = stackQueueContestCluster.problems.filter(p => solvedSet.has(p.id)).length
  const total = stackQueueContestCluster.problems.length
      ...
      <main className="max-w-2xl mx-auto px-4 pt-4">
        <div className="mb-4">
          <JourneyBreadcrumb items={[
            ...
            { label: "스택/큐", labelEn: "Stack/Queue", href: "/algo/stackqueue", emoji: "📊" },
            { label: "문제 풀이", labelEn: "Practice" },
          ]} />
          <div className="flex items-center gap-2 mb-2">
            <span className="text-3xl">{stackQueueContestCluster.emoji}</span>
            <h1 className="text-xl sm:text-2xl font-black text-gray-900">
              {t(stackQueueContestCluster.title, stackQueueContestCluster.en?.title ?? stackQueueContestCluster.title)}
            </h1>
          </div>
          <p className="text-sm text-gray-600 mb-3">
            {t(stackQueueContestCluster.description, stackQueueContestCluster.en?.description ?? stackQueueContestCluster.description)}
          </p>
        ...
        {(() => {
          const warmup = stackQueueContestCluster.problems.filter(p => p.difficulty === "쉬움")
          const easy = stackQueueContestCluster.problems.filter(p => p.difficulty === "보통")
          const hard = stackQueueContestCluster.problems.filter(p => p.difficulty === "어려움")
          const renderItem = (p: any) => {
            const isSolved = solvedSet.has(p.id)
            const globalIdx = stackQueueContestCluster.problems.findIndex(pp => pp.id === p.id) + 1
            return (
              <Link
                key={p.id}
                href={`/algo/stackqueue/practice?p=${p.id}`}
                className={cn(
