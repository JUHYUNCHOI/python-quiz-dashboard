...
const FULL_CPP = [
  ...
  "",
  "    if (N % 2 == 0) {",
  "        if (f != 0) return -1;   // even N: alternating sum must be 0",
  "    } else {",
  "        if (f < 0) return -1;    // odd N: final value f must be >= 0",
  "    }",
  "",
  "    // o[i] = bags spent on pair (i, i+1)",
  ...
  "    if (N % 2 == 0) {",
  "        long long mn = o[0];",
  "        for (int i = 0; i < N - 1; i += 2) mn = min(mn, o[i]);",
  "        for (int i = 0; i < N - 1; i += 2) o[i] -= mn;",
  "    }",
  "",
  "    long long bags = 0;",
  "    for (long long x : o) bags += x;",
  "    return 2 * bags;",
  "}",
  "",
  "int main() {",
