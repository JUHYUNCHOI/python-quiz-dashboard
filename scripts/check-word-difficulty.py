...
SYNONYMS = [
    ...
    ("통",     ["버킷", "바구니"]),
    ("표",     ["테이블"]),
    # ⚠️ `목록` 을 권하면 안 된다. **레슨이 쓰는 말은 따로 있다** (2026-09-17 실측):
    #    파이썬 레슨 `리스트` 344번 vs `배열` 5번 · C++ 레슨 `배열` 206번 vs `리스트` 15번.
    #    그러니 파이썬 글은 `리스트`, C++ 글은 `배열` 이 맞고, 둘이 같이 있는 건 정상이다.
    #    어긋나는 건 **셋째 이름 `목록`** 이다 (quest 20개가 쓰고 있다).
    ("리스트", ["목록"]),
    ("배열",   ["목록"]),
    ("사전",   ["딕셔너리", "맵"]),
    ("반복",   ["루프"]),
    ...
    ("글",     ["문자열", "스트링"]),
    ("자리",   ["인덱스"]),
    # ⚠️ `스택` 은 여기 없다. **레슨 23~26 이 "스택 (Stack)" 이라는 이름으로 직접 가르친다.**
    #    학생이 배운 말을 화면에서만 "쌓기" 로 바꾸면 오히려 코드의 `stack` 과 못 잇는다.
    #    (2026-09-17: presents·mcc15choco 담당 둘이 각각 같은 이유로 "못 바꾼다" 고 돌려보냈다.)
    ("줄",     ["행", "로우"]),
    ("칸",     ["셀", "열"]),
]
...
INSIDE = {
    "글":   ["글자", "한글", "글씨", "글쓰기"],
    "문자": ["문자열", "소문자", "대문자", "특수문자", "한글자"],
    "열":   ["문자열", "수열", "순열", "배열", "나열", "열다", "열어", "열고", "열린", "열리", "열기", "열림", "열려", "열쇠",
             "열심", "계열", "서열", "열째", "열번", "열혈"],
    "줄":   ["줄이", "줄어", "줄기", "줄자"],
    "행":   ["평행", "행동", "행복", "실행", "진행", "은행", "여행", "수행", "시행", "행렬",
             "행운", "행사", "행성", "비행", "행위"],
    ...
    "표":   ["표시", "발표", "목표", "표준", "대표", "투표", "표현", "표정", "좌표",
             "표기", "표면"],
    "칸":   ["칸막"],
    # ⚠️ 2026-09-17: `연산` 이 `연산자` 안에 들어 있어 10건이 통째로 허수였다.
    #    `연산자` 는 이 문제의 **원문 용어**다(P 가 고르는 연산자). 바꾸면 안 된다.
    #    같은 모양의 실수를 이 표에서 여덟 번째로 고친다 — 짧은 말이 긴 말 안에 숨는다.
    "연산": ["연산자", "사칙연산", "비트연산"],
    "맵":   ["맵다", "맵게", "매맵"],
    "딜":   ["딜러", "딜리", "딜레"],
    "행렬": [],
    ...
    "상한": ["이상한", "비정상한"],
    "이하": ["이하다"],
    "탐색": ["이분 탐색", "이분탐색", "완전 탐색", "완전탐색", "이진 탐색"],
    "셀":   ["셀프", "셀까", "셀 수", "세셀"],
    "반복": [],
}
...
TRANSLATIONESE = [
    ...
    ("영어를 음만 옮겼다 (쿼리·버킷…)",
     re.compile(r"쿼리|버킷|루프|인덱스|테이블|딜(?![러리])")),
]
