...
const FULL_PY = [
  ...
  "",
  "        # (A) 넉넉히 사고 끝내기",
  "        buy_up = (left + block_size - 1) // block_size",
...
const _BM_VARS = [
  ...
  { v: "paid", ko: "여기까지 확정으로 낸 값", en: "paid so far" },
  { v: "best", ko: "지금까지 제일 싼 값", en: "cheapest so far" },
];
export function getBuyMilkWalk(E, lang = "py") {
  if (lang === "cpp") {
    return { code: FULL_CPP, vars: _BM_VARS, beats: [
      ...
      { hi: [39, 50], bubble: t(E, "Now the real work. At each pack there are only two things to do —\n(A) buyUp: how many packs it takes to cover what is left.\nDivision rounds down, so we add (pack size − 1) first — that rounds up.\n(B) buyDown: how many fit without going over, so plain division works.",
                                 "이제 진짜 할 일이에요. 묶음마다 할 수 있는 건 둘뿐이에요 —\nbuyUp 은 넉넉히 사고 끝내기. 덮으려면 몇 개가 필요한지예요.\n나누기는 내림이라 모자라요. 그래서 나누기 전에 (묶음크기 − 1) 을 더해 올림해요.\nbuyDown 은 넘지 않게 사는 개수라 그냥 나누기고, 남은 통은 작은 묶음에 넘겨요.") },
  ...
  return [
    {
      ...
      why: [
        t(E, "Step 1 — is the price on a pack its real price?\nNot always: two half-size packs can be cheaper.\nSo block_cost[i] keeps the cheaper of the two — buy_the_deal or buy_two_halves.",
            "1단계 — 묶음에 붙은 값이 진짜 값일까요?\n아니에요. 절반짜리를 두 번 사는 게 쌀 수 있어요.\n그래서 block_cost[i] 에 싼 쪽을 담아요 — buy_the_deal 과 buy_two_halves 중에서요."),
        t(E, "That one step pays off: now a bigger pack is never worse per bucket.\nSo we never have to skip a big pack in favour of small ones —\nwhich is exactly why no recursion is needed.",
            "이 한 걸음이 값을 해요. 이제 큰 묶음이 한 통당 더 비쌀 수가 없어요.\n그래서 큰 묶음을 건너뛰고 작은 것만 쓸 이유가 없어요.\n재귀가 필요 없는 이유가 바로 이거예요."),
        t(E, "Step 2 — walk from the biggest pack down. At each pack there are only two moves:\nbuy enough with this pack and stop, or buy fewer and hand the rest to smaller packs.\nKeep the cheapest. One sweep per question, no recursion.",
            "2단계 — 큰 묶음부터 차례로 내려와요. 묶음마다 할 수 있는 건 둘뿐이에요.\n이 묶음으로 넉넉히 사고 끝내거나, 모자라게 사고 남은 통을 작은 묶음에 넘기거나.\n그중 제일 싼 값을 남겨요. 질문 하나에 한 번만 훑고, 재귀가 없어요."),
        t(E, "Why buy more than you need? One big cheap pack can cover x and spill over,\nand that can still cost less than matching x exactly. That is the buy_up branch.",
            "왜 필요한 것보다 많이 살까요? 싼 큰 묶음 하나가 x 를 덮고 남을 수 있어요.\n그게 딱 맞추기보다 쌀 때가 있어요. 그게 buy_up 쪽이에요."),
      ],
      pyOnly: [
