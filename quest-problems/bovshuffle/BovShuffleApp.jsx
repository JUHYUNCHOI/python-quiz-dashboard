...
import { BovShuffleProgressiveCode, downloadBovShufflePDF, getBovShuffleSections, getBovShuffleWalk } from "./components";
import { makeShuffleCh1, makeShuffleCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#f97316";

export default function BovShuffleApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeShuffleCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeShuffleCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeShuffleCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeShuffleCh1, 1: (e) => makeShuffleCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <BovShuffleProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "bovshuffle-codewalk") return <CodeWalk E={E} lang={codeLang} {...getBovShuffleWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <BovShuffleProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "bovshuffle-codewalk") return <CodeWalk E={E} lang={codeLang} {...getBovShuffleWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadBovShufflePDF(E, getBovShuffleSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`bovs-${tab}-${cur}-${lang}`} text={step.narr} />}
