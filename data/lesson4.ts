...
⚠️ \`=\`는 저장, \`==\`는 비교!`
        ...
        },
        {
          ...
          content: `수학 시간에 이런 거 본 적 있죠?

> **\`0 < x < 10\`** — "x 는 0 보다 크고 10 보다 작다"

한 줄로 쓰는 부등식. **파이썬은 이걸 그대로 코드로 쓸 수 있어요!**
...
C++ / Java 같은 다른 언어는 두 비교를 따로 써야 해요:

\`\`\`python
# 길게 — 두 비교 + and 로 연결
print(0 < x and x < 10)   # True (같은 의미, 더 김)
\`\`\`

같은 의미지만 파이썬 식이 짧고 수학에 가까워서 읽기 좋음.
...
### 언제 쓰나? — 값이 범위 안에 있나 검사

"점수가 70 점 이상 90 점 미만인가?", "나이가 13 ~ 19 (10대) 인가?" 같은 **범위 검사** 의 단골.
...
> 🎯 한 줄: **수학의 \`a < x < b\` 부등식을 파이썬 코드 그대로 쓸 수 있다.**

(조건문은 lesson 11 에서 배워요 — 이 결과를 if 와 같이 쓰는 식으로.)`
        },
        {
          id: "try-chained",
          type: "tryit",
