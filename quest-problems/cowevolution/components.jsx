// 🔒 USACO_VERIFIED — cpid=941, cowevolution (2019 Open Bronze #3)
...
const FULL_CPP = [
  ...
  "    for (const string& c : all_chars) char_list.push_back(c);",
  "    int L = char_list.size();",
  "    // 두 특성 a, b 가 'cross' 하면 invalid:",
  "    //   어떤 집단은 a 만, 어떤 집단은 b 만, 어떤 집단은 a 와 b 둘 다",
  "    bool valid = true;",
  "    for (int i = 0; i < L && valid; i++) {",
  "        for (int j = i + 1; j < L && valid; j++) {",
  "            bool a_only = false, b_only = false, both = false;",
  "            for (int p = 0; p < N; p++) {",
  "                bool ha = pops[p].count(char_list[i]) > 0;",
  "                bool hb = pops[p].count(char_list[j]) > 0;",
  "                if (ha && !hb) a_only = true;",
  "                if (hb && !ha) b_only = true;",
  "                if (ha && hb) both = true;",
  "            }",
  "            if (a_only && b_only && both) valid = false;",
  "        }",
  "    }",
  "    if (valid) {",
  "        fout << \"yes\\n\";",
