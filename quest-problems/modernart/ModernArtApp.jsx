...
import { ModernArtProgressiveCode, downloadModernArtPDF, getModernArtSections } from "./components";
import { makeModernArtCh1, makeModernArtCh2 } from "./chapters";

const A = "#059669";

export default function ModernArtApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeModernArtCh1(false));
  const [ch2Q, setCh2Q] = useState(() => makeModernArtCh2(false, "py"));

  useEffect(() => {
    setCh2Q(prev => makeModernArtCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeModernArtCh1, 1: (e) => makeModernArtCh2(e, codeLang) };
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(1100px, 100%)", margin: "0 auto", padding: "0 12px" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 12, alignItems: "center" }}>
          ...
          {showCodeControls && (
            <div style={{ display: "flex", gap: 0, alignItems: "stretch", flexShrink: 0 }}>
              ...
              <button onClick={() => downloadModernArtPDF(E, getModernArtSections(E), codeLang)} style={{
        ...
        {step.narr && <Narration key={`mod-${tab}-${cur}-${lang}`} text={step.narr} />}
