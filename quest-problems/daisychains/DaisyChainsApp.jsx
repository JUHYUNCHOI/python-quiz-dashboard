...
import { DaisyChainsProgressiveCode, downloadDaisyChainsPDF, getDaisyChainsSections } from "./components";
import { makeDaisyCh1, makeDaisyCh2, makeDaisyCh3, getDaisyChainsWalk } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#f97316";

export default function DaisyChainsApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeDaisyCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeDaisyCh2(lang === "en"));
  const [ch3Q, setCh3Q] = useState(() => makeDaisyCh3(lang === "en", "py"));

  useEffect(() => {
    setCh3Q(prev => makeDaisyCh3(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeDaisyCh1, 1: makeDaisyCh2, 2: (e) => makeDaisyCh3(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <DaisyChainsProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "daisychains-codewalk") return <CodeWalk E={E} lang={codeLang} {...getDaisyChainsWalk(E)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <DaisyChainsProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "daisychains-codewalk") return <CodeWalk E={E} lang={codeLang} {...getDaisyChainsWalk(E)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadDaisyChainsPDF(E, getDaisyChainsSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`daisycha-${tab}-${cur}-${lang}`} text={step.narr} />}
