...
import { MilkFactoryProgressiveCode, downloadMilkFactoryPDF, getMilkFactorySections, getMilkFactoryWalk } from "./components";
import { makeFactoryCh1, makeFactoryCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#2563eb";

export default function MilkFactoryApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeFactoryCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeFactoryCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeFactoryCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeFactoryCh1, 1: (e) => makeFactoryCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <MilkFactoryProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "milkfactory-codewalk") return <CodeWalk E={E} lang={codeLang} {...getMilkFactoryWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <MilkFactoryProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "milkfactory-codewalk") return <CodeWalk E={E} lang={codeLang} {...getMilkFactoryWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadMilkFactoryPDF(E, getMilkFactorySections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`milk-${tab}-${cur}-${lang}`} text={step.narr} />}
