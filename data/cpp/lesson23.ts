...
> **🎯 정렬해두면 검색이 빨라져요 — 이 챕터 통째로 그 이야기**

전화번호부에서 "Kim" 을 찾을 때 1 페이지부터 안 보죠? **가운데를 펼쳐서** 앞/뒤를 결정해요. 그게 가능한 건 이름이 **알파벳 순서로 정렬되어 있기 때문**.

컴퓨터도 똑같아요. 데이터가 정렬되어 있으면 **"가운데 보고 절반 버리기"** 가 가능해져서 검색이 폭발적으로 빨라져요.

---

### 📊 같은 일, 정렬 전후 속도 차이

100 만 개 데이터에서 검색한다고 해봐요:

| 하고 싶은 일 | 정렬 안 됨 | 정렬 후 |
|---|---|---|
| "x 가 있나?" | 100 만 번 비교 | **~20 번 비교** |
| "x 가 몇 개?" | 100 만 번 비교 | **~40 번 비교** |
| "x 이상 첫 거?" | 100 만 번 비교 | **~20 번 비교** |
| "중복 제거" | 직접 짜야 함 | **한 줄로 끝** |

정렬 한 번이 이렇게 많은 일을 한꺼번에 풀어줘요.

---

### 🔑 왜 이렇게 빨라져?

정렬돼 있으면 **"중간을 봤더니 X 면, 답은 한쪽에만 있다"** 가 가능해져요. 한 번 비교에 절반을 버릴 수 있어요. (앞 페이지의 이진 탐색이 정확히 이거.)

다음 페이지부터 **이 아이디어 위에 얹힌 도구들** 을 하나씩 배워볼 거예요. 각 도구마다 "이건 어디 쓰는 거지?" 가 분명해서 외울 게 줄어들 거예요.

> 💡 **학생용 한 줄:** "정렬은 끝이 아니라 **시작**. 정렬 한 번 = 검색이 빨라짐."`
        },
        {
          id: "s23-ch2-iter",
          type: "explain",
