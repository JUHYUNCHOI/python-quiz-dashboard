...
export const FULL_CPP = [
  ...
  "    vector<vector<long long>> beauty(N + 2, vector<long long>(N + 2, 0));",
  "",
  "    int W = N - K + 1; // valid top-left range per dim",
  "    vector<vector<long long>> S(W + 2, vector<long long>(W + 2, 0));",
  "",
  "    long long cur_max = 0;",
  "",
  "    while (Q--) {",
  ...
  "                S[i][j] += delta;",
  "                if (S[i][j] > cur_max) cur_max = S[i][j];",
  "            }",
  "        }",
  "        cout << cur_max << \"\\n\";",
  "    }",
  ...
  return [
    {
      ...
      cppOnly: [
        t(E,
          "long long for window sums: up to 25*25 = 625 cells * 10^6 = 6.25 x 10^8 (fits in 32-bit, but be safe).",
          "윈도우 합은 long long: 25*25 = 625 칸 * 10^6 = 6.25 x 10^8 (32 비트도 가능하지만 안전하게)."),
        t(E,
          "ios::sync_with_stdio(false) + cin.tie(nullptr) gives fast I/O for Q lines.",
          "ios::sync_with_stdio(false) + cin.tie(nullptr) 로 Q 줄 입출력 가속."),
      ],
    },
  ];
}
