...
**아니요.** STL 컨테이너 다룰 땐 거의 다 이터레이터 (또는 \`range-for\`) 써요.

| 상황 | 요즘 쓰는 것 |
|---|---|
| vector / map / set 순회 | \`for (auto x : v)\` ← 내부적으로 이터레이터 |
| STL 함수 (\`sort\`, \`find\`, \`lower_bound\` …) | 이터레이터 받음 — 다른 선택지가 없음 |
| C 라이브러리 호출 / 저수준 메모리 | 포인터 |

이번 챕터부터 \`sort(v.begin(), v.end())\` 같은 거 계속 쓰는데, 그 \`v.begin()\` 이 바로 이터레이터예요. 포인터로는 \`map\`, \`set\` 같은 거 못 다루니까 자연스럽게 이터레이터가 기본이 됐어요.
