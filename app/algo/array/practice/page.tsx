...
const SOLVED_KEY = "algo-array-contest-solved"
...
export default function SortingPracticePage() {
  ...
  // 단일 문제 풀이 화면
  if (problem) {
    const problemIdx = arrayContestCluster.problems.findIndex(p => p.id === problem.id) + 1
    return (
      <div className="min-h-screen bg-gray-50 pb-24">
        ...
        <main className="max-w-5xl mx-auto px-4 pt-4">
          <div className="mb-3 flex items-center justify-between gap-2">
            <button
              onClick={() => router.push("/algo/array")}
        ...
        <LessonPanel open={showLesson} onClose={() => setShowLesson(false)} title={t("배열 수업", "Array lesson")}>
          <ArrayLesson />
        </LessonPanel>
        <BottomNav />
      </div>
  ...
  // 리스트 화면
  const totalSolved = arrayContestCluster.problems.filter(p => solvedSet.has(p.id)).length
  const total = arrayContestCluster.problems.length
      ...
      <main className="max-w-2xl mx-auto px-4 pt-4">
        <div className="mb-4">
          <JourneyBreadcrumb items={[
            ...
            { label: "배열", labelEn: "Array", href: "/algo/array", emoji: "📊" },
            { label: "문제 풀이", labelEn: "Practice" },
          ]} />
          <div className="flex items-center gap-2 mb-2">
            <span className="text-3xl">{arrayContestCluster.emoji}</span>
            <h1 className="text-xl sm:text-2xl font-black text-gray-900">
              {t(arrayContestCluster.title, arrayContestCluster.en?.title ?? arrayContestCluster.title)}
            </h1>
          </div>
          <p className="text-sm text-gray-600 mb-3">
            {t(arrayContestCluster.description, arrayContestCluster.en?.description ?? arrayContestCluster.description)}
          </p>
        ...
        {(() => {
          const warmup = arrayContestCluster.problems.filter(p => p.difficulty === "쉬움")
          const easy = arrayContestCluster.problems.filter(p => p.difficulty === "보통")
          const hard = arrayContestCluster.problems.filter(p => p.difficulty === "어려움")
          const renderItem = (p: any) => {
            const isSolved = solvedSet.has(p.id)
            const globalIdx = arrayContestCluster.problems.findIndex(pp => pp.id === p.id) + 1
            return (
              <Link
                key={p.id}
                href={`/algo/array/practice?p=${p.id}`}
                className={cn(
