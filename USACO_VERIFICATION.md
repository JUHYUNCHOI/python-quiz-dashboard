...
# 🇲🇾 MCC 2019 · 2020 · 2021 · 2022 — 공식 풀이·독립 브루트포스 대조 (2026-09-24)

이 네 해는 **공식 테스트 데이터가 공개되지 않았다.** 그래서 두 가지 방법을 썼다.

## 방법 ① 공식 풀이 코드를 «정답 기계» 로 (2019 · 2021)

| 연도 | 공식 풀이 | 주소 |
|---|---|---|
| 2019 | Python + C++ (에디토리얼 안) | `ioimalaysia.org/competition/mcc/2019-editorial/` |
| 2021 | Python 6 + C++ 6 (코드만, 해설 없음) | `drive.google.com/drive/folders/1SVEIzqw6GIc-nbJRZiee_SsuXZ-oZLnF` |

## 방법 ② 원문만 읽고 **독립 브루트포스**를 새로 짜서 (2020 · 2022)

⚠️ **2022 는 Drive 의 «Solutions» 폴더가 실제로는 문제 원문 PDF 복사본이었다** —
알고리즘 해설도 코드도 없다(`pdftotext` 로 확인, 로컬 `public/problems/mcc22*.pdf` 와 동일).
**2020 도 공식 풀이가 없다.**

→ **우리 코드를 보지 않고** 원문 문장에서 직접 규칙을 읽어 **가장 단순한 전수 탐색**을 짰다.
   (`mcc22aliens` 순열 전체 · `mcc20zigzag` `itertools.combinations` 전체 ·
    `mcc20knight` 「정확히 j걸음에 닿는 좌표 집합」 직접 전개 — 최단거리·홀짝 트릭 안 씀 등)

## 결과 — **스물네 quest 전부 불일치 0건**

| 연도 | quest | 케이스 |
|---|---|---|
| 2019 | rect · bakery · candy · ditcoin · elim · palindrome | 300~500씩 + 경계(k=0·k=n·n=1) |
| 2020 | cipher · citytour · kitty · knight · missing · zigzag | 120~602씩 + 공식 샘플 |
| 2021 | carrots · dvd · marbles · glass · simplemath · menu | 300~2000씩 + 경계 |
| 2022 | grammar · aliens · maze · birthday · cardshark · lamp | 전수 탐색 160~2000씩 |

## ⭐ `mcc21glass` — **공식 코드 쪽이 틀렸다**

Drive 의 `round_glass.py` 가 «빠진 반지름이 중간» 루프를 `range(N - 1)` 로 돌리는데
`R`·`dp_r` 은 길이 `N-1`(인덱스 0..N-2)이라 **마지막 반복에서 IndexError 로 죽는다.**
`N=4, A=10, R=[2,3,4]`(정답 1) 에서 실제로 크래시했다.
**우리 quest 코드는 이 케이스에서 정답 `1` 을 정확히 낸다.**
루프 범위를 `N-2` 로 고친 뒤 2000케이스 전부 일치했다.
→ **«공식 풀이가 곧 정답» 이 아니다.** 2023 tichu 에서도 공식 py 가 틀렸다.

## 입출력 형식도 다시 봤다

MCC 2020 여섯 개는 `chapters.jsx` 에 **2026-09-17 자 수정 주석**이 이미 있다 —
*"원문에 없는 줄 형식을 원문인 것처럼 적어 두던 것을 지웠다"*.
지금은 원문처럼 변수 형태로만 표시한다. **새로 발견된 형식 불일치 없음.**
...
| 방법 | 연도 | quest 수 |
|---|---|---|
| **실제 채점기 제출** (HackerRank) | 2015 | 5 |
| **공식 테스트 데이터로 채점** | 2023 · 2024 · 2025 | 18 |
| **공식 풀이 코드와 대조** | 2019 · 2021 | 12 |
| **원문 읽고 독립 브루트포스 대조** | 2020 · 2022 | 12 |
| | **합계** | **47** |

남은 하나는 `mcc19rect2` — **대응하는 공식 문제가 없다**(2019 공식 문제는 6개뿐).
오늘 «MCC 2019 P7» 라벨과 지어낸 제약을 뺐다. 문제 자체는 정직하고 잘 풀린다.

## 오늘 이 과정에서 잡은 진짜 결함 넷

| quest | 무엇이 틀렸나 | 전 → 후 |
|---|---|---|
| `mcc15bahasaf` | 규칙을 «첫 **글자**» 로 읽었다. 원문은 «첫 **자음**» | 3/17 → **17/17** |
| `feb23` (USACO) | K줄을 증가 순서로 내야 하는데 **최솟값·최댓값만** + C++ 오버플로 | 2/20 → **9/20, 오답 0** |
| `tichu` | `N=K` 면 둘째 줄이 **없는데 읽어서** 런타임 에러 | 6/7 → **7/7** |
| `reach` | `S=0` 이면 둘째 줄이 **빈 줄로 있는데 안 읽어서** 입력이 밀림 | 4/10 → **6/10** |

⭐ **넷 다 공식 샘플로는 안 걸리던 것들이다.**
