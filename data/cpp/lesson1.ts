...
그 단계는 뭘까요?`,
        ...
        // ── 자연스러운 전환: 이제 직접 찍어보자! ──
        {
          ...
          content: `이제 직접 코드를 써볼 거예요!

C++ 코드를 쓰려면 먼저 파일 확장자를 알아야 해요.
...
이 레슨에서는 브라우저 안에서 바로 C++ 코드를 써볼 거예요. 나중에 VS Code 같은 에디터에서 직접 \`.cpp\` 파일을 만들어볼 수도 있어요! 😊`,
        },
      ...
      steps: [
        {
          ...
          type: "explain",
          title: "⚡ 컴파일하고 실행하는 법!",
          content: `코드를 작성했으면 이제 컴파일해서 실행해봐요!

아래 애니메이션에서 **다음 →** 버튼을 눌러보며 흐름을 따라가보세요 👇`,
      ...
      steps: [
        {
          ...
          type: "explain",
          title: "💬 // 이게 뭐예요? — 주석",
          content: `코드에 **//** 가 보이면 **주석**이에요!

\`\`\`cpp
// 이건 주석이에요 — 컴파일러가 무시해요
std::cout << "Hello!" << std::endl;  // 출력!
\`\`\`

- **//** 뒤의 내용은 컴파일러가 **완전히 무시**해요
- 실행도 안 되고, 에러도 아니에요
- 코드에 **설명을 달아두는 메모**예요 📝
