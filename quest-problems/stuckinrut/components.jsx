...
const FULL_CPP = [
  ...
  "        // cows.append((d, int(x), int(y)))",
  "",
  "    auto north = [(x, y, i) for i, (d, x, y) in enumerate(cows) if d == 'N'];",
  "    auto east = [(x, y, i) for i, (d, x, y) in enumerate(cows) if d == 'E'];",
  "",
  "    auto stopped = [False] * N;",
  "    auto grazed = [0] * N  # 0 means Infinity;",
  "",
  "    // For each pair (N-cow, E-cow), check if they intersect",
  "    auto events = [];",
  "    // for nx, ny, ni in north:",
  ...
  "            // E-cow reaches col nx at time nx - ex (if nx > ex)",
  "            if (nx > ex and ey > ny) {",
  "                auto tn = ey - ny  # time for N-cow to reach intersection row;",
  "                auto te = nx - ex  # time for E-cow to reach intersection col;",
  "                if (tn < te) {",
  "                    // events.append((te, ei, ni))  # E-cow stopped by N-cow",
  "                else if (te < tn) {",
  "                    // events.append((tn, ni, ei))  # N-cow stopped by E-cow",
