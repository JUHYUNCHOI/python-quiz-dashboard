...
import { MagicOrbsProgressiveCode, downloadMagicOrbsPDF, getMagicOrbsSections } from "./components";
import { makeMagicOrbsCh1, makeMagicOrbsCh2 } from "./chapters";
...
export default function MagicOrbsApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeMagicOrbsCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeMagicOrbsCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeMagicOrbsCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeMagicOrbsCh1, 1: (e) => makeMagicOrbsCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <MagicOrbsProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <MagicOrbsProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadMagicOrbsPDF(E, getMagicOrbsSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`magi-${tab}-${cur}-${lang}`} text={step.narr} />}
