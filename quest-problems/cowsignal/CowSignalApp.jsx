...
import { CowSignalProgressiveCode, downloadCowSignalPDF, getCowSignalSections, getCowSignalWalk } from "./components";
import { makeCowSignalCh1, makeCowSignalCh2, makeCowSignalCh3 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#8b5cf6";

export default function CowSignalApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeCowSignalCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeCowSignalCh2(lang === "en"));
  const [ch3Q, setCh3Q] = useState(() => makeCowSignalCh3(lang === "en", "py"));

  useEffect(() => {
    setCh3Q(prev => makeCowSignalCh3(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeCowSignalCh1, 1: makeCowSignalCh2, 2: (e) => makeCowSignalCh3(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <CowSignalProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "cowsignal-codewalk") return <CodeWalk E={E} lang={codeLang} {...getCowSignalWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <CowSignalProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "cowsignal-codewalk") return <CodeWalk E={E} lang={codeLang} {...getCowSignalWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`cowsigna-${tab}-${cur}-${lang}`} text={step.narr} />}
