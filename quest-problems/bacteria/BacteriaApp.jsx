...
import { BacteriaProgressiveCode, downloadBacteriaPDF, getBacteriaSections, getBacteriaWalk } from "./components";
import { makeBacteriaCh1, makeBacteriaCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";

const A = "#059669";

export default function BacteriaApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeBacteriaCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeBacteriaCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeBacteriaCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeBacteriaCh1, 1: (e) => makeBacteriaCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <BacteriaProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getBacteriaWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <BacteriaProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getBacteriaWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadBacteriaPDF(E, getBacteriaSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`bact-${tab}-${cur}-${lang}`} text={step.narr} />}
