...
export function getMakeDistinctWalk(E, lang = "py") {
  if (lang === "cpp") {
    return { code: FULL_CPP, vars: _MD_VARS_CPP, beats: [
      /* ⚠️ 2026-09-25 선생님: *"이것도 너무 설명이 많잖아."* 말풍선 하나(11줄 코드)에
         일곱 문장이 얹혀 있었다 — 목적/T읽기 + long long 이유 + 배열 생성 + llabs, 서로
         다른 코드 구간 넷을 한 덩어리로 설명했다. **코드 구간이 실제로 다르므로 가른다.** */
      { hi: [7, 10],  bubble: t(E, "What are we solving for? One number per test — the fewest operations needed.\ncin >> T reads the test count, then the loop runs once per test.", "무엇을 구해야 하나요? 테스트마다 최소 횟수 하나예요.\ncin >> T 로 테스트 개수를 받고, 테스트마다 돌아요.") },
      { hi: [11, 12], bubble: t(E, "n and k are long long — values can pass 2 billion, too big for int.\nN can be up to 200,000 — if many numbers pile up, one may be pushed almost that far,\nand with many such numbers the total easily passes 2 billion.", "n, k 는 long long 타입이에요 — 값이 20억을 넘을 수 있어 int 로는 부족해요.\nN 은 최대 200,000 이에요 — 같은 수가 몰리면 하나가 그 근처까지 밀릴 수 있고,\n그런 수가 여럿이면 답이 20억을 훌쩍 넘어요.") },
      { hi: [13, 17], bubble: t(E, "vector<long long> a(n) makes the array, and cin >> a[i] fills it one by one.\nllabs(k) is just abs() for long long — we need it because k can be negative.", "vector<long long> a(n) 으로 배열을 만들고 cin >> a[i] 로 하나씩 채워요.\nllabs(k) 는 long long 용 abs() 예요 — k 가 음수일 수 있어서 절댓값이 필요해요.") },
      { hi: [19, 19], bubble: t(E, "Adding K never changes a value's remainder mod m.\nFor example, with K = 2: 3+2=5, 5+2=7 … remainder is always 1.", "K 를 더해도 m 으로 나눈 나머지는 안 바뀌어요.\n예를 들어 K=2 라면 3+2=5, 5+2=7 … 나머지가 늘 1 이에요.") },
      { hi: [20, 20], bubble: t(E, "So different-remainder groups never collide with each other — we can handle each group separately.\nInside one group every gap is a multiple of m — 7−3=4 is a multiple of 2.", "그래서 나머지가 다르면 서로 절대 안 부딪혀요 — 묶음끼리 따로 봐도 돼요.\n같은 묶음 안에서는 차이가 늘 m 의 배수예요 — 7−3=4 는 2 의 배수죠.") },
      { hi: [21, 24], bubble: t(E, "C++'s % can return a negative number for negative input, so ((x%m)+m)%m nudges it back to 0..m-1.", "C++ 의 % 는 음수가 들어오면 음수를 돌려줄 수 있어요.\n((x%m)+m)%m 으로 0 이상으로 보정해요.") },
      { hi: [26, 26], bubble: t(E, "total starts at 0 — every push adds to it.", "total 은 0 에서 시작해요 — 밀 때마다 여기 더해요.") },
      { hi: [27, 28], bubble: t(E, "for (auto &kv : groups) pulls out one group at a time —\nkv.second is that group's list of values (vals).", "for (auto &kv : groups) 로 묶음을 하나씩 꺼내요 —\nkv.second 가 그 묶음의 값들(vals)이에요.") },
      { hi: [29, 33], bubble: t(E, "For K>0, sort ascending;\nfor K<0, sort(…, greater<long long>()) sorts descending.\nvals.begin() and vals.end() just mark the start and end of the range to sort.", "K>0 이면 오름차순,\nK<0 이면 sort(…, greater<long long>()) 로 내림차순 정렬해요.\nvals.begin(), vals.end() 는 정렬할 범위의 시작과 끝을 가리켜요.") },
      { hi: [34, 35], bubble: t(E, "The first value stays put as cur.", "첫 값은 그대로 두고 cur 로 삼아요.") },
      ...
      { hi: [45, 46], bubble: t(E, "cur = cur + k moves to the next free spot — each push adds exactly K.\ncur was already pushed ahead by earlier values, so vals[i] leaps straight to cur in one jump.", "cur = cur + k 로 다음 빈 자리로 밀어요 — 한 번 밀 때마다 딱 K 만큼 커져요.\ncur 는 앞 반복들에서 이미 여러 번 밀려 있어서, vals[i] 는 한 번에 cur 까지 건너뛰어요.") },
      { hi: [47, 50], bubble: t(E, "total += (cur - vals[i]) / k adds that count.\nC++'s / is integer division — it divides evenly here.\ntotal is long long, so it stays correct at scale.", "total += (cur - vals[i]) / k 로 그 횟수를 더해요.\nC++ 의 / 는 정수 나눗셈이라 나머지 없이 딱 떨어져요.\ntotal 이 long long 이라 값이 커져도 안전해요.") },
  ...
  return { code: FULL_PY, vars: _MD_VARS, beats: [
    ...
    /* ⚠️ 2026-09-25 선생님: *"이것도 너무 설명이 많잖아."* 아래 두 자리가 각각
       말풍선 하나에 네 문장 이상 얹혀 있었다. 서로 다른 코드 구간을 한 덩어리로
       설명하고 있어서 가른다(여담을 붙인 게 아니라 진짜 다른 줄을 설명하는 문장들). */
    { hi: [2, 4],   bubble: t(E, "solve() handles ONE test: read n, k, the array.", "solve() 는 테스트 하나를 맡아요. n, k, 배열을 읽어요.") },
    { hi: [5, 5],   bubble: t(E, "K can be negative,\nso the grouping size takes its absolute value — m = abs(k).\nabs strips the sign — abs(-2) is 2.", "K 가 음수일 수도 있어서,\n묶을 때 쓸 크기는 절댓값으로 잡아요 — m = abs(k)\nabs 는 부호를 떼는 것 — abs(-2) 는 2 예요.") },
    { hi: [7, 7],   bubble: t(E, "Adding K never changes a value's remainder mod m.\nFor example, with K = 2: 3+2=5, 5+2=7 … remainder is always 1.", "K 를 더해도 m 으로 나눈 나머지는 안 바뀌어요.\n예를 들어 K=2 라면 3+2=5, 5+2=7 … 나머지가 늘 1 이에요.") },
    { hi: [8, 8],   bubble: t(E, "So different-remainder groups never collide with each other — we can handle each group separately.\nInside one group every gap is a multiple of m — 7−3=4 is a multiple of 2.", "그래서 나머지가 다르면 서로 절대 안 부딪혀요 — 묶음끼리 따로 봐도 돼요.\n같은 묶음 안에서는 차이가 늘 m 의 배수예요 — 7−3=4 는 2 의 배수죠.") },
    { hi: [9, 10],  bubble: t(E, "defaultdict(list) makes each remainder's list for us.", "defaultdict(list) 가 나머지마다 통을 알아서 만들어 줘요.") },
    ...
    { hi: [21, 22], bubble: t(E, "On collision, push to cur + K, the next free spot — each push adds exactly K.\ncur isn't just the value we last compared — it's already been pushed several times by earlier steps.\nSo vals[i] leaps straight to cur in one jump.", "부딪히면 cur + K, 다음 빈 자리로 밀어요 — 한 번 밀 때마다 딱 K 만큼 커져요.\ncur 는 방금 비교한 값이 아니라, 앞선 반복들에서 이미 여러 번 밀린 값이에요.\n그래서 vals[i] 는 한 번에 cur 까지 건너뛰어요.") },
    { hi: [23, 23], bubble: t(E, "Same remainder means that gap is a multiple of m,\nso dividing it gives the exact push count.", "나머지가 같으면 그 거리는 m 의 배수라서,\n나누면 민 횟수가 그대로 나와요.") },
