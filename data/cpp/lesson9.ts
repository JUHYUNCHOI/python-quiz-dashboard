...
왜 이런 게 필요할까요? 학생 30명의 점수를 저장한다고 생각해보세요. 변수 30개를 만들 건가요? \`score1, score2, score3...\` 😱

이럴 때 쓰라고 있는 게 **같은 종류의 데이터를 한 줄에 보관하는 서랍장** — C++에서는 **배열(array)** 이에요.
...
파이썬 list와 비슷해 보이지만 **결정적인 차이**가 있어요. 다음 페이지에서 정리할게요.`,
          component: "cppArrayBuilder",
        },
        {
          id: "ch1-intro-diff",
          type: "explain",
          title: "🐍 vs ⚡ 파이썬 list와 무엇이 다를까?",
          content: `C++ 배열은 **처음부터 크기를 정하는 좌석 예약**과 같아요. 좌석을 나중에 늘릴 수 없듯이, 배열도 크기를 나중에 바꿀 수 없어요.
...
파이썬처럼 유연한 자료구조는 C++에도 있어요 — 이 레슨 후반에 배울 **vector** 가 그 역할을 해요. 하지만 배열을 먼저 이해해야 vector도 쉬워져요.`,
        },
        {
          id: "ch1-fb1",
...
💡 사실 합계는 \`sum += input\` 만으로도 구할 수 있어요. 근데 이번 레슨은 **배열 연습**이 목적이니 \`int scores[5]\` 에 5개를 먼저 저장한 뒤, for문으로 합계를 구해봐요.`,
          code: `#include <iostream>
using namespace std;
...
💡 이 문제도 배열 없이 \`if\`로 비교만 해도 풀 수 있지만, 이번엔 **배열 연습**이니 \`int scores[5]\` 에 5개를 저장한 뒤 for문으로 최댓값을 찾아봐요.`,
          code: `#include <iostream>
using namespace std;
...
C++에서 문자열 타입은 \`string\`이에요. (파이썬의 \`str\`)`,
        ...
        },
        {
          ...
          content: `이미 봤던 방식 외에, **크기(와 초기값)를 지정해서** vector를 만들 수 있어요.
...
\`\`\`cpp
vector<int> v(5);       // {0, 0, 0, 0, 0}  — int는 0으로 자동 초기화
vector<string> s(3);    // {"", "", ""}      — string은 빈 문자열
vector<double> d(4);    // {0.0, 0.0, 0.0, 0.0}
\`\`\`

초기값을 생략하면 **타입의 기본값으로 자동 초기화**돼요. 그래서 **쓰레기 값은 안 들어가요.**

### ⚠️ 배열과는 다르다

\`\`\`cpp
int arr[5];             // 쓰레기 값! (초기화 안 됨)
vector<int> v(5);       // {0, 0, 0, 0, 0} ✅
\`\`\`

일반 배열(\`int arr[5]\`)은 초기화 안 하면 쓰레기 값이 들어가지만, **vector**는 알아서 기본값으로 채워줘요. 실수 방지에 좋은 점이에요.
...
vector<int> v(5);     // 이것도 같은 결과 (int 기본값 = 0)
\`\`\`

@핵심: 알고리즘 문제에서 "n개짜리 0으로 초기화"할 때 \`vector<int> v(n);\` 한 줄로 끝!`,
        },
        {
          id: "ch2-fb-init",
          type: "fillblank" as const,
...
- \`vector<int> v(5, 1)\` → {1, 1, 1, 1, 1}`,
        ...
        },
        {
          id: "ch2-methods-add",
          type: "explain",
          title: "🛠️ vector — 끝에 추가/제거",
          content: `이제 vector의 주요 기능을 하나씩 살펴봐요. 모든 예시는 \`vector<int> v = {10, 20, 30};\` 기준이에요.
...
끝에서 추가/제거하는 건 가장 흔한 조작이라 파이썬 \`append\`/\`pop\` 과 짝이 맞아요.

**다음 페이지**: 중간에 끼워 넣거나 빼는 방법 — \`insert\`/\`erase\`.`
        },
        {
          id: "ch2-methods-middle",
          type: "explain",
          title: "➕ vector — 중간에 넣고 빼기",
          content: `끝뿐만 아니라 **중간 조작**도 가능해요. 위치는 **iterator** 로 지정해요 (곧 설명).

\`\`\`cpp
// v = {10, 20, 30}
v.insert(v.begin() + 1, 99);  // 1번 위치에 99 삽입 → {10, 99, 20, 30}
v.erase(v.begin() + 2);       // 2번 위치 삭제 → {10, 99, 30}
// 파이썬: v.insert(1, 99) / del v[2]
\`\`\`

💡 지금은 \`v.begin() + 1\` 을 **"1번 위치"** 라고만 받아들이면 충분해요. 왜 이렇게 쓰는지는 뒤에 \`begin()/end()\` 페이지에서 풀 거예요.

🐍 **파이썬과 다른 점** — 파이썬은 \`v.insert(1, 99)\` 처럼 **인덱스 숫자**를 바로 넣어요. 하지만 **C++ vector는 안 돼요.** 반드시 \`v.begin() + 1\` 같은 **iterator(위치)** 로 넘겨야 해요.

\`\`\`cpp
v.insert(1, 99);             // ❌ 컴파일 에러
v.insert(v.begin() + 1, 99); // ✅ 이렇게
\`\`\`

{expand:🤔 근데 v.begin()이 뭐예요? 왜 이렇게 복잡하게?}
C++ 컨테이너는 vector 말고도 \`list\` (linked list — 파이썬의 list 아님!), \`set\` (중복 없는 집합), \`map\` (키→값 딕셔너리) 등 여러 종류가 있어요. 그런데 이 중 \`list\`/\`set\`/\`map\` 은 **숫자 인덱스라는 개념이 아예 없어요.** "3번째 원소" 를 바로 집어낼 수가 없거든요.

그래서 C++은 **모든 컨테이너가 공통으로 쓸 수 있는 "위치 표기"** 로 iterator 를 만들었어요. \`insert\` 를 한 번 익히면 **vector/list/set/map 어디든 똑같이** 써먹을 수 있어요.

즉, "인덱스를 못 써서 불편" 이 아니라 **"하나의 공통 언어로 모든 자료구조를 다룬다"** 가 진짜 이유예요. (더 자세한 위치 개념은 곧 \`begin()/end()\` 페이지에서.)
{/expand}

⚠️ 중간 삽입/삭제는 뒤 원소를 **전부 한 칸씩 밀어야** 해서 느려요(O(n)). 가끔 쓰는 건 OK, 반복문 안에서 남발하면 성능 문제.`
        },
        {
          id: "ch2-fb-insert",
          type: "fillblank" as const,
          title: "insert 빈칸 채우기",
          content: "현재 \`v = {10, 20, 30}\` 이에요.\n\n**1번 자리**에 값 \`99\`를 넣어 \`{10, 99, 20, 30}\` 으로 만들려면 어떻게 써야 할까요?",
          code: "v.insert(v.begin() + ___, ___);",
          reviewHint: `insert 사용법:
\`v.insert(위치, 값);\`

- 위치는 iterator (숫자 인덱스가 아님!) — \`v.begin() + i\` 로 만들어요
- \`v.begin() + 0\` = 0번 자리
- \`v.begin() + 1\` = 1번 자리
- 값은 그 자리에 **삽입**할 값 (뒤 원소들은 한 칸씩 밀림)`,
          fillBlanks: [
            { id: 0, answer: "1", options: ["1", "0", "2", "3"] },
            { id: 1, answer: "99", options: ["99", "20", "10", "1"] }
          ],
          explanation: "v.insert(v.begin() + 1, 99)는 1번 자리에 99를 삽입해요. 원래 1번 자리였던 20과 그 뒤는 한 칸씩 뒤로 밀려요. 결과: {10, 99, 20, 30}"
        },
        {
          id: "ch2-methods-access",
          type: "explain",
          title: "🔍 vector — 원소 접근 / 크기",
          content: `이어서 **크기와 원소 접근** 관련 기능이에요. (\`v = {10, 20, 30}\` 기준)
...
💡 꼭 써야 하는 건 아니에요. \`v[0]\`, \`v[v.size()-1]\` 로도 똑같아요. \`front()/back()\`은 "앞/뒤"를 의미적으로 강조하고 싶을 때.`
        },
        {
          id: "ch2-methods-state",
          type: "explain",
          title: "🧹 vector — 상태 확인 / 비우기",
          content: `마지막으로 **상태 확인과 정리** 기능이에요.
...
💡 **find, sort** 같은 검색/정렬 기능은 \`#include <algorithm>\` 헤더에서 제공해요. 곧 배울게요!`
        },
        {
          id: "ch2-begin-end",
          type: "explain",
          title: "🧭 begin() / end() — 범위 표기",
          content: `\`insert(v.begin() + 1, ...)\` 에서 봤던 \`v.begin()\`, 곧 배울 \`sort(v.begin(), v.end())\` — 두 친구를 한 번에 정리할게요.

![begin과 end는 원소가 아니라 위치를 가리킵니다](/images/cpp/vector-begin-end.svg)

- \`v.begin()\` → **첫 원소의 위치** (\`v[0]\` 자리)
- \`v.end()\` → **마지막 원소의 바로 다음 위치** (원소 없는 자리!)

\`end()\` 가 "마지막 다음" 인 건 약속이에요. 이렇게 쓰면 **원소 개수 = v.end() - v.begin()** 으로 딱 떨어지고, 빈 vector 는 \`v.begin() == v.end()\` 로 자연스럽게 표현돼요.

@핵심: 둘을 짝지으면 "vector 전체 범위" 가 돼요. 그래서 \`sort(v.begin(), v.end())\` 는 "vector 전체 정렬".`
        },
        {
          id: "ch2-begin-end-arith",
          type: "explain",
          title: "➕ 위치끼리 덧셈/뺄셈",
          content: `그럼 **"가운데 자리"** 는 어떻게 가리킬까요?

위치는 **인덱스처럼 ± 가 돼요.**

\`\`\`cpp
v.begin()        // 첫 원소
v.begin() + 2    // 3번째 원소 (0번부터 세면 2칸 이동)
v.end() - 1      // 마지막 원소
v.end()          // 끝 표지판
\`\`\`

그래서 \`insert(v.begin() + 1, 99)\` 는 **"1번 위치에 99 넣기"** 라는 뜻이에요.

💡 간단히 외울 것: \`begin() + i\` = "i번 자리".`
        },
        {
          id: "ch2-array-arith",
          type: "explain",
          title: "📍 배열의 arr / arr+n 이 왜 통해?",
          content: `배열은 vector 보다 오래된 방식이라 조금 **날것**이에요. \`v.begin()\` 같이 친절한 메서드가 없어요. 대신 **배열 이름 자체가 "첫 원소 위치"** 를 의미해요.

![배열 이름은 첫 원소 위치, arr+n은 n칸 뒤 위치](/images/cpp/array-arr-n.svg)

- \`arr\` → 첫 원소 위치 (\`v.begin()\` 과 같은 역할)
- \`arr + 5\` → 원소 다섯 칸 뒤 = **끝 표지판** (\`v.end()\` 와 같은 역할)

{expand:🤔 이게 어떻게 되는 거예요? (주소 얘기)}
배열은 메모리에 **연속된 칸**으로 놓여요. 예를 들어 \`int arr[5]\` 가 1000번지에서 시작한다고 하면:

\`\`\`
arr     →  1000번지  → 10
arr + 1 →  1004번지  → 20    (int 가 4바이트라서 +4)
arr + 2 →  1008번지  → 30
arr + 3 →  1012번지  → 40
arr + 4 →  1016번지  → 50
arr + 5 →  1020번지  → (마지막 다음, 원소 없음)
\`\`\`

- \`arr\` 을 수식에서 쓰면 **"첫 원소의 주소"** 로 자동 변환돼요.
- \`arr + 1\` 은 \`1001\` 이 아니라 \`1000 + sizeof(int)\` 예요. 타입 크기에 맞춰 C++ 이 알아서 곱해줘요.
- 그래서 \`arr + 5\` 는 "5칸 뒤 주소" = \`end()\` 역할을 해요.

지금은 **"아, 주소 기반 트릭이구나"** 정도만 느끼면 충분해요. 진짜 포인터 연산은 **포인터 기초** 레슨에서 자세히 배워요.
{/expand}

그래서 아래 두 호출은 **"같은 의미의 다른 표기"** 예요.

\`\`\`cpp
vector<int> v = {5, 2, 8};
sort(v.begin(), v.end());     // vector 방식

int arr[3] = {5, 2, 8};
sort(arr, arr + 3);           // 배열 방식 — 시작 / 끝표지판 쌍
\`\`\`

@핵심: STL 함수들은 전부 **(시작 위치, 끝 표지판)** 쌍 패턴을 써요. vector면 \`v.begin()/v.end()\`, 배열이면 \`arr/arr+n\`.`
        },
        {
          id: "ch2-sort",
          type: "explain",
          title: "📊 기본 정렬 — sort()",
          content: `**sort()** 는 \`<algorithm>\` 헤더의 정렬 함수예요. 방금 배운 **[시작, 끝)** 범위 쌍을 받아요.
...
**사용법**:
- \`sort(v.begin(), v.end())\` — vector 전체 오름차순
- \`sort(arr, arr + n)\` — 배열(0번부터 n-1번까지) 오름차순
- \`sort(v.begin() + 2, v.end())\` — 2번부터 끝까지만 정렬
...
💡 **내림차순**, **구조체 정렬**, 정렬 알고리즘 내부 원리는 *sort & 이진탐색* 레슨에서 자세히 배워요. 여기선 "쓸 수 있다" 만 익히면 충분.`
        },
        {
          id: "ch2-pred-sort",
          type: "predict" as const,
...
int main() {
    ...
    return 0;
          ...
          reviewHint: `\`sort(v.begin(), v.end())\` 는 **v 전체를 오름차순**으로 정렬해요.
- \`v.begin()\` = 첫 원소 위치
- \`v.end()\` = 마지막 다음 위치 (끝 표지판)

range-for (\`for (int x : v)\`) 로 순회하며 공백으로 출력하면 정렬된 순서가 보여요.`,
          options: ["1 2 5 8 9 ", "5 2 8 1 9 ", "9 8 5 2 1 ", "에러"],
          answer: 0,
          explanation: "sort(v.begin(), v.end())는 오름차순 정렬이에요. {5,2,8,1,9} → {1,2,5,8,9}. 각 원소를 공백과 함께 출력하니 '1 2 5 8 9 '가 나와요."
        },
        {
          id: "ch2-pred1",
          type: "predict" as const,
...
💡 더 간결한 순회 방법인 **range-based for**는 다음 레슨에서 배워요!`
        },
        {
          id: "ch2-pred2",
          type: "predict" as const,
...
- \`empty()\` → 비어있으면 1(true), 아니면 0(false)`,
        ...
        },
        {
          id: "ch2-compare",
          type: "explain",
          title: "📊 배열 vs vector — 기본 원칙",
          content: `언제 뭘 써야 할까요? 표로 한눈에 봐요.
...
**기본 원칙:**
- 크기가 바뀔 수 있으면 → **vector**
- 잘 모르겠으면 → **vector** (훨씬 편하고 안전)

"그럼 배열은 아예 안 써도 되나요?" — 그건 아니에요. **다음 페이지에서** 배열이 더 자연스러운 두 경우를 볼게요.`
        },
        {
          id: "ch2-array-when",
          type: "explain",
          title: "🤔 그럼 배열은 언제 써요?",
          content: `두 가지 경우가 대표적이에요.

**① 크기가 처음부터 확실히 고정일 때**

예를 들어 요일은 언제나 7개예요. 주사위는 6면, 한 해의 월은 12개. 커질 일도, 줄 일도 없죠.

\`\`\`cpp
int daysInMonth[12];   // ✅ 자연스러움 (1~12월)
int diceCount[6];      // ✅ 주사위 6면 카운트
\`\`\`

이런 **절대 안 변하는 개수**는 vector보다 배열이 짧고 깔끔해요. (다음에 배우는 **2차원 배열**도 체스판(8×8) 같은 고정 크기 상황에서 요긴해요.)

**② 경쟁 프로그래밍(CP)에서 속도/간결함이 중요할 때**

USACO/백준 같은 문제풀이 코드는 이런 스타일이 흔해요:

\`\`\`cpp
int arr[1000001];   // 최대 100만 칸을 전역에 미리 확보
\`\`\`

- 선언이 짧아서 타이핑이 빠름
- 전역 배열은 자동으로 0 초기화됨
- 크기 바꿀 일 없어서 \`push_back\` 도 필요 없음

속도 차이는 실제론 크지 않지만, CP에서는 **코드가 짧고 친숙해서** 관행처럼 써요.

💡 정리: **일반 프로그램은 vector**, **크기 고정 / CP 코드는 배열** 이 자연스러워요. 둘 다 읽을 수 있어야 실무/문제풀이 양쪽에서 편해요.`
        },
        {
          id: "ch2-fb2",
          type: "fillblank" as const,
...
둘 다 **\`vector<int> nums;\` (빈 상태)** 로 시작해서 하나씩 채우는 방식이에요.

근데 만약 vector를 **미리 크기 정해서 만들었다면?** 그때도 push_back 써도 될까요? **다음 페이지에서** 살펴볼게요.`
        },
        {
          id: "ch2-cin-sized",
          type: "explain",
...
v.push_back(7);        // {0, 0, 0, 0, 0, 7}  크기 6 — 원한 결과 아님! ❌
\`\`\`

미리 크기를 정했다면 **\`cin >> v[i]\`** 로 그 자리에 덮어써요:

\`\`\`cpp
int n;
cin >> n;
vector<int> v(n);                    // 크기 n, 전부 0
for (int i = 0; i < n; i++) {
    cin >> v[i];                     // ← push_back 아님, 직접 접근!
}
\`\`\`
...
| vector 선언 | 입력 방식 |
|---|---|
| \`vector<int> v;\` (빈 상태) | \`v.push_back(x)\` |
| \`vector<int> v(n);\` (크기 지정) | \`cin >> v[i]\` |

@핵심: 빈 vector면 push_back, 미리 크기를 정한 vector면 \`v[i]\` 직접 접근. 섞어 쓰면 크기가 이상해져요!`
        },
        {
          id: "ch2-fb-cin",
          type: "fillblank" as const,
