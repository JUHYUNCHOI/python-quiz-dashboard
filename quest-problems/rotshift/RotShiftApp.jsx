...
import { RotShiftProgressiveCode, downloadRotShiftPDF, getRotShiftSections, getRotShiftWalk } from "./components";
import { makeRotShiftCh1, makeRotShiftCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";

const A = "#8b5cf6";

export default function RotShiftApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeRotShiftCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeRotShiftCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeRotShiftCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeRotShiftCh1, 1: (e) => makeRotShiftCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <RotShiftProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "rotshift-walk") return <CodeWalk E={E} lang={codeLang} {...getRotShiftWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <RotShiftProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "rotshift-walk") return <CodeWalk E={E} lang={codeLang} {...getRotShiftWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadRotShiftPDF(E, getRotShiftSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`rots-${tab}-${cur}-${lang}`} text={step.narr} />}
