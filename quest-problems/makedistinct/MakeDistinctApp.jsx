...
import { MakeDistinctProgressiveCode, downloadMakeDistinctPDF, getMakeDistinctSections } from "./components";
import { makeMakeDistinctCh1, makeMakeDistinctCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";

const A = "#2563eb";

export default function MakeDistinctApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeMakeDistinctCh1(false));
  const [ch2Q, setCh2Q] = useState(() => makeMakeDistinctCh2(false, "py"));
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <MakeDistinctProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    return null;
  };
  ...
  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      ...
      <button onClick={() => downloadMakeDistinctPDF(E, getMakeDistinctSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`makedistinct-${tab}-${cur}-${lang}`} text={step.narr} />}
