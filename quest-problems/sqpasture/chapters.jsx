...
export function makeSqPastureCh1(E) {
  return [
          ...
          <div style={{ background: "#fff7ed", border: "1.5px solid #d97706", borderRadius: 10, padding: "10px 14px", marginBottom: 10, textAlign: "center" }}>
            ...
            <div style={{ fontSize: 13, color: "#9a3412", lineHeight: 1.5 }}>
              {t(E,
                ...
                "두 목초지를 모두 감싸는 가장 작은 정사각형의 면적을 출력해요. 정사각형의 변은 축과 나란해요.")}
            </div>
          </div>
    ...
    },
    {
      type: "quiz",
      narr: t(E,
        "The bounding box uses: left = min(all x-lefts), right = max(all x-rights), bottom = min(all y-bottoms), top = max(all y-tops).", "바운딩 박스의 네 변은 min 과 max 로 정해요."),
      question: t(E,
        "Rect A: (1,2)→(4,5). Rect B: (6,1)→(9,4). Bounding box width (x-span)?",
        "A 는 (1,2)→(4,5), B 는 (6,1)→(9,4) 예요. 바운딩 박스의 가로는 얼마일까요?"),
    ...
    },
    {
      type: "reveal",
      narr: t(E,
        "Step 2: The bounding box might not be square!\nTo make it a square, take the LARGER of width and height as the side length.", "2단계예요. 가로와 세로 중 큰 쪽이 정사각형의 변이 돼요."),
      content: (
    ...
    },
    {
      ...
      question: t(E,
        ...
        "바운딩 박스의 가로가 5, 세로가 3 이에요. 정사각형 변 = max(5,3) = 5. 면적은 얼마일까요?"),
      options: ["25", "15", "8"],
      correct: 0,
    ...
    },
    {
      type: "input",
      narr: t(E,
        "A: (0,0)→(3,2).\nB: (5,1)→(8,6).\nBounding box: left=0, right=8, bottom=0, top=6.\nWidth=8, Height=6.\nSide = max(8,6) = 8.\nArea?", "두 목초지를 감싸는 정사각형의 면적을 구해 봐요."),
      question: t(E, "Side = 8. Area = 8² = ?", "A 는 (0,0)→(3,2), B 는 (5,1)→(8,6) 이에요. 둘을 감싸면 가로=8, 세로=6 이니까 변 = max(8,6) = 8 이에요. 면적 = 8² = ?"),
      answer: 64,
    },
  ];
}
...
export function makeSqPastureCh2(E, lang = "py") {
  return [
    ...
    },
    {
      type: "reveal",
      narr: t(E,
        "Find the bounding box: min of all lefts/bottoms, max of all rights/tops.", "왼쪽과 아래는 min 으로, 오른쪽과 위는 max 로 찾아요."),
      content: (
        <div style={{ padding: 16 }}>
    ...
    },
    {
      type: "reveal",
      narr: t(E,
        "Finally: square side = max of width and height. Area = side².", "정사각형 변은 가로와 세로 중 큰 값이고, 면적은 변² 이에요."),
      content: (
        <div style={{ padding: 16 }}>
          <div style={{ background: C.codeBg, borderRadius: 10, padding: "12px 14px", fontFamily: "'JetBrains Mono',monospace", fontSize: 13, lineHeight: 1.8 }}>
