...
//   (2026-07-02: SMART 코드를 prefix → center-expansion 으로 교체 (선생님 제출로 통과 확인).
//    CK_SMART_* = 중심에서 넓히며 두 끝만 갱신하는 O(N²). ch4·PDF 가 이 코드를 씀.
//    옛 prefix 시뮬/aside(OutPrefix·InPrefix·MatchUpToSim·CkSmartAside 등)는 quest 미사용 = 죽은 코드.)
...
// Page 6 — baseMatches: 안 뒤집었을 때 맞는 수 (모든 구간의 출발점).
...
const CK_SMART_S7_PY = [
  ...
  "        if cow[left]  == want[left]:",
  "            matches -= 1",
  "        if cow[right] == want[right]:",
  "            matches -= 1",
  "        if cow[left]  == want[right]:",
  "            matches += 1",
  "        if cow[right] == want[left]:",
  "            matches += 1",
  "        answer[matches] += 1",
  "        left  -= 1",
  "        right += 1",
...
export function getCheckupsSections(E) {
  return [
    ...
    {
      label: t(E, "5️⃣ Idea — reversing is symmetric, so only 2 ends change", "5️⃣ 아이디어 — 뒤집기는 대칭, 두 끝만 바뀜"),
      color: "#0891b2",
    ...
    {
      label: t(E, "6️⃣ baseMatches — matches with no flip", "6️⃣ baseMatches — 안 뒤집었을 때 맞는 수"),
      color: "#0d9488",
    ...
    {
      label: t(E, "7️⃣ expand — widen from a center, fix only 2 ends", "7️⃣ expand — 가운데서 넓히며 두 끝만"),
      color: "#15803d",
    ...
    {
      label: t(E, "8️⃣ Run every center + print", "8️⃣ 모든 중심 돌기 + 출력"),
      color: "#0d9488",
    ...
    {
      label: t(E, "9️⃣ Full code — everything together", "9️⃣ 전체 코드 — 한 번에"),
      color: "#15803d",
