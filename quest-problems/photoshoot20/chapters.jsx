...
export function makePhoto20Ch1(E) {
  return [
    // 1-1: Title reveal
    {
      type: "reveal",
      narr: t(E,
        "Given b[i] = a[i] + a[i+1] for a permutation a of 1..N, find the lexicographically smallest permutation a.\nTry each possible a[0] from 1 to N and derive the rest!", "순열 a의 b[i] = a[i] + a[i+1]이 주어질 때, 사전순으로 가장 작은 순열 a를 구해. a[0]을 1부터 N까지 시도하고 나머지를 유도해!"),
      content: (
    ...
    // 1-2: Quiz
    {
      type: "quiz",
      narr: t(E,
        "Example: b = [3], N = 2.\nWe need a[0] + a[1] = 3.\nIf a[0] = 1, then a[1] = 2.\nIs [1, 2] a valid permutation of {1, 2}?", "예시: b = [3], N = 2. a[0] + a[1] = 3이 필요해. a[0] = 1이면, a[1] = 2. [1, 2]는 {1, 2}의 유효한 순열일까?"),
      question: t(E,
...
export function makePhoto20Ch2(E, lang = "py") {
  return [
    // 2-1: Complexity reveal
    {
      type: "reveal",
      narr: t(E,
        "Try each a[0] from 1 to N, derive the rest in O(N).\nTotal: O(N^2).\nFor Bronze constraints, this is fast enough!", "a[0]을 1부터 N까지 시도하고 나머지를 O(N)에 유도. 총: O(N^2). 브론즈 제한에서는 충분히 빨라!"),
      content: (
        <div style={{ padding: 16, textAlign: "center" }}>
