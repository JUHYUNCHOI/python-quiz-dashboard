...
import { Photoshoot25ProgressiveCode, downloadPhotoshoot25PDF, getPhotoshoot25Sections } from "./components";
import { makePhotoshoot25Ch1, makePhotoshoot25Ch2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";

const A = "#8b5cf6";

export default function Photoshoot25App(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makePhotoshoot25Ch1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makePhotoshoot25Ch2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makePhotoshoot25Ch2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makePhotoshoot25Ch1, 1: (e) => makePhotoshoot25Ch2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <Photoshoot25ProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <Photoshoot25ProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadPhotoshoot25PDF(E, getPhotoshoot25Sections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`photoshoot25-${tab}-${cur}-${lang}`} text={step.narr} />}
