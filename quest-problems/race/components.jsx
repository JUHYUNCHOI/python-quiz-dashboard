...
const FULL_CPP = [
  ...
  "    while (N--) {",
  "        long long target_K, X; cin >> target_K >> X;",
  "        // Brute simulation: track speed and distance",
  "        long long bestT = LLONG_MAX;",
  "        for (long long peak = 1; peak <= 100000; peak++) {",
  "            // Accelerate 1..peak, then decelerate peak..X",
  "            long long dist = peak * (peak + 1) / 2 + (peak + X) * (peak - X + 1) / 2 - peak;",
  "            long long time = peak + (peak - X);",
  "            if (dist >= target_K && time < bestT) bestT = time;",
  "            if (dist > target_K + 1000000) break;",
  "        }",
  "        cout << bestT << \"\n\";",
  "    }",
  "    return 0;",
...
export function RaceProgressiveCode({ E, lang = "py", sections }) {
