...
\`\`\`python
print(money)   # ❌ money 라는 상자 만든 적 없음
\`\`\`

### 💁 파이썬이 하는 말 (한국어로 번역)

> "어? \`money\` 라는 상자 못 찾겠어! 아직 안 만들었나봐?" 😵

실제로 화면에 뜨는 빨간 영어 메시지:

\`\`\`
NameError: name 'money' is not defined
\`\`\`

→ \`NameError\` (이름 에러) = "그런 이름의 상자 없어!" 라는 뜻.
      ...
      steps: [
        {
          id: "rules-explain",
          type: "explain",
          title: "📋 변수 이름 규칙 — 4 가지만!",
          content: `### 1️⃣ **영문, 숫자, \`_\` 만** 쓸 수 있어요

- ✅ \`age\`, \`score_1\`, \`my_hp\`
- ❌ \`점수\` (한글) / \`my hp\` (공백) / \`my-hp\` (하이픈)
...
- ✅ \`my_if\`
- ❌ \`if\`, \`for\`, \`print\` (이건 다음 스텝에서 자세히)

### 4️⃣ **대문자, 소문자 다른 글자**

- \`age\` 와 \`Age\` 는 **완전 다른 상자**
- 실수로 대소문자 섞으면 다른 변수가 돼요!

> 🎯 한 줄: **영문/숫자/\`_\`, 첫 글자는 영문 아니면 \`_\`.**`
        },
        {
...
\`\`\`python
age = 15
print(Age)   # ❌ 에러! Age 는 만든 적 없음 (대문자 A!)
\`\`\`

\`age\`, \`Age\`, \`AGE\` → 파이썬한테는 **다 다른 상자**. 대문자 하나 차이도 다른 변수예요.`
        },
        {
...
playerHp = 100    # △ 낙타 — 동작은 하지만 파이썬에선 잘 안 씀
\`\`\`
...
> 🎯 컴퓨터는 \`x\` 든 \`player_hp\` 든 똑같이 동작해요. **한 달 후의 나** 가 읽으려고 쓰는 약속이에요.`
        },
        {
          id: "quiz3",
          type: "quiz",
...
✅ **다시 넣기** (재할당) — 같은 변수에 새 값 넣으면 덮어써짐 (\`x = x + 3\` 도 가능)
✅ **변수 이름 규칙** — 영문/숫자/_, 첫 글자 영문 또는 _, 파이썬 특별 단어 X, 언더바 스타일 (snake_case), 의미 있게
