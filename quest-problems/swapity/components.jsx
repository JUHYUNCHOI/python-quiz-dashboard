...
const FULL_PY = [
  ...
  "A1, A2 = map(int, lines[1].split())",
  "B1, B2 = map(int, lines[2].split())",
  "",
  "# 한 라운드의 perm 구축 (2번의 reverse 적용)",
  "perm = list(range(N))",
  "# 첫번째 reverse: 위치 A1-1 ~ A2-1 (0-indexed)",
  "perm[A1-1:A2] = perm[A1-1:A2][::-1]",
  "# 두번째 reverse: 위치 B1-1 ~ B2-1",
  "perm[B1-1:B2] = perm[B1-1:B2][::-1]",
  "",
  ...
  "        result[cycle[j]] = cycle[(j + shift) % L] + 1",
  "",
  "# 출력: 각 줄에 위치 i 의 소 번호",
...
const FULL_CPP = [
  ...
  "    reverse(perm.begin() + (A1 - 1), perm.begin() + A2);",
  "    reverse(perm.begin() + (B1 - 1), perm.begin() + B2);",
  "",
  "    // 사이클 분해로 K 라운드 효율적 계산",
