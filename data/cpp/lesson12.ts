      ...
      steps: [
        {
          ...
          content: `C++에서 함수에 변수를 넘기면, 기본적으로 **값이 복사**돼요! 이걸 **Call by Value** 라고 해요. 다음/이전 버튼으로 한 단계씩 어떻게 흘러가는지 보세요:`,
          contentAfter: `\`tryChange\` 에 \`num\` 을 넘기면 \`num\` 의 **복사본** 이 새로 생기고, 함수 안의 \`x\` 는 그 복사본이에요. 함수 안에서 \`x\` 를 바꿔도 원본 \`num\` 은 그대로!
...
아래 시뮬레이션에서 단계를 넘겨가며 봐봐요. \`int& x\`로 받으면 \`x\` **는** \`num\` **의 또 다른 이름 (별명)**이 돼요. 한 상자에 이름표가 두 개 붙은 셈이라, \`x\`를 바꾸면 \`num\`도 같이 바뀌어요.`,
          contentAfter: `이걸 활용한 대표적인 예: **swap 함수!**
...
💡 포인터('주소를 저장하는 변수')는 다음 레슨에서 배워요!`
        ...
        },
        {
          ...
          content: `지금까지 \`int&\`로 정수의 참조를 받았어요. **vector 도 똑같이** \`&\` **를 붙여서 참조로 받을 수 있어요!**
