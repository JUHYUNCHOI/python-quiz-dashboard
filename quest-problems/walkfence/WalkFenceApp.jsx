...
export default function WalkFenceApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeWalkCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeWalkCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeWalkCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeWalkCh1, 1: (e) => makeWalkCh2(e, codeLang) };
  ...
  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadWalkFencePDF(E, getWalkFenceSections(E), codeLang)} style={{
