...
function AstralDpWalk({ E }) {
  ...
  const steps = [
    ...
    { c: 0, box: "B", bubble: t(E,
      "Same first cell. Even if this star moves to the next cell, it was a star we PLACED right here (in photo 1) — moving is just how it looks in photo 2. So photo 1 still had 1 star here → send = 1.",
      "같은 첫 칸. 이 별을 옆으로 '보내도' — 그 별은 원래 이 자리(사진 1)에 놨던 거예요. 보낸 건 사진 2에서 옆 칸으로 간 모습일 뿐, 사진 1엔 여기 별이 1 개 있었던 거죠 → 보냄 = 1.") },
    ...
    { c: 3, box: "A", bubble: t(E,
      "Last cell G. Two ways to fill this spot: ① the previous star CAME here → it makes this G, so no new star → 2. ② the previous star did NOT come (it vanished, or there was none) → we must place a new star here → 2+1 = 3. Use fewer stars → the smaller, 2 → don't-send = 2. (So 1,1,2,2 — NOT 3!)",
      "마지막 칸 G. 이 자리를 채우는 길은 둘이에요. ① 앞 별이 여기로 '왔으면' → 그 별이 이 G가 돼요 → 새 별 안 놔도 됨 = 2. ② 앞 별이 '안 왔으면'(사라졌거나 없거나) → 여기 새 별을 놔야 해요 = 2+1 = 3. 별을 더 적게 쓰는 쪽! 더 작은 2 → 안 보냄 = 2. (그래서 1,1,2,2 — 3 이 아니에요!)") },
