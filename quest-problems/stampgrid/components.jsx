...
const FULL_CPP = [
  ...
  "int main() {",
  "    int N, K; cin >> N >> K;",
  "    vector<string> canvas(N), stamp(K);",
  "    for (auto& r : canvas) cin >> r;",
  "    for (auto& r : stamp) cin >> r;",
  "    // Check if canvas can be reproduced by stamping anywhere",
  "    bool possible = true;",
  "    for (int r = 0; r < N; r++) {",
  "        for (int c = 0; c < N; c++) {",
  "            if (canvas[r][c] == '.') continue;",
  "            // Check at least one stamp position covers (r,c) with same color",
  "            bool found = false;",
  "            for (int sr = 0; sr <= r && r - sr < K; sr++) {",
  "                for (int sc = 0; sc <= c && c - sc < K; sc++) {",
  "                    if (sr + K > N || sc + K > N) continue;",
  "                    if (stamp[r - sr][c - sc] == canvas[r][c]) { found = true; break; }",
  "                }",
  "                if (found) break;",
  "            }",
  "            if (!found) { possible = false; goto done; }",
  "        }",
  "    }",
  "    done:",
...
export function StampGridProgressiveCode({ E, lang = "py", sections }) {
