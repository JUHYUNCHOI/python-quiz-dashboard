...
export const cppLesson20Data: LessonData = {
  ...
  title: "CP 실전 팁",
  emoji: "🏆",
  description: "참고용 — 알고리즘 갈 때 필요해지면 다시 보기",
  chapters: [
    // ============================================
...
이 레슨이 다루는 내용은 대부분 **알고리즘 문제 풀 때 필요한 자잘한 기술** 들이에요:
...
| **비트마스크 부분집합** | 알고리즘 영역 — DP 배우면서 자연스럽게 |
| USACO 템플릿 | 옛날 USACO 기준 (모던 USACO 는 stdin/stdout) |
...
- **건너뛰어도 됨** — 다음 단계 (cpp-p3 → 알고리즘) 잠금 안 됨
- **궁금하면 둘러봐도 됨** — 콘텐츠 그대로
- **외우려고 애쓰지 마세요** — 컨텍스트 없으면 비효율

> 알고리즘 랩으로 진행해도 OK. 각 토픽에서 필요한 C++ 기술은 거기서 다시 안내될 거예요.`
        }
      ]
    },
    // ============================================
...
맞아요 — 맥 기본 컴파일러(clang)엔 \`bits/stdc++.h\` 가 **아예 없어서** \`'bits/stdc++.h' file not found\` 에러가 떠요. 세팅하느라 끙끙댈 필요 없어요. 쉬운 길부터:

| 방법 | 어떻게 |
|---|---|
| ⭐ **사이트의 [▶ 실행] 버튼** | 우리 실행기는 GCC라 \`bits/stdc++.h\` 가 그대로 돌아가요. **세팅 0** — 제일 편해요. |
| **백준·Codeforces·USACO 제출** | 채점기가 전부 GCC → 거기서도 잘 통해요 (실제 제출용) |
| 로컬 VS Code 에서 꼭 쓰려면 | \`g++\`(GCC) 를 설치하거나, 번거로우면 필요한 헤더만 개별 include (\`#include <iostream>\` 등) |

> 💡 즉 \`bits/stdc++.h\` 는 **GCC(채점기·우리 실행기) 세상의 편의 도구**예요. 로컬 맥에서 안 된다고 네가 틀린 게 아니에요!`
        },
        {
          id: "ch1-fb1",
