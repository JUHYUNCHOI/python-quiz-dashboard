...
import { PhotoshootProgressiveCode, downloadPhotoshootPDF, getPhotoshootSections, getPhotoshootWalk } from "./components";
import { makePhotoshootCh1, makePhotoshootCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#dc2626";

export default function PhotoshootApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makePhotoshootCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makePhotoshootCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makePhotoshootCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makePhotoshootCh1, 1: (e) => makePhotoshootCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <PhotoshootProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "photoshoot-walk") return <CodeWalk E={E} lang={codeLang} {...getPhotoshootWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <PhotoshootProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "photoshoot-walk") return <CodeWalk E={E} lang={codeLang} {...getPhotoshootWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadPhotoshootPDF(E, getPhotoshootSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`phot-${tab}-${cur}-${lang}`} text={step.narr} />}
