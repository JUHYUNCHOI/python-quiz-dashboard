...
const FULL_CPP = [
  ...
  "",
  "    long long N; cin >> N;",
  "    auto gestures = [input().strip() for _ in range(N)];",
  "",
  "    // Map 1,2,3 to H,P,S in all 6 permutations",
  "    auto labels = ['H', 'P', 'S'];",
  "    auto best = 0;",
  "",
  "    // for perm in permutations(labels):",
  "        auto mapping = {str(i+1): perm[i] for i in range(3)};",
  "        auto wins = 0;",
  "        // for g in gestures:",
  "            auto mine = mapping[g];",
  "            // H beats S, S beats P, P beats H",
  ...
  "        auto wins = 0;",
  "        // for g in gestures:",
  "            auto cow_gesture = perm[int(g)-1];",
  "            // FJ picks the gesture that beats cow's",
  "            wins += 1  # FJ can always win each round;",
  "        auto best = max(best, wins);",
  "",
  "    // Wait - re-read: FJ uses SAME gesture every game",
  "    // So FJ picks one gesture, count wins across all rounds",
  "    auto best = 0;",
