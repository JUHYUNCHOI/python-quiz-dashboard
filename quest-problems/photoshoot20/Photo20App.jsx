...
import { Photo20ProgressiveCode, downloadPhoto20PDF, getPhoto20Sections, getPhoto20Walk } from "./components";
import { makePhoto20Ch1, makePhoto20Ch2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#2563eb";

export default function Photo20App(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makePhoto20Ch1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makePhoto20Ch2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makePhoto20Ch2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makePhoto20Ch1, 1: (e) => makePhoto20Ch2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <Photo20ProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "photo20-codewalk") return <CodeWalk E={E} lang={codeLang} {...getPhoto20Walk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <Photo20ProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "photo20-codewalk") return <CodeWalk E={E} lang={codeLang} {...getPhoto20Walk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadPhoto20PDF(E, getPhoto20Sections(E), codeLang)} style={{
