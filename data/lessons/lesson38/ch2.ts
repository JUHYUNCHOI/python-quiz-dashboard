...
- **\`open(...)\`** — 공책을 *펼치는* 단계. \`'w'\` 는 "쓰기 모드로 펼쳐줘" 라는 뜻이에요.
  ⚠️ 이때 **파일에 원래 있던 내용은 지워져요.** 새 공책으로 바꾸는 셈이에요. (메모리 속 변수와 달리, 파일은 *먼저 열어야* 손댈 수 있어요.)
- **\`write(...)\`** — 펼친 공책에 *글자를 적는* 단계.
  ⚠️ \`print\` 와 달리 **줄바꿈을 안 해줘요.** \`write('A')\` 를 세 번 하면 \`AAA\` 가 돼요.
  줄을 나누고 싶으면 \`write('A\\n')\` 처럼 **직접** 넣어야 해요.
