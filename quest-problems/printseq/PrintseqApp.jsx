...
export default function PrintseqApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makePrintseqCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makePrintseqCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makePrintseqCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makePrintseqCh1, 1: (e) => makePrintseqCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <PrintseqProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "sim") return <PrintseqSim E={E} />;
    if (step.type === "runner") return <PrintseqRunner E={E} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <PrintseqProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "sim") return <PrintseqSim E={E} />;
    if (s.type === "runner") return <PrintseqRunner E={E} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadPrintseqPDF(E, getPrintseqSections(E), codeLang)} style={{
    ...
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        <AlgorithmTags E={E} tags={[
          { icon: "📋", ko: "할 일 목록 + 메모", en: "TO-DO list + memo", href: "/algo/dp/learn?from=quest" },
          { icon: "🔁", ko: "재귀 (심화)", en: "Recursion (bonus)", href: "/algo/recursion/learn?from=quest" },
        ]} />
        ...
        {step.narr && <Narration key={`prin-${tab}-${cur}-${lang}`} text={step.narr} />}
