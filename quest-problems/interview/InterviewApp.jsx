...
import { InterviewProgressiveCode, downloadInterviewPDF, getInterviewSections, getInterviewWalk, InterviewSim, InterviewRunner, InterviewHeapAudit } from "./components";
...
export default function InterviewApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeInterviewCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeInterviewCh2(lang === "en"));
  const [ch3Q, setCh3Q] = useState(() => makeInterviewCh3(lang === "en", "py"));
  ...
  const renderContent = () => {
    ...
    if (step.type === "interview-walk") return <CodeWalk E={E} lang={codeLang} {...getInterviewWalk(E, codeLang)} accent={A} />;
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "interview-walk") return <CodeWalk E={E} lang={codeLang} {...getInterviewWalk(E, codeLang)} accent={A} />;
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`intervie-${tab}-${cur}-${lang}`} text={step.narr} />}
