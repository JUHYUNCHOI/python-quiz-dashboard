...
const FULL_CPP = [
  ...
  "        for (char c : a) ca[c]++;",
  "        for (char c : b) cb[c]++;",
  "        for (auto& [c, _] : ca) need[c] = max(need[c], max(ca[c], cb[c]));",
  "        for (auto& [c, _] : cb) need[c] = max(need[c], max(ca[c], cb[c]));",
  "    }",
  "    for (char c = 'a'; c <= 'z'; c++) cout << (need.count(c) ? need[c] : 0) << \"\n\";",
  "    return 0;",
  "}",
];
...
export function BlockGameProgressiveCode({ E, lang = "py", sections }) {
