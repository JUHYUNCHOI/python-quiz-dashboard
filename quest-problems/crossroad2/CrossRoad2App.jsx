...
import { CrossRoad2ProgressiveCode, downloadCrossRoad2PDF, getCrossRoad2Sections, getCrossRoad2Walk } from "./components";
import { makeCrossRd2Ch1, makeCrossRd2Ch2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#f97316";

export default function CrossRoad2App(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeCrossRd2Ch1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeCrossRd2Ch2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeCrossRd2Ch2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeCrossRd2Ch1, 1: (e) => makeCrossRd2Ch2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <CrossRoad2ProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "cr2-codewalk") return <CodeWalk E={E} lang={codeLang} {...getCrossRoad2Walk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <CrossRoad2ProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "cr2-codewalk") return <CodeWalk E={E} lang={codeLang} {...getCrossRoad2Walk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadCrossRoad2PDF(E, getCrossRoad2Sections(E), codeLang)} style={{
