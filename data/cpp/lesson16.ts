...
> • **빈도 카운팅** — \`map<string, int> freq; for (단어) freq[w]++;\` — 입력에 나온 단어/숫자 각각 몇 번 등장?
> • **이름 → 점수 매핑** — \`map<string, int> scores;\` 학생 이름으로 점수 조회.
> • **좌표 → 인덱스 매핑** — 큰 좌표 (10⁹) 를 작은 번호로 재매김.
> • **"이 키가 처음 등장한 시각"** 추적 — \`if (!m.count(k)) m[k] = time;\`
...
> • **"본 적 있나?" 추적** — \`set<string> seen;\` 단어/좌표/ID 가 이미 등장했는지 빠른 체크.
> • **중복 제거된 종류 수** — 입력에 중복 섞여 있을 때 \`set\` 에 다 넣고 \`s.size()\` 가 답.
> • **방문한 위치** — 격자 BFS/DFS 에서 \`set<pair<int,int>>\` 로 이미 방문한 좌표 추적.
