...
export function getPrintseqWalk(E, lang = "py") {
  if (lang === "cpp") {
    return {
      ...
      marks: [{ from: 31, to: 34, ko: "✋ 베이스 케이스", en: "✋ base case" }],
      vars: [
      ...
      beats: [
        ...
        { hi: [29, 47], bubble: t(E,
          "Inside can(): the ✋ BASE CASE comes FIRST — empty / no budget → return false RIGHT HERE (recursion stops).\nBelow runs only if we didn't stop. Then trick ①: all same → 1 PRINT → true.",
          "can() 안: 맨 위에서 ✋ 베이스 케이스부터 — 빈 수열 / 예산 0 → 그 자리에서 바로 return false (재귀 멈춤).\n안 멈췄을 때만 아래로. 그다음 요령 ①: 다 같음 → PRINT 1 개면 됨 → true.") },
        { hi: [49, 55], bubble: t(E,
  ...
  return {
    ...
    // ✋ 베이스 케이스 상시 강조 (선생님 2026-07-17: "base case 가 눈에 잘 보이게")
    marks: [{ from: 13, to: 15, ko: "✋ 베이스 케이스", en: "✋ base case" }],
    vars: [
      { v: "seq", ko: "지금 보는 수열", en: "current sequence" },
      { v: "n", ko: "seq 길이", en: "length of seq" },
      { v: "budget", ko: "남은 PRINT 예산", en: "PRINT budget left" },
    ...
    beats: [
      ...
      { hi: [6, 6], bubble: t(E,
        ...
        "여기서 can 이라는 함수를 정의해요.\n수열(seq)과 예산(budget)을 받아서 딱 한 가지에 답해요:\n\"이 수열을 PRINT budget 개 이하로 만들 수 있나?\" → True / False.") },
      { hi: [11, 16], bubble: t(E,
        "Inside can(): the ✋ BASE CASE comes FIRST, at the top —\nempty sequence or no budget → return False RIGHT HERE.\nEverything below runs only when we didn't stop — no else needed!",
        "can() 안: 맨 위에서 ✋ 베이스 케이스부터 —\n빈 수열이거나 예산이 없으면 → 그 자리에서 바로 return False.\n여기서 안 멈췄을 때만 아래로 내려가요 — else 없이 깔끔!") },
      { hi: [19, 26], bubble: t(E,
