...
const SOLVED_KEY = "algo-trie-contest-solved"
...
export default function SortingPracticePage() {
  ...
  const problem = problemId ? trieContestCluster.problems.find(p => p.id === problemId) : null
  ...
  // 단일 문제 풀이 화면
  if (problem) {
    const problemIdx = trieContestCluster.problems.findIndex(p => p.id === problem.id) + 1
    return (
      <div className="min-h-screen bg-gray-50 pb-24">
        ...
        <LessonPanel open={showLesson} onClose={() => setShowLesson(false)} title={t("트라이 수업", "Trie lesson")}>
          <TrieLesson />
        </LessonPanel>
        <BottomNav />
      </div>
  ...
  // 리스트 화면
  const totalSolved = trieContestCluster.problems.filter(p => solvedSet.has(p.id)).length
  const total = trieContestCluster.problems.length
      ...
      <main className="max-w-2xl mx-auto px-4 pt-4">
        <div className="mb-4">
          <JourneyBreadcrumb items={[
            ...
            { label: "트라이", labelEn: "Trie", href: "/algo/trie", emoji: "📊" },
            { label: "문제 풀이", labelEn: "Practice" },
          ]} />
          <div className="flex items-center gap-2 mb-2">
            <span className="text-3xl">{trieContestCluster.emoji}</span>
            <h1 className="text-xl sm:text-2xl font-black text-gray-900">
              {t(trieContestCluster.title, trieContestCluster.en?.title ?? trieContestCluster.title)}
            </h1>
          </div>
          <p className="text-sm text-gray-600 mb-3">
            {t(trieContestCluster.description, trieContestCluster.en?.description ?? trieContestCluster.description)}
          </p>
        ...
        {(() => {
          const warmup = trieContestCluster.problems.filter(p => p.difficulty === "쉬움")
          const easy = trieContestCluster.problems.filter(p => p.difficulty === "보통")
          const hard = trieContestCluster.problems.filter(p => p.difficulty === "어려움")
          const renderItem = (p: any) => {
            const isSolved = solvedSet.has(p.id)
            const globalIdx = trieContestCluster.problems.findIndex(pp => pp.id === p.id) + 1
            return (
              <Link
                key={p.id}
                href={`/algo/trie/practice?p=${p.id}`}
                className={cn(
