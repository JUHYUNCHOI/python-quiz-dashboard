...
import { Mcc19PalProgressiveCode, Mcc19PalSim, downloadMcc19PalPDF, getMcc19PalSections, getMcc19PalWalk } from "./components";
...
export default function Mcc19PalApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeMcc19PalCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeMcc19PalCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeMcc19PalCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeMcc19PalCh1, 1: (e) => makeMcc19PalCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "sim") return <Mcc19PalSim E={E} />;
    if (step.type === "progressive") return <Mcc19PalProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getMcc19PalWalk(E)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "sim") return <Mcc19PalSim E={E} />;
    if (s.type === "progressive") return <Mcc19PalProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getMcc19PalWalk(E)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadMcc19PalPDF(E, getMcc19PalSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`mcc1-${tab}-${cur}-${lang}`} text={step.narr} />}
