...
export default function CowPhotosApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeCowPhotosCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeCowPhotosCh2(lang === "en", "py"));
  ...
  // Hover preview body for the segmented progress bar.
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <CowPhotosProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "sim") return <CowPhotosSim E={E} />;
    if (s.type === "runner") return <CowPhotosRunner E={E} />;
    return null;
  };

  // Code-tab-only controls slot.
