...
export default function LonelyPhotoApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeLonelyPhotoCh1(false));
  const [ch2Q, setCh2Q] = useState(() => makeLonelyPhotoCh2(false, "py"));

  useEffect(() => {
    setCh2Q(prev => makeLonelyPhotoCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeLonelyPhotoCh1, 1: (e) => makeLonelyPhotoCh2(e, codeLang) };
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(1300px, 100%)", margin: "0 auto", padding: "0 12px" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 12, alignItems: "center" }}>
          ...
          {showCodeControls && (
            <div style={{ display: "flex", gap: 0, alignItems: "stretch", flexShrink: 0 }}>
              ...
              <button onClick={() => downloadLonelyPhotoPDF(E, getLonelyPhotoSections(E), codeLang)} style={{
        ...
        {step.narr && <Narration key={`lp-${tab}-${cur}-${lang}`} text={step.narr} />}
