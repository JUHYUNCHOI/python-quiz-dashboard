...
import { TrainsProgressiveCode, downloadTrainsPDF, getTrainsSections, getTrainsWalk, TrainsAuditSim } from "./components";
import { makeTrainsCh1, makeTrainsCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";

const A = "#2563eb";

export default function TrainsApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeTrainsCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeTrainsCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeTrainsCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeTrainsCh1, 1: (e) => makeTrainsCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <TrainsProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "trains-walk") return <CodeWalk E={E} lang={codeLang} {...getTrainsWalk(E, codeLang)} accent={A} />;
    if (step.type === "auditSim") return <TrainsAuditSim E={E} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <TrainsProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "trains-walk") return <CodeWalk E={E} lang={codeLang} {...getTrainsWalk(E, codeLang)} accent={A} />;
    if (s.type === "auditSim") return <TrainsAuditSim E={E} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadTrainsPDF(E, getTrainsSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`trai-${tab}-${cur}-${lang}`} text={step.narr} />}
