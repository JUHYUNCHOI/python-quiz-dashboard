...
import { CornerCoverProgressiveCode, downloadCornerCoverPDF, getCornerCoverSections, getCornerCoverWalk } from "./components";
...
export default function CornerCoverApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeCornerCoverCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeCornerCoverCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeCornerCoverCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeCornerCoverCh1, 1: (e) => makeCornerCoverCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <CornerCoverProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getCornerCoverWalk(E)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <CornerCoverProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getCornerCoverWalk(E)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadCornerCoverPDF(E, getCornerCoverSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`corn-${tab}-${cur}-${lang}`} text={step.narr} />}
