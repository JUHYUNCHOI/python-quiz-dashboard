...
/**
 * 정렬문제 풀이 — /algo/topologicalsort/practice
 *
...
const SOLVED_KEY = "algo-topologicalsort-contest-solved"
...
export default function SortingPracticePage() {
  ...
  const problem = problemId ? topologicalSortContestCluster.problems.find(p => p.id === problemId) : null
  ...
  // 단일 문제 풀이 화면
  if (problem) {
    const problemIdx = topologicalSortContestCluster.problems.findIndex(p => p.id === problem.id) + 1
    return (
      <div className="min-h-screen bg-gray-50 pb-24">
        ...
        <main className="max-w-5xl mx-auto px-4 pt-4">
          <div className="mb-3 flex items-center justify-between gap-2">
            <button
              onClick={() => router.push("/algo/topologicalsort")}
        ...
        <LessonPanel open={showLesson} onClose={() => setShowLesson(false)} title={t("위상 정렬 수업", "Topological Sort lesson")}>
  ...
  // 리스트 화면
  const totalSolved = topologicalSortContestCluster.problems.filter(p => solvedSet.has(p.id)).length
  const total = topologicalSortContestCluster.problems.length
      ...
      <main className="max-w-2xl mx-auto px-4 pt-4">
        <div className="mb-4">
          <JourneyBreadcrumb items={[
            ...
            { label: "위상 정렬", labelEn: "Topological Sort", href: "/algo/topologicalsort", emoji: "📊" },
            { label: "문제 풀이", labelEn: "Practice" },
          ]} />
          <div className="flex items-center gap-2 mb-2">
            <span className="text-3xl">{topologicalSortContestCluster.emoji}</span>
            <h1 className="text-xl sm:text-2xl font-black text-gray-900">
              {t(topologicalSortContestCluster.title, topologicalSortContestCluster.en?.title ?? topologicalSortContestCluster.title)}
            </h1>
          </div>
          <p className="text-sm text-gray-600 mb-3">
            {t(topologicalSortContestCluster.description, topologicalSortContestCluster.en?.description ?? topologicalSortContestCluster.description)}
          </p>
        ...
        {(() => {
          const warmup = topologicalSortContestCluster.problems.filter(p => p.difficulty === "쉬움")
          const easy = topologicalSortContestCluster.problems.filter(p => p.difficulty === "보통")
          const hard = topologicalSortContestCluster.problems.filter(p => p.difficulty === "어려움")
          const renderItem = (p: any) => {
            const isSolved = solvedSet.has(p.id)
            const globalIdx = topologicalSortContestCluster.problems.findIndex(pp => pp.id === p.id) + 1
            return (
              <Link
                key={p.id}
                href={`/algo/topologicalsort/practice?p=${p.id}`}
                className={cn(
