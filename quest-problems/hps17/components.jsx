...
const FULL_PY = [
  ...
  "",
  "# beats[c] = opponent gesture that c defeats",
  "beats = {'H': 'S', 'P': 'H', 'S': 'P'}",
  "idx = {'H': 0, 'P': 1, 'S': 2}",
  "",
...
const FULL_CPP = [
  ...
  "using namespace std;",
  "",
  "int idx(char c) { return c == 'H' ? 0 : (c == 'P' ? 1 : 2); }",
  "// beats[c] = opponent gesture that c defeats",
  ...
  "    for (int i = 0; i < N; i++) {",
  "        for (char c : {'H', 'P', 'S'}) {",
  "            pre[i + 1][idx(c)] = pre[i][idx(c)] + (plays[i] == beats(c) ? 1 : 0);",
  "        }",
  "    }",
  "",
  "    int best = 0;",
  ...
  "        for (char c2 : {'H', 'P', 'S'}) {",
  "            for (int k = 0; k <= N; k++) {",
  "                int wins = pre[k][idx(c1)] + (pre[N][idx(c2)] - pre[k][idx(c2)]);",
  "                best = max(best, wins);",
  "            }",
  "        }",
  "    }",
