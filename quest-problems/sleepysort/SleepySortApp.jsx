...
import { SleepySortProgressiveCode, downloadSleepySortPDF, getSleepySortSections } from "./components";
import { makeSleepySortCh1, makeSleepySortCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";

const A = "#2563eb";

export default function SleepySortApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeSleepySortCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeSleepySortCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeSleepySortCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeSleepySortCh1, 1: (e) => makeSleepySortCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <SleepySortProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <SleepySortProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    return null;
  };
  ...
  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      ...
      <button onClick={() => downloadSleepySortPDF(E, getSleepySortSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`slee-${tab}-${cur}-${lang}`} text={step.narr} />}
