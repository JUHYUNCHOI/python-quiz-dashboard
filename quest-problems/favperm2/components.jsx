...
const FULL_CPP = [
  ...
  "        long long N; cin >> N;",
  "        vector<long long> h; { long long _x; while (cin >> _x) h.push_back(_x); if (!cin) cin.clear(); } // adapt: read N values",
  "        // Build permutation using hints in reverse",
  "        auto perm = deque();",
  "        auto used = [False] * (N + 1);",
  "        // Last remaining element: find the one not in hints",
  ...
  "            auto front = perm[0];",
  "            auto back = perm[-1];",
  "            auto can_prepend = True  # val becomes 2nd, need new front;",
  "            auto can_append = True   # val becomes 2nd-to-last, need new back;",
  "            // used[val] = True",
  "            // Find smallest unused for front or back",
  "            // Greedy: try both, pick lex smaller result",
  "            // Simplified: prepend val, then prepend smallest unused",
  ...
  "            cout << -1 << \"\\n\";",
  "        else {",
  "            cout << ' '.join(map(str, perm)) << \"\\n\";",
  "",
  "    return 0;",
  "}",
];

export function getFavPerm2Sections(E) {
  return [
    {
...
export function FavPerm2ProgressiveCode({ E, lang = "py", sections }) {
...
export function downloadFavPerm2PDF(E, sections, lang = "py") {
  const win = window.open("", "_blank");
  if (!win) { alert(t(E, "Pop-up blocked.", "팝업 차단됨.")); return; }
  const esc = (s) => String(s).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
  const langLabel = lang === "py" ? "🐍 Python" : "💻 C++";
  const fileTitle = t(E, "FavPerm2 — Full Study Guide", "FavPerm2 — 종합 풀이 노트");
