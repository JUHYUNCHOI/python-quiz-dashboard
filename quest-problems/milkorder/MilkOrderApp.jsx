...
import { MilkOrderProgressiveCode, downloadMilkOrderPDF, getMilkOrderSections, getMilkOrderWalk } from "./components";
import { makeMilkOrderCh1, makeMilkOrderCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";

const A = "#2563eb";

export default function MilkOrderApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeMilkOrderCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeMilkOrderCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeMilkOrderCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeMilkOrderCh1, 1: (e) => makeMilkOrderCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <MilkOrderProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "milkorder-walk") return <CodeWalk E={E} lang={codeLang} {...getMilkOrderWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <MilkOrderProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "milkorder-walk") return <CodeWalk E={E} lang={codeLang} {...getMilkOrderWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadMilkOrderPDF(E, getMilkOrderSections(E), codeLang)} style={{
