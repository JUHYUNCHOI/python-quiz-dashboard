...
const FULL_CPP = [
  ...
  "",
  "    // Via teleporter: a->x, teleport to y, y->b",
  "    auto via1 = abs(a - x) + abs(y - b);",
  "",
  "    // Via teleporter: a->y, teleport to x, x->b",
  "    auto via2 = abs(a - y) + abs(x - b);",
  "",
  "    cout << min(direct, via1, via2) << \"\\n\";",
  "",
  "    return 0;",
  "}",
];

export function getTeleportSections(E) {
  return [
    {
...
export function TeleportProgressiveCode({ E, lang = "py", sections }) {
...
export function downloadTeleportPDF(E, sections, lang = "py") {
  const win = window.open("", "_blank");
  if (!win) { alert(t(E, "Pop-up blocked.", "팝업 차단됨.")); return; }
  const esc = (s) => String(s).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
  const langLabel = lang === "py" ? "🐍 Python" : "💻 C++";
  const fileTitle = t(E, "Teleport — Full Study Guide", "Teleport — 종합 풀이 노트");
