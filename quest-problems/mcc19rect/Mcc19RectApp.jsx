...
import { Mcc19RectProgressiveCode, downloadMcc19RectPDF, getMcc19RectSections, getMcc19RectWalk } from "./components";
...
export default function Mcc19RectApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeMcc19RectCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeMcc19RectCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeMcc19RectCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeMcc19RectCh1, 1: (e) => makeMcc19RectCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <Mcc19RectProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getMcc19RectWalk(E)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <Mcc19RectProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getMcc19RectWalk(E)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadMcc19RectPDF(E, getMcc19RectSections(E), codeLang)} style={{
