...
/**
 * 정렬문제 풀이 — /algo/binarysearch/practice
 *
...
const SOLVED_KEY = "algo-binarysearch-contest-solved"
...
export default function SortingPracticePage() {
  ...
  const problem = problemId ? binarySearchContestCluster.problems.find(p => p.id === problemId) : null
  ...
  // 단일 문제 풀이 화면
  if (problem) {
    const problemIdx = binarySearchContestCluster.problems.findIndex(p => p.id === problem.id) + 1
    return (
      <div className="min-h-screen bg-gray-50 pb-24">
        ...
        <main className="max-w-5xl mx-auto px-4 pt-4">
          <div className="mb-3 flex items-center justify-between gap-2">
            <button
              onClick={() => router.push("/algo/binarysearch")}
        ...
        <LessonPanel open={showLesson} onClose={() => setShowLesson(false)} title={t("이분탐색 수업", "Binary Search lesson")}>
          <BinarySearchLesson />
        </LessonPanel>
        <BottomNav />
      </div>
  ...
  // 리스트 화면
  const totalSolved = binarySearchContestCluster.problems.filter(p => solvedSet.has(p.id)).length
  const total = binarySearchContestCluster.problems.length
      ...
      <main className="max-w-2xl mx-auto px-4 pt-4">
        <div className="mb-4">
          <JourneyBreadcrumb items={[
            ...
            { label: "이분탐색", labelEn: "Binary Search", href: "/algo/binarysearch", emoji: "📊" },
            { label: "문제 풀이", labelEn: "Practice" },
          ]} />
          <div className="flex items-center gap-2 mb-2">
            <span className="text-3xl">{binarySearchContestCluster.emoji}</span>
            <h1 className="text-xl sm:text-2xl font-black text-gray-900">
              {t(binarySearchContestCluster.title, binarySearchContestCluster.en?.title ?? binarySearchContestCluster.title)}
            </h1>
          </div>
          <p className="text-sm text-gray-600 mb-3">
            {t(binarySearchContestCluster.description, binarySearchContestCluster.en?.description ?? binarySearchContestCluster.description)}
          </p>
        ...
        {(() => {
          const warmup = binarySearchContestCluster.problems.filter(p => p.difficulty === "쉬움")
          const easy = binarySearchContestCluster.problems.filter(p => p.difficulty === "보통")
          const hard = binarySearchContestCluster.problems.filter(p => p.difficulty === "어려움")
          const renderItem = (p: any) => {
            const isSolved = solvedSet.has(p.id)
            const globalIdx = binarySearchContestCluster.problems.findIndex(pp => pp.id === p.id) + 1
            return (
              <Link
                key={p.id}
                href={`/algo/binarysearch/practice?p=${p.id}`}
                className={cn(
