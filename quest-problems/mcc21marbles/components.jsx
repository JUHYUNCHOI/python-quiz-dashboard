...
const FULL_PY = [
  ...
  "target = sum(a) // N",
  "",
  "# 왼쪽부터: 누적 불균형(carry)이 그 경계를 건너는 구슬 수.",
  "# 인접 이동 최소 횟수 = 경계마다 |carry| 의 합.",
  "ops = 0",
  "carry = 0",
  "for i in range(N):",
  "    carry += a[i] - target",
  ...
  return [
    {
      ...
      why: [
        t(E, "One left-to-right pass, O(N): keep a running prefix imbalance (carry); every unit that must cross a boundary adds 1.",
            "왼쪽부터 한 번 훑기, O(N): 누적 불균형(carry)을 들고 가며, 경계를 건너야 하는 단위마다 답에 1 더해요."),
        t(E, "Answer = sum of |carry| at each boundary. Use 64-bit (long long) — totals can be large.",
            "답 = 경계마다 |carry| 의 합. 합이 커질 수 있으니 64비트(long long)."),
      ],
      pyOnly: [
