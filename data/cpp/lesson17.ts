...
- 값을 찾으면 → 그 위치의 **iterator** 반환
- 값이 없으면 → **\`v.end()\`** 반환
- \`v.end()\` 와 비교해서 존재 여부 확인

⚠️ **항상 \`v.end()\` 와 비교 후 사용:**
\`\`\`cpp
auto it = find(v.begin(), v.end(), 42);
if (it != v.end()) { /* 찾음 */ }
\`\`\`

> 다음 페이지 — 파이썬 \`in\` / \`.index()\` 와 어떻게 다른지 비교 + 시간복잡도.`
        },
        {
          id: "ch1-find-py",
          type: "explain",
          title: "🐍 find() vs 파이썬 — 시간복잡도까지",
          content: `### 파이썬과 비교
...
C++ 은 한 번 호출로 두 정보 (있나? 어디?) 를 한꺼번에 — iterator 가 그 둘을 다 담고 있어서.

### ⏱️ 시간복잡도

\`find()\` 는 **앞에서부터 순서대로** 찾아요 → **O(n)**.

데이터가 정렬돼 있으면 다음 챕터에서 배울 \`binary_search\` 가 훨씬 빨라요 (O(log n)). 정렬 안 된 일반 벡터에서는 \`find()\` 가 정답.`
        },
        {
          id: "ch1-pred1",
      ...
      steps: [
        {
          id: "ch2-intro",
          type: "explain",
          title: "🎯 왜 더 빠른 탐색이 필요한가 — binary_search 개념",
          content: `> 📌 **이미 본 적 있죠?** 이진탐색은 **레슨 18 (sort 마스터)** 의 ch2 에서 깊이 다뤘어요 — \`binary_search\` / \`lower_bound\` / \`upper_bound\` 3 형제, 동작 원리, 실수 함정까지. 이번 장은 **STL 탐색 함수 한 묶음** 으로 정리하면서 \`find()\` 와 짝지어 보는 게 목적이에요. 깊이는 sort 레슨이 더 풍부.

상상해봐요 — 학생 100 만 명의 학번 명단에서 특정 학번 한 명을 찾아야 해요. \`find()\` 로 하면 어떻게 될까요?

\`find()\` 는 **앞에서부터 한 명씩** 확인해요. 운이 나쁘면 100 만 번 비교. 컴퓨터한테도 부담스러운 양.

### 사전 찾기 직관

근데 학번이 **이미 정렬되어 있다면?** 사람도 그렇잖아요 — 사전에서 단어 찾을 때 "ㄱ" 부터 한 페이지씩 안 넘기죠. 가운데 펴서 "내가 찾는 단어가 앞에 있나 뒤에 있나" 보고, 그 절반만 다시 가운데로… 이런 식으로 **빨리 좁혀요**.
...
| 방법 | 동작 | 1 백만 개에서 |
|---|---|---|
| \`find()\` | 앞에서 한 칸씩 | 최대 100 만 번 비교 |
| \`binary_search()\` | 가운데 잘라서 반씩 | 최대 **~20 번** 비교 |

차이 어마어마하죠? 단 **하나의 전제 조건** — 데이터가 **정렬되어 있어야** 함. 정렬 안 된 데이터에선 "가운데 보면 앞쪽인지 뒤쪽인지" 자체를 알 수 없으니까요.

> 💡 한 번 검색하고 끝낼 거면 정렬 비용이 더 클 수도. **여러 번 검색** 할 때 이득이 커져요.
...
다음 페이지에서 실제 사용법을 봐요 👇`
        },
        {
          id: "ch2-intro-usage",
          type: "explain",
          title: "🔧 binary_search 사용법 + 정렬 필수 규칙",
          content: `\`\`\`cpp
#include <algorithm>
#include <vector>
using namespace std;
...
import bisect
lst = [1, 3, 5, 7, 9, 11]
# 파이썬에는 binary_search 가 없어서 bisect 로 구현
...
> 💡 데이터가 정렬되어 있으면 항상 \`binary_search()\` 가 효율적.`
        },
        {
          id: "ch2-fb1",
          type: "fillblank" as const,
