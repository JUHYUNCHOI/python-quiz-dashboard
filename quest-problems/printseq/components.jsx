...
const PSQ_STK_PY = [
  ...
  "        for length in range(block_len * 2, N + 1, block_len):",
  "            divisors[length].append(block_len)",
  "",
  "    # memo = 답 공책 = 메모이제이션(memoization)",
  "    #   memo[(start, end, budget)] = 그 조각, 만들 수 있나?",
  "    memo = {}",
  "    todo = [(0, N, K)]     # 할 일 목록 — 전체 수열부터 (재귀 대신 이걸 굴려요)",
  "",
  "    # 조각마다 '어디까지 해봤는지' 기억해두는 곳",
  "    block_lens = {}        # 요령②: 반복 가능한 블록 길이 후보들",
  "    block_tried = {}       # 요령②: 몇 번째 후보까지 봤나",
  "    cut_at = {}            # 요령③: 어디서 자를 차례인가",
  "    give_left = {}         # 요령③: 왼쪽에 예산 몇 개 줄 차례인가",
  "",
  "    while todo:",
  "        piece = todo[-1]",
  "        start, end, budget = piece",
  "",
  "        if piece in memo:              # 이미 푼 조각이면 넘어가기",
  "            todo.pop()",
  "            continue",
  "        if start >= end or budget <= 0:  # 빈 조각 / 예산 없음 → 못 만듦",
  "            memo[piece] = False",
  "            todo.pop()",
  "            continue",
  "",
  "        if piece not in block_tried:     # 이 조각을 처음 꺼냈을 때만",
  "            all_same = True              # 요령 ①: 숫자가 다 같으면 PRINT 하나면 끝",
  ...
  "                todo.pop()",
  "                continue",
  "            found = []                   # 요령 ②: 반복되는 블록 길이 모으기",
  "            for block_len in divisors[end - start]:",
  "                repeats = True",
  "                for pos in range(start + block_len, end):",
  "                    if target[pos] != target[pos - block_len]:",
  "                        repeats = False",
  "                        break",
  "                if repeats:",
  "                    found.append(block_len)",
  "            block_lens[piece] = found",
  "            block_tried[piece] = 0",
  "            cut_at[piece] = start + 1",
  "            give_left[piece] = 1",
  "",
  "        unknown = None                   # 아직 답을 모르는 '더 작은 조각'",
  "        can_make = False                 # 이 조각을 만들 수 있나",
  "",
  "        while block_tried[piece] < len(block_lens[piece]):     # 요령 ②",
  "            block_len = block_lens[piece][block_tried[piece]]",
  "            block = (start, start + block_len, budget)",
  ...
  "            block_tried[piece] += 1",
  "",
  "        if unknown is None and not can_make:                   # 요령 ③",
  "            while cut_at[piece] < end:",
  "                if give_left[piece] >= budget:",
  "                    cut_at[piece] += 1",
  "                    give_left[piece] = 1",
  "                    continue",
  "                left = (start, cut_at[piece], give_left[piece])",
  "                right = (cut_at[piece], end, budget - give_left[piece])",
  "                if left not in memo:",
  "                    unknown = left",
  "                    break",
  "                if memo[left]:",
  ...
  "                give_left[piece] += 1",
  "",
  "        if unknown is not None:          # 모르는 조각이 있으면 그것부터 할 일 목록에",
  "            todo.append(unknown)",
  "            continue",
  "",
  "        memo[piece] = can_make         # 세 요령 다 해봤으면 답 확정",
  "        del block_lens[piece], block_tried[piece], cut_at[piece], give_left[piece]",
  "        todo.pop()",
  "",
  "    if memo[(0, N, K)]:",
  "        print('YES')",
...
const PSQ_STK_PY_EN = [
  ...
  "    target = list(map(int, input().split()))",
  "",
  "    # divisors by length — trick 2 needs a block length that divides the piece",
  "    divisors = [[] for _ in range(N + 1)]",
  "    for block_len in range(1, N + 1):",
  "        for length in range(block_len * 2, N + 1, block_len):",
  "            divisors[length].append(block_len)",
  ...
  "",
  "    # where we remember how far we got on each piece",
  "    block_lens = {}        # trick 2: repeatable block lengths",
  "    block_tried = {}       # trick 2: how many candidates we checked",
  "    cut_at = {}            # trick 3: where to cut next",
  "    give_left = {}         # trick 3: how much budget to give the left side",
  "",
  "    while todo:",
  "        piece = todo[-1]",
  "        start, end, budget = piece",
  "",
  "        if piece in memo:              # already solved — skip",
  "            todo.pop()",
  "            continue",
  "        if start >= end or budget <= 0:  # empty piece / no budget -> impossible",
  "            memo[piece] = False",
  "            todo.pop()",
  "            continue",
  "",
  "        if piece not in block_tried:     # only the first time we pop this piece",
  "            all_same = True              # trick 1: all same -> one PRINT is enough",
  "            for pos in range(start + 1, end):",
  "                if target[pos] != target[start]:",
  "                    all_same = False",
  "                    break",
  ...
  "                todo.pop()",
  "                continue",
  "            found = []                   # trick 2: collect repeating block lengths",
  "            for block_len in divisors[end - start]:",
  "                repeats = True",
  "                for pos in range(start + block_len, end):",
  "                    if target[pos] != target[pos - block_len]:",
  ...
  "            give_left[piece] = 1",
  "",
  "        unknown = None                   # a smaller piece we don't know yet",
  "        can_make = False                 # can this piece be made?",
  "",
  "        while block_tried[piece] < len(block_lens[piece]):     # trick 2",
  "            block_len = block_lens[piece][block_tried[piece]]",
  "            block = (start, start + block_len, budget)",
  "            if block not in memo:",
  "                unknown = block",
  ...
  "            block_tried[piece] += 1",
  "",
  "        if unknown is None and not can_make:                   # trick 3",
  "            while cut_at[piece] < end:",
  "                if give_left[piece] >= budget:",
  "                    cut_at[piece] += 1",
  "                    give_left[piece] = 1",
  ...
  "                give_left[piece] += 1",
  "",
  "        if unknown is not None:          # unknown piece -> put it on the TO-DO list first",
  "            todo.append(unknown)",
  "            continue",
  "",
  "        memo[piece] = can_make         # all three tricks tried -> answer settled",
  "        del block_lens[piece], block_tried[piece], cut_at[piece], give_left[piece]",
  "        todo.pop()",
  "",
  "    if memo[(0, N, K)]:",
...
const PSQ_STK_CPP = [
  ...
  "",
  "// memo = 답 공책 = 메모이제이션(memoization)",
  "int  memo[101][101][4];      // -1 = 아직 모름, 0 = 못 만듦, 1 = 만들 수 있음",
  "bool opened  [101][101][4];      // 이 조각을 이미 꺼내 봤나",
  "int  blockTried[101][101][4];    // 요령②: 몇 번째 블록 후보까지 봤나",
  "int  cutAt     [101][101][4];    // 요령③: 어디서 자를 차례인가",
  "int  giveLeft  [101][101][4];    // 요령③: 왼쪽에 예산 몇 개 줄 차례인가",
  "vector<int> blockLens[101][101]; // 요령②: 반복 가능한 블록 길이 후보들",
  "bool blockLensReady[101][101];",
  "",
  "// 조각 target[start:end) 의 숫자가 다 같은가?",
  "bool all_same(int start, int end) {",
  ...
  "bool repeats_every(int start, int end, int blockLen) {",
  "    for (int pos = start + blockLen; pos < end; pos++)",
  "        if (target[pos] != target[pos - blockLen]) {",
  "            return false;",
  "        }",
  "    return true;",
  "}",
  ...
  "            }",
  "",
  "        vector<vector<int>> todo;                 // 할 일 목록 (재귀 대신)",
  "        todo.push_back({0, N, K});",
  "        while (!todo.empty()) {",
  "            int start = todo.back()[0];",
  "            int end = todo.back()[1];",
  "            int budget = todo.back()[2];",
  "            if (memo[start][end][budget] != -1) {",
  "                todo.pop_back();",
  "                continue;",
  "            }",
  "            if (start >= end || budget <= 0) {",
  "                memo[start][end][budget] = 0;",
  "                todo.pop_back();",
  "                continue;",
  "            }",
  "",
  "            if (!opened[start][end][budget]) {    // 이 조각을 처음 꺼냈을 때만",
  "                opened[start][end][budget] = true;",
  "                if (all_same(start, end)) {",
  "                    memo[start][end][budget] = 1;",
  "                    todo.pop_back();",
  "                    continue;",
  "                }",
  "                if (!blockLensReady[start][end]) {",
  "                    blockLensReady[start][end] = true;",
  "                    int length = end - start;",
  "                    for (int blockLen = 1; blockLen < length; blockLen++)",
  "                        if (length % blockLen == 0 && repeats_every(start, end, blockLen))",
  "                            blockLens[start][end].push_back(blockLen);",
  "                }",
  "                blockTried[start][end][budget] = 0;",
  "                cutAt[start][end][budget] = start + 1;",
  "                giveLeft[start][end][budget] = 1;",
  "            }",
  "",
  "            bool unknown = false;",
  "            bool canMake = false;",
  "            while (blockTried[start][end][budget] < (int)blockLens[start][end].size()) {   // 요령 ②",
  "                int blockLen = blockLens[start][end][blockTried[start][end][budget]];",
  "                int mid = start + blockLen;",
  "                if (memo[start][mid][budget] == -1) {",
  "                    todo.push_back({start, mid, budget});",
  "                    unknown = true;",
  "                    break;",
  "                }",
  "                if (memo[start][mid][budget] == 1) {",
  ...
  "                    break;",
  "                }",
  "                blockTried[start][end][budget]++;",
  "            }",
  "            if (!unknown && !canMake) {",
  "                while (cutAt[start][end][budget] < end) {                                   // 요령 ③",
  "                    int cut = cutAt[start][end][budget];",
  "                    int toLeft = giveLeft[start][end][budget];",
  "                    if (toLeft >= budget) {",
  "                        cutAt[start][end][budget]++;",
  "                        giveLeft[start][end][budget] = 1;",
  "                        continue;",
  "                    }",
  "                    if (memo[start][cut][toLeft] == -1) {",
  "                        todo.push_back({start, cut, toLeft});",
  "                        unknown = true;",
  "                        break;",
  "                    }",
  "                    if (memo[start][cut][toLeft] == 1) {",
  "                        if (memo[cut][end][budget - toLeft] == -1) {",
  "                            todo.push_back({cut, end, budget - toLeft});",
  "                            unknown = true;",
  "                            break;",
  "                        }",
  "                        if (memo[cut][end][budget - toLeft] == 1) {",
  ...
  "                        }",
  "                    }",
  "                    giveLeft[start][end][budget]++;",
  "                }",
  "            }",
  "            if (unknown) {",
  "                continue;               // 모르는 조각부터 먼저 (할 일 목록에 올라감)",
  "            }",
  "            if (canMake) {",
  "                memo[start][end][budget] = 1;",
  "            } else {",
...
const PSQ_STK_CPP_EN = [
  ...
  "",
  "// memo = the answer notebook = memoization",
  "int  memo[101][101][4];      // -1 = unknown, 0 = cannot, 1 = can",
  "bool opened  [101][101][4];      // have we opened this piece before?",
  "int  blockTried[101][101][4];    // trick 2: how many candidates checked",
  "int  cutAt     [101][101][4];    // trick 3: where to cut next",
  "int  giveLeft  [101][101][4];    // trick 3: budget for the left side",
  "vector<int> blockLens[101][101]; // trick 2: repeatable block lengths",
  "bool blockLensReady[101][101];",
  "",
  "// are all numbers in target[start:end) the same?",
  "bool all_same(int start, int end) {",
  "    for (int pos = start + 1; pos < end; pos++)",
  "        if (target[pos] != target[start]) {",
  "            return false;",
  ...
  "            }",
  "",
  "        vector<vector<int>> todo;                 // TO-DO list (instead of recursion)",
  "        todo.push_back({0, N, K});",
  "        while (!todo.empty()) {",
  "            int start = todo.back()[0];",
  "            int end = todo.back()[1];",
  ...
  "            }",
  "",
  "            if (!opened[start][end][budget]) {    // only the first time we pop it",
  "                opened[start][end][budget] = true;",
  "                if (all_same(start, end)) {",
  "                    memo[start][end][budget] = 1;",
  "                    todo.pop_back();",
  ...
  "            bool unknown = false;",
  "            bool canMake = false;",
  "            while (blockTried[start][end][budget] < (int)blockLens[start][end].size()) {   // trick 2",
  "                int blockLen = blockLens[start][end][blockTried[start][end][budget]];",
  "                int mid = start + blockLen;",
  "                if (memo[start][mid][budget] == -1) {",
  "                    todo.push_back({start, mid, budget});",
  ...
  "            }",
  "            if (!unknown && !canMake) {",
  "                while (cutAt[start][end][budget] < end) {                                   // trick 3",
  "                    int cut = cutAt[start][end][budget];",
  "                    int toLeft = giveLeft[start][end][budget];",
  "                    if (toLeft >= budget) {",
  "                        cutAt[start][end][budget]++;",
  ...
  "            }",
  "            if (unknown) {",
  "                continue;               // solve the unknown piece first (already on the list)",
  "            }",
  "            if (canMake) {",
  "                memo[start][end][budget] = 1;",
  "            } else {",
    ...
    return {
      ...
      beats: [
        ...
        { hi: [115, 119], bubble: t(E,
          ...
          "can(목표, K) 를 불러서 YES / NO 를 출력해요. 이게 우리 목표예요.\n이제 일하는 '두 명'을 만나요 👇") },
        { hi: [56, 63], bubble: t(E,
        ...
        { hi: [79, 88], bubble: t(E,
          ...
          "요령 ② — 블록 길이 1, 2, … 를 하나씩 해봐요. 반복 모양이면(is_repeat_of)\n↺ can(블록)! 같은 질문을 더 작게 묻는 거예요 = 재귀.\n예를 들어 [1 2 1 2] = 블록 [1 2] 의 반복 → can([1 2]) 만 풀면 끝 ✓") },
        { hi: [90, 99], bubble: t(E,
          ...
          "요령 ③ — 둘로 자르고 예산도 나눠요. ↺ can(왼쪽) 그리고 ↺ can(오른쪽)!\n둘 다 되면 return true. 자르는 자리와 나누는 방법을 전부 해봐요.\n예를 들어 [1 1 2 2] 예산 2 → [1 1] 예산 1 + [2 2] 예산 1 ✓") },
  ...
  return {
    ...
    beats: [
      ...
      { hi: [41, 46], bubble: t(E,
        ...
        "요령 ② — 블록 길이 1, 2, … 를 하나씩 해봐요. 반복 모양이면(is_repeat_of)\n↺ can(seq[:block_len])! 같은 질문을 더 작게 묻는 거예요 = 재귀.\n예를 들어 [1 2 1 2] = 블록 [1 2] 의 반복 → can([1 2]) 만 풀면 끝 ✓") },
      { hi: [48, 53], bubble: t(E,
        "Trick ③: cut in two, split the budget — ↺ can(left) AND ↺ can(right)!\nBoth yes → return True. Try every cut × every split.\ne.g. [1 1 2 2] budget 2 → [1 1]+budget 1, [2 2]+budget 1 ✓",
        "요령 ③ — 둘로 자르고 예산도 나눠요. ↺ can(왼쪽) 그리고 ↺ can(오른쪽)!\n둘 다 되면 return True. 자르는 자리와 나누는 방법을 전부 해봐요.\n예를 들어 [1 1 2 2] 예산 2 → [1 1] 예산 1 + [2 2] 예산 1 ✓") },
      { hi: [55, 55], bubble: t(E,
...
export function getPrintseqStkWalk(E, lang = "py") {
  ...
  if (lang === "cpp") {
    return {
      ...
      vars,
      marks: [
        { from: 52, to: 53, color: "#0d9488", ko: "📋 재귀 대신 이것!", en: "📋 instead of recursion!" },
      ],
      beats: [
        { hi: [33, 41], bubble: t(E,
          "Start where we always start — the INPUT.\nmain reads T, then each case: N, K, and the target sequence.",
        ...
        { hi: [87, 102], bubble: t(E,
          ...
          "이제 요령 ② 를 해봐요. 그 블록은 만들 수 있나? 공책을 봐요.\n공책에 아직 없으면 → 목록에 올려두고 나중에 다시 와요.") },
        { hi: [103, 130], bubble: t(E,
          "Trick ③: cut in two and split the budget.\nLeft makable AND right makable? Same rule — unknown piece goes on the list.\ne.g. [1 1 2 2] → [1 1] + [2 2] ✓",
          "요령 ③ 은 둘로 자르고 예산도 나누는 거예요.\n왼쪽도 되고 오른쪽도 되나? 규칙은 같아요 — 모르는 조각은 목록에 올려요.\n예를 들어 [1 1 2 2] → [1 1] + [2 2] ✓") },
        { hi: [131, 133], bubble: t(E,
  ...
  return {
    ...
    vars,
    marks: [
      { from: 93, to: 96, color: "#0d9488", ko: "📋 재귀 대신 이것!", en: "📋 instead of recursion!" },
    ],
    beats: [
      { hi: [0, 5], bubble: t(E,
      ...
      { hi: [62, 71], bubble: t(E,
        ...
        "이제 요령 ② 를 해봐요. 그 블록은 만들 수 있나? 공책을 봐요.\n아직 없으면 → 그 블록이 `need` 가 돼요 (그것부터 처리해요).") },
      { hi: [73, 92], bubble: t(E,
        "Trick ③: cut in two and split the budget.\nLeft makable AND right makable? Unknown pieces become `need` too.\ne.g. [1 1 2 2] → [1 1] + [2 2] ✓",
        "요령 ③ 은 둘로 자르고 예산도 나누는 거예요.\n왼쪽도 되고 오른쪽도 되나? 모르는 조각은 여기서도 `need` 가 돼요.\n예를 들어 [1 1 2 2] → [1 1] + [2 2] ✓") },
      { hi: [93, 96], bubble: t(E,
  ...
  return [
    {
      ...
      why: [
        t(E, "T test cases; each gives N, K (the PRINT budget) and the target sequence.",
            "케이스가 T 개예요. 케이스마다 N, K(PRINT 예산), 목표 수열이 주어져요."),
      ],
    },
    {
    ...
    },
    {
      ...
      cpp: [
        ...
        "                    int len = j - i;",
        "                    for (int b = 1; b < len; b++) {",
        "                        if (len % b == 0 && is_repeat_of(i, j, b)) {",
        "                            reps[i][j].push_back(b);",
        "                        }",
        "                    }",
        "                }",
        "                pRep[i][j][k] = 0;",
        ...
        "                    }",
        "                    if (memo[i][cut][k1] == -1) {",
        "                        stk.push_back({i, cut, k1});",
        "                        need = true;",
        "                        break;",
        "                    }",
        "                    if (memo[i][cut][k1] == 1) {",
        "                        if (memo[cut][j][k - k1] == -1) {",
        "                            stk.push_back({cut, j, k - k1});",
        "                            need = true;",
        "                            break;",
        "                        }",
        "                        if (memo[cut][j][k - k1] == 1) {",
      ...
      why: [
        ...
        t(E, "Key: if a smaller piece's answer isn't known yet, we push it on the TO-DO list and come back — prog remembers where we stopped. That's exactly what recursion used to do.",
            "여기가 제일 중요해요. 더 작은 조각의 답을 모르면 할 일 목록에 올리고 나중에 다시 와요 — prog 가 멈춘 자리를 기억하고요. 재귀가 대신 해주던 일이 이거예요."),
      ],
    },
    {
      ...
      why: [
        t(E, "All three tricks tried → write the answer in the notebook and drop the piece. The notebook entry for the whole sequence is the answer.",
            "세 요령 다 해봤으면 → 답을 공책에 적고 조각을 치워요. 전체 수열의 공책 값이 곧 답이에요."),
      ...
      cppOnly: [
        t(E, "memo/started/pRep/pCut/pK1 are plain 3-D arrays; the TO-DO list is a vector we push and pop.",
            "memo/started/pRep/pCut/pK1 은 3차원 배열이에요. 할 일 목록은 push/pop 을 쓰는 vector 고요."),
      ],
    },
  ];
}
