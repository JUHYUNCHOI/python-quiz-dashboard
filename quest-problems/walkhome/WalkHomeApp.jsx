...
import { WalkHomeProgressiveCode, downloadWalkHomePDF, getWalkHomeSections, getWalkHomeWalk } from "./components";
import { makeWalkHomeCh1, makeWalkHomeCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#8b5cf6";

export default function WalkHomeApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeWalkHomeCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeWalkHomeCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeWalkHomeCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeWalkHomeCh1, 1: (e) => makeWalkHomeCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <WalkHomeProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getWalkHomeWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <WalkHomeProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "opt-codewalk") return <CodeWalk E={E} lang={codeLang} {...getWalkHomeWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadWalkHomePDF(E, getWalkHomeSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`walk-${tab}-${cur}-${lang}`} text={step.narr} />}
