...
export function makeAcow1Ch1(E) {
  return [
    // 1-1: Title reveal
    {
      type: "reveal",
      narr: t(E,
        "Bessie wants to maximize her h-index! She has N papers and can add L extra citations. The h-index is the largest h such that at least h papers have at least h citations.",
        "Bessie가 h-index를 최대화하고 싶어! N개의 논문이 있고 L개의 추가 인용을 넣을 수 있어. h-index는 최소 h개의 논문이 각각 h 이상의 인용을 가지는 최대 h야."),
      content: (
    ...
    // 1-2: Quiz
    {
      type: "quiz",
      narr: t(E,
        "Consider papers with citations [1, 3, 3, 100]. Without any extra citations, what is the h-index?",
        "인용수가 [1, 3, 3, 100]인 논문들이 있어. 추가 인용 없이 h-index는 얼마일까?"),
      question: t(E,
    ...
    // 1-3: Input
    {
      type: "input",
      narr: t(E,
        "Papers [1, 3, 3, 100] with no extra citations. What is the h-index?",
        "논문 [1, 3, 3, 100]에 추가 인용이 없어. h-index는?"),
      question: t(E,
...
export function makeAcow1Ch2(E, lang = "py") {
  return [
    // 2-1: Complexity reveal
    {
      type: "reveal",
      narr: t(E,
        "Sort the papers, then binary search on h. For each candidate h, greedily check if L citations suffice. O(N log N) time!",
        "논문을 정렬하고 h를 이분 탐색. 각 후보 h에 대해 L개 인용이 충분한지 그리디로 확인. O(N log N) 시간!"),
      content: (
        <div style={{ padding: 16, textAlign: "center" }}>
