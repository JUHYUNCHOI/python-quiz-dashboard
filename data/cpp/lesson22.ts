...
**방법 2: 이니셜라이저 리스트 (새로 배우는 방식)**
\`\`\`cpp
BankAccount(string name, double initial)
    : owner(name), balance(initial) {}
\`\`\`

콜론(\`:\`) 뒤에 \`멤버변수(값)\` 형태로 쭉 쓰고, 여러 개면 \`,\` 로 구분. 중괄호는 빈 채로 둬요.

---

눈으로 보면 **결과는 똑같아 보여요.** 둘 다 \`owner\`, \`balance\` 에 값이 들어가니까요.

🤔 **그런데 C++에서는 이니셜라이저 리스트를 "권장"해요.** 이유가 뭘까요? 단순한 선호 문제가 아니라 **내부 동작이 다르기 때문**이에요. 다음 페이지에서 살펴봐요.`
        },
        {
          id: "ch3-initlist-lifecycle",
          type: "explain",
          title: "🔬 내부 동작 — 멤버가 언제 생성되나요?",
          content: `멤버 변수가 **언제 값을 갖게 되는지** 를 따라가 보면 두 방식의 차이가 보여요.
...
1. 먼저 \`owner\` 가 **"빈 문자열"** 로 태어나고, \`balance\` 는 **쓰레기 값** 으로 태어나요 (기본 생성)
2. 그 다음 바디 \`{ ... }\` 안에서 \`owner = name;\`, \`balance = initial;\` 로 **덮어써요** (대입)

→ **2번의 일** 이 일어나요: (1) 기본 생성 → (2) 덮어쓰기
...
여기선 \`owner\` 가 **처음부터 \`name\` 값으로** 태어나요. 기본값으로 먼저 만들고 덮어쓰는 과정이 없어요.

→ **1번의 일** 만 일어나요: (1) 원하는 값으로 바로 생성

---

\`int\`, \`double\` 같은 단순 타입은 속도 차이를 못 느낄 정도로 미미해요. **진짜 중요한 차이는** 어떤 멤버들은 **"나중에 덮어쓰기"가 아예 불가능** 하다는 점이에요. 다음 페이지에서 그 주인공들을 만나봐요.`
        },
        {
