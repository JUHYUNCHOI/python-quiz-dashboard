...
import { BitcoinProgressiveCode, downloadBitcoinPDF, getBitcoinSections, getBitcoinWalk1, getBitcoinWalk2 } from "./components";
import { makeBitcoinCh1, makeBitcoinCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";

const A = "#f97316";

export default function BitcoinApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeBitcoinCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeBitcoinCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeBitcoinCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeBitcoinCh1, 1: (e) => makeBitcoinCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <BitcoinProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "bitcoin-walk-1") return <CodeWalk E={E} lang={codeLang} {...getBitcoinWalk1(E, codeLang)} accent={A} />;
    if (step.type === "bitcoin-walk-2") return <CodeWalk E={E} lang={codeLang} {...getBitcoinWalk2(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <BitcoinProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "bitcoin-walk-1") return <CodeWalk E={E} lang={codeLang} {...getBitcoinWalk1(E, codeLang)} accent={A} />;
    if (s.type === "bitcoin-walk-2") return <CodeWalk E={E} lang={codeLang} {...getBitcoinWalk2(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadBitcoinPDF(E, getBitcoinSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`bitc-${tab}-${cur}-${lang}`} text={step.narr} />}
