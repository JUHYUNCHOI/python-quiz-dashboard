...
> **정렬 한 번 해두면 검색이 거의 5 만 배 빨라져요.** 그래서 USACO 문제는 거의 "일단 sort 부터" 하고 시작.`
        },
        {
          ...
          content: `다음 페이지의 \`lower_bound\` 가 숫자가 아니라 **이터레이터 (iterator)** 라는 걸 돌려줘요. 한 번 정리하고 가요.

---

### 이터레이터 = "벡터 안의 한 자리"

sort 할 때마다 쓰던 \`v.begin()\`, \`v.end()\` — 그게 이미 이터레이터였어요.
...
- \`it != v.end()\` — 끝 표시 전까지만 (end 는 값 없으니 안 봄)
- \`++it\` 로 다음 칸, \`*it\` 로 그 자리 값

> 💡 평소엔 \`for (int x : v)\` (range-for) 가 더 짧아서 그걸 써요. 위 패턴은 **이터레이터 동작 이해용**.

---

### 포인터 들어봤다면?

이터레이터는 **포인터의 사촌** 이라고 보면 돼요. \`*\` 로 값 보고 \`++\` 로 다음 가는 문법이 똑같음. 차이는 vector / list / map 어디서나 같은 코드로 돌아간다는 점 (포인터는 vector 까지만). **다음 페이지에서 직접 눈으로 보여줄게요.**`
        },
        {
          id: "s23-ch2-iter-sim",
          type: "animation" as const,
