...
import { Mooin4ProgressiveCode, downloadMooin4PDF, getMooin4Sections } from "./components";
import { makeMooin4Ch1, makeMooin4Ch2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";

const A = "#f97316";

export default function Mooin4App(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeMooin4Ch1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeMooin4Ch2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeMooin4Ch2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeMooin4Ch1, 1: (e) => makeMooin4Ch2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <Mooin4ProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <Mooin4ProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadMooin4PDF(E, getMooin4Sections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`mooin4-${tab}-${cur}-${lang}`} text={step.narr} />}
