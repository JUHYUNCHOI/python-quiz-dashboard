...
import { RaceProgressiveCode, downloadRacePDF, getRaceSections, getRaceWalk } from "./components";
import { makeRaceCh1, makeRaceCh2 } from "./chapters";
import { useCodeLang } from "@/components/quest/use-code-lang";
import { CodeWalk } from "@/components/quest/CodeWalk";

const A = "#059669";

export default function RaceApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeRaceCh1(lang === "en"));
  const [ch2Q, setCh2Q] = useState(() => makeRaceCh2(lang === "en", "py"));

  useEffect(() => {
    setCh2Q(prev => makeRaceCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeRaceCh1, 1: (e) => makeRaceCh2(e, codeLang) };
  ...
  const renderContent = () => {
    ...
    if (step.type === "progressive") return <RaceProgressiveCode E={E} lang={codeLang} sections={step.sections} />;
    if (step.type === "race-codewalk") return <CodeWalk E={E} lang={codeLang} {...getRaceWalk(E, codeLang)} accent={A} />;
    return null;
  };
  ...
  const renderPreviewBody = (s) => {
    ...
    if (s.type === "progressive") return <RaceProgressiveCode E={E} lang={codeLang} sections={s.sections} />;
    if (s.type === "race-codewalk") return <CodeWalk E={E} lang={codeLang} {...getRaceWalk(E, codeLang)} accent={A} />;
    return null;
  };

  const codeControlsSlot = showCodeControls ? (
    <div style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
      <button onClick={() => downloadRacePDF(E, getRaceSections(E), codeLang)} style={{
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(880px, 100%)", margin: "0 auto", padding: "0 clamp(4px, 2vw, 16px)" }}>
        ...
        {step.narr && <Narration key={`race-${tab}-${cur}-${lang}`} text={step.narr} />}
