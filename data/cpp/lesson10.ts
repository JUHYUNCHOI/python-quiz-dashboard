...
💡 auto는 "타입 안 쓰는 것"이 아니라 "컴파일러가 대신 써주는 것"이에요! 파이썬처럼 자유롭지는 않아요.`
        ...
        },
        {
          ...
          content: `range-for 와 auto 같이 쓰면 정말 편해요.
...
### ⚠️ 그런데 auto 가 vector<int> 대신은 안 돼요

range-for 의 변수 자리엔 auto 가 잘 통하지만, **vector 자체 선언은 다름**:
...
이유: \`{1, 2, 3}\` 자체는 "중괄호로 묶은 임시 값 묶음" (initializer_list) 이에요. \`vector<int>\` 라고 명시하면 vector 생성자가 그 값들로 vector 를 만들어주지만, \`auto\` 는 보이는 그대로 추론해서 initializer_list 가 돼요.

**그러면 auto 로 vector 를 만들고 싶으면?**

\`\`\`cpp
auto v = vector<int>{1, 2, 3};   // 명시하면 OK (근데 그냥 vector<int> v = {1,2,3} 가 더 짧음)
\`\`\`

> 💡 **정리**: 짧은 타입 (\`int\`, \`double\`, \`vector<int>\`) 은 직접 쓰는 게 명확. \`vector<vector<int>>\` 처럼 길어지면 auto 가 빛남. 다음 페이지에서 함정 자세히.`
        },
        {
