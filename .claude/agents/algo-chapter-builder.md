...
**실측 — 진짜 재귀는 셋뿐이다**: `alchemy`🔒 · `permutation`🔒 · `printseq`🔒 — **셋 다 🔒 다.**
⚠️ 재귀는 "이름이 두 번 나온다" 가 아니라 **"자기 본문 안에서 자기를 부른다"** 다.
   **두 번 틀리게 셌다** — 1차 51개(`main` 까지 셌다), 2차 4개(`mcc20kitty` 의 `sum(` 은
   **파이썬 내장함수**였다). `def 이름` 을 잡고 **그 본문 안의 같은 이름 호출**을 봐라.

1. **새 코드는 반복문을 먼저 시도해라.** 재귀가 짧아도 반복문이 읽힌다.
   **예외는 하나** — 반복문으로 바꾸면 **학생에게 오히려 더 어려워지는 경우**
   (손으로 스택을 만들고 되돌리기까지 해야 하는 경우)만 재귀를 쓴다.
   PM 판정(2026-09-22): 위 셋은 **재분류하지 않고 그대로 둔다.**
