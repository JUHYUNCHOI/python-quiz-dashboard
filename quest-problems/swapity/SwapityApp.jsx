...
export default function SwapityApp(props = {}) {
  ...
  const [ch1Q, setCh1Q] = useState(() => makeSwapityCh1(false));
  const [ch2Q, setCh2Q] = useState(() => makeSwapityCh2(false, "py"));

  useEffect(() => {
    setCh2Q(prev => makeSwapityCh2(E, codeLang).map((s, i) => ({ ...s, answered: prev[i]?.answered, solved: prev[i]?.solved })));
  }, [codeLang, E]);
  ...
  const makers  = { 0: makeSwapityCh1, 1: (e) => makeSwapityCh2(e, codeLang) };
  ...
  return (
    <div>
      <div style={{ maxWidth: "min(1100px, 100%)", margin: "0 auto", padding: "0 12px" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 12, alignItems: "center" }}>
          ...
          {showCodeControls && (
            <div style={{ display: "flex", gap: 0, alignItems: "stretch", flexShrink: 0 }}>
              ...
              <button onClick={() => downloadSwapityPDF(E, getSwapitySections(E), codeLang)} style={{
        ...
        {step.narr && <Narration key={`swa-${tab}-${cur}-${lang}`} text={step.narr} />}
