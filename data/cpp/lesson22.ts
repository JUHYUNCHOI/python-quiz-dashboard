...
🤔 **Python 에 없는 낯선 문법인데, 왜 실무는 방법 2 를 쓸까?** 이유가 있어요. 다음 페이지 시뮬레이터에서 직접 봐요.`
        },
        {
          ...
          content: `시뮬레이터 상단에서 **두 가지 상황** 을 비교해봐요.

### 상황 1: 일반 멤버 (int, string 등)
**둘 다 작동.** 학생이 체감하는 차이 거의 없음. 편한 거 써도 OK.

### 상황 2: const 멤버
**Python 스타일은 컴파일 에러.** 리스트만 작동.

\`const int id\` 는 "한 번 정해지면 못 바꾸는 값". Python 스타일은 바디에서 \`id = i\` 로 **재대입** 하는데, const 는 재대입 금지 → 에러.

리스트는 "태어날 때 값 정함" 이라 재대입이 아님 → OK.

---

### 💡 이게 왜 중요해?

클래스에 const (나 reference) 멤버 하나라도 생기면 **Python 스타일 선택지가 사라져요**.

학생은 "아 지금은 const 없으니 Python 스타일 써도 되겠지" 생각할 수 있지만, 실무에선 const 를 **나중에 추가** 하는 경우 많음. 그때마다 모든 생성자를 리스트로 고쳐야 함.

👉 그래서 실무는 **처음부터 리스트** 씀. const 추가돼도 안 건드려도 되고, 새 팀원이 "어느 스타일 써야지?" 고민 안 해도 되니까.

→ 다음 페이지에서 \`const\`, \`reference\` 가 왜 안 되는지 원리를 더 자세히 봐요.`
        },
        {
