...
const FULL_PY = [
  ...
  "",
  "# Can `word` be spelled? Try to give each letter a distinct block.",
  "def can_spell(word):",
  "    used = [False] * 4",
  "    def solve(pos):",
  "        if pos == len(word):",
  "            return True            # every letter placed",
  "        for b in range(4):",
  "            # block b unused AND it has the needed letter on a face",
  "            if not used[b] and word[pos] in blocks[b]:",
  "                used[b] = True",
  "                if solve(pos + 1):",
  "                    used[b] = False",
  "                    return True",
  "                used[b] = False    # undo, try a different block",
  "        return False",
  "    return solve(0)",
  "",
  "for _ in range(N):",
...
const FULL_CPP = [
  ...
  "bool used[4];",
  "",
  "// Can `w` be spelled? Give each letter a distinct block.",
  "bool solve(const string &w, int pos) {",
  "    if (pos == (int)w.size()) return true;   // every letter placed",
  "    for (int b = 0; b < 4; b++) {",
  "        // block b unused AND it has the needed letter on a face",
  "        if (!used[b] && blocks[b].find(w[pos]) != string::npos) {",
  "            used[b] = true;",
  "            if (solve(w, pos + 1)) { used[b] = false; return true; }",
  "            used[b] = false;                 // undo, try another block",
  "        }",
  "    }",
  "    return false;",
  "}",
  ...
  return [
    {
      ...
      cppOnly: [
        ...
        t(E, "used[] + backtracking: mark a block, recurse, then unmark to try another.",
            "used[] + 백트래킹: 블록 표시 → 재귀 → 실패하면 표시 해제하고 다른 블록 시도."),
      ],
    },
  ];
}
