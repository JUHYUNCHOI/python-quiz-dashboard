...
const FULL_CPP = [
  ...
  "        cnt[{x - 1, y - 1, z - 1}] += 1;",
  "    }",
  "    vector<tuple<int,int,int,long long>> triples;",
  "    for (auto& [k, v] : cnt) {",
  "        auto [x, y, z] = k;",
  "        triples.push_back({x, y, z, v});",
  "    }",
  "",
  "    long long best = -1, ways = 0;",
  "    for (int b = 0; b < (1 << N); b++) {",
  "        long long score = 0;",
  "        for (auto& [x, y, z, c] : triples) {",
  "            if (((b >> x) & 1) && !((b >> y) & 1) && !((b >> z) & 1))",
  "                score += c;",
  "        }",
  "        if (score > best) { best = score; ways = 1; }",
  "        else if (score == best) ways++;",
  "    }",
  "    cout << best << \" \" << ways << \"\\n\";",
  "    return 0;",
  "}",
  ...
  return [
    {
      ...
      cppOnly: [
        t(E, "std::map<tuple<int,int,int>, long long> auto-default-initializes to 0.",
            "std::map<tuple<int,int,int>, long long> 은 기본 0 으로 초기화."),
      ],
    },
    {
      ...
      cppOnly: [
        t(E, "Structured bindings (auto& [x,y,z,c]) make the inner loop readable.",
            "구조 분해 바인딩으로 내부 루프가 깔끔."),
      ],
    },
    {
