...
const FULL_PY = [
  ...
  "    # Adversary picks b in [0, x] to minimize",
  "    # A + (x - b) + ((B + b) // cB) * cA.",
  "    # Try a few candidate b values that hit the worst remainders.",
  "    if x == 0:",
  "        return A + (B // cB) * cA",
  "    cands = {0, x}",
  "    r1 = (cB - 1 - (B % cB)) % cB        # max-waste remainder",
  "    if r1 <= x:",
  "        cands.add(r1)",
  "        cands.add(r1 + ((x - r1) // cB) * cB)",
  "    r0 = (-B) % cB                       # zero-waste remainder",
  "    if r0 <= x:",
  "        cands.add(r0)",
...
const FULL_CPP = [
  ...
  "    while (lo < hi) {",
  "        ll mid = lo + (hi - lo) / 2;",
  "        if (minFinalA(A, B, cA, cB, mid) >= fA) hi = mid;",
  "        else lo = mid + 1;",
  "    }",
  "    return lo;",
  "}",
