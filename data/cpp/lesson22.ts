      ...
      steps: [
        {
          ...
          type: "explain",
          title: "🚗 class란?",
          content: `컴퓨터 세상에는 자동차 같은 건 없어요. **직접 만들어야 해요.**

\`int\`로 정수, \`string\`으로 문자열을 만들 수 있어요.
그럼 **자동차**는? → **class**로 정의해요!

---

## 🎮 게임 아이템으로 이해하기

RPG 게임에서 "불꽃 검" 아이템을 본 적 있나요?

게임 상점에서 아이템 설명을 보면 이런 게 적혀 있어요:
...
이 설명만 보는 건 **내 아이템이 아니에요.** 설명을 읽었다고 해서 실제로 검이 생기지는 않아요.

**실제로 구매하거나 획득해야** 내 인벤토리에 생기는 거잖아요!

→ 이게 바로 **class와 객체(object)의 관계**예요.
...
**class를 만든다** = 설명서를 쓰는 것
**객체를 만든다** = 실제로 그 아이템을 손에 넣는 것`,
        },
        {
          id: "ch1-build",
          type: "explain",
          title: "🚗 자동차 class 만들어보기",
          content: `class 안에는 딱 두 가지가 들어가요:

| | 설명 | C++ |
|---|---|---|
| **기억해야 할 것** | 색깔, 속도 등 | 멤버변수 |
| **해야 할 것** | 앞으로 가기, 뒤로 가기 등 | 멤버함수 |

> 💡 친구한테 "김밥 만들려면 뭐가 필요해?" 물어봤더니 밥, 단무지, 소고기, 계란은 다 얘기하면서 **김**을 빠뜨렸어요.
> class를 설계할 때도 이렇게 **필요한 게 빠지지 않도록** 꼼꼼히 생각해야 해요.

\`\`\`cpp
class Car {
public:
    string color;   // 기억해야 할 것 (멤버변수)
    double speed;

    void forward()  { speed += 10; }  // 해야 할 것 (멤버함수)
...
> 틀 하나로 붕어빵을 여러 개 찍을 수 있듯, class 하나로 객체를 여러 개 만들 수 있어요.
