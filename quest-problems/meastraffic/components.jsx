// 🔒 USACO_VERIFIED — cpid=917, meastraffic (2019 Feb Bronze #3)
...
const FULL_PY = [
  ...
  "    los.append(int(parts[1]))",
  "    his.append(int(parts[2]))",
  "",
  ...
  "    file.write(str(b_lo) + ' ' + str(b_hi) + '\\n')",
  "    file.write(str(f_lo) + ' ' + str(f_hi) + '\\n')",
];
...
const FULL_CPP = [
  ...
  "    for (int i = 0; i < N; i++) {",
  "        if (types[i] == \"none\") {",
  "            if (los[i] > f_lo) f_lo = los[i];",
  "            if (his[i] < f_hi) f_hi = his[i];",
  "        } else if (types[i] == \"on\") {",
  "            f_lo += los[i];",
  "            f_hi += his[i];",
  "        } else {  // off",
  ...
  "    for (int i = N - 1; i >= 0; i--) {",
  "        if (types[i] == \"none\") {",
  "            if (los[i] > b_lo) b_lo = los[i];",
  "            if (his[i] < b_hi) b_hi = his[i];",
  "        } else if (types[i] == \"on\") {",
  "            b_lo -= his[i];",
  "            b_hi -= los[i];",
  "            if (b_lo < 0) b_lo = 0;",
