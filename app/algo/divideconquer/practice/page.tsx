...
/**
 * 정렬문제 풀이 — /algo/divideconquer/practice
 *
...
const SOLVED_KEY = "algo-divideconquer-contest-solved"
...
export default function SortingPracticePage() {
  ...
  const problem = problemId ? divideConquerContestCluster.problems.find(p => p.id === problemId) : null
  ...
  // 단일 문제 풀이 화면
  if (problem) {
    const problemIdx = divideConquerContestCluster.problems.findIndex(p => p.id === problem.id) + 1
    return (
      <div className="min-h-screen bg-gray-50 pb-24">
        ...
        <main className="max-w-5xl mx-auto px-4 pt-4">
          <div className="mb-3 flex items-center justify-between gap-2">
            <button
              onClick={() => router.push("/algo/divideconquer")}
        ...
        <LessonPanel open={showLesson} onClose={() => setShowLesson(false)} title={t("분할 정복 수업", "Divide & Conquer lesson")}>
          <DivideConquerLesson />
        </LessonPanel>
        <BottomNav />
      </div>
  ...
  // 리스트 화면
  const totalSolved = divideConquerContestCluster.problems.filter(p => solvedSet.has(p.id)).length
  const total = divideConquerContestCluster.problems.length
      ...
      <main className="max-w-2xl mx-auto px-4 pt-4">
        <div className="mb-4">
          <JourneyBreadcrumb items={[
            ...
            { label: "분할 정복", labelEn: "Divide and Conquer", href: "/algo/divideconquer", emoji: "📊" },
            { label: "문제 풀이", labelEn: "Practice" },
          ]} />
          <div className="flex items-center gap-2 mb-2">
            <span className="text-3xl">{divideConquerContestCluster.emoji}</span>
            <h1 className="text-xl sm:text-2xl font-black text-gray-900">
              {t(divideConquerContestCluster.title, divideConquerContestCluster.en?.title ?? divideConquerContestCluster.title)}
            </h1>
          </div>
          <p className="text-sm text-gray-600 mb-3">
            {t(divideConquerContestCluster.description, divideConquerContestCluster.en?.description ?? divideConquerContestCluster.description)}
          </p>
        ...
        {(() => {
          const warmup = divideConquerContestCluster.problems.filter(p => p.difficulty === "쉬움")
          const easy = divideConquerContestCluster.problems.filter(p => p.difficulty === "보통")
          const hard = divideConquerContestCluster.problems.filter(p => p.difficulty === "어려움")
          const renderItem = (p: any) => {
            const isSolved = solvedSet.has(p.id)
            const globalIdx = divideConquerContestCluster.problems.findIndex(pp => pp.id === p.id) + 1
            return (
              <Link
                key={p.id}
                href={`/algo/divideconquer/practice?p=${p.id}`}
                className={cn(
