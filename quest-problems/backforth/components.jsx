// 🔒 USACO_VERIFIED — cpid=857, backforth (2018 Dec Bronze #3)
...
const FULL_PY = [
  ...
  "with open('backforth.in', 'r') as file:",
  "    lines = file.readlines()",
  "",
  "b1 = list(map(int, lines[0].split()))",
  "b2 = list(map(int, lines[1].split()))",
  "",
  ...
  "states = set()",
  "for i1 in range(10):",
  "    b1a = b1[:i1] + b1[i1+1:]      # 9개",
  "    b2a = b2 + [b1[i1]]              # 11개",
  "    for j1 in range(11):",
  "        b1b = b1a + [b2a[j1]]        # 10개",
  "        b2b = b2a[:j1] + b2a[j1+1:]  # 10개",
  "        for i2 in range(10):",
  "            b1c = b1b[:i2] + b1b[i2+1:]  # 9개",
  "            b2c = b2b + [b1b[i2]]         # 11개",
  "            sum_b1c = sum(b1c)",
  "            for j2 in range(11):",
  "                final_sum = sum_b1c + b2c[j2]",
  "                states.add(final_sum)",
  "",
...
const FULL_CPP = [
  ...
  "    for (int i = 0; i < 10; i++) fin >> b1[i];",
  "    for (int i = 0; i < 10; i++) fin >> b2[i];",
  "    // 4일 동안 barn1 <-> barn2 옮기기, 최종 b1 총합 distinct",
  "    set<long long> states;",
  "    for (int i1 = 0; i1 < 10; i1++) {",
  "        vector<int> b1a;  // 9개",
  "        for (int k = 0; k < 10; k++) if (k != i1) b1a.push_back(b1[k]);",
  "        vector<int> b2a = b2;",
  "        b2a.push_back(b1[i1]);  // 11개",
  "        for (int j1 = 0; j1 < 11; j1++) {",
  "            vector<int> b1b = b1a;",
  "            b1b.push_back(b2a[j1]);  // 10개",
  "            vector<int> b2b;  // 10개",
  "            for (int k = 0; k < 11; k++) if (k != j1) b2b.push_back(b2a[k]);",
  "            for (int i2 = 0; i2 < 10; i2++) {",
  "                vector<int> b1c;  // 9개",
  "                for (int k = 0; k < 10; k++) if (k != i2) b1c.push_back(b1b[k]);",
  "                vector<int> b2c = b2b;",
  "                b2c.push_back(b1b[i2]);  // 11개",
  "                long long sum_b1c = 0;",
  "                for (int x : b1c) sum_b1c += x;",
  "                for (int j2 = 0; j2 < 11; j2++) {",
  "                    long long final_sum = sum_b1c + b2c[j2];",
  "                    states.insert(final_sum);",
  "                }",
  "            }",
  "        }",
  "    }",
